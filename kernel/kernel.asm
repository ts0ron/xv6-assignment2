
kernel/kernel:     file format elf64-littleriscv


Disassembly of section .text:

0000000080000000 <_entry>:
    80000000:	00009117          	auipc	sp,0x9
    80000004:	90013103          	ld	sp,-1792(sp) # 80008900 <_GLOBAL_OFFSET_TABLE_+0x8>
    80000008:	6505                	lui	a0,0x1
    8000000a:	f14025f3          	csrr	a1,mhartid
    8000000e:	0585                	addi	a1,a1,1
    80000010:	02b50533          	mul	a0,a0,a1
    80000014:	912a                	add	sp,sp,a0
    80000016:	078000ef          	jal	ra,8000008e <start>

000000008000001a <spin>:
    8000001a:	a001                	j	8000001a <spin>

000000008000001c <timerinit>:
// which arrive at timervec in kernelvec.S,
// which turns them into software interrupts for
// devintr() in trap.c.
void
timerinit()
{
    8000001c:	1141                	addi	sp,sp,-16
    8000001e:	e422                	sd	s0,8(sp)
    80000020:	0800                	addi	s0,sp,16
// which hart (core) is this?
static inline uint64
r_mhartid()
{
  uint64 x;
  asm volatile("csrr %0, mhartid" : "=r" (x) );
    80000022:	f14027f3          	csrr	a5,mhartid
  // each CPU has a separate source of timer interrupts.
  int id = r_mhartid();
    80000026:	0007869b          	sext.w	a3,a5

  // ask the CLINT for a timer interrupt.
  int interval = 1000000; // cycles; about 1/10th second in qemu.
  *(uint64*)CLINT_MTIMECMP(id) = *(uint64*)CLINT_MTIME + interval;
    8000002a:	0037979b          	slliw	a5,a5,0x3
    8000002e:	02004737          	lui	a4,0x2004
    80000032:	97ba                	add	a5,a5,a4
    80000034:	0200c737          	lui	a4,0x200c
    80000038:	ff873583          	ld	a1,-8(a4) # 200bff8 <_entry-0x7dff4008>
    8000003c:	000f4637          	lui	a2,0xf4
    80000040:	24060613          	addi	a2,a2,576 # f4240 <_entry-0x7ff0bdc0>
    80000044:	95b2                	add	a1,a1,a2
    80000046:	e38c                	sd	a1,0(a5)

  // prepare information in scratch[] for timervec.
  // scratch[0..2] : space for timervec to save registers.
  // scratch[3] : address of CLINT MTIMECMP register.
  // scratch[4] : desired interval (in cycles) between timer interrupts.
  uint64 *scratch = &timer_scratch[id][0];
    80000048:	00269713          	slli	a4,a3,0x2
    8000004c:	9736                	add	a4,a4,a3
    8000004e:	00371693          	slli	a3,a4,0x3
    80000052:	00009717          	auipc	a4,0x9
    80000056:	fee70713          	addi	a4,a4,-18 # 80009040 <timer_scratch>
    8000005a:	9736                	add	a4,a4,a3
  scratch[3] = CLINT_MTIMECMP(id);
    8000005c:	ef1c                	sd	a5,24(a4)
  scratch[4] = interval;
    8000005e:	f310                	sd	a2,32(a4)
}

static inline void 
w_mscratch(uint64 x)
{
  asm volatile("csrw mscratch, %0" : : "r" (x));
    80000060:	34071073          	csrw	mscratch,a4
  asm volatile("csrw mtvec, %0" : : "r" (x));
    80000064:	00006797          	auipc	a5,0x6
    80000068:	f2c78793          	addi	a5,a5,-212 # 80005f90 <timervec>
    8000006c:	30579073          	csrw	mtvec,a5
  asm volatile("csrr %0, mstatus" : "=r" (x) );
    80000070:	300027f3          	csrr	a5,mstatus

  // set the machine-mode trap handler.
  w_mtvec((uint64)timervec);

  // enable machine-mode interrupts.
  w_mstatus(r_mstatus() | MSTATUS_MIE);
    80000074:	0087e793          	ori	a5,a5,8
  asm volatile("csrw mstatus, %0" : : "r" (x));
    80000078:	30079073          	csrw	mstatus,a5
  asm volatile("csrr %0, mie" : "=r" (x) );
    8000007c:	304027f3          	csrr	a5,mie

  // enable machine-mode timer interrupts.
  w_mie(r_mie() | MIE_MTIE);
    80000080:	0807e793          	ori	a5,a5,128
  asm volatile("csrw mie, %0" : : "r" (x));
    80000084:	30479073          	csrw	mie,a5
}
    80000088:	6422                	ld	s0,8(sp)
    8000008a:	0141                	addi	sp,sp,16
    8000008c:	8082                	ret

000000008000008e <start>:
{
    8000008e:	1141                	addi	sp,sp,-16
    80000090:	e406                	sd	ra,8(sp)
    80000092:	e022                	sd	s0,0(sp)
    80000094:	0800                	addi	s0,sp,16
  asm volatile("csrr %0, mstatus" : "=r" (x) );
    80000096:	300027f3          	csrr	a5,mstatus
  x &= ~MSTATUS_MPP_MASK;
    8000009a:	7779                	lui	a4,0xffffe
    8000009c:	7ff70713          	addi	a4,a4,2047 # ffffffffffffe7ff <end+0xffffffff7ffd87ff>
    800000a0:	8ff9                	and	a5,a5,a4
  x |= MSTATUS_MPP_S;
    800000a2:	6705                	lui	a4,0x1
    800000a4:	80070713          	addi	a4,a4,-2048 # 800 <_entry-0x7ffff800>
    800000a8:	8fd9                	or	a5,a5,a4
  asm volatile("csrw mstatus, %0" : : "r" (x));
    800000aa:	30079073          	csrw	mstatus,a5
  asm volatile("csrw mepc, %0" : : "r" (x));
    800000ae:	00001797          	auipc	a5,0x1
    800000b2:	de078793          	addi	a5,a5,-544 # 80000e8e <main>
    800000b6:	34179073          	csrw	mepc,a5
  asm volatile("csrw satp, %0" : : "r" (x));
    800000ba:	4781                	li	a5,0
    800000bc:	18079073          	csrw	satp,a5
  asm volatile("csrw medeleg, %0" : : "r" (x));
    800000c0:	67c1                	lui	a5,0x10
    800000c2:	17fd                	addi	a5,a5,-1
    800000c4:	30279073          	csrw	medeleg,a5
  asm volatile("csrw mideleg, %0" : : "r" (x));
    800000c8:	30379073          	csrw	mideleg,a5
  asm volatile("csrr %0, sie" : "=r" (x) );
    800000cc:	104027f3          	csrr	a5,sie
  w_sie(r_sie() | SIE_SEIE | SIE_STIE | SIE_SSIE);
    800000d0:	2227e793          	ori	a5,a5,546
  asm volatile("csrw sie, %0" : : "r" (x));
    800000d4:	10479073          	csrw	sie,a5
  asm volatile("csrw pmpaddr0, %0" : : "r" (x));
    800000d8:	57fd                	li	a5,-1
    800000da:	83a9                	srli	a5,a5,0xa
    800000dc:	3b079073          	csrw	pmpaddr0,a5
  asm volatile("csrw pmpcfg0, %0" : : "r" (x));
    800000e0:	47bd                	li	a5,15
    800000e2:	3a079073          	csrw	pmpcfg0,a5
  timerinit();
    800000e6:	00000097          	auipc	ra,0x0
    800000ea:	f36080e7          	jalr	-202(ra) # 8000001c <timerinit>
  asm volatile("csrr %0, mhartid" : "=r" (x) );
    800000ee:	f14027f3          	csrr	a5,mhartid
  w_tp(id);
    800000f2:	2781                	sext.w	a5,a5
}

static inline void 
w_tp(uint64 x)
{
  asm volatile("mv tp, %0" : : "r" (x));
    800000f4:	823e                	mv	tp,a5
  asm volatile("mret");
    800000f6:	30200073          	mret
}
    800000fa:	60a2                	ld	ra,8(sp)
    800000fc:	6402                	ld	s0,0(sp)
    800000fe:	0141                	addi	sp,sp,16
    80000100:	8082                	ret

0000000080000102 <consolewrite>:
//
// user write()s to the console go here.
//
int
consolewrite(int user_src, uint64 src, int n)
{
    80000102:	715d                	addi	sp,sp,-80
    80000104:	e486                	sd	ra,72(sp)
    80000106:	e0a2                	sd	s0,64(sp)
    80000108:	fc26                	sd	s1,56(sp)
    8000010a:	f84a                	sd	s2,48(sp)
    8000010c:	f44e                	sd	s3,40(sp)
    8000010e:	f052                	sd	s4,32(sp)
    80000110:	ec56                	sd	s5,24(sp)
    80000112:	0880                	addi	s0,sp,80
  int i;

  for(i = 0; i < n; i++){
    80000114:	04c05663          	blez	a2,80000160 <consolewrite+0x5e>
    80000118:	8a2a                	mv	s4,a0
    8000011a:	84ae                	mv	s1,a1
    8000011c:	89b2                	mv	s3,a2
    8000011e:	4901                	li	s2,0
    char c;
    if(either_copyin(&c, user_src, src+i, 1) == -1)
    80000120:	5afd                	li	s5,-1
    80000122:	4685                	li	a3,1
    80000124:	8626                	mv	a2,s1
    80000126:	85d2                	mv	a1,s4
    80000128:	fbf40513          	addi	a0,s0,-65
    8000012c:	00002097          	auipc	ra,0x2
    80000130:	7a2080e7          	jalr	1954(ra) # 800028ce <either_copyin>
    80000134:	01550c63          	beq	a0,s5,8000014c <consolewrite+0x4a>
      break;
    uartputc(c);
    80000138:	fbf44503          	lbu	a0,-65(s0)
    8000013c:	00000097          	auipc	ra,0x0
    80000140:	78e080e7          	jalr	1934(ra) # 800008ca <uartputc>
  for(i = 0; i < n; i++){
    80000144:	2905                	addiw	s2,s2,1
    80000146:	0485                	addi	s1,s1,1
    80000148:	fd299de3          	bne	s3,s2,80000122 <consolewrite+0x20>
  }

  return i;
}
    8000014c:	854a                	mv	a0,s2
    8000014e:	60a6                	ld	ra,72(sp)
    80000150:	6406                	ld	s0,64(sp)
    80000152:	74e2                	ld	s1,56(sp)
    80000154:	7942                	ld	s2,48(sp)
    80000156:	79a2                	ld	s3,40(sp)
    80000158:	7a02                	ld	s4,32(sp)
    8000015a:	6ae2                	ld	s5,24(sp)
    8000015c:	6161                	addi	sp,sp,80
    8000015e:	8082                	ret
  for(i = 0; i < n; i++){
    80000160:	4901                	li	s2,0
    80000162:	b7ed                	j	8000014c <consolewrite+0x4a>

0000000080000164 <consoleread>:
// user_dist indicates whether dst is a user
// or kernel address.
//
int
consoleread(int user_dst, uint64 dst, int n)
{
    80000164:	7119                	addi	sp,sp,-128
    80000166:	fc86                	sd	ra,120(sp)
    80000168:	f8a2                	sd	s0,112(sp)
    8000016a:	f4a6                	sd	s1,104(sp)
    8000016c:	f0ca                	sd	s2,96(sp)
    8000016e:	ecce                	sd	s3,88(sp)
    80000170:	e8d2                	sd	s4,80(sp)
    80000172:	e4d6                	sd	s5,72(sp)
    80000174:	e0da                	sd	s6,64(sp)
    80000176:	fc5e                	sd	s7,56(sp)
    80000178:	f862                	sd	s8,48(sp)
    8000017a:	f466                	sd	s9,40(sp)
    8000017c:	f06a                	sd	s10,32(sp)
    8000017e:	ec6e                	sd	s11,24(sp)
    80000180:	0100                	addi	s0,sp,128
    80000182:	8b2a                	mv	s6,a0
    80000184:	8aae                	mv	s5,a1
    80000186:	8a32                	mv	s4,a2
  uint target;
  int c;
  char cbuf;

  target = n;
    80000188:	00060b9b          	sext.w	s7,a2
  acquire(&cons.lock);
    8000018c:	00011517          	auipc	a0,0x11
    80000190:	ff450513          	addi	a0,a0,-12 # 80011180 <cons>
    80000194:	00001097          	auipc	ra,0x1
    80000198:	a50080e7          	jalr	-1456(ra) # 80000be4 <acquire>
  while(n > 0){
    // wait until interrupt handler has put some
    // input into cons.buffer.
    while(cons.r == cons.w){
    8000019c:	00011497          	auipc	s1,0x11
    800001a0:	fe448493          	addi	s1,s1,-28 # 80011180 <cons>
      if(myproc()->killed){
        release(&cons.lock);
        return -1;
      }
      sleep(&cons.r, &cons.lock);
    800001a4:	89a6                	mv	s3,s1
    800001a6:	00011917          	auipc	s2,0x11
    800001aa:	07290913          	addi	s2,s2,114 # 80011218 <cons+0x98>
    }

    c = cons.buf[cons.r++ % INPUT_BUF];

    if(c == C('D')){  // end-of-file
    800001ae:	4c91                	li	s9,4
      break;
    }

    // copy the input byte to the user-space buffer.
    cbuf = c;
    if(either_copyout(user_dst, dst, &cbuf, 1) == -1)
    800001b0:	5d7d                	li	s10,-1
      break;

    dst++;
    --n;

    if(c == '\n'){
    800001b2:	4da9                	li	s11,10
  while(n > 0){
    800001b4:	07405863          	blez	s4,80000224 <consoleread+0xc0>
    while(cons.r == cons.w){
    800001b8:	0984a783          	lw	a5,152(s1)
    800001bc:	09c4a703          	lw	a4,156(s1)
    800001c0:	02f71463          	bne	a4,a5,800001e8 <consoleread+0x84>
      if(myproc()->killed){
    800001c4:	00002097          	auipc	ra,0x2
    800001c8:	bc8080e7          	jalr	-1080(ra) # 80001d8c <myproc>
    800001cc:	551c                	lw	a5,40(a0)
    800001ce:	e7b5                	bnez	a5,8000023a <consoleread+0xd6>
      sleep(&cons.r, &cons.lock);
    800001d0:	85ce                	mv	a1,s3
    800001d2:	854a                	mv	a0,s2
    800001d4:	00002097          	auipc	ra,0x2
    800001d8:	276080e7          	jalr	630(ra) # 8000244a <sleep>
    while(cons.r == cons.w){
    800001dc:	0984a783          	lw	a5,152(s1)
    800001e0:	09c4a703          	lw	a4,156(s1)
    800001e4:	fef700e3          	beq	a4,a5,800001c4 <consoleread+0x60>
    c = cons.buf[cons.r++ % INPUT_BUF];
    800001e8:	0017871b          	addiw	a4,a5,1
    800001ec:	08e4ac23          	sw	a4,152(s1)
    800001f0:	07f7f713          	andi	a4,a5,127
    800001f4:	9726                	add	a4,a4,s1
    800001f6:	01874703          	lbu	a4,24(a4)
    800001fa:	00070c1b          	sext.w	s8,a4
    if(c == C('D')){  // end-of-file
    800001fe:	079c0663          	beq	s8,s9,8000026a <consoleread+0x106>
    cbuf = c;
    80000202:	f8e407a3          	sb	a4,-113(s0)
    if(either_copyout(user_dst, dst, &cbuf, 1) == -1)
    80000206:	4685                	li	a3,1
    80000208:	f8f40613          	addi	a2,s0,-113
    8000020c:	85d6                	mv	a1,s5
    8000020e:	855a                	mv	a0,s6
    80000210:	00002097          	auipc	ra,0x2
    80000214:	668080e7          	jalr	1640(ra) # 80002878 <either_copyout>
    80000218:	01a50663          	beq	a0,s10,80000224 <consoleread+0xc0>
    dst++;
    8000021c:	0a85                	addi	s5,s5,1
    --n;
    8000021e:	3a7d                	addiw	s4,s4,-1
    if(c == '\n'){
    80000220:	f9bc1ae3          	bne	s8,s11,800001b4 <consoleread+0x50>
      // a whole line has arrived, return to
      // the user-level read().
      break;
    }
  }
  release(&cons.lock);
    80000224:	00011517          	auipc	a0,0x11
    80000228:	f5c50513          	addi	a0,a0,-164 # 80011180 <cons>
    8000022c:	00001097          	auipc	ra,0x1
    80000230:	a6c080e7          	jalr	-1428(ra) # 80000c98 <release>

  return target - n;
    80000234:	414b853b          	subw	a0,s7,s4
    80000238:	a811                	j	8000024c <consoleread+0xe8>
        release(&cons.lock);
    8000023a:	00011517          	auipc	a0,0x11
    8000023e:	f4650513          	addi	a0,a0,-186 # 80011180 <cons>
    80000242:	00001097          	auipc	ra,0x1
    80000246:	a56080e7          	jalr	-1450(ra) # 80000c98 <release>
        return -1;
    8000024a:	557d                	li	a0,-1
}
    8000024c:	70e6                	ld	ra,120(sp)
    8000024e:	7446                	ld	s0,112(sp)
    80000250:	74a6                	ld	s1,104(sp)
    80000252:	7906                	ld	s2,96(sp)
    80000254:	69e6                	ld	s3,88(sp)
    80000256:	6a46                	ld	s4,80(sp)
    80000258:	6aa6                	ld	s5,72(sp)
    8000025a:	6b06                	ld	s6,64(sp)
    8000025c:	7be2                	ld	s7,56(sp)
    8000025e:	7c42                	ld	s8,48(sp)
    80000260:	7ca2                	ld	s9,40(sp)
    80000262:	7d02                	ld	s10,32(sp)
    80000264:	6de2                	ld	s11,24(sp)
    80000266:	6109                	addi	sp,sp,128
    80000268:	8082                	ret
      if(n < target){
    8000026a:	000a071b          	sext.w	a4,s4
    8000026e:	fb777be3          	bgeu	a4,s7,80000224 <consoleread+0xc0>
        cons.r--;
    80000272:	00011717          	auipc	a4,0x11
    80000276:	faf72323          	sw	a5,-90(a4) # 80011218 <cons+0x98>
    8000027a:	b76d                	j	80000224 <consoleread+0xc0>

000000008000027c <consputc>:
{
    8000027c:	1141                	addi	sp,sp,-16
    8000027e:	e406                	sd	ra,8(sp)
    80000280:	e022                	sd	s0,0(sp)
    80000282:	0800                	addi	s0,sp,16
  if(c == BACKSPACE){
    80000284:	10000793          	li	a5,256
    80000288:	00f50a63          	beq	a0,a5,8000029c <consputc+0x20>
    uartputc_sync(c);
    8000028c:	00000097          	auipc	ra,0x0
    80000290:	564080e7          	jalr	1380(ra) # 800007f0 <uartputc_sync>
}
    80000294:	60a2                	ld	ra,8(sp)
    80000296:	6402                	ld	s0,0(sp)
    80000298:	0141                	addi	sp,sp,16
    8000029a:	8082                	ret
    uartputc_sync('\b'); uartputc_sync(' '); uartputc_sync('\b');
    8000029c:	4521                	li	a0,8
    8000029e:	00000097          	auipc	ra,0x0
    800002a2:	552080e7          	jalr	1362(ra) # 800007f0 <uartputc_sync>
    800002a6:	02000513          	li	a0,32
    800002aa:	00000097          	auipc	ra,0x0
    800002ae:	546080e7          	jalr	1350(ra) # 800007f0 <uartputc_sync>
    800002b2:	4521                	li	a0,8
    800002b4:	00000097          	auipc	ra,0x0
    800002b8:	53c080e7          	jalr	1340(ra) # 800007f0 <uartputc_sync>
    800002bc:	bfe1                	j	80000294 <consputc+0x18>

00000000800002be <consoleintr>:
// do erase/kill processing, append to cons.buf,
// wake up consoleread() if a whole line has arrived.
//
void
consoleintr(int c)
{
    800002be:	1101                	addi	sp,sp,-32
    800002c0:	ec06                	sd	ra,24(sp)
    800002c2:	e822                	sd	s0,16(sp)
    800002c4:	e426                	sd	s1,8(sp)
    800002c6:	e04a                	sd	s2,0(sp)
    800002c8:	1000                	addi	s0,sp,32
    800002ca:	84aa                	mv	s1,a0
  acquire(&cons.lock);
    800002cc:	00011517          	auipc	a0,0x11
    800002d0:	eb450513          	addi	a0,a0,-332 # 80011180 <cons>
    800002d4:	00001097          	auipc	ra,0x1
    800002d8:	910080e7          	jalr	-1776(ra) # 80000be4 <acquire>

  switch(c){
    800002dc:	47d5                	li	a5,21
    800002de:	0af48663          	beq	s1,a5,8000038a <consoleintr+0xcc>
    800002e2:	0297ca63          	blt	a5,s1,80000316 <consoleintr+0x58>
    800002e6:	47a1                	li	a5,8
    800002e8:	0ef48763          	beq	s1,a5,800003d6 <consoleintr+0x118>
    800002ec:	47c1                	li	a5,16
    800002ee:	10f49a63          	bne	s1,a5,80000402 <consoleintr+0x144>
  case C('P'):  // Print process list.
    procdump();
    800002f2:	00002097          	auipc	ra,0x2
    800002f6:	632080e7          	jalr	1586(ra) # 80002924 <procdump>
      }
    }
    break;
  }
  
  release(&cons.lock);
    800002fa:	00011517          	auipc	a0,0x11
    800002fe:	e8650513          	addi	a0,a0,-378 # 80011180 <cons>
    80000302:	00001097          	auipc	ra,0x1
    80000306:	996080e7          	jalr	-1642(ra) # 80000c98 <release>
}
    8000030a:	60e2                	ld	ra,24(sp)
    8000030c:	6442                	ld	s0,16(sp)
    8000030e:	64a2                	ld	s1,8(sp)
    80000310:	6902                	ld	s2,0(sp)
    80000312:	6105                	addi	sp,sp,32
    80000314:	8082                	ret
  switch(c){
    80000316:	07f00793          	li	a5,127
    8000031a:	0af48e63          	beq	s1,a5,800003d6 <consoleintr+0x118>
    if(c != 0 && cons.e-cons.r < INPUT_BUF){
    8000031e:	00011717          	auipc	a4,0x11
    80000322:	e6270713          	addi	a4,a4,-414 # 80011180 <cons>
    80000326:	0a072783          	lw	a5,160(a4)
    8000032a:	09872703          	lw	a4,152(a4)
    8000032e:	9f99                	subw	a5,a5,a4
    80000330:	07f00713          	li	a4,127
    80000334:	fcf763e3          	bltu	a4,a5,800002fa <consoleintr+0x3c>
      c = (c == '\r') ? '\n' : c;
    80000338:	47b5                	li	a5,13
    8000033a:	0cf48763          	beq	s1,a5,80000408 <consoleintr+0x14a>
      consputc(c);
    8000033e:	8526                	mv	a0,s1
    80000340:	00000097          	auipc	ra,0x0
    80000344:	f3c080e7          	jalr	-196(ra) # 8000027c <consputc>
      cons.buf[cons.e++ % INPUT_BUF] = c;
    80000348:	00011797          	auipc	a5,0x11
    8000034c:	e3878793          	addi	a5,a5,-456 # 80011180 <cons>
    80000350:	0a07a703          	lw	a4,160(a5)
    80000354:	0017069b          	addiw	a3,a4,1
    80000358:	0006861b          	sext.w	a2,a3
    8000035c:	0ad7a023          	sw	a3,160(a5)
    80000360:	07f77713          	andi	a4,a4,127
    80000364:	97ba                	add	a5,a5,a4
    80000366:	00978c23          	sb	s1,24(a5)
      if(c == '\n' || c == C('D') || cons.e == cons.r+INPUT_BUF){
    8000036a:	47a9                	li	a5,10
    8000036c:	0cf48563          	beq	s1,a5,80000436 <consoleintr+0x178>
    80000370:	4791                	li	a5,4
    80000372:	0cf48263          	beq	s1,a5,80000436 <consoleintr+0x178>
    80000376:	00011797          	auipc	a5,0x11
    8000037a:	ea27a783          	lw	a5,-350(a5) # 80011218 <cons+0x98>
    8000037e:	0807879b          	addiw	a5,a5,128
    80000382:	f6f61ce3          	bne	a2,a5,800002fa <consoleintr+0x3c>
      cons.buf[cons.e++ % INPUT_BUF] = c;
    80000386:	863e                	mv	a2,a5
    80000388:	a07d                	j	80000436 <consoleintr+0x178>
    while(cons.e != cons.w &&
    8000038a:	00011717          	auipc	a4,0x11
    8000038e:	df670713          	addi	a4,a4,-522 # 80011180 <cons>
    80000392:	0a072783          	lw	a5,160(a4)
    80000396:	09c72703          	lw	a4,156(a4)
          cons.buf[(cons.e-1) % INPUT_BUF] != '\n'){
    8000039a:	00011497          	auipc	s1,0x11
    8000039e:	de648493          	addi	s1,s1,-538 # 80011180 <cons>
    while(cons.e != cons.w &&
    800003a2:	4929                	li	s2,10
    800003a4:	f4f70be3          	beq	a4,a5,800002fa <consoleintr+0x3c>
          cons.buf[(cons.e-1) % INPUT_BUF] != '\n'){
    800003a8:	37fd                	addiw	a5,a5,-1
    800003aa:	07f7f713          	andi	a4,a5,127
    800003ae:	9726                	add	a4,a4,s1
    while(cons.e != cons.w &&
    800003b0:	01874703          	lbu	a4,24(a4)
    800003b4:	f52703e3          	beq	a4,s2,800002fa <consoleintr+0x3c>
      cons.e--;
    800003b8:	0af4a023          	sw	a5,160(s1)
      consputc(BACKSPACE);
    800003bc:	10000513          	li	a0,256
    800003c0:	00000097          	auipc	ra,0x0
    800003c4:	ebc080e7          	jalr	-324(ra) # 8000027c <consputc>
    while(cons.e != cons.w &&
    800003c8:	0a04a783          	lw	a5,160(s1)
    800003cc:	09c4a703          	lw	a4,156(s1)
    800003d0:	fcf71ce3          	bne	a4,a5,800003a8 <consoleintr+0xea>
    800003d4:	b71d                	j	800002fa <consoleintr+0x3c>
    if(cons.e != cons.w){
    800003d6:	00011717          	auipc	a4,0x11
    800003da:	daa70713          	addi	a4,a4,-598 # 80011180 <cons>
    800003de:	0a072783          	lw	a5,160(a4)
    800003e2:	09c72703          	lw	a4,156(a4)
    800003e6:	f0f70ae3          	beq	a4,a5,800002fa <consoleintr+0x3c>
      cons.e--;
    800003ea:	37fd                	addiw	a5,a5,-1
    800003ec:	00011717          	auipc	a4,0x11
    800003f0:	e2f72a23          	sw	a5,-460(a4) # 80011220 <cons+0xa0>
      consputc(BACKSPACE);
    800003f4:	10000513          	li	a0,256
    800003f8:	00000097          	auipc	ra,0x0
    800003fc:	e84080e7          	jalr	-380(ra) # 8000027c <consputc>
    80000400:	bded                	j	800002fa <consoleintr+0x3c>
    if(c != 0 && cons.e-cons.r < INPUT_BUF){
    80000402:	ee048ce3          	beqz	s1,800002fa <consoleintr+0x3c>
    80000406:	bf21                	j	8000031e <consoleintr+0x60>
      consputc(c);
    80000408:	4529                	li	a0,10
    8000040a:	00000097          	auipc	ra,0x0
    8000040e:	e72080e7          	jalr	-398(ra) # 8000027c <consputc>
      cons.buf[cons.e++ % INPUT_BUF] = c;
    80000412:	00011797          	auipc	a5,0x11
    80000416:	d6e78793          	addi	a5,a5,-658 # 80011180 <cons>
    8000041a:	0a07a703          	lw	a4,160(a5)
    8000041e:	0017069b          	addiw	a3,a4,1
    80000422:	0006861b          	sext.w	a2,a3
    80000426:	0ad7a023          	sw	a3,160(a5)
    8000042a:	07f77713          	andi	a4,a4,127
    8000042e:	97ba                	add	a5,a5,a4
    80000430:	4729                	li	a4,10
    80000432:	00e78c23          	sb	a4,24(a5)
        cons.w = cons.e;
    80000436:	00011797          	auipc	a5,0x11
    8000043a:	dec7a323          	sw	a2,-538(a5) # 8001121c <cons+0x9c>
        wakeup(&cons.r);
    8000043e:	00011517          	auipc	a0,0x11
    80000442:	dda50513          	addi	a0,a0,-550 # 80011218 <cons+0x98>
    80000446:	00002097          	auipc	ra,0x2
    8000044a:	190080e7          	jalr	400(ra) # 800025d6 <wakeup>
    8000044e:	b575                	j	800002fa <consoleintr+0x3c>

0000000080000450 <consoleinit>:

void
consoleinit(void)
{
    80000450:	1141                	addi	sp,sp,-16
    80000452:	e406                	sd	ra,8(sp)
    80000454:	e022                	sd	s0,0(sp)
    80000456:	0800                	addi	s0,sp,16
  initlock(&cons.lock, "cons");
    80000458:	00008597          	auipc	a1,0x8
    8000045c:	bb858593          	addi	a1,a1,-1096 # 80008010 <etext+0x10>
    80000460:	00011517          	auipc	a0,0x11
    80000464:	d2050513          	addi	a0,a0,-736 # 80011180 <cons>
    80000468:	00000097          	auipc	ra,0x0
    8000046c:	6ec080e7          	jalr	1772(ra) # 80000b54 <initlock>

  uartinit();
    80000470:	00000097          	auipc	ra,0x0
    80000474:	330080e7          	jalr	816(ra) # 800007a0 <uartinit>

  // connect read and write system calls
  // to consoleread and consolewrite.
  devsw[CONSOLE].read = consoleread;
    80000478:	00021797          	auipc	a5,0x21
    8000047c:	16078793          	addi	a5,a5,352 # 800215d8 <devsw>
    80000480:	00000717          	auipc	a4,0x0
    80000484:	ce470713          	addi	a4,a4,-796 # 80000164 <consoleread>
    80000488:	eb98                	sd	a4,16(a5)
  devsw[CONSOLE].write = consolewrite;
    8000048a:	00000717          	auipc	a4,0x0
    8000048e:	c7870713          	addi	a4,a4,-904 # 80000102 <consolewrite>
    80000492:	ef98                	sd	a4,24(a5)
}
    80000494:	60a2                	ld	ra,8(sp)
    80000496:	6402                	ld	s0,0(sp)
    80000498:	0141                	addi	sp,sp,16
    8000049a:	8082                	ret

000000008000049c <printint>:

static char digits[] = "0123456789abcdef";

static void
printint(int xx, int base, int sign)
{
    8000049c:	7179                	addi	sp,sp,-48
    8000049e:	f406                	sd	ra,40(sp)
    800004a0:	f022                	sd	s0,32(sp)
    800004a2:	ec26                	sd	s1,24(sp)
    800004a4:	e84a                	sd	s2,16(sp)
    800004a6:	1800                	addi	s0,sp,48
  char buf[16];
  int i;
  uint x;

  if(sign && (sign = xx < 0))
    800004a8:	c219                	beqz	a2,800004ae <printint+0x12>
    800004aa:	08054663          	bltz	a0,80000536 <printint+0x9a>
    x = -xx;
  else
    x = xx;
    800004ae:	2501                	sext.w	a0,a0
    800004b0:	4881                	li	a7,0
    800004b2:	fd040693          	addi	a3,s0,-48

  i = 0;
    800004b6:	4701                	li	a4,0
  do {
    buf[i++] = digits[x % base];
    800004b8:	2581                	sext.w	a1,a1
    800004ba:	00008617          	auipc	a2,0x8
    800004be:	b8660613          	addi	a2,a2,-1146 # 80008040 <digits>
    800004c2:	883a                	mv	a6,a4
    800004c4:	2705                	addiw	a4,a4,1
    800004c6:	02b577bb          	remuw	a5,a0,a1
    800004ca:	1782                	slli	a5,a5,0x20
    800004cc:	9381                	srli	a5,a5,0x20
    800004ce:	97b2                	add	a5,a5,a2
    800004d0:	0007c783          	lbu	a5,0(a5)
    800004d4:	00f68023          	sb	a5,0(a3)
  } while((x /= base) != 0);
    800004d8:	0005079b          	sext.w	a5,a0
    800004dc:	02b5553b          	divuw	a0,a0,a1
    800004e0:	0685                	addi	a3,a3,1
    800004e2:	feb7f0e3          	bgeu	a5,a1,800004c2 <printint+0x26>

  if(sign)
    800004e6:	00088b63          	beqz	a7,800004fc <printint+0x60>
    buf[i++] = '-';
    800004ea:	fe040793          	addi	a5,s0,-32
    800004ee:	973e                	add	a4,a4,a5
    800004f0:	02d00793          	li	a5,45
    800004f4:	fef70823          	sb	a5,-16(a4)
    800004f8:	0028071b          	addiw	a4,a6,2

  while(--i >= 0)
    800004fc:	02e05763          	blez	a4,8000052a <printint+0x8e>
    80000500:	fd040793          	addi	a5,s0,-48
    80000504:	00e784b3          	add	s1,a5,a4
    80000508:	fff78913          	addi	s2,a5,-1
    8000050c:	993a                	add	s2,s2,a4
    8000050e:	377d                	addiw	a4,a4,-1
    80000510:	1702                	slli	a4,a4,0x20
    80000512:	9301                	srli	a4,a4,0x20
    80000514:	40e90933          	sub	s2,s2,a4
    consputc(buf[i]);
    80000518:	fff4c503          	lbu	a0,-1(s1)
    8000051c:	00000097          	auipc	ra,0x0
    80000520:	d60080e7          	jalr	-672(ra) # 8000027c <consputc>
  while(--i >= 0)
    80000524:	14fd                	addi	s1,s1,-1
    80000526:	ff2499e3          	bne	s1,s2,80000518 <printint+0x7c>
}
    8000052a:	70a2                	ld	ra,40(sp)
    8000052c:	7402                	ld	s0,32(sp)
    8000052e:	64e2                	ld	s1,24(sp)
    80000530:	6942                	ld	s2,16(sp)
    80000532:	6145                	addi	sp,sp,48
    80000534:	8082                	ret
    x = -xx;
    80000536:	40a0053b          	negw	a0,a0
  if(sign && (sign = xx < 0))
    8000053a:	4885                	li	a7,1
    x = -xx;
    8000053c:	bf9d                	j	800004b2 <printint+0x16>

000000008000053e <panic>:
    release(&pr.lock);
}

void
panic(char *s)
{
    8000053e:	1101                	addi	sp,sp,-32
    80000540:	ec06                	sd	ra,24(sp)
    80000542:	e822                	sd	s0,16(sp)
    80000544:	e426                	sd	s1,8(sp)
    80000546:	1000                	addi	s0,sp,32
    80000548:	84aa                	mv	s1,a0
  pr.locking = 0;
    8000054a:	00011797          	auipc	a5,0x11
    8000054e:	ce07ab23          	sw	zero,-778(a5) # 80011240 <pr+0x18>
  printf("panic: ");
    80000552:	00008517          	auipc	a0,0x8
    80000556:	ac650513          	addi	a0,a0,-1338 # 80008018 <etext+0x18>
    8000055a:	00000097          	auipc	ra,0x0
    8000055e:	02e080e7          	jalr	46(ra) # 80000588 <printf>
  printf(s);
    80000562:	8526                	mv	a0,s1
    80000564:	00000097          	auipc	ra,0x0
    80000568:	024080e7          	jalr	36(ra) # 80000588 <printf>
  printf("\n");
    8000056c:	00008517          	auipc	a0,0x8
    80000570:	b5c50513          	addi	a0,a0,-1188 # 800080c8 <digits+0x88>
    80000574:	00000097          	auipc	ra,0x0
    80000578:	014080e7          	jalr	20(ra) # 80000588 <printf>
  panicked = 1; // freeze uart output from other CPUs
    8000057c:	4785                	li	a5,1
    8000057e:	00009717          	auipc	a4,0x9
    80000582:	a8f72123          	sw	a5,-1406(a4) # 80009000 <panicked>
  for(;;)
    80000586:	a001                	j	80000586 <panic+0x48>

0000000080000588 <printf>:
{
    80000588:	7131                	addi	sp,sp,-192
    8000058a:	fc86                	sd	ra,120(sp)
    8000058c:	f8a2                	sd	s0,112(sp)
    8000058e:	f4a6                	sd	s1,104(sp)
    80000590:	f0ca                	sd	s2,96(sp)
    80000592:	ecce                	sd	s3,88(sp)
    80000594:	e8d2                	sd	s4,80(sp)
    80000596:	e4d6                	sd	s5,72(sp)
    80000598:	e0da                	sd	s6,64(sp)
    8000059a:	fc5e                	sd	s7,56(sp)
    8000059c:	f862                	sd	s8,48(sp)
    8000059e:	f466                	sd	s9,40(sp)
    800005a0:	f06a                	sd	s10,32(sp)
    800005a2:	ec6e                	sd	s11,24(sp)
    800005a4:	0100                	addi	s0,sp,128
    800005a6:	8a2a                	mv	s4,a0
    800005a8:	e40c                	sd	a1,8(s0)
    800005aa:	e810                	sd	a2,16(s0)
    800005ac:	ec14                	sd	a3,24(s0)
    800005ae:	f018                	sd	a4,32(s0)
    800005b0:	f41c                	sd	a5,40(s0)
    800005b2:	03043823          	sd	a6,48(s0)
    800005b6:	03143c23          	sd	a7,56(s0)
  locking = pr.locking;
    800005ba:	00011d97          	auipc	s11,0x11
    800005be:	c86dad83          	lw	s11,-890(s11) # 80011240 <pr+0x18>
  if(locking)
    800005c2:	020d9b63          	bnez	s11,800005f8 <printf+0x70>
  if (fmt == 0)
    800005c6:	040a0263          	beqz	s4,8000060a <printf+0x82>
  va_start(ap, fmt);
    800005ca:	00840793          	addi	a5,s0,8
    800005ce:	f8f43423          	sd	a5,-120(s0)
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
    800005d2:	000a4503          	lbu	a0,0(s4)
    800005d6:	16050263          	beqz	a0,8000073a <printf+0x1b2>
    800005da:	4481                	li	s1,0
    if(c != '%'){
    800005dc:	02500a93          	li	s5,37
    switch(c){
    800005e0:	07000b13          	li	s6,112
  consputc('x');
    800005e4:	4d41                	li	s10,16
    consputc(digits[x >> (sizeof(uint64) * 8 - 4)]);
    800005e6:	00008b97          	auipc	s7,0x8
    800005ea:	a5ab8b93          	addi	s7,s7,-1446 # 80008040 <digits>
    switch(c){
    800005ee:	07300c93          	li	s9,115
    800005f2:	06400c13          	li	s8,100
    800005f6:	a82d                	j	80000630 <printf+0xa8>
    acquire(&pr.lock);
    800005f8:	00011517          	auipc	a0,0x11
    800005fc:	c3050513          	addi	a0,a0,-976 # 80011228 <pr>
    80000600:	00000097          	auipc	ra,0x0
    80000604:	5e4080e7          	jalr	1508(ra) # 80000be4 <acquire>
    80000608:	bf7d                	j	800005c6 <printf+0x3e>
    panic("null fmt");
    8000060a:	00008517          	auipc	a0,0x8
    8000060e:	a1e50513          	addi	a0,a0,-1506 # 80008028 <etext+0x28>
    80000612:	00000097          	auipc	ra,0x0
    80000616:	f2c080e7          	jalr	-212(ra) # 8000053e <panic>
      consputc(c);
    8000061a:	00000097          	auipc	ra,0x0
    8000061e:	c62080e7          	jalr	-926(ra) # 8000027c <consputc>
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
    80000622:	2485                	addiw	s1,s1,1
    80000624:	009a07b3          	add	a5,s4,s1
    80000628:	0007c503          	lbu	a0,0(a5)
    8000062c:	10050763          	beqz	a0,8000073a <printf+0x1b2>
    if(c != '%'){
    80000630:	ff5515e3          	bne	a0,s5,8000061a <printf+0x92>
    c = fmt[++i] & 0xff;
    80000634:	2485                	addiw	s1,s1,1
    80000636:	009a07b3          	add	a5,s4,s1
    8000063a:	0007c783          	lbu	a5,0(a5)
    8000063e:	0007891b          	sext.w	s2,a5
    if(c == 0)
    80000642:	cfe5                	beqz	a5,8000073a <printf+0x1b2>
    switch(c){
    80000644:	05678a63          	beq	a5,s6,80000698 <printf+0x110>
    80000648:	02fb7663          	bgeu	s6,a5,80000674 <printf+0xec>
    8000064c:	09978963          	beq	a5,s9,800006de <printf+0x156>
    80000650:	07800713          	li	a4,120
    80000654:	0ce79863          	bne	a5,a4,80000724 <printf+0x19c>
      printint(va_arg(ap, int), 16, 1);
    80000658:	f8843783          	ld	a5,-120(s0)
    8000065c:	00878713          	addi	a4,a5,8
    80000660:	f8e43423          	sd	a4,-120(s0)
    80000664:	4605                	li	a2,1
    80000666:	85ea                	mv	a1,s10
    80000668:	4388                	lw	a0,0(a5)
    8000066a:	00000097          	auipc	ra,0x0
    8000066e:	e32080e7          	jalr	-462(ra) # 8000049c <printint>
      break;
    80000672:	bf45                	j	80000622 <printf+0x9a>
    switch(c){
    80000674:	0b578263          	beq	a5,s5,80000718 <printf+0x190>
    80000678:	0b879663          	bne	a5,s8,80000724 <printf+0x19c>
      printint(va_arg(ap, int), 10, 1);
    8000067c:	f8843783          	ld	a5,-120(s0)
    80000680:	00878713          	addi	a4,a5,8
    80000684:	f8e43423          	sd	a4,-120(s0)
    80000688:	4605                	li	a2,1
    8000068a:	45a9                	li	a1,10
    8000068c:	4388                	lw	a0,0(a5)
    8000068e:	00000097          	auipc	ra,0x0
    80000692:	e0e080e7          	jalr	-498(ra) # 8000049c <printint>
      break;
    80000696:	b771                	j	80000622 <printf+0x9a>
      printptr(va_arg(ap, uint64));
    80000698:	f8843783          	ld	a5,-120(s0)
    8000069c:	00878713          	addi	a4,a5,8
    800006a0:	f8e43423          	sd	a4,-120(s0)
    800006a4:	0007b983          	ld	s3,0(a5)
  consputc('0');
    800006a8:	03000513          	li	a0,48
    800006ac:	00000097          	auipc	ra,0x0
    800006b0:	bd0080e7          	jalr	-1072(ra) # 8000027c <consputc>
  consputc('x');
    800006b4:	07800513          	li	a0,120
    800006b8:	00000097          	auipc	ra,0x0
    800006bc:	bc4080e7          	jalr	-1084(ra) # 8000027c <consputc>
    800006c0:	896a                	mv	s2,s10
    consputc(digits[x >> (sizeof(uint64) * 8 - 4)]);
    800006c2:	03c9d793          	srli	a5,s3,0x3c
    800006c6:	97de                	add	a5,a5,s7
    800006c8:	0007c503          	lbu	a0,0(a5)
    800006cc:	00000097          	auipc	ra,0x0
    800006d0:	bb0080e7          	jalr	-1104(ra) # 8000027c <consputc>
  for (i = 0; i < (sizeof(uint64) * 2); i++, x <<= 4)
    800006d4:	0992                	slli	s3,s3,0x4
    800006d6:	397d                	addiw	s2,s2,-1
    800006d8:	fe0915e3          	bnez	s2,800006c2 <printf+0x13a>
    800006dc:	b799                	j	80000622 <printf+0x9a>
      if((s = va_arg(ap, char*)) == 0)
    800006de:	f8843783          	ld	a5,-120(s0)
    800006e2:	00878713          	addi	a4,a5,8
    800006e6:	f8e43423          	sd	a4,-120(s0)
    800006ea:	0007b903          	ld	s2,0(a5)
    800006ee:	00090e63          	beqz	s2,8000070a <printf+0x182>
      for(; *s; s++)
    800006f2:	00094503          	lbu	a0,0(s2)
    800006f6:	d515                	beqz	a0,80000622 <printf+0x9a>
        consputc(*s);
    800006f8:	00000097          	auipc	ra,0x0
    800006fc:	b84080e7          	jalr	-1148(ra) # 8000027c <consputc>
      for(; *s; s++)
    80000700:	0905                	addi	s2,s2,1
    80000702:	00094503          	lbu	a0,0(s2)
    80000706:	f96d                	bnez	a0,800006f8 <printf+0x170>
    80000708:	bf29                	j	80000622 <printf+0x9a>
        s = "(null)";
    8000070a:	00008917          	auipc	s2,0x8
    8000070e:	91690913          	addi	s2,s2,-1770 # 80008020 <etext+0x20>
      for(; *s; s++)
    80000712:	02800513          	li	a0,40
    80000716:	b7cd                	j	800006f8 <printf+0x170>
      consputc('%');
    80000718:	8556                	mv	a0,s5
    8000071a:	00000097          	auipc	ra,0x0
    8000071e:	b62080e7          	jalr	-1182(ra) # 8000027c <consputc>
      break;
    80000722:	b701                	j	80000622 <printf+0x9a>
      consputc('%');
    80000724:	8556                	mv	a0,s5
    80000726:	00000097          	auipc	ra,0x0
    8000072a:	b56080e7          	jalr	-1194(ra) # 8000027c <consputc>
      consputc(c);
    8000072e:	854a                	mv	a0,s2
    80000730:	00000097          	auipc	ra,0x0
    80000734:	b4c080e7          	jalr	-1204(ra) # 8000027c <consputc>
      break;
    80000738:	b5ed                	j	80000622 <printf+0x9a>
  if(locking)
    8000073a:	020d9163          	bnez	s11,8000075c <printf+0x1d4>
}
    8000073e:	70e6                	ld	ra,120(sp)
    80000740:	7446                	ld	s0,112(sp)
    80000742:	74a6                	ld	s1,104(sp)
    80000744:	7906                	ld	s2,96(sp)
    80000746:	69e6                	ld	s3,88(sp)
    80000748:	6a46                	ld	s4,80(sp)
    8000074a:	6aa6                	ld	s5,72(sp)
    8000074c:	6b06                	ld	s6,64(sp)
    8000074e:	7be2                	ld	s7,56(sp)
    80000750:	7c42                	ld	s8,48(sp)
    80000752:	7ca2                	ld	s9,40(sp)
    80000754:	7d02                	ld	s10,32(sp)
    80000756:	6de2                	ld	s11,24(sp)
    80000758:	6129                	addi	sp,sp,192
    8000075a:	8082                	ret
    release(&pr.lock);
    8000075c:	00011517          	auipc	a0,0x11
    80000760:	acc50513          	addi	a0,a0,-1332 # 80011228 <pr>
    80000764:	00000097          	auipc	ra,0x0
    80000768:	534080e7          	jalr	1332(ra) # 80000c98 <release>
}
    8000076c:	bfc9                	j	8000073e <printf+0x1b6>

000000008000076e <printfinit>:
    ;
}

void
printfinit(void)
{
    8000076e:	1101                	addi	sp,sp,-32
    80000770:	ec06                	sd	ra,24(sp)
    80000772:	e822                	sd	s0,16(sp)
    80000774:	e426                	sd	s1,8(sp)
    80000776:	1000                	addi	s0,sp,32
  initlock(&pr.lock, "pr");
    80000778:	00011497          	auipc	s1,0x11
    8000077c:	ab048493          	addi	s1,s1,-1360 # 80011228 <pr>
    80000780:	00008597          	auipc	a1,0x8
    80000784:	8b858593          	addi	a1,a1,-1864 # 80008038 <etext+0x38>
    80000788:	8526                	mv	a0,s1
    8000078a:	00000097          	auipc	ra,0x0
    8000078e:	3ca080e7          	jalr	970(ra) # 80000b54 <initlock>
  pr.locking = 1;
    80000792:	4785                	li	a5,1
    80000794:	cc9c                	sw	a5,24(s1)
}
    80000796:	60e2                	ld	ra,24(sp)
    80000798:	6442                	ld	s0,16(sp)
    8000079a:	64a2                	ld	s1,8(sp)
    8000079c:	6105                	addi	sp,sp,32
    8000079e:	8082                	ret

00000000800007a0 <uartinit>:

void uartstart();

void
uartinit(void)
{
    800007a0:	1141                	addi	sp,sp,-16
    800007a2:	e406                	sd	ra,8(sp)
    800007a4:	e022                	sd	s0,0(sp)
    800007a6:	0800                	addi	s0,sp,16
  // disable interrupts.
  WriteReg(IER, 0x00);
    800007a8:	100007b7          	lui	a5,0x10000
    800007ac:	000780a3          	sb	zero,1(a5) # 10000001 <_entry-0x6fffffff>

  // special mode to set baud rate.
  WriteReg(LCR, LCR_BAUD_LATCH);
    800007b0:	f8000713          	li	a4,-128
    800007b4:	00e781a3          	sb	a4,3(a5)

  // LSB for baud rate of 38.4K.
  WriteReg(0, 0x03);
    800007b8:	470d                	li	a4,3
    800007ba:	00e78023          	sb	a4,0(a5)

  // MSB for baud rate of 38.4K.
  WriteReg(1, 0x00);
    800007be:	000780a3          	sb	zero,1(a5)

  // leave set-baud mode,
  // and set word length to 8 bits, no parity.
  WriteReg(LCR, LCR_EIGHT_BITS);
    800007c2:	00e781a3          	sb	a4,3(a5)

  // reset and enable FIFOs.
  WriteReg(FCR, FCR_FIFO_ENABLE | FCR_FIFO_CLEAR);
    800007c6:	469d                	li	a3,7
    800007c8:	00d78123          	sb	a3,2(a5)

  // enable transmit and receive interrupts.
  WriteReg(IER, IER_TX_ENABLE | IER_RX_ENABLE);
    800007cc:	00e780a3          	sb	a4,1(a5)

  initlock(&uart_tx_lock, "uart");
    800007d0:	00008597          	auipc	a1,0x8
    800007d4:	88858593          	addi	a1,a1,-1912 # 80008058 <digits+0x18>
    800007d8:	00011517          	auipc	a0,0x11
    800007dc:	a7050513          	addi	a0,a0,-1424 # 80011248 <uart_tx_lock>
    800007e0:	00000097          	auipc	ra,0x0
    800007e4:	374080e7          	jalr	884(ra) # 80000b54 <initlock>
}
    800007e8:	60a2                	ld	ra,8(sp)
    800007ea:	6402                	ld	s0,0(sp)
    800007ec:	0141                	addi	sp,sp,16
    800007ee:	8082                	ret

00000000800007f0 <uartputc_sync>:
// use interrupts, for use by kernel printf() and
// to echo characters. it spins waiting for the uart's
// output register to be empty.
void
uartputc_sync(int c)
{
    800007f0:	1101                	addi	sp,sp,-32
    800007f2:	ec06                	sd	ra,24(sp)
    800007f4:	e822                	sd	s0,16(sp)
    800007f6:	e426                	sd	s1,8(sp)
    800007f8:	1000                	addi	s0,sp,32
    800007fa:	84aa                	mv	s1,a0
  push_off();
    800007fc:	00000097          	auipc	ra,0x0
    80000800:	39c080e7          	jalr	924(ra) # 80000b98 <push_off>

  if(panicked){
    80000804:	00008797          	auipc	a5,0x8
    80000808:	7fc7a783          	lw	a5,2044(a5) # 80009000 <panicked>
    for(;;)
      ;
  }

  // wait for Transmit Holding Empty to be set in LSR.
  while((ReadReg(LSR) & LSR_TX_IDLE) == 0)
    8000080c:	10000737          	lui	a4,0x10000
  if(panicked){
    80000810:	c391                	beqz	a5,80000814 <uartputc_sync+0x24>
    for(;;)
    80000812:	a001                	j	80000812 <uartputc_sync+0x22>
  while((ReadReg(LSR) & LSR_TX_IDLE) == 0)
    80000814:	00574783          	lbu	a5,5(a4) # 10000005 <_entry-0x6ffffffb>
    80000818:	0ff7f793          	andi	a5,a5,255
    8000081c:	0207f793          	andi	a5,a5,32
    80000820:	dbf5                	beqz	a5,80000814 <uartputc_sync+0x24>
    ;
  WriteReg(THR, c);
    80000822:	0ff4f793          	andi	a5,s1,255
    80000826:	10000737          	lui	a4,0x10000
    8000082a:	00f70023          	sb	a5,0(a4) # 10000000 <_entry-0x70000000>

  pop_off();
    8000082e:	00000097          	auipc	ra,0x0
    80000832:	40a080e7          	jalr	1034(ra) # 80000c38 <pop_off>
}
    80000836:	60e2                	ld	ra,24(sp)
    80000838:	6442                	ld	s0,16(sp)
    8000083a:	64a2                	ld	s1,8(sp)
    8000083c:	6105                	addi	sp,sp,32
    8000083e:	8082                	ret

0000000080000840 <uartstart>:
// called from both the top- and bottom-half.
void
uartstart()
{
  while(1){
    if(uart_tx_w == uart_tx_r){
    80000840:	00008717          	auipc	a4,0x8
    80000844:	7c873703          	ld	a4,1992(a4) # 80009008 <uart_tx_r>
    80000848:	00008797          	auipc	a5,0x8
    8000084c:	7c87b783          	ld	a5,1992(a5) # 80009010 <uart_tx_w>
    80000850:	06e78c63          	beq	a5,a4,800008c8 <uartstart+0x88>
{
    80000854:	7139                	addi	sp,sp,-64
    80000856:	fc06                	sd	ra,56(sp)
    80000858:	f822                	sd	s0,48(sp)
    8000085a:	f426                	sd	s1,40(sp)
    8000085c:	f04a                	sd	s2,32(sp)
    8000085e:	ec4e                	sd	s3,24(sp)
    80000860:	e852                	sd	s4,16(sp)
    80000862:	e456                	sd	s5,8(sp)
    80000864:	0080                	addi	s0,sp,64
      // transmit buffer is empty.
      return;
    }
    
    if((ReadReg(LSR) & LSR_TX_IDLE) == 0){
    80000866:	10000937          	lui	s2,0x10000
      // so we cannot give it another byte.
      // it will interrupt when it's ready for a new byte.
      return;
    }
    
    int c = uart_tx_buf[uart_tx_r % UART_TX_BUF_SIZE];
    8000086a:	00011a17          	auipc	s4,0x11
    8000086e:	9dea0a13          	addi	s4,s4,-1570 # 80011248 <uart_tx_lock>
    uart_tx_r += 1;
    80000872:	00008497          	auipc	s1,0x8
    80000876:	79648493          	addi	s1,s1,1942 # 80009008 <uart_tx_r>
    if(uart_tx_w == uart_tx_r){
    8000087a:	00008997          	auipc	s3,0x8
    8000087e:	79698993          	addi	s3,s3,1942 # 80009010 <uart_tx_w>
    if((ReadReg(LSR) & LSR_TX_IDLE) == 0){
    80000882:	00594783          	lbu	a5,5(s2) # 10000005 <_entry-0x6ffffffb>
    80000886:	0ff7f793          	andi	a5,a5,255
    8000088a:	0207f793          	andi	a5,a5,32
    8000088e:	c785                	beqz	a5,800008b6 <uartstart+0x76>
    int c = uart_tx_buf[uart_tx_r % UART_TX_BUF_SIZE];
    80000890:	01f77793          	andi	a5,a4,31
    80000894:	97d2                	add	a5,a5,s4
    80000896:	0187ca83          	lbu	s5,24(a5)
    uart_tx_r += 1;
    8000089a:	0705                	addi	a4,a4,1
    8000089c:	e098                	sd	a4,0(s1)
    
    // maybe uartputc() is waiting for space in the buffer.
    wakeup(&uart_tx_r);
    8000089e:	8526                	mv	a0,s1
    800008a0:	00002097          	auipc	ra,0x2
    800008a4:	d36080e7          	jalr	-714(ra) # 800025d6 <wakeup>
    
    WriteReg(THR, c);
    800008a8:	01590023          	sb	s5,0(s2)
    if(uart_tx_w == uart_tx_r){
    800008ac:	6098                	ld	a4,0(s1)
    800008ae:	0009b783          	ld	a5,0(s3)
    800008b2:	fce798e3          	bne	a5,a4,80000882 <uartstart+0x42>
  }
}
    800008b6:	70e2                	ld	ra,56(sp)
    800008b8:	7442                	ld	s0,48(sp)
    800008ba:	74a2                	ld	s1,40(sp)
    800008bc:	7902                	ld	s2,32(sp)
    800008be:	69e2                	ld	s3,24(sp)
    800008c0:	6a42                	ld	s4,16(sp)
    800008c2:	6aa2                	ld	s5,8(sp)
    800008c4:	6121                	addi	sp,sp,64
    800008c6:	8082                	ret
    800008c8:	8082                	ret

00000000800008ca <uartputc>:
{
    800008ca:	7179                	addi	sp,sp,-48
    800008cc:	f406                	sd	ra,40(sp)
    800008ce:	f022                	sd	s0,32(sp)
    800008d0:	ec26                	sd	s1,24(sp)
    800008d2:	e84a                	sd	s2,16(sp)
    800008d4:	e44e                	sd	s3,8(sp)
    800008d6:	e052                	sd	s4,0(sp)
    800008d8:	1800                	addi	s0,sp,48
    800008da:	89aa                	mv	s3,a0
  acquire(&uart_tx_lock);
    800008dc:	00011517          	auipc	a0,0x11
    800008e0:	96c50513          	addi	a0,a0,-1684 # 80011248 <uart_tx_lock>
    800008e4:	00000097          	auipc	ra,0x0
    800008e8:	300080e7          	jalr	768(ra) # 80000be4 <acquire>
  if(panicked){
    800008ec:	00008797          	auipc	a5,0x8
    800008f0:	7147a783          	lw	a5,1812(a5) # 80009000 <panicked>
    800008f4:	c391                	beqz	a5,800008f8 <uartputc+0x2e>
    for(;;)
    800008f6:	a001                	j	800008f6 <uartputc+0x2c>
    if(uart_tx_w == uart_tx_r + UART_TX_BUF_SIZE){
    800008f8:	00008797          	auipc	a5,0x8
    800008fc:	7187b783          	ld	a5,1816(a5) # 80009010 <uart_tx_w>
    80000900:	00008717          	auipc	a4,0x8
    80000904:	70873703          	ld	a4,1800(a4) # 80009008 <uart_tx_r>
    80000908:	02070713          	addi	a4,a4,32
    8000090c:	02f71b63          	bne	a4,a5,80000942 <uartputc+0x78>
      sleep(&uart_tx_r, &uart_tx_lock);
    80000910:	00011a17          	auipc	s4,0x11
    80000914:	938a0a13          	addi	s4,s4,-1736 # 80011248 <uart_tx_lock>
    80000918:	00008497          	auipc	s1,0x8
    8000091c:	6f048493          	addi	s1,s1,1776 # 80009008 <uart_tx_r>
    if(uart_tx_w == uart_tx_r + UART_TX_BUF_SIZE){
    80000920:	00008917          	auipc	s2,0x8
    80000924:	6f090913          	addi	s2,s2,1776 # 80009010 <uart_tx_w>
      sleep(&uart_tx_r, &uart_tx_lock);
    80000928:	85d2                	mv	a1,s4
    8000092a:	8526                	mv	a0,s1
    8000092c:	00002097          	auipc	ra,0x2
    80000930:	b1e080e7          	jalr	-1250(ra) # 8000244a <sleep>
    if(uart_tx_w == uart_tx_r + UART_TX_BUF_SIZE){
    80000934:	00093783          	ld	a5,0(s2)
    80000938:	6098                	ld	a4,0(s1)
    8000093a:	02070713          	addi	a4,a4,32
    8000093e:	fef705e3          	beq	a4,a5,80000928 <uartputc+0x5e>
      uart_tx_buf[uart_tx_w % UART_TX_BUF_SIZE] = c;
    80000942:	00011497          	auipc	s1,0x11
    80000946:	90648493          	addi	s1,s1,-1786 # 80011248 <uart_tx_lock>
    8000094a:	01f7f713          	andi	a4,a5,31
    8000094e:	9726                	add	a4,a4,s1
    80000950:	01370c23          	sb	s3,24(a4)
      uart_tx_w += 1;
    80000954:	0785                	addi	a5,a5,1
    80000956:	00008717          	auipc	a4,0x8
    8000095a:	6af73d23          	sd	a5,1722(a4) # 80009010 <uart_tx_w>
      uartstart();
    8000095e:	00000097          	auipc	ra,0x0
    80000962:	ee2080e7          	jalr	-286(ra) # 80000840 <uartstart>
      release(&uart_tx_lock);
    80000966:	8526                	mv	a0,s1
    80000968:	00000097          	auipc	ra,0x0
    8000096c:	330080e7          	jalr	816(ra) # 80000c98 <release>
}
    80000970:	70a2                	ld	ra,40(sp)
    80000972:	7402                	ld	s0,32(sp)
    80000974:	64e2                	ld	s1,24(sp)
    80000976:	6942                	ld	s2,16(sp)
    80000978:	69a2                	ld	s3,8(sp)
    8000097a:	6a02                	ld	s4,0(sp)
    8000097c:	6145                	addi	sp,sp,48
    8000097e:	8082                	ret

0000000080000980 <uartgetc>:

// read one input character from the UART.
// return -1 if none is waiting.
int
uartgetc(void)
{
    80000980:	1141                	addi	sp,sp,-16
    80000982:	e422                	sd	s0,8(sp)
    80000984:	0800                	addi	s0,sp,16
  if(ReadReg(LSR) & 0x01){
    80000986:	100007b7          	lui	a5,0x10000
    8000098a:	0057c783          	lbu	a5,5(a5) # 10000005 <_entry-0x6ffffffb>
    8000098e:	8b85                	andi	a5,a5,1
    80000990:	cb91                	beqz	a5,800009a4 <uartgetc+0x24>
    // input data is ready.
    return ReadReg(RHR);
    80000992:	100007b7          	lui	a5,0x10000
    80000996:	0007c503          	lbu	a0,0(a5) # 10000000 <_entry-0x70000000>
    8000099a:	0ff57513          	andi	a0,a0,255
  } else {
    return -1;
  }
}
    8000099e:	6422                	ld	s0,8(sp)
    800009a0:	0141                	addi	sp,sp,16
    800009a2:	8082                	ret
    return -1;
    800009a4:	557d                	li	a0,-1
    800009a6:	bfe5                	j	8000099e <uartgetc+0x1e>

00000000800009a8 <uartintr>:
// handle a uart interrupt, raised because input has
// arrived, or the uart is ready for more output, or
// both. called from trap.c.
void
uartintr(void)
{
    800009a8:	1101                	addi	sp,sp,-32
    800009aa:	ec06                	sd	ra,24(sp)
    800009ac:	e822                	sd	s0,16(sp)
    800009ae:	e426                	sd	s1,8(sp)
    800009b0:	1000                	addi	s0,sp,32
  // read and process incoming characters.
  while(1){
    int c = uartgetc();
    if(c == -1)
    800009b2:	54fd                	li	s1,-1
    int c = uartgetc();
    800009b4:	00000097          	auipc	ra,0x0
    800009b8:	fcc080e7          	jalr	-52(ra) # 80000980 <uartgetc>
    if(c == -1)
    800009bc:	00950763          	beq	a0,s1,800009ca <uartintr+0x22>
      break;
    consoleintr(c);
    800009c0:	00000097          	auipc	ra,0x0
    800009c4:	8fe080e7          	jalr	-1794(ra) # 800002be <consoleintr>
  while(1){
    800009c8:	b7f5                	j	800009b4 <uartintr+0xc>
  }

  // send buffered characters.
  acquire(&uart_tx_lock);
    800009ca:	00011497          	auipc	s1,0x11
    800009ce:	87e48493          	addi	s1,s1,-1922 # 80011248 <uart_tx_lock>
    800009d2:	8526                	mv	a0,s1
    800009d4:	00000097          	auipc	ra,0x0
    800009d8:	210080e7          	jalr	528(ra) # 80000be4 <acquire>
  uartstart();
    800009dc:	00000097          	auipc	ra,0x0
    800009e0:	e64080e7          	jalr	-412(ra) # 80000840 <uartstart>
  release(&uart_tx_lock);
    800009e4:	8526                	mv	a0,s1
    800009e6:	00000097          	auipc	ra,0x0
    800009ea:	2b2080e7          	jalr	690(ra) # 80000c98 <release>
}
    800009ee:	60e2                	ld	ra,24(sp)
    800009f0:	6442                	ld	s0,16(sp)
    800009f2:	64a2                	ld	s1,8(sp)
    800009f4:	6105                	addi	sp,sp,32
    800009f6:	8082                	ret

00000000800009f8 <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(void *pa)
{
    800009f8:	1101                	addi	sp,sp,-32
    800009fa:	ec06                	sd	ra,24(sp)
    800009fc:	e822                	sd	s0,16(sp)
    800009fe:	e426                	sd	s1,8(sp)
    80000a00:	e04a                	sd	s2,0(sp)
    80000a02:	1000                	addi	s0,sp,32
  struct run *r;

  if(((uint64)pa % PGSIZE) != 0 || (char*)pa < end || (uint64)pa >= PHYSTOP)
    80000a04:	03451793          	slli	a5,a0,0x34
    80000a08:	ebb9                	bnez	a5,80000a5e <kfree+0x66>
    80000a0a:	84aa                	mv	s1,a0
    80000a0c:	00025797          	auipc	a5,0x25
    80000a10:	5f478793          	addi	a5,a5,1524 # 80026000 <end>
    80000a14:	04f56563          	bltu	a0,a5,80000a5e <kfree+0x66>
    80000a18:	47c5                	li	a5,17
    80000a1a:	07ee                	slli	a5,a5,0x1b
    80000a1c:	04f57163          	bgeu	a0,a5,80000a5e <kfree+0x66>
    panic("kfree");

  // Fill with junk to catch dangling refs.
  memset(pa, 1, PGSIZE);
    80000a20:	6605                	lui	a2,0x1
    80000a22:	4585                	li	a1,1
    80000a24:	00000097          	auipc	ra,0x0
    80000a28:	2bc080e7          	jalr	700(ra) # 80000ce0 <memset>

  r = (struct run*)pa;

  acquire(&kmem.lock);
    80000a2c:	00011917          	auipc	s2,0x11
    80000a30:	85490913          	addi	s2,s2,-1964 # 80011280 <kmem>
    80000a34:	854a                	mv	a0,s2
    80000a36:	00000097          	auipc	ra,0x0
    80000a3a:	1ae080e7          	jalr	430(ra) # 80000be4 <acquire>
  r->next = kmem.freelist;
    80000a3e:	01893783          	ld	a5,24(s2)
    80000a42:	e09c                	sd	a5,0(s1)
  kmem.freelist = r;
    80000a44:	00993c23          	sd	s1,24(s2)
  release(&kmem.lock);
    80000a48:	854a                	mv	a0,s2
    80000a4a:	00000097          	auipc	ra,0x0
    80000a4e:	24e080e7          	jalr	590(ra) # 80000c98 <release>
}
    80000a52:	60e2                	ld	ra,24(sp)
    80000a54:	6442                	ld	s0,16(sp)
    80000a56:	64a2                	ld	s1,8(sp)
    80000a58:	6902                	ld	s2,0(sp)
    80000a5a:	6105                	addi	sp,sp,32
    80000a5c:	8082                	ret
    panic("kfree");
    80000a5e:	00007517          	auipc	a0,0x7
    80000a62:	60250513          	addi	a0,a0,1538 # 80008060 <digits+0x20>
    80000a66:	00000097          	auipc	ra,0x0
    80000a6a:	ad8080e7          	jalr	-1320(ra) # 8000053e <panic>

0000000080000a6e <freerange>:
{
    80000a6e:	7179                	addi	sp,sp,-48
    80000a70:	f406                	sd	ra,40(sp)
    80000a72:	f022                	sd	s0,32(sp)
    80000a74:	ec26                	sd	s1,24(sp)
    80000a76:	e84a                	sd	s2,16(sp)
    80000a78:	e44e                	sd	s3,8(sp)
    80000a7a:	e052                	sd	s4,0(sp)
    80000a7c:	1800                	addi	s0,sp,48
  p = (char*)PGROUNDUP((uint64)pa_start);
    80000a7e:	6785                	lui	a5,0x1
    80000a80:	fff78493          	addi	s1,a5,-1 # fff <_entry-0x7ffff001>
    80000a84:	94aa                	add	s1,s1,a0
    80000a86:	757d                	lui	a0,0xfffff
    80000a88:	8ce9                	and	s1,s1,a0
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000a8a:	94be                	add	s1,s1,a5
    80000a8c:	0095ee63          	bltu	a1,s1,80000aa8 <freerange+0x3a>
    80000a90:	892e                	mv	s2,a1
    kfree(p);
    80000a92:	7a7d                	lui	s4,0xfffff
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000a94:	6985                	lui	s3,0x1
    kfree(p);
    80000a96:	01448533          	add	a0,s1,s4
    80000a9a:	00000097          	auipc	ra,0x0
    80000a9e:	f5e080e7          	jalr	-162(ra) # 800009f8 <kfree>
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000aa2:	94ce                	add	s1,s1,s3
    80000aa4:	fe9979e3          	bgeu	s2,s1,80000a96 <freerange+0x28>
}
    80000aa8:	70a2                	ld	ra,40(sp)
    80000aaa:	7402                	ld	s0,32(sp)
    80000aac:	64e2                	ld	s1,24(sp)
    80000aae:	6942                	ld	s2,16(sp)
    80000ab0:	69a2                	ld	s3,8(sp)
    80000ab2:	6a02                	ld	s4,0(sp)
    80000ab4:	6145                	addi	sp,sp,48
    80000ab6:	8082                	ret

0000000080000ab8 <kinit>:
{
    80000ab8:	1141                	addi	sp,sp,-16
    80000aba:	e406                	sd	ra,8(sp)
    80000abc:	e022                	sd	s0,0(sp)
    80000abe:	0800                	addi	s0,sp,16
  initlock(&kmem.lock, "kmem");
    80000ac0:	00007597          	auipc	a1,0x7
    80000ac4:	5a858593          	addi	a1,a1,1448 # 80008068 <digits+0x28>
    80000ac8:	00010517          	auipc	a0,0x10
    80000acc:	7b850513          	addi	a0,a0,1976 # 80011280 <kmem>
    80000ad0:	00000097          	auipc	ra,0x0
    80000ad4:	084080e7          	jalr	132(ra) # 80000b54 <initlock>
  freerange(end, (void*)PHYSTOP);
    80000ad8:	45c5                	li	a1,17
    80000ada:	05ee                	slli	a1,a1,0x1b
    80000adc:	00025517          	auipc	a0,0x25
    80000ae0:	52450513          	addi	a0,a0,1316 # 80026000 <end>
    80000ae4:	00000097          	auipc	ra,0x0
    80000ae8:	f8a080e7          	jalr	-118(ra) # 80000a6e <freerange>
}
    80000aec:	60a2                	ld	ra,8(sp)
    80000aee:	6402                	ld	s0,0(sp)
    80000af0:	0141                	addi	sp,sp,16
    80000af2:	8082                	ret

0000000080000af4 <kalloc>:
// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
void *
kalloc(void)
{
    80000af4:	1101                	addi	sp,sp,-32
    80000af6:	ec06                	sd	ra,24(sp)
    80000af8:	e822                	sd	s0,16(sp)
    80000afa:	e426                	sd	s1,8(sp)
    80000afc:	1000                	addi	s0,sp,32
  struct run *r;

  acquire(&kmem.lock);
    80000afe:	00010497          	auipc	s1,0x10
    80000b02:	78248493          	addi	s1,s1,1922 # 80011280 <kmem>
    80000b06:	8526                	mv	a0,s1
    80000b08:	00000097          	auipc	ra,0x0
    80000b0c:	0dc080e7          	jalr	220(ra) # 80000be4 <acquire>
  r = kmem.freelist;
    80000b10:	6c84                	ld	s1,24(s1)
  if(r)
    80000b12:	c885                	beqz	s1,80000b42 <kalloc+0x4e>
    kmem.freelist = r->next;
    80000b14:	609c                	ld	a5,0(s1)
    80000b16:	00010517          	auipc	a0,0x10
    80000b1a:	76a50513          	addi	a0,a0,1898 # 80011280 <kmem>
    80000b1e:	ed1c                	sd	a5,24(a0)
  release(&kmem.lock);
    80000b20:	00000097          	auipc	ra,0x0
    80000b24:	178080e7          	jalr	376(ra) # 80000c98 <release>

  if(r)
    memset((char*)r, 5, PGSIZE); // fill with junk
    80000b28:	6605                	lui	a2,0x1
    80000b2a:	4595                	li	a1,5
    80000b2c:	8526                	mv	a0,s1
    80000b2e:	00000097          	auipc	ra,0x0
    80000b32:	1b2080e7          	jalr	434(ra) # 80000ce0 <memset>
  return (void*)r;
}
    80000b36:	8526                	mv	a0,s1
    80000b38:	60e2                	ld	ra,24(sp)
    80000b3a:	6442                	ld	s0,16(sp)
    80000b3c:	64a2                	ld	s1,8(sp)
    80000b3e:	6105                	addi	sp,sp,32
    80000b40:	8082                	ret
  release(&kmem.lock);
    80000b42:	00010517          	auipc	a0,0x10
    80000b46:	73e50513          	addi	a0,a0,1854 # 80011280 <kmem>
    80000b4a:	00000097          	auipc	ra,0x0
    80000b4e:	14e080e7          	jalr	334(ra) # 80000c98 <release>
  if(r)
    80000b52:	b7d5                	j	80000b36 <kalloc+0x42>

0000000080000b54 <initlock>:
#include "proc.h"
#include "defs.h"

void
initlock(struct spinlock *lk, char *name)
{
    80000b54:	1141                	addi	sp,sp,-16
    80000b56:	e422                	sd	s0,8(sp)
    80000b58:	0800                	addi	s0,sp,16
  lk->name = name;
    80000b5a:	e50c                	sd	a1,8(a0)
  lk->locked = 0;
    80000b5c:	00052023          	sw	zero,0(a0)
  lk->cpu = 0;
    80000b60:	00053823          	sd	zero,16(a0)
}
    80000b64:	6422                	ld	s0,8(sp)
    80000b66:	0141                	addi	sp,sp,16
    80000b68:	8082                	ret

0000000080000b6a <holding>:
// Interrupts must be off.
int
holding(struct spinlock *lk)
{
  int r;
  r = (lk->locked && lk->cpu == mycpu());
    80000b6a:	411c                	lw	a5,0(a0)
    80000b6c:	e399                	bnez	a5,80000b72 <holding+0x8>
    80000b6e:	4501                	li	a0,0
  return r;
}
    80000b70:	8082                	ret
{
    80000b72:	1101                	addi	sp,sp,-32
    80000b74:	ec06                	sd	ra,24(sp)
    80000b76:	e822                	sd	s0,16(sp)
    80000b78:	e426                	sd	s1,8(sp)
    80000b7a:	1000                	addi	s0,sp,32
  r = (lk->locked && lk->cpu == mycpu());
    80000b7c:	6904                	ld	s1,16(a0)
    80000b7e:	00001097          	auipc	ra,0x1
    80000b82:	1f2080e7          	jalr	498(ra) # 80001d70 <mycpu>
    80000b86:	40a48533          	sub	a0,s1,a0
    80000b8a:	00153513          	seqz	a0,a0
}
    80000b8e:	60e2                	ld	ra,24(sp)
    80000b90:	6442                	ld	s0,16(sp)
    80000b92:	64a2                	ld	s1,8(sp)
    80000b94:	6105                	addi	sp,sp,32
    80000b96:	8082                	ret

0000000080000b98 <push_off>:
// it takes two pop_off()s to undo two push_off()s.  Also, if interrupts
// are initially off, then push_off, pop_off leaves them off.

void
push_off(void)
{
    80000b98:	1101                	addi	sp,sp,-32
    80000b9a:	ec06                	sd	ra,24(sp)
    80000b9c:	e822                	sd	s0,16(sp)
    80000b9e:	e426                	sd	s1,8(sp)
    80000ba0:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000ba2:	100024f3          	csrr	s1,sstatus
    80000ba6:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() & ~SSTATUS_SIE);
    80000baa:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80000bac:	10079073          	csrw	sstatus,a5
  int old = intr_get();

  intr_off();
  if(mycpu()->noff == 0)
    80000bb0:	00001097          	auipc	ra,0x1
    80000bb4:	1c0080e7          	jalr	448(ra) # 80001d70 <mycpu>
    80000bb8:	5d3c                	lw	a5,120(a0)
    80000bba:	cf89                	beqz	a5,80000bd4 <push_off+0x3c>
    mycpu()->intena = old;
  mycpu()->noff += 1;
    80000bbc:	00001097          	auipc	ra,0x1
    80000bc0:	1b4080e7          	jalr	436(ra) # 80001d70 <mycpu>
    80000bc4:	5d3c                	lw	a5,120(a0)
    80000bc6:	2785                	addiw	a5,a5,1
    80000bc8:	dd3c                	sw	a5,120(a0)
}
    80000bca:	60e2                	ld	ra,24(sp)
    80000bcc:	6442                	ld	s0,16(sp)
    80000bce:	64a2                	ld	s1,8(sp)
    80000bd0:	6105                	addi	sp,sp,32
    80000bd2:	8082                	ret
    mycpu()->intena = old;
    80000bd4:	00001097          	auipc	ra,0x1
    80000bd8:	19c080e7          	jalr	412(ra) # 80001d70 <mycpu>
  return (x & SSTATUS_SIE) != 0;
    80000bdc:	8085                	srli	s1,s1,0x1
    80000bde:	8885                	andi	s1,s1,1
    80000be0:	dd64                	sw	s1,124(a0)
    80000be2:	bfe9                	j	80000bbc <push_off+0x24>

0000000080000be4 <acquire>:
{
    80000be4:	1101                	addi	sp,sp,-32
    80000be6:	ec06                	sd	ra,24(sp)
    80000be8:	e822                	sd	s0,16(sp)
    80000bea:	e426                	sd	s1,8(sp)
    80000bec:	1000                	addi	s0,sp,32
    80000bee:	84aa                	mv	s1,a0
  push_off(); // disable interrupts to avoid deadlock.
    80000bf0:	00000097          	auipc	ra,0x0
    80000bf4:	fa8080e7          	jalr	-88(ra) # 80000b98 <push_off>
  if(holding(lk))
    80000bf8:	8526                	mv	a0,s1
    80000bfa:	00000097          	auipc	ra,0x0
    80000bfe:	f70080e7          	jalr	-144(ra) # 80000b6a <holding>
  while(__sync_lock_test_and_set(&lk->locked, 1) != 0)
    80000c02:	4705                	li	a4,1
  if(holding(lk))
    80000c04:	e115                	bnez	a0,80000c28 <acquire+0x44>
  while(__sync_lock_test_and_set(&lk->locked, 1) != 0)
    80000c06:	87ba                	mv	a5,a4
    80000c08:	0cf4a7af          	amoswap.w.aq	a5,a5,(s1)
    80000c0c:	2781                	sext.w	a5,a5
    80000c0e:	ffe5                	bnez	a5,80000c06 <acquire+0x22>
  __sync_synchronize();
    80000c10:	0ff0000f          	fence
  lk->cpu = mycpu();
    80000c14:	00001097          	auipc	ra,0x1
    80000c18:	15c080e7          	jalr	348(ra) # 80001d70 <mycpu>
    80000c1c:	e888                	sd	a0,16(s1)
}
    80000c1e:	60e2                	ld	ra,24(sp)
    80000c20:	6442                	ld	s0,16(sp)
    80000c22:	64a2                	ld	s1,8(sp)
    80000c24:	6105                	addi	sp,sp,32
    80000c26:	8082                	ret
    panic("acquire");
    80000c28:	00007517          	auipc	a0,0x7
    80000c2c:	44850513          	addi	a0,a0,1096 # 80008070 <digits+0x30>
    80000c30:	00000097          	auipc	ra,0x0
    80000c34:	90e080e7          	jalr	-1778(ra) # 8000053e <panic>

0000000080000c38 <pop_off>:

void
pop_off(void)
{
    80000c38:	1141                	addi	sp,sp,-16
    80000c3a:	e406                	sd	ra,8(sp)
    80000c3c:	e022                	sd	s0,0(sp)
    80000c3e:	0800                	addi	s0,sp,16
  struct cpu *c = mycpu();
    80000c40:	00001097          	auipc	ra,0x1
    80000c44:	130080e7          	jalr	304(ra) # 80001d70 <mycpu>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000c48:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    80000c4c:	8b89                	andi	a5,a5,2
  if(intr_get())
    80000c4e:	e78d                	bnez	a5,80000c78 <pop_off+0x40>
    panic("pop_off - interruptible");
  if(c->noff < 1)
    80000c50:	5d3c                	lw	a5,120(a0)
    80000c52:	02f05b63          	blez	a5,80000c88 <pop_off+0x50>
    panic("pop_off");
  c->noff -= 1;
    80000c56:	37fd                	addiw	a5,a5,-1
    80000c58:	0007871b          	sext.w	a4,a5
    80000c5c:	dd3c                	sw	a5,120(a0)
  if(c->noff == 0 && c->intena)
    80000c5e:	eb09                	bnez	a4,80000c70 <pop_off+0x38>
    80000c60:	5d7c                	lw	a5,124(a0)
    80000c62:	c799                	beqz	a5,80000c70 <pop_off+0x38>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000c64:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80000c68:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80000c6c:	10079073          	csrw	sstatus,a5
    intr_on();
}
    80000c70:	60a2                	ld	ra,8(sp)
    80000c72:	6402                	ld	s0,0(sp)
    80000c74:	0141                	addi	sp,sp,16
    80000c76:	8082                	ret
    panic("pop_off - interruptible");
    80000c78:	00007517          	auipc	a0,0x7
    80000c7c:	40050513          	addi	a0,a0,1024 # 80008078 <digits+0x38>
    80000c80:	00000097          	auipc	ra,0x0
    80000c84:	8be080e7          	jalr	-1858(ra) # 8000053e <panic>
    panic("pop_off");
    80000c88:	00007517          	auipc	a0,0x7
    80000c8c:	40850513          	addi	a0,a0,1032 # 80008090 <digits+0x50>
    80000c90:	00000097          	auipc	ra,0x0
    80000c94:	8ae080e7          	jalr	-1874(ra) # 8000053e <panic>

0000000080000c98 <release>:
{
    80000c98:	1101                	addi	sp,sp,-32
    80000c9a:	ec06                	sd	ra,24(sp)
    80000c9c:	e822                	sd	s0,16(sp)
    80000c9e:	e426                	sd	s1,8(sp)
    80000ca0:	1000                	addi	s0,sp,32
    80000ca2:	84aa                	mv	s1,a0
  if(!holding(lk))
    80000ca4:	00000097          	auipc	ra,0x0
    80000ca8:	ec6080e7          	jalr	-314(ra) # 80000b6a <holding>
    80000cac:	c115                	beqz	a0,80000cd0 <release+0x38>
  lk->cpu = 0;
    80000cae:	0004b823          	sd	zero,16(s1)
  __sync_synchronize();
    80000cb2:	0ff0000f          	fence
  __sync_lock_release(&lk->locked);
    80000cb6:	0f50000f          	fence	iorw,ow
    80000cba:	0804a02f          	amoswap.w	zero,zero,(s1)
  pop_off();
    80000cbe:	00000097          	auipc	ra,0x0
    80000cc2:	f7a080e7          	jalr	-134(ra) # 80000c38 <pop_off>
}
    80000cc6:	60e2                	ld	ra,24(sp)
    80000cc8:	6442                	ld	s0,16(sp)
    80000cca:	64a2                	ld	s1,8(sp)
    80000ccc:	6105                	addi	sp,sp,32
    80000cce:	8082                	ret
    panic("release");
    80000cd0:	00007517          	auipc	a0,0x7
    80000cd4:	3c850513          	addi	a0,a0,968 # 80008098 <digits+0x58>
    80000cd8:	00000097          	auipc	ra,0x0
    80000cdc:	866080e7          	jalr	-1946(ra) # 8000053e <panic>

0000000080000ce0 <memset>:
#include "types.h"

void*
memset(void *dst, int c, uint n)
{
    80000ce0:	1141                	addi	sp,sp,-16
    80000ce2:	e422                	sd	s0,8(sp)
    80000ce4:	0800                	addi	s0,sp,16
  char *cdst = (char *) dst;
  int i;
  for(i = 0; i < n; i++){
    80000ce6:	ce09                	beqz	a2,80000d00 <memset+0x20>
    80000ce8:	87aa                	mv	a5,a0
    80000cea:	fff6071b          	addiw	a4,a2,-1
    80000cee:	1702                	slli	a4,a4,0x20
    80000cf0:	9301                	srli	a4,a4,0x20
    80000cf2:	0705                	addi	a4,a4,1
    80000cf4:	972a                	add	a4,a4,a0
    cdst[i] = c;
    80000cf6:	00b78023          	sb	a1,0(a5)
  for(i = 0; i < n; i++){
    80000cfa:	0785                	addi	a5,a5,1
    80000cfc:	fee79de3          	bne	a5,a4,80000cf6 <memset+0x16>
  }
  return dst;
}
    80000d00:	6422                	ld	s0,8(sp)
    80000d02:	0141                	addi	sp,sp,16
    80000d04:	8082                	ret

0000000080000d06 <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
    80000d06:	1141                	addi	sp,sp,-16
    80000d08:	e422                	sd	s0,8(sp)
    80000d0a:	0800                	addi	s0,sp,16
  const uchar *s1, *s2;

  s1 = v1;
  s2 = v2;
  while(n-- > 0){
    80000d0c:	ca05                	beqz	a2,80000d3c <memcmp+0x36>
    80000d0e:	fff6069b          	addiw	a3,a2,-1
    80000d12:	1682                	slli	a3,a3,0x20
    80000d14:	9281                	srli	a3,a3,0x20
    80000d16:	0685                	addi	a3,a3,1
    80000d18:	96aa                	add	a3,a3,a0
    if(*s1 != *s2)
    80000d1a:	00054783          	lbu	a5,0(a0)
    80000d1e:	0005c703          	lbu	a4,0(a1)
    80000d22:	00e79863          	bne	a5,a4,80000d32 <memcmp+0x2c>
      return *s1 - *s2;
    s1++, s2++;
    80000d26:	0505                	addi	a0,a0,1
    80000d28:	0585                	addi	a1,a1,1
  while(n-- > 0){
    80000d2a:	fed518e3          	bne	a0,a3,80000d1a <memcmp+0x14>
  }

  return 0;
    80000d2e:	4501                	li	a0,0
    80000d30:	a019                	j	80000d36 <memcmp+0x30>
      return *s1 - *s2;
    80000d32:	40e7853b          	subw	a0,a5,a4
}
    80000d36:	6422                	ld	s0,8(sp)
    80000d38:	0141                	addi	sp,sp,16
    80000d3a:	8082                	ret
  return 0;
    80000d3c:	4501                	li	a0,0
    80000d3e:	bfe5                	j	80000d36 <memcmp+0x30>

0000000080000d40 <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
    80000d40:	1141                	addi	sp,sp,-16
    80000d42:	e422                	sd	s0,8(sp)
    80000d44:	0800                	addi	s0,sp,16
  const char *s;
  char *d;

  if(n == 0)
    80000d46:	ca0d                	beqz	a2,80000d78 <memmove+0x38>
    return dst;
  
  s = src;
  d = dst;
  if(s < d && s + n > d){
    80000d48:	00a5f963          	bgeu	a1,a0,80000d5a <memmove+0x1a>
    80000d4c:	02061693          	slli	a3,a2,0x20
    80000d50:	9281                	srli	a3,a3,0x20
    80000d52:	00d58733          	add	a4,a1,a3
    80000d56:	02e56463          	bltu	a0,a4,80000d7e <memmove+0x3e>
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
    80000d5a:	fff6079b          	addiw	a5,a2,-1
    80000d5e:	1782                	slli	a5,a5,0x20
    80000d60:	9381                	srli	a5,a5,0x20
    80000d62:	0785                	addi	a5,a5,1
    80000d64:	97ae                	add	a5,a5,a1
    80000d66:	872a                	mv	a4,a0
      *d++ = *s++;
    80000d68:	0585                	addi	a1,a1,1
    80000d6a:	0705                	addi	a4,a4,1
    80000d6c:	fff5c683          	lbu	a3,-1(a1)
    80000d70:	fed70fa3          	sb	a3,-1(a4)
    while(n-- > 0)
    80000d74:	fef59ae3          	bne	a1,a5,80000d68 <memmove+0x28>

  return dst;
}
    80000d78:	6422                	ld	s0,8(sp)
    80000d7a:	0141                	addi	sp,sp,16
    80000d7c:	8082                	ret
    d += n;
    80000d7e:	96aa                	add	a3,a3,a0
    while(n-- > 0)
    80000d80:	fff6079b          	addiw	a5,a2,-1
    80000d84:	1782                	slli	a5,a5,0x20
    80000d86:	9381                	srli	a5,a5,0x20
    80000d88:	fff7c793          	not	a5,a5
    80000d8c:	97ba                	add	a5,a5,a4
      *--d = *--s;
    80000d8e:	177d                	addi	a4,a4,-1
    80000d90:	16fd                	addi	a3,a3,-1
    80000d92:	00074603          	lbu	a2,0(a4)
    80000d96:	00c68023          	sb	a2,0(a3)
    while(n-- > 0)
    80000d9a:	fef71ae3          	bne	a4,a5,80000d8e <memmove+0x4e>
    80000d9e:	bfe9                	j	80000d78 <memmove+0x38>

0000000080000da0 <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
    80000da0:	1141                	addi	sp,sp,-16
    80000da2:	e406                	sd	ra,8(sp)
    80000da4:	e022                	sd	s0,0(sp)
    80000da6:	0800                	addi	s0,sp,16
  return memmove(dst, src, n);
    80000da8:	00000097          	auipc	ra,0x0
    80000dac:	f98080e7          	jalr	-104(ra) # 80000d40 <memmove>
}
    80000db0:	60a2                	ld	ra,8(sp)
    80000db2:	6402                	ld	s0,0(sp)
    80000db4:	0141                	addi	sp,sp,16
    80000db6:	8082                	ret

0000000080000db8 <strncmp>:

int
strncmp(const char *p, const char *q, uint n)
{
    80000db8:	1141                	addi	sp,sp,-16
    80000dba:	e422                	sd	s0,8(sp)
    80000dbc:	0800                	addi	s0,sp,16
  while(n > 0 && *p && *p == *q)
    80000dbe:	ce11                	beqz	a2,80000dda <strncmp+0x22>
    80000dc0:	00054783          	lbu	a5,0(a0)
    80000dc4:	cf89                	beqz	a5,80000dde <strncmp+0x26>
    80000dc6:	0005c703          	lbu	a4,0(a1)
    80000dca:	00f71a63          	bne	a4,a5,80000dde <strncmp+0x26>
    n--, p++, q++;
    80000dce:	367d                	addiw	a2,a2,-1
    80000dd0:	0505                	addi	a0,a0,1
    80000dd2:	0585                	addi	a1,a1,1
  while(n > 0 && *p && *p == *q)
    80000dd4:	f675                	bnez	a2,80000dc0 <strncmp+0x8>
  if(n == 0)
    return 0;
    80000dd6:	4501                	li	a0,0
    80000dd8:	a809                	j	80000dea <strncmp+0x32>
    80000dda:	4501                	li	a0,0
    80000ddc:	a039                	j	80000dea <strncmp+0x32>
  if(n == 0)
    80000dde:	ca09                	beqz	a2,80000df0 <strncmp+0x38>
  return (uchar)*p - (uchar)*q;
    80000de0:	00054503          	lbu	a0,0(a0)
    80000de4:	0005c783          	lbu	a5,0(a1)
    80000de8:	9d1d                	subw	a0,a0,a5
}
    80000dea:	6422                	ld	s0,8(sp)
    80000dec:	0141                	addi	sp,sp,16
    80000dee:	8082                	ret
    return 0;
    80000df0:	4501                	li	a0,0
    80000df2:	bfe5                	j	80000dea <strncmp+0x32>

0000000080000df4 <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
    80000df4:	1141                	addi	sp,sp,-16
    80000df6:	e422                	sd	s0,8(sp)
    80000df8:	0800                	addi	s0,sp,16
  char *os;

  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
    80000dfa:	872a                	mv	a4,a0
    80000dfc:	8832                	mv	a6,a2
    80000dfe:	367d                	addiw	a2,a2,-1
    80000e00:	01005963          	blez	a6,80000e12 <strncpy+0x1e>
    80000e04:	0705                	addi	a4,a4,1
    80000e06:	0005c783          	lbu	a5,0(a1)
    80000e0a:	fef70fa3          	sb	a5,-1(a4)
    80000e0e:	0585                	addi	a1,a1,1
    80000e10:	f7f5                	bnez	a5,80000dfc <strncpy+0x8>
    ;
  while(n-- > 0)
    80000e12:	00c05d63          	blez	a2,80000e2c <strncpy+0x38>
    80000e16:	86ba                	mv	a3,a4
    *s++ = 0;
    80000e18:	0685                	addi	a3,a3,1
    80000e1a:	fe068fa3          	sb	zero,-1(a3)
  while(n-- > 0)
    80000e1e:	fff6c793          	not	a5,a3
    80000e22:	9fb9                	addw	a5,a5,a4
    80000e24:	010787bb          	addw	a5,a5,a6
    80000e28:	fef048e3          	bgtz	a5,80000e18 <strncpy+0x24>
  return os;
}
    80000e2c:	6422                	ld	s0,8(sp)
    80000e2e:	0141                	addi	sp,sp,16
    80000e30:	8082                	ret

0000000080000e32 <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
    80000e32:	1141                	addi	sp,sp,-16
    80000e34:	e422                	sd	s0,8(sp)
    80000e36:	0800                	addi	s0,sp,16
  char *os;

  os = s;
  if(n <= 0)
    80000e38:	02c05363          	blez	a2,80000e5e <safestrcpy+0x2c>
    80000e3c:	fff6069b          	addiw	a3,a2,-1
    80000e40:	1682                	slli	a3,a3,0x20
    80000e42:	9281                	srli	a3,a3,0x20
    80000e44:	96ae                	add	a3,a3,a1
    80000e46:	87aa                	mv	a5,a0
    return os;
  while(--n > 0 && (*s++ = *t++) != 0)
    80000e48:	00d58963          	beq	a1,a3,80000e5a <safestrcpy+0x28>
    80000e4c:	0585                	addi	a1,a1,1
    80000e4e:	0785                	addi	a5,a5,1
    80000e50:	fff5c703          	lbu	a4,-1(a1)
    80000e54:	fee78fa3          	sb	a4,-1(a5)
    80000e58:	fb65                	bnez	a4,80000e48 <safestrcpy+0x16>
    ;
  *s = 0;
    80000e5a:	00078023          	sb	zero,0(a5)
  return os;
}
    80000e5e:	6422                	ld	s0,8(sp)
    80000e60:	0141                	addi	sp,sp,16
    80000e62:	8082                	ret

0000000080000e64 <strlen>:

int
strlen(const char *s)
{
    80000e64:	1141                	addi	sp,sp,-16
    80000e66:	e422                	sd	s0,8(sp)
    80000e68:	0800                	addi	s0,sp,16
  int n;

  for(n = 0; s[n]; n++)
    80000e6a:	00054783          	lbu	a5,0(a0)
    80000e6e:	cf91                	beqz	a5,80000e8a <strlen+0x26>
    80000e70:	0505                	addi	a0,a0,1
    80000e72:	87aa                	mv	a5,a0
    80000e74:	4685                	li	a3,1
    80000e76:	9e89                	subw	a3,a3,a0
    80000e78:	00f6853b          	addw	a0,a3,a5
    80000e7c:	0785                	addi	a5,a5,1
    80000e7e:	fff7c703          	lbu	a4,-1(a5)
    80000e82:	fb7d                	bnez	a4,80000e78 <strlen+0x14>
    ;
  return n;
}
    80000e84:	6422                	ld	s0,8(sp)
    80000e86:	0141                	addi	sp,sp,16
    80000e88:	8082                	ret
  for(n = 0; s[n]; n++)
    80000e8a:	4501                	li	a0,0
    80000e8c:	bfe5                	j	80000e84 <strlen+0x20>

0000000080000e8e <main>:
volatile static int started = 0;

// start() jumps here in supervisor mode on all CPUs.
void
main()
{
    80000e8e:	1141                	addi	sp,sp,-16
    80000e90:	e406                	sd	ra,8(sp)
    80000e92:	e022                	sd	s0,0(sp)
    80000e94:	0800                	addi	s0,sp,16
  if(cpuid() == 0){
    80000e96:	00001097          	auipc	ra,0x1
    80000e9a:	eca080e7          	jalr	-310(ra) # 80001d60 <cpuid>
    virtio_disk_init(); // emulated hard disk
    userinit();      // first user process
    __sync_synchronize();
    started = 1;
  } else {
    while(started == 0)
    80000e9e:	00008717          	auipc	a4,0x8
    80000ea2:	17a70713          	addi	a4,a4,378 # 80009018 <started>
  if(cpuid() == 0){
    80000ea6:	c139                	beqz	a0,80000eec <main+0x5e>
    while(started == 0)
    80000ea8:	431c                	lw	a5,0(a4)
    80000eaa:	2781                	sext.w	a5,a5
    80000eac:	dff5                	beqz	a5,80000ea8 <main+0x1a>
      ;
    __sync_synchronize();
    80000eae:	0ff0000f          	fence
    printf("hart %d starting\n", cpuid());
    80000eb2:	00001097          	auipc	ra,0x1
    80000eb6:	eae080e7          	jalr	-338(ra) # 80001d60 <cpuid>
    80000eba:	85aa                	mv	a1,a0
    80000ebc:	00007517          	auipc	a0,0x7
    80000ec0:	1fc50513          	addi	a0,a0,508 # 800080b8 <digits+0x78>
    80000ec4:	fffff097          	auipc	ra,0xfffff
    80000ec8:	6c4080e7          	jalr	1732(ra) # 80000588 <printf>
    kvminithart();    // turn on paging
    80000ecc:	00000097          	auipc	ra,0x0
    80000ed0:	0d8080e7          	jalr	216(ra) # 80000fa4 <kvminithart>
    trapinithart();   // install kernel trap vector
    80000ed4:	00002097          	auipc	ra,0x2
    80000ed8:	b90080e7          	jalr	-1136(ra) # 80002a64 <trapinithart>
    plicinithart();   // ask PLIC for device interrupts
    80000edc:	00005097          	auipc	ra,0x5
    80000ee0:	0f4080e7          	jalr	244(ra) # 80005fd0 <plicinithart>
  }

  scheduler();        
    80000ee4:	00001097          	auipc	ra,0x1
    80000ee8:	3b4080e7          	jalr	948(ra) # 80002298 <scheduler>
    consoleinit();
    80000eec:	fffff097          	auipc	ra,0xfffff
    80000ef0:	564080e7          	jalr	1380(ra) # 80000450 <consoleinit>
    printfinit();
    80000ef4:	00000097          	auipc	ra,0x0
    80000ef8:	87a080e7          	jalr	-1926(ra) # 8000076e <printfinit>
    printf("\n");
    80000efc:	00007517          	auipc	a0,0x7
    80000f00:	1cc50513          	addi	a0,a0,460 # 800080c8 <digits+0x88>
    80000f04:	fffff097          	auipc	ra,0xfffff
    80000f08:	684080e7          	jalr	1668(ra) # 80000588 <printf>
    printf("xv6 kernel is booting\n");
    80000f0c:	00007517          	auipc	a0,0x7
    80000f10:	19450513          	addi	a0,a0,404 # 800080a0 <digits+0x60>
    80000f14:	fffff097          	auipc	ra,0xfffff
    80000f18:	674080e7          	jalr	1652(ra) # 80000588 <printf>
    printf("\n");
    80000f1c:	00007517          	auipc	a0,0x7
    80000f20:	1ac50513          	addi	a0,a0,428 # 800080c8 <digits+0x88>
    80000f24:	fffff097          	auipc	ra,0xfffff
    80000f28:	664080e7          	jalr	1636(ra) # 80000588 <printf>
    kinit();         // physical page allocator
    80000f2c:	00000097          	auipc	ra,0x0
    80000f30:	b8c080e7          	jalr	-1140(ra) # 80000ab8 <kinit>
    kvminit();       // create kernel page table
    80000f34:	00000097          	auipc	ra,0x0
    80000f38:	322080e7          	jalr	802(ra) # 80001256 <kvminit>
    kvminithart();   // turn on paging
    80000f3c:	00000097          	auipc	ra,0x0
    80000f40:	068080e7          	jalr	104(ra) # 80000fa4 <kvminithart>
    procinit();      // process table
    80000f44:	00001097          	auipc	ra,0x1
    80000f48:	d42080e7          	jalr	-702(ra) # 80001c86 <procinit>
    trapinit();      // trap vectors
    80000f4c:	00002097          	auipc	ra,0x2
    80000f50:	af0080e7          	jalr	-1296(ra) # 80002a3c <trapinit>
    trapinithart();  // install kernel trap vector
    80000f54:	00002097          	auipc	ra,0x2
    80000f58:	b10080e7          	jalr	-1264(ra) # 80002a64 <trapinithart>
    plicinit();      // set up interrupt controller
    80000f5c:	00005097          	auipc	ra,0x5
    80000f60:	05e080e7          	jalr	94(ra) # 80005fba <plicinit>
    plicinithart();  // ask PLIC for device interrupts
    80000f64:	00005097          	auipc	ra,0x5
    80000f68:	06c080e7          	jalr	108(ra) # 80005fd0 <plicinithart>
    binit();         // buffer cache
    80000f6c:	00002097          	auipc	ra,0x2
    80000f70:	252080e7          	jalr	594(ra) # 800031be <binit>
    iinit();         // inode table
    80000f74:	00003097          	auipc	ra,0x3
    80000f78:	8e2080e7          	jalr	-1822(ra) # 80003856 <iinit>
    fileinit();      // file table
    80000f7c:	00004097          	auipc	ra,0x4
    80000f80:	88c080e7          	jalr	-1908(ra) # 80004808 <fileinit>
    virtio_disk_init(); // emulated hard disk
    80000f84:	00005097          	auipc	ra,0x5
    80000f88:	16e080e7          	jalr	366(ra) # 800060f2 <virtio_disk_init>
    userinit();      // first user process
    80000f8c:	00001097          	auipc	ra,0x1
    80000f90:	0d2080e7          	jalr	210(ra) # 8000205e <userinit>
    __sync_synchronize();
    80000f94:	0ff0000f          	fence
    started = 1;
    80000f98:	4785                	li	a5,1
    80000f9a:	00008717          	auipc	a4,0x8
    80000f9e:	06f72f23          	sw	a5,126(a4) # 80009018 <started>
    80000fa2:	b789                	j	80000ee4 <main+0x56>

0000000080000fa4 <kvminithart>:

// Switch h/w page table register to the kernel's page table,
// and enable paging.
void
kvminithart()
{
    80000fa4:	1141                	addi	sp,sp,-16
    80000fa6:	e422                	sd	s0,8(sp)
    80000fa8:	0800                	addi	s0,sp,16
  w_satp(MAKE_SATP(kernel_pagetable));
    80000faa:	00008797          	auipc	a5,0x8
    80000fae:	0767b783          	ld	a5,118(a5) # 80009020 <kernel_pagetable>
    80000fb2:	83b1                	srli	a5,a5,0xc
    80000fb4:	577d                	li	a4,-1
    80000fb6:	177e                	slli	a4,a4,0x3f
    80000fb8:	8fd9                	or	a5,a5,a4
  asm volatile("csrw satp, %0" : : "r" (x));
    80000fba:	18079073          	csrw	satp,a5
// flush the TLB.
static inline void
sfence_vma()
{
  // the zero, zero means flush all TLB entries.
  asm volatile("sfence.vma zero, zero");
    80000fbe:	12000073          	sfence.vma
  sfence_vma();
}
    80000fc2:	6422                	ld	s0,8(sp)
    80000fc4:	0141                	addi	sp,sp,16
    80000fc6:	8082                	ret

0000000080000fc8 <walk>:
//   21..29 -- 9 bits of level-1 index.
//   12..20 -- 9 bits of level-0 index.
//    0..11 -- 12 bits of byte offset within the page.
pte_t *
walk(pagetable_t pagetable, uint64 va, int alloc)
{
    80000fc8:	7139                	addi	sp,sp,-64
    80000fca:	fc06                	sd	ra,56(sp)
    80000fcc:	f822                	sd	s0,48(sp)
    80000fce:	f426                	sd	s1,40(sp)
    80000fd0:	f04a                	sd	s2,32(sp)
    80000fd2:	ec4e                	sd	s3,24(sp)
    80000fd4:	e852                	sd	s4,16(sp)
    80000fd6:	e456                	sd	s5,8(sp)
    80000fd8:	e05a                	sd	s6,0(sp)
    80000fda:	0080                	addi	s0,sp,64
    80000fdc:	84aa                	mv	s1,a0
    80000fde:	89ae                	mv	s3,a1
    80000fe0:	8ab2                	mv	s5,a2
  if(va >= MAXVA)
    80000fe2:	57fd                	li	a5,-1
    80000fe4:	83e9                	srli	a5,a5,0x1a
    80000fe6:	4a79                	li	s4,30
    panic("walk");

  for(int level = 2; level > 0; level--) {
    80000fe8:	4b31                	li	s6,12
  if(va >= MAXVA)
    80000fea:	04b7f263          	bgeu	a5,a1,8000102e <walk+0x66>
    panic("walk");
    80000fee:	00007517          	auipc	a0,0x7
    80000ff2:	0e250513          	addi	a0,a0,226 # 800080d0 <digits+0x90>
    80000ff6:	fffff097          	auipc	ra,0xfffff
    80000ffa:	548080e7          	jalr	1352(ra) # 8000053e <panic>
    pte_t *pte = &pagetable[PX(level, va)];
    if(*pte & PTE_V) {
      pagetable = (pagetable_t)PTE2PA(*pte);
    } else {
      if(!alloc || (pagetable = (pde_t*)kalloc()) == 0)
    80000ffe:	060a8663          	beqz	s5,8000106a <walk+0xa2>
    80001002:	00000097          	auipc	ra,0x0
    80001006:	af2080e7          	jalr	-1294(ra) # 80000af4 <kalloc>
    8000100a:	84aa                	mv	s1,a0
    8000100c:	c529                	beqz	a0,80001056 <walk+0x8e>
        return 0;
      memset(pagetable, 0, PGSIZE);
    8000100e:	6605                	lui	a2,0x1
    80001010:	4581                	li	a1,0
    80001012:	00000097          	auipc	ra,0x0
    80001016:	cce080e7          	jalr	-818(ra) # 80000ce0 <memset>
      *pte = PA2PTE(pagetable) | PTE_V;
    8000101a:	00c4d793          	srli	a5,s1,0xc
    8000101e:	07aa                	slli	a5,a5,0xa
    80001020:	0017e793          	ori	a5,a5,1
    80001024:	00f93023          	sd	a5,0(s2)
  for(int level = 2; level > 0; level--) {
    80001028:	3a5d                	addiw	s4,s4,-9
    8000102a:	036a0063          	beq	s4,s6,8000104a <walk+0x82>
    pte_t *pte = &pagetable[PX(level, va)];
    8000102e:	0149d933          	srl	s2,s3,s4
    80001032:	1ff97913          	andi	s2,s2,511
    80001036:	090e                	slli	s2,s2,0x3
    80001038:	9926                	add	s2,s2,s1
    if(*pte & PTE_V) {
    8000103a:	00093483          	ld	s1,0(s2)
    8000103e:	0014f793          	andi	a5,s1,1
    80001042:	dfd5                	beqz	a5,80000ffe <walk+0x36>
      pagetable = (pagetable_t)PTE2PA(*pte);
    80001044:	80a9                	srli	s1,s1,0xa
    80001046:	04b2                	slli	s1,s1,0xc
    80001048:	b7c5                	j	80001028 <walk+0x60>
    }
  }
  return &pagetable[PX(0, va)];
    8000104a:	00c9d513          	srli	a0,s3,0xc
    8000104e:	1ff57513          	andi	a0,a0,511
    80001052:	050e                	slli	a0,a0,0x3
    80001054:	9526                	add	a0,a0,s1
}
    80001056:	70e2                	ld	ra,56(sp)
    80001058:	7442                	ld	s0,48(sp)
    8000105a:	74a2                	ld	s1,40(sp)
    8000105c:	7902                	ld	s2,32(sp)
    8000105e:	69e2                	ld	s3,24(sp)
    80001060:	6a42                	ld	s4,16(sp)
    80001062:	6aa2                	ld	s5,8(sp)
    80001064:	6b02                	ld	s6,0(sp)
    80001066:	6121                	addi	sp,sp,64
    80001068:	8082                	ret
        return 0;
    8000106a:	4501                	li	a0,0
    8000106c:	b7ed                	j	80001056 <walk+0x8e>

000000008000106e <walkaddr>:
walkaddr(pagetable_t pagetable, uint64 va)
{
  pte_t *pte;
  uint64 pa;

  if(va >= MAXVA)
    8000106e:	57fd                	li	a5,-1
    80001070:	83e9                	srli	a5,a5,0x1a
    80001072:	00b7f463          	bgeu	a5,a1,8000107a <walkaddr+0xc>
    return 0;
    80001076:	4501                	li	a0,0
    return 0;
  if((*pte & PTE_U) == 0)
    return 0;
  pa = PTE2PA(*pte);
  return pa;
}
    80001078:	8082                	ret
{
    8000107a:	1141                	addi	sp,sp,-16
    8000107c:	e406                	sd	ra,8(sp)
    8000107e:	e022                	sd	s0,0(sp)
    80001080:	0800                	addi	s0,sp,16
  pte = walk(pagetable, va, 0);
    80001082:	4601                	li	a2,0
    80001084:	00000097          	auipc	ra,0x0
    80001088:	f44080e7          	jalr	-188(ra) # 80000fc8 <walk>
  if(pte == 0)
    8000108c:	c105                	beqz	a0,800010ac <walkaddr+0x3e>
  if((*pte & PTE_V) == 0)
    8000108e:	611c                	ld	a5,0(a0)
  if((*pte & PTE_U) == 0)
    80001090:	0117f693          	andi	a3,a5,17
    80001094:	4745                	li	a4,17
    return 0;
    80001096:	4501                	li	a0,0
  if((*pte & PTE_U) == 0)
    80001098:	00e68663          	beq	a3,a4,800010a4 <walkaddr+0x36>
}
    8000109c:	60a2                	ld	ra,8(sp)
    8000109e:	6402                	ld	s0,0(sp)
    800010a0:	0141                	addi	sp,sp,16
    800010a2:	8082                	ret
  pa = PTE2PA(*pte);
    800010a4:	00a7d513          	srli	a0,a5,0xa
    800010a8:	0532                	slli	a0,a0,0xc
  return pa;
    800010aa:	bfcd                	j	8000109c <walkaddr+0x2e>
    return 0;
    800010ac:	4501                	li	a0,0
    800010ae:	b7fd                	j	8000109c <walkaddr+0x2e>

00000000800010b0 <mappages>:
// physical addresses starting at pa. va and size might not
// be page-aligned. Returns 0 on success, -1 if walk() couldn't
// allocate a needed page-table page.
int
mappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
    800010b0:	715d                	addi	sp,sp,-80
    800010b2:	e486                	sd	ra,72(sp)
    800010b4:	e0a2                	sd	s0,64(sp)
    800010b6:	fc26                	sd	s1,56(sp)
    800010b8:	f84a                	sd	s2,48(sp)
    800010ba:	f44e                	sd	s3,40(sp)
    800010bc:	f052                	sd	s4,32(sp)
    800010be:	ec56                	sd	s5,24(sp)
    800010c0:	e85a                	sd	s6,16(sp)
    800010c2:	e45e                	sd	s7,8(sp)
    800010c4:	0880                	addi	s0,sp,80
  uint64 a, last;
  pte_t *pte;

  if(size == 0)
    800010c6:	c205                	beqz	a2,800010e6 <mappages+0x36>
    800010c8:	8aaa                	mv	s5,a0
    800010ca:	8b3a                	mv	s6,a4
    panic("mappages: size");
  
  a = PGROUNDDOWN(va);
    800010cc:	77fd                	lui	a5,0xfffff
    800010ce:	00f5fa33          	and	s4,a1,a5
  last = PGROUNDDOWN(va + size - 1);
    800010d2:	15fd                	addi	a1,a1,-1
    800010d4:	00c589b3          	add	s3,a1,a2
    800010d8:	00f9f9b3          	and	s3,s3,a5
  a = PGROUNDDOWN(va);
    800010dc:	8952                	mv	s2,s4
    800010de:	41468a33          	sub	s4,a3,s4
    if(*pte & PTE_V)
      panic("mappages: remap");
    *pte = PA2PTE(pa) | perm | PTE_V;
    if(a == last)
      break;
    a += PGSIZE;
    800010e2:	6b85                	lui	s7,0x1
    800010e4:	a015                	j	80001108 <mappages+0x58>
    panic("mappages: size");
    800010e6:	00007517          	auipc	a0,0x7
    800010ea:	ff250513          	addi	a0,a0,-14 # 800080d8 <digits+0x98>
    800010ee:	fffff097          	auipc	ra,0xfffff
    800010f2:	450080e7          	jalr	1104(ra) # 8000053e <panic>
      panic("mappages: remap");
    800010f6:	00007517          	auipc	a0,0x7
    800010fa:	ff250513          	addi	a0,a0,-14 # 800080e8 <digits+0xa8>
    800010fe:	fffff097          	auipc	ra,0xfffff
    80001102:	440080e7          	jalr	1088(ra) # 8000053e <panic>
    a += PGSIZE;
    80001106:	995e                	add	s2,s2,s7
  for(;;){
    80001108:	012a04b3          	add	s1,s4,s2
    if((pte = walk(pagetable, a, 1)) == 0)
    8000110c:	4605                	li	a2,1
    8000110e:	85ca                	mv	a1,s2
    80001110:	8556                	mv	a0,s5
    80001112:	00000097          	auipc	ra,0x0
    80001116:	eb6080e7          	jalr	-330(ra) # 80000fc8 <walk>
    8000111a:	cd19                	beqz	a0,80001138 <mappages+0x88>
    if(*pte & PTE_V)
    8000111c:	611c                	ld	a5,0(a0)
    8000111e:	8b85                	andi	a5,a5,1
    80001120:	fbf9                	bnez	a5,800010f6 <mappages+0x46>
    *pte = PA2PTE(pa) | perm | PTE_V;
    80001122:	80b1                	srli	s1,s1,0xc
    80001124:	04aa                	slli	s1,s1,0xa
    80001126:	0164e4b3          	or	s1,s1,s6
    8000112a:	0014e493          	ori	s1,s1,1
    8000112e:	e104                	sd	s1,0(a0)
    if(a == last)
    80001130:	fd391be3          	bne	s2,s3,80001106 <mappages+0x56>
    pa += PGSIZE;
  }
  return 0;
    80001134:	4501                	li	a0,0
    80001136:	a011                	j	8000113a <mappages+0x8a>
      return -1;
    80001138:	557d                	li	a0,-1
}
    8000113a:	60a6                	ld	ra,72(sp)
    8000113c:	6406                	ld	s0,64(sp)
    8000113e:	74e2                	ld	s1,56(sp)
    80001140:	7942                	ld	s2,48(sp)
    80001142:	79a2                	ld	s3,40(sp)
    80001144:	7a02                	ld	s4,32(sp)
    80001146:	6ae2                	ld	s5,24(sp)
    80001148:	6b42                	ld	s6,16(sp)
    8000114a:	6ba2                	ld	s7,8(sp)
    8000114c:	6161                	addi	sp,sp,80
    8000114e:	8082                	ret

0000000080001150 <kvmmap>:
{
    80001150:	1141                	addi	sp,sp,-16
    80001152:	e406                	sd	ra,8(sp)
    80001154:	e022                	sd	s0,0(sp)
    80001156:	0800                	addi	s0,sp,16
    80001158:	87b6                	mv	a5,a3
  if(mappages(kpgtbl, va, sz, pa, perm) != 0)
    8000115a:	86b2                	mv	a3,a2
    8000115c:	863e                	mv	a2,a5
    8000115e:	00000097          	auipc	ra,0x0
    80001162:	f52080e7          	jalr	-174(ra) # 800010b0 <mappages>
    80001166:	e509                	bnez	a0,80001170 <kvmmap+0x20>
}
    80001168:	60a2                	ld	ra,8(sp)
    8000116a:	6402                	ld	s0,0(sp)
    8000116c:	0141                	addi	sp,sp,16
    8000116e:	8082                	ret
    panic("kvmmap");
    80001170:	00007517          	auipc	a0,0x7
    80001174:	f8850513          	addi	a0,a0,-120 # 800080f8 <digits+0xb8>
    80001178:	fffff097          	auipc	ra,0xfffff
    8000117c:	3c6080e7          	jalr	966(ra) # 8000053e <panic>

0000000080001180 <kvmmake>:
{
    80001180:	1101                	addi	sp,sp,-32
    80001182:	ec06                	sd	ra,24(sp)
    80001184:	e822                	sd	s0,16(sp)
    80001186:	e426                	sd	s1,8(sp)
    80001188:	e04a                	sd	s2,0(sp)
    8000118a:	1000                	addi	s0,sp,32
  kpgtbl = (pagetable_t) kalloc();
    8000118c:	00000097          	auipc	ra,0x0
    80001190:	968080e7          	jalr	-1688(ra) # 80000af4 <kalloc>
    80001194:	84aa                	mv	s1,a0
  memset(kpgtbl, 0, PGSIZE);
    80001196:	6605                	lui	a2,0x1
    80001198:	4581                	li	a1,0
    8000119a:	00000097          	auipc	ra,0x0
    8000119e:	b46080e7          	jalr	-1210(ra) # 80000ce0 <memset>
  kvmmap(kpgtbl, UART0, UART0, PGSIZE, PTE_R | PTE_W);
    800011a2:	4719                	li	a4,6
    800011a4:	6685                	lui	a3,0x1
    800011a6:	10000637          	lui	a2,0x10000
    800011aa:	100005b7          	lui	a1,0x10000
    800011ae:	8526                	mv	a0,s1
    800011b0:	00000097          	auipc	ra,0x0
    800011b4:	fa0080e7          	jalr	-96(ra) # 80001150 <kvmmap>
  kvmmap(kpgtbl, VIRTIO0, VIRTIO0, PGSIZE, PTE_R | PTE_W);
    800011b8:	4719                	li	a4,6
    800011ba:	6685                	lui	a3,0x1
    800011bc:	10001637          	lui	a2,0x10001
    800011c0:	100015b7          	lui	a1,0x10001
    800011c4:	8526                	mv	a0,s1
    800011c6:	00000097          	auipc	ra,0x0
    800011ca:	f8a080e7          	jalr	-118(ra) # 80001150 <kvmmap>
  kvmmap(kpgtbl, PLIC, PLIC, 0x400000, PTE_R | PTE_W);
    800011ce:	4719                	li	a4,6
    800011d0:	004006b7          	lui	a3,0x400
    800011d4:	0c000637          	lui	a2,0xc000
    800011d8:	0c0005b7          	lui	a1,0xc000
    800011dc:	8526                	mv	a0,s1
    800011de:	00000097          	auipc	ra,0x0
    800011e2:	f72080e7          	jalr	-142(ra) # 80001150 <kvmmap>
  kvmmap(kpgtbl, KERNBASE, KERNBASE, (uint64)etext-KERNBASE, PTE_R | PTE_X);
    800011e6:	00007917          	auipc	s2,0x7
    800011ea:	e1a90913          	addi	s2,s2,-486 # 80008000 <etext>
    800011ee:	4729                	li	a4,10
    800011f0:	80007697          	auipc	a3,0x80007
    800011f4:	e1068693          	addi	a3,a3,-496 # 8000 <_entry-0x7fff8000>
    800011f8:	4605                	li	a2,1
    800011fa:	067e                	slli	a2,a2,0x1f
    800011fc:	85b2                	mv	a1,a2
    800011fe:	8526                	mv	a0,s1
    80001200:	00000097          	auipc	ra,0x0
    80001204:	f50080e7          	jalr	-176(ra) # 80001150 <kvmmap>
  kvmmap(kpgtbl, (uint64)etext, (uint64)etext, PHYSTOP-(uint64)etext, PTE_R | PTE_W);
    80001208:	4719                	li	a4,6
    8000120a:	46c5                	li	a3,17
    8000120c:	06ee                	slli	a3,a3,0x1b
    8000120e:	412686b3          	sub	a3,a3,s2
    80001212:	864a                	mv	a2,s2
    80001214:	85ca                	mv	a1,s2
    80001216:	8526                	mv	a0,s1
    80001218:	00000097          	auipc	ra,0x0
    8000121c:	f38080e7          	jalr	-200(ra) # 80001150 <kvmmap>
  kvmmap(kpgtbl, TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
    80001220:	4729                	li	a4,10
    80001222:	6685                	lui	a3,0x1
    80001224:	00006617          	auipc	a2,0x6
    80001228:	ddc60613          	addi	a2,a2,-548 # 80007000 <_trampoline>
    8000122c:	040005b7          	lui	a1,0x4000
    80001230:	15fd                	addi	a1,a1,-1
    80001232:	05b2                	slli	a1,a1,0xc
    80001234:	8526                	mv	a0,s1
    80001236:	00000097          	auipc	ra,0x0
    8000123a:	f1a080e7          	jalr	-230(ra) # 80001150 <kvmmap>
  proc_mapstacks(kpgtbl);
    8000123e:	8526                	mv	a0,s1
    80001240:	00001097          	auipc	ra,0x1
    80001244:	9b0080e7          	jalr	-1616(ra) # 80001bf0 <proc_mapstacks>
}
    80001248:	8526                	mv	a0,s1
    8000124a:	60e2                	ld	ra,24(sp)
    8000124c:	6442                	ld	s0,16(sp)
    8000124e:	64a2                	ld	s1,8(sp)
    80001250:	6902                	ld	s2,0(sp)
    80001252:	6105                	addi	sp,sp,32
    80001254:	8082                	ret

0000000080001256 <kvminit>:
{
    80001256:	1141                	addi	sp,sp,-16
    80001258:	e406                	sd	ra,8(sp)
    8000125a:	e022                	sd	s0,0(sp)
    8000125c:	0800                	addi	s0,sp,16
  kernel_pagetable = kvmmake();
    8000125e:	00000097          	auipc	ra,0x0
    80001262:	f22080e7          	jalr	-222(ra) # 80001180 <kvmmake>
    80001266:	00008797          	auipc	a5,0x8
    8000126a:	daa7bd23          	sd	a0,-582(a5) # 80009020 <kernel_pagetable>
}
    8000126e:	60a2                	ld	ra,8(sp)
    80001270:	6402                	ld	s0,0(sp)
    80001272:	0141                	addi	sp,sp,16
    80001274:	8082                	ret

0000000080001276 <uvmunmap>:
// Remove npages of mappings starting from va. va must be
// page-aligned. The mappings must exist.
// Optionally free the physical memory.
void
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
    80001276:	715d                	addi	sp,sp,-80
    80001278:	e486                	sd	ra,72(sp)
    8000127a:	e0a2                	sd	s0,64(sp)
    8000127c:	fc26                	sd	s1,56(sp)
    8000127e:	f84a                	sd	s2,48(sp)
    80001280:	f44e                	sd	s3,40(sp)
    80001282:	f052                	sd	s4,32(sp)
    80001284:	ec56                	sd	s5,24(sp)
    80001286:	e85a                	sd	s6,16(sp)
    80001288:	e45e                	sd	s7,8(sp)
    8000128a:	0880                	addi	s0,sp,80
  uint64 a;
  pte_t *pte;

  if((va % PGSIZE) != 0)
    8000128c:	03459793          	slli	a5,a1,0x34
    80001290:	e795                	bnez	a5,800012bc <uvmunmap+0x46>
    80001292:	8a2a                	mv	s4,a0
    80001294:	892e                	mv	s2,a1
    80001296:	8ab6                	mv	s5,a3
    panic("uvmunmap: not aligned");

  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    80001298:	0632                	slli	a2,a2,0xc
    8000129a:	00b609b3          	add	s3,a2,a1
    if((pte = walk(pagetable, a, 0)) == 0)
      panic("uvmunmap: walk");
    if((*pte & PTE_V) == 0)
      panic("uvmunmap: not mapped");
    if(PTE_FLAGS(*pte) == PTE_V)
    8000129e:	4b85                	li	s7,1
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    800012a0:	6b05                	lui	s6,0x1
    800012a2:	0735e863          	bltu	a1,s3,80001312 <uvmunmap+0x9c>
      uint64 pa = PTE2PA(*pte);
      kfree((void*)pa);
    }
    *pte = 0;
  }
}
    800012a6:	60a6                	ld	ra,72(sp)
    800012a8:	6406                	ld	s0,64(sp)
    800012aa:	74e2                	ld	s1,56(sp)
    800012ac:	7942                	ld	s2,48(sp)
    800012ae:	79a2                	ld	s3,40(sp)
    800012b0:	7a02                	ld	s4,32(sp)
    800012b2:	6ae2                	ld	s5,24(sp)
    800012b4:	6b42                	ld	s6,16(sp)
    800012b6:	6ba2                	ld	s7,8(sp)
    800012b8:	6161                	addi	sp,sp,80
    800012ba:	8082                	ret
    panic("uvmunmap: not aligned");
    800012bc:	00007517          	auipc	a0,0x7
    800012c0:	e4450513          	addi	a0,a0,-444 # 80008100 <digits+0xc0>
    800012c4:	fffff097          	auipc	ra,0xfffff
    800012c8:	27a080e7          	jalr	634(ra) # 8000053e <panic>
      panic("uvmunmap: walk");
    800012cc:	00007517          	auipc	a0,0x7
    800012d0:	e4c50513          	addi	a0,a0,-436 # 80008118 <digits+0xd8>
    800012d4:	fffff097          	auipc	ra,0xfffff
    800012d8:	26a080e7          	jalr	618(ra) # 8000053e <panic>
      panic("uvmunmap: not mapped");
    800012dc:	00007517          	auipc	a0,0x7
    800012e0:	e4c50513          	addi	a0,a0,-436 # 80008128 <digits+0xe8>
    800012e4:	fffff097          	auipc	ra,0xfffff
    800012e8:	25a080e7          	jalr	602(ra) # 8000053e <panic>
      panic("uvmunmap: not a leaf");
    800012ec:	00007517          	auipc	a0,0x7
    800012f0:	e5450513          	addi	a0,a0,-428 # 80008140 <digits+0x100>
    800012f4:	fffff097          	auipc	ra,0xfffff
    800012f8:	24a080e7          	jalr	586(ra) # 8000053e <panic>
      uint64 pa = PTE2PA(*pte);
    800012fc:	8129                	srli	a0,a0,0xa
      kfree((void*)pa);
    800012fe:	0532                	slli	a0,a0,0xc
    80001300:	fffff097          	auipc	ra,0xfffff
    80001304:	6f8080e7          	jalr	1784(ra) # 800009f8 <kfree>
    *pte = 0;
    80001308:	0004b023          	sd	zero,0(s1)
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    8000130c:	995a                	add	s2,s2,s6
    8000130e:	f9397ce3          	bgeu	s2,s3,800012a6 <uvmunmap+0x30>
    if((pte = walk(pagetable, a, 0)) == 0)
    80001312:	4601                	li	a2,0
    80001314:	85ca                	mv	a1,s2
    80001316:	8552                	mv	a0,s4
    80001318:	00000097          	auipc	ra,0x0
    8000131c:	cb0080e7          	jalr	-848(ra) # 80000fc8 <walk>
    80001320:	84aa                	mv	s1,a0
    80001322:	d54d                	beqz	a0,800012cc <uvmunmap+0x56>
    if((*pte & PTE_V) == 0)
    80001324:	6108                	ld	a0,0(a0)
    80001326:	00157793          	andi	a5,a0,1
    8000132a:	dbcd                	beqz	a5,800012dc <uvmunmap+0x66>
    if(PTE_FLAGS(*pte) == PTE_V)
    8000132c:	3ff57793          	andi	a5,a0,1023
    80001330:	fb778ee3          	beq	a5,s7,800012ec <uvmunmap+0x76>
    if(do_free){
    80001334:	fc0a8ae3          	beqz	s5,80001308 <uvmunmap+0x92>
    80001338:	b7d1                	j	800012fc <uvmunmap+0x86>

000000008000133a <uvmcreate>:

// create an empty user page table.
// returns 0 if out of memory.
pagetable_t
uvmcreate()
{
    8000133a:	1101                	addi	sp,sp,-32
    8000133c:	ec06                	sd	ra,24(sp)
    8000133e:	e822                	sd	s0,16(sp)
    80001340:	e426                	sd	s1,8(sp)
    80001342:	1000                	addi	s0,sp,32
  pagetable_t pagetable;
  pagetable = (pagetable_t) kalloc();
    80001344:	fffff097          	auipc	ra,0xfffff
    80001348:	7b0080e7          	jalr	1968(ra) # 80000af4 <kalloc>
    8000134c:	84aa                	mv	s1,a0
  if(pagetable == 0)
    8000134e:	c519                	beqz	a0,8000135c <uvmcreate+0x22>
    return 0;
  memset(pagetable, 0, PGSIZE);
    80001350:	6605                	lui	a2,0x1
    80001352:	4581                	li	a1,0
    80001354:	00000097          	auipc	ra,0x0
    80001358:	98c080e7          	jalr	-1652(ra) # 80000ce0 <memset>
  return pagetable;
}
    8000135c:	8526                	mv	a0,s1
    8000135e:	60e2                	ld	ra,24(sp)
    80001360:	6442                	ld	s0,16(sp)
    80001362:	64a2                	ld	s1,8(sp)
    80001364:	6105                	addi	sp,sp,32
    80001366:	8082                	ret

0000000080001368 <uvminit>:
// Load the user initcode into address 0 of pagetable,
// for the very first process.
// sz must be less than a page.
void
uvminit(pagetable_t pagetable, uchar *src, uint sz)
{
    80001368:	7179                	addi	sp,sp,-48
    8000136a:	f406                	sd	ra,40(sp)
    8000136c:	f022                	sd	s0,32(sp)
    8000136e:	ec26                	sd	s1,24(sp)
    80001370:	e84a                	sd	s2,16(sp)
    80001372:	e44e                	sd	s3,8(sp)
    80001374:	e052                	sd	s4,0(sp)
    80001376:	1800                	addi	s0,sp,48
  char *mem;

  if(sz >= PGSIZE)
    80001378:	6785                	lui	a5,0x1
    8000137a:	04f67863          	bgeu	a2,a5,800013ca <uvminit+0x62>
    8000137e:	8a2a                	mv	s4,a0
    80001380:	89ae                	mv	s3,a1
    80001382:	84b2                	mv	s1,a2
    panic("inituvm: more than a page");
  mem = kalloc();
    80001384:	fffff097          	auipc	ra,0xfffff
    80001388:	770080e7          	jalr	1904(ra) # 80000af4 <kalloc>
    8000138c:	892a                	mv	s2,a0
  memset(mem, 0, PGSIZE);
    8000138e:	6605                	lui	a2,0x1
    80001390:	4581                	li	a1,0
    80001392:	00000097          	auipc	ra,0x0
    80001396:	94e080e7          	jalr	-1714(ra) # 80000ce0 <memset>
  mappages(pagetable, 0, PGSIZE, (uint64)mem, PTE_W|PTE_R|PTE_X|PTE_U);
    8000139a:	4779                	li	a4,30
    8000139c:	86ca                	mv	a3,s2
    8000139e:	6605                	lui	a2,0x1
    800013a0:	4581                	li	a1,0
    800013a2:	8552                	mv	a0,s4
    800013a4:	00000097          	auipc	ra,0x0
    800013a8:	d0c080e7          	jalr	-756(ra) # 800010b0 <mappages>
  memmove(mem, src, sz);
    800013ac:	8626                	mv	a2,s1
    800013ae:	85ce                	mv	a1,s3
    800013b0:	854a                	mv	a0,s2
    800013b2:	00000097          	auipc	ra,0x0
    800013b6:	98e080e7          	jalr	-1650(ra) # 80000d40 <memmove>
}
    800013ba:	70a2                	ld	ra,40(sp)
    800013bc:	7402                	ld	s0,32(sp)
    800013be:	64e2                	ld	s1,24(sp)
    800013c0:	6942                	ld	s2,16(sp)
    800013c2:	69a2                	ld	s3,8(sp)
    800013c4:	6a02                	ld	s4,0(sp)
    800013c6:	6145                	addi	sp,sp,48
    800013c8:	8082                	ret
    panic("inituvm: more than a page");
    800013ca:	00007517          	auipc	a0,0x7
    800013ce:	d8e50513          	addi	a0,a0,-626 # 80008158 <digits+0x118>
    800013d2:	fffff097          	auipc	ra,0xfffff
    800013d6:	16c080e7          	jalr	364(ra) # 8000053e <panic>

00000000800013da <uvmdealloc>:
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
uint64
uvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
    800013da:	1101                	addi	sp,sp,-32
    800013dc:	ec06                	sd	ra,24(sp)
    800013de:	e822                	sd	s0,16(sp)
    800013e0:	e426                	sd	s1,8(sp)
    800013e2:	1000                	addi	s0,sp,32
  if(newsz >= oldsz)
    return oldsz;
    800013e4:	84ae                	mv	s1,a1
  if(newsz >= oldsz)
    800013e6:	00b67d63          	bgeu	a2,a1,80001400 <uvmdealloc+0x26>
    800013ea:	84b2                	mv	s1,a2

  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz)){
    800013ec:	6785                	lui	a5,0x1
    800013ee:	17fd                	addi	a5,a5,-1
    800013f0:	00f60733          	add	a4,a2,a5
    800013f4:	767d                	lui	a2,0xfffff
    800013f6:	8f71                	and	a4,a4,a2
    800013f8:	97ae                	add	a5,a5,a1
    800013fa:	8ff1                	and	a5,a5,a2
    800013fc:	00f76863          	bltu	a4,a5,8000140c <uvmdealloc+0x32>
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, 1);
  }

  return newsz;
}
    80001400:	8526                	mv	a0,s1
    80001402:	60e2                	ld	ra,24(sp)
    80001404:	6442                	ld	s0,16(sp)
    80001406:	64a2                	ld	s1,8(sp)
    80001408:	6105                	addi	sp,sp,32
    8000140a:	8082                	ret
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    8000140c:	8f99                	sub	a5,a5,a4
    8000140e:	83b1                	srli	a5,a5,0xc
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, 1);
    80001410:	4685                	li	a3,1
    80001412:	0007861b          	sext.w	a2,a5
    80001416:	85ba                	mv	a1,a4
    80001418:	00000097          	auipc	ra,0x0
    8000141c:	e5e080e7          	jalr	-418(ra) # 80001276 <uvmunmap>
    80001420:	b7c5                	j	80001400 <uvmdealloc+0x26>

0000000080001422 <uvmalloc>:
  if(newsz < oldsz)
    80001422:	0ab66163          	bltu	a2,a1,800014c4 <uvmalloc+0xa2>
{
    80001426:	7139                	addi	sp,sp,-64
    80001428:	fc06                	sd	ra,56(sp)
    8000142a:	f822                	sd	s0,48(sp)
    8000142c:	f426                	sd	s1,40(sp)
    8000142e:	f04a                	sd	s2,32(sp)
    80001430:	ec4e                	sd	s3,24(sp)
    80001432:	e852                	sd	s4,16(sp)
    80001434:	e456                	sd	s5,8(sp)
    80001436:	0080                	addi	s0,sp,64
    80001438:	8aaa                	mv	s5,a0
    8000143a:	8a32                	mv	s4,a2
  oldsz = PGROUNDUP(oldsz);
    8000143c:	6985                	lui	s3,0x1
    8000143e:	19fd                	addi	s3,s3,-1
    80001440:	95ce                	add	a1,a1,s3
    80001442:	79fd                	lui	s3,0xfffff
    80001444:	0135f9b3          	and	s3,a1,s3
  for(a = oldsz; a < newsz; a += PGSIZE){
    80001448:	08c9f063          	bgeu	s3,a2,800014c8 <uvmalloc+0xa6>
    8000144c:	894e                	mv	s2,s3
    mem = kalloc();
    8000144e:	fffff097          	auipc	ra,0xfffff
    80001452:	6a6080e7          	jalr	1702(ra) # 80000af4 <kalloc>
    80001456:	84aa                	mv	s1,a0
    if(mem == 0){
    80001458:	c51d                	beqz	a0,80001486 <uvmalloc+0x64>
    memset(mem, 0, PGSIZE);
    8000145a:	6605                	lui	a2,0x1
    8000145c:	4581                	li	a1,0
    8000145e:	00000097          	auipc	ra,0x0
    80001462:	882080e7          	jalr	-1918(ra) # 80000ce0 <memset>
    if(mappages(pagetable, a, PGSIZE, (uint64)mem, PTE_W|PTE_X|PTE_R|PTE_U) != 0){
    80001466:	4779                	li	a4,30
    80001468:	86a6                	mv	a3,s1
    8000146a:	6605                	lui	a2,0x1
    8000146c:	85ca                	mv	a1,s2
    8000146e:	8556                	mv	a0,s5
    80001470:	00000097          	auipc	ra,0x0
    80001474:	c40080e7          	jalr	-960(ra) # 800010b0 <mappages>
    80001478:	e905                	bnez	a0,800014a8 <uvmalloc+0x86>
  for(a = oldsz; a < newsz; a += PGSIZE){
    8000147a:	6785                	lui	a5,0x1
    8000147c:	993e                	add	s2,s2,a5
    8000147e:	fd4968e3          	bltu	s2,s4,8000144e <uvmalloc+0x2c>
  return newsz;
    80001482:	8552                	mv	a0,s4
    80001484:	a809                	j	80001496 <uvmalloc+0x74>
      uvmdealloc(pagetable, a, oldsz);
    80001486:	864e                	mv	a2,s3
    80001488:	85ca                	mv	a1,s2
    8000148a:	8556                	mv	a0,s5
    8000148c:	00000097          	auipc	ra,0x0
    80001490:	f4e080e7          	jalr	-178(ra) # 800013da <uvmdealloc>
      return 0;
    80001494:	4501                	li	a0,0
}
    80001496:	70e2                	ld	ra,56(sp)
    80001498:	7442                	ld	s0,48(sp)
    8000149a:	74a2                	ld	s1,40(sp)
    8000149c:	7902                	ld	s2,32(sp)
    8000149e:	69e2                	ld	s3,24(sp)
    800014a0:	6a42                	ld	s4,16(sp)
    800014a2:	6aa2                	ld	s5,8(sp)
    800014a4:	6121                	addi	sp,sp,64
    800014a6:	8082                	ret
      kfree(mem);
    800014a8:	8526                	mv	a0,s1
    800014aa:	fffff097          	auipc	ra,0xfffff
    800014ae:	54e080e7          	jalr	1358(ra) # 800009f8 <kfree>
      uvmdealloc(pagetable, a, oldsz);
    800014b2:	864e                	mv	a2,s3
    800014b4:	85ca                	mv	a1,s2
    800014b6:	8556                	mv	a0,s5
    800014b8:	00000097          	auipc	ra,0x0
    800014bc:	f22080e7          	jalr	-222(ra) # 800013da <uvmdealloc>
      return 0;
    800014c0:	4501                	li	a0,0
    800014c2:	bfd1                	j	80001496 <uvmalloc+0x74>
    return oldsz;
    800014c4:	852e                	mv	a0,a1
}
    800014c6:	8082                	ret
  return newsz;
    800014c8:	8532                	mv	a0,a2
    800014ca:	b7f1                	j	80001496 <uvmalloc+0x74>

00000000800014cc <freewalk>:

// Recursively free page-table pages.
// All leaf mappings must already have been removed.
void
freewalk(pagetable_t pagetable)
{
    800014cc:	7179                	addi	sp,sp,-48
    800014ce:	f406                	sd	ra,40(sp)
    800014d0:	f022                	sd	s0,32(sp)
    800014d2:	ec26                	sd	s1,24(sp)
    800014d4:	e84a                	sd	s2,16(sp)
    800014d6:	e44e                	sd	s3,8(sp)
    800014d8:	e052                	sd	s4,0(sp)
    800014da:	1800                	addi	s0,sp,48
    800014dc:	8a2a                	mv	s4,a0
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    800014de:	84aa                	mv	s1,a0
    800014e0:	6905                	lui	s2,0x1
    800014e2:	992a                	add	s2,s2,a0
    pte_t pte = pagetable[i];
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
    800014e4:	4985                	li	s3,1
    800014e6:	a821                	j	800014fe <freewalk+0x32>
      // this PTE points to a lower-level page table.
      uint64 child = PTE2PA(pte);
    800014e8:	8129                	srli	a0,a0,0xa
      freewalk((pagetable_t)child);
    800014ea:	0532                	slli	a0,a0,0xc
    800014ec:	00000097          	auipc	ra,0x0
    800014f0:	fe0080e7          	jalr	-32(ra) # 800014cc <freewalk>
      pagetable[i] = 0;
    800014f4:	0004b023          	sd	zero,0(s1)
  for(int i = 0; i < 512; i++){
    800014f8:	04a1                	addi	s1,s1,8
    800014fa:	03248163          	beq	s1,s2,8000151c <freewalk+0x50>
    pte_t pte = pagetable[i];
    800014fe:	6088                	ld	a0,0(s1)
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
    80001500:	00f57793          	andi	a5,a0,15
    80001504:	ff3782e3          	beq	a5,s3,800014e8 <freewalk+0x1c>
    } else if(pte & PTE_V){
    80001508:	8905                	andi	a0,a0,1
    8000150a:	d57d                	beqz	a0,800014f8 <freewalk+0x2c>
      panic("freewalk: leaf");
    8000150c:	00007517          	auipc	a0,0x7
    80001510:	c6c50513          	addi	a0,a0,-916 # 80008178 <digits+0x138>
    80001514:	fffff097          	auipc	ra,0xfffff
    80001518:	02a080e7          	jalr	42(ra) # 8000053e <panic>
    }
  }
  kfree((void*)pagetable);
    8000151c:	8552                	mv	a0,s4
    8000151e:	fffff097          	auipc	ra,0xfffff
    80001522:	4da080e7          	jalr	1242(ra) # 800009f8 <kfree>
}
    80001526:	70a2                	ld	ra,40(sp)
    80001528:	7402                	ld	s0,32(sp)
    8000152a:	64e2                	ld	s1,24(sp)
    8000152c:	6942                	ld	s2,16(sp)
    8000152e:	69a2                	ld	s3,8(sp)
    80001530:	6a02                	ld	s4,0(sp)
    80001532:	6145                	addi	sp,sp,48
    80001534:	8082                	ret

0000000080001536 <uvmfree>:

// Free user memory pages,
// then free page-table pages.
void
uvmfree(pagetable_t pagetable, uint64 sz)
{
    80001536:	1101                	addi	sp,sp,-32
    80001538:	ec06                	sd	ra,24(sp)
    8000153a:	e822                	sd	s0,16(sp)
    8000153c:	e426                	sd	s1,8(sp)
    8000153e:	1000                	addi	s0,sp,32
    80001540:	84aa                	mv	s1,a0
  if(sz > 0)
    80001542:	e999                	bnez	a1,80001558 <uvmfree+0x22>
    uvmunmap(pagetable, 0, PGROUNDUP(sz)/PGSIZE, 1);
  freewalk(pagetable);
    80001544:	8526                	mv	a0,s1
    80001546:	00000097          	auipc	ra,0x0
    8000154a:	f86080e7          	jalr	-122(ra) # 800014cc <freewalk>
}
    8000154e:	60e2                	ld	ra,24(sp)
    80001550:	6442                	ld	s0,16(sp)
    80001552:	64a2                	ld	s1,8(sp)
    80001554:	6105                	addi	sp,sp,32
    80001556:	8082                	ret
    uvmunmap(pagetable, 0, PGROUNDUP(sz)/PGSIZE, 1);
    80001558:	6605                	lui	a2,0x1
    8000155a:	167d                	addi	a2,a2,-1
    8000155c:	962e                	add	a2,a2,a1
    8000155e:	4685                	li	a3,1
    80001560:	8231                	srli	a2,a2,0xc
    80001562:	4581                	li	a1,0
    80001564:	00000097          	auipc	ra,0x0
    80001568:	d12080e7          	jalr	-750(ra) # 80001276 <uvmunmap>
    8000156c:	bfe1                	j	80001544 <uvmfree+0xe>

000000008000156e <uvmcopy>:
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  char *mem;

  for(i = 0; i < sz; i += PGSIZE){
    8000156e:	c679                	beqz	a2,8000163c <uvmcopy+0xce>
{
    80001570:	715d                	addi	sp,sp,-80
    80001572:	e486                	sd	ra,72(sp)
    80001574:	e0a2                	sd	s0,64(sp)
    80001576:	fc26                	sd	s1,56(sp)
    80001578:	f84a                	sd	s2,48(sp)
    8000157a:	f44e                	sd	s3,40(sp)
    8000157c:	f052                	sd	s4,32(sp)
    8000157e:	ec56                	sd	s5,24(sp)
    80001580:	e85a                	sd	s6,16(sp)
    80001582:	e45e                	sd	s7,8(sp)
    80001584:	0880                	addi	s0,sp,80
    80001586:	8b2a                	mv	s6,a0
    80001588:	8aae                	mv	s5,a1
    8000158a:	8a32                	mv	s4,a2
  for(i = 0; i < sz; i += PGSIZE){
    8000158c:	4981                	li	s3,0
    if((pte = walk(old, i, 0)) == 0)
    8000158e:	4601                	li	a2,0
    80001590:	85ce                	mv	a1,s3
    80001592:	855a                	mv	a0,s6
    80001594:	00000097          	auipc	ra,0x0
    80001598:	a34080e7          	jalr	-1484(ra) # 80000fc8 <walk>
    8000159c:	c531                	beqz	a0,800015e8 <uvmcopy+0x7a>
      panic("uvmcopy: pte should exist");
    if((*pte & PTE_V) == 0)
    8000159e:	6118                	ld	a4,0(a0)
    800015a0:	00177793          	andi	a5,a4,1
    800015a4:	cbb1                	beqz	a5,800015f8 <uvmcopy+0x8a>
      panic("uvmcopy: page not present");
    pa = PTE2PA(*pte);
    800015a6:	00a75593          	srli	a1,a4,0xa
    800015aa:	00c59b93          	slli	s7,a1,0xc
    flags = PTE_FLAGS(*pte);
    800015ae:	3ff77493          	andi	s1,a4,1023
    if((mem = kalloc()) == 0)
    800015b2:	fffff097          	auipc	ra,0xfffff
    800015b6:	542080e7          	jalr	1346(ra) # 80000af4 <kalloc>
    800015ba:	892a                	mv	s2,a0
    800015bc:	c939                	beqz	a0,80001612 <uvmcopy+0xa4>
      goto err;
    memmove(mem, (char*)pa, PGSIZE);
    800015be:	6605                	lui	a2,0x1
    800015c0:	85de                	mv	a1,s7
    800015c2:	fffff097          	auipc	ra,0xfffff
    800015c6:	77e080e7          	jalr	1918(ra) # 80000d40 <memmove>
    if(mappages(new, i, PGSIZE, (uint64)mem, flags) != 0){
    800015ca:	8726                	mv	a4,s1
    800015cc:	86ca                	mv	a3,s2
    800015ce:	6605                	lui	a2,0x1
    800015d0:	85ce                	mv	a1,s3
    800015d2:	8556                	mv	a0,s5
    800015d4:	00000097          	auipc	ra,0x0
    800015d8:	adc080e7          	jalr	-1316(ra) # 800010b0 <mappages>
    800015dc:	e515                	bnez	a0,80001608 <uvmcopy+0x9a>
  for(i = 0; i < sz; i += PGSIZE){
    800015de:	6785                	lui	a5,0x1
    800015e0:	99be                	add	s3,s3,a5
    800015e2:	fb49e6e3          	bltu	s3,s4,8000158e <uvmcopy+0x20>
    800015e6:	a081                	j	80001626 <uvmcopy+0xb8>
      panic("uvmcopy: pte should exist");
    800015e8:	00007517          	auipc	a0,0x7
    800015ec:	ba050513          	addi	a0,a0,-1120 # 80008188 <digits+0x148>
    800015f0:	fffff097          	auipc	ra,0xfffff
    800015f4:	f4e080e7          	jalr	-178(ra) # 8000053e <panic>
      panic("uvmcopy: page not present");
    800015f8:	00007517          	auipc	a0,0x7
    800015fc:	bb050513          	addi	a0,a0,-1104 # 800081a8 <digits+0x168>
    80001600:	fffff097          	auipc	ra,0xfffff
    80001604:	f3e080e7          	jalr	-194(ra) # 8000053e <panic>
      kfree(mem);
    80001608:	854a                	mv	a0,s2
    8000160a:	fffff097          	auipc	ra,0xfffff
    8000160e:	3ee080e7          	jalr	1006(ra) # 800009f8 <kfree>
    }
  }
  return 0;

 err:
  uvmunmap(new, 0, i / PGSIZE, 1);
    80001612:	4685                	li	a3,1
    80001614:	00c9d613          	srli	a2,s3,0xc
    80001618:	4581                	li	a1,0
    8000161a:	8556                	mv	a0,s5
    8000161c:	00000097          	auipc	ra,0x0
    80001620:	c5a080e7          	jalr	-934(ra) # 80001276 <uvmunmap>
  return -1;
    80001624:	557d                	li	a0,-1
}
    80001626:	60a6                	ld	ra,72(sp)
    80001628:	6406                	ld	s0,64(sp)
    8000162a:	74e2                	ld	s1,56(sp)
    8000162c:	7942                	ld	s2,48(sp)
    8000162e:	79a2                	ld	s3,40(sp)
    80001630:	7a02                	ld	s4,32(sp)
    80001632:	6ae2                	ld	s5,24(sp)
    80001634:	6b42                	ld	s6,16(sp)
    80001636:	6ba2                	ld	s7,8(sp)
    80001638:	6161                	addi	sp,sp,80
    8000163a:	8082                	ret
  return 0;
    8000163c:	4501                	li	a0,0
}
    8000163e:	8082                	ret

0000000080001640 <uvmclear>:

// mark a PTE invalid for user access.
// used by exec for the user stack guard page.
void
uvmclear(pagetable_t pagetable, uint64 va)
{
    80001640:	1141                	addi	sp,sp,-16
    80001642:	e406                	sd	ra,8(sp)
    80001644:	e022                	sd	s0,0(sp)
    80001646:	0800                	addi	s0,sp,16
  pte_t *pte;
  
  pte = walk(pagetable, va, 0);
    80001648:	4601                	li	a2,0
    8000164a:	00000097          	auipc	ra,0x0
    8000164e:	97e080e7          	jalr	-1666(ra) # 80000fc8 <walk>
  if(pte == 0)
    80001652:	c901                	beqz	a0,80001662 <uvmclear+0x22>
    panic("uvmclear");
  *pte &= ~PTE_U;
    80001654:	611c                	ld	a5,0(a0)
    80001656:	9bbd                	andi	a5,a5,-17
    80001658:	e11c                	sd	a5,0(a0)
}
    8000165a:	60a2                	ld	ra,8(sp)
    8000165c:	6402                	ld	s0,0(sp)
    8000165e:	0141                	addi	sp,sp,16
    80001660:	8082                	ret
    panic("uvmclear");
    80001662:	00007517          	auipc	a0,0x7
    80001666:	b6650513          	addi	a0,a0,-1178 # 800081c8 <digits+0x188>
    8000166a:	fffff097          	auipc	ra,0xfffff
    8000166e:	ed4080e7          	jalr	-300(ra) # 8000053e <panic>

0000000080001672 <copyout>:
int
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  uint64 n, va0, pa0;

  while(len > 0){
    80001672:	c6bd                	beqz	a3,800016e0 <copyout+0x6e>
{
    80001674:	715d                	addi	sp,sp,-80
    80001676:	e486                	sd	ra,72(sp)
    80001678:	e0a2                	sd	s0,64(sp)
    8000167a:	fc26                	sd	s1,56(sp)
    8000167c:	f84a                	sd	s2,48(sp)
    8000167e:	f44e                	sd	s3,40(sp)
    80001680:	f052                	sd	s4,32(sp)
    80001682:	ec56                	sd	s5,24(sp)
    80001684:	e85a                	sd	s6,16(sp)
    80001686:	e45e                	sd	s7,8(sp)
    80001688:	e062                	sd	s8,0(sp)
    8000168a:	0880                	addi	s0,sp,80
    8000168c:	8b2a                	mv	s6,a0
    8000168e:	8c2e                	mv	s8,a1
    80001690:	8a32                	mv	s4,a2
    80001692:	89b6                	mv	s3,a3
    va0 = PGROUNDDOWN(dstva);
    80001694:	7bfd                	lui	s7,0xfffff
    pa0 = walkaddr(pagetable, va0);
    if(pa0 == 0)
      return -1;
    n = PGSIZE - (dstva - va0);
    80001696:	6a85                	lui	s5,0x1
    80001698:	a015                	j	800016bc <copyout+0x4a>
    if(n > len)
      n = len;
    memmove((void *)(pa0 + (dstva - va0)), src, n);
    8000169a:	9562                	add	a0,a0,s8
    8000169c:	0004861b          	sext.w	a2,s1
    800016a0:	85d2                	mv	a1,s4
    800016a2:	41250533          	sub	a0,a0,s2
    800016a6:	fffff097          	auipc	ra,0xfffff
    800016aa:	69a080e7          	jalr	1690(ra) # 80000d40 <memmove>

    len -= n;
    800016ae:	409989b3          	sub	s3,s3,s1
    src += n;
    800016b2:	9a26                	add	s4,s4,s1
    dstva = va0 + PGSIZE;
    800016b4:	01590c33          	add	s8,s2,s5
  while(len > 0){
    800016b8:	02098263          	beqz	s3,800016dc <copyout+0x6a>
    va0 = PGROUNDDOWN(dstva);
    800016bc:	017c7933          	and	s2,s8,s7
    pa0 = walkaddr(pagetable, va0);
    800016c0:	85ca                	mv	a1,s2
    800016c2:	855a                	mv	a0,s6
    800016c4:	00000097          	auipc	ra,0x0
    800016c8:	9aa080e7          	jalr	-1622(ra) # 8000106e <walkaddr>
    if(pa0 == 0)
    800016cc:	cd01                	beqz	a0,800016e4 <copyout+0x72>
    n = PGSIZE - (dstva - va0);
    800016ce:	418904b3          	sub	s1,s2,s8
    800016d2:	94d6                	add	s1,s1,s5
    if(n > len)
    800016d4:	fc99f3e3          	bgeu	s3,s1,8000169a <copyout+0x28>
    800016d8:	84ce                	mv	s1,s3
    800016da:	b7c1                	j	8000169a <copyout+0x28>
  }
  return 0;
    800016dc:	4501                	li	a0,0
    800016de:	a021                	j	800016e6 <copyout+0x74>
    800016e0:	4501                	li	a0,0
}
    800016e2:	8082                	ret
      return -1;
    800016e4:	557d                	li	a0,-1
}
    800016e6:	60a6                	ld	ra,72(sp)
    800016e8:	6406                	ld	s0,64(sp)
    800016ea:	74e2                	ld	s1,56(sp)
    800016ec:	7942                	ld	s2,48(sp)
    800016ee:	79a2                	ld	s3,40(sp)
    800016f0:	7a02                	ld	s4,32(sp)
    800016f2:	6ae2                	ld	s5,24(sp)
    800016f4:	6b42                	ld	s6,16(sp)
    800016f6:	6ba2                	ld	s7,8(sp)
    800016f8:	6c02                	ld	s8,0(sp)
    800016fa:	6161                	addi	sp,sp,80
    800016fc:	8082                	ret

00000000800016fe <copyin>:
int
copyin(pagetable_t pagetable, char *dst, uint64 srcva, uint64 len)
{
  uint64 n, va0, pa0;

  while(len > 0){
    800016fe:	c6bd                	beqz	a3,8000176c <copyin+0x6e>
{
    80001700:	715d                	addi	sp,sp,-80
    80001702:	e486                	sd	ra,72(sp)
    80001704:	e0a2                	sd	s0,64(sp)
    80001706:	fc26                	sd	s1,56(sp)
    80001708:	f84a                	sd	s2,48(sp)
    8000170a:	f44e                	sd	s3,40(sp)
    8000170c:	f052                	sd	s4,32(sp)
    8000170e:	ec56                	sd	s5,24(sp)
    80001710:	e85a                	sd	s6,16(sp)
    80001712:	e45e                	sd	s7,8(sp)
    80001714:	e062                	sd	s8,0(sp)
    80001716:	0880                	addi	s0,sp,80
    80001718:	8b2a                	mv	s6,a0
    8000171a:	8a2e                	mv	s4,a1
    8000171c:	8c32                	mv	s8,a2
    8000171e:	89b6                	mv	s3,a3
    va0 = PGROUNDDOWN(srcva);
    80001720:	7bfd                	lui	s7,0xfffff
    pa0 = walkaddr(pagetable, va0);
    if(pa0 == 0)
      return -1;
    n = PGSIZE - (srcva - va0);
    80001722:	6a85                	lui	s5,0x1
    80001724:	a015                	j	80001748 <copyin+0x4a>
    if(n > len)
      n = len;
    memmove(dst, (void *)(pa0 + (srcva - va0)), n);
    80001726:	9562                	add	a0,a0,s8
    80001728:	0004861b          	sext.w	a2,s1
    8000172c:	412505b3          	sub	a1,a0,s2
    80001730:	8552                	mv	a0,s4
    80001732:	fffff097          	auipc	ra,0xfffff
    80001736:	60e080e7          	jalr	1550(ra) # 80000d40 <memmove>

    len -= n;
    8000173a:	409989b3          	sub	s3,s3,s1
    dst += n;
    8000173e:	9a26                	add	s4,s4,s1
    srcva = va0 + PGSIZE;
    80001740:	01590c33          	add	s8,s2,s5
  while(len > 0){
    80001744:	02098263          	beqz	s3,80001768 <copyin+0x6a>
    va0 = PGROUNDDOWN(srcva);
    80001748:	017c7933          	and	s2,s8,s7
    pa0 = walkaddr(pagetable, va0);
    8000174c:	85ca                	mv	a1,s2
    8000174e:	855a                	mv	a0,s6
    80001750:	00000097          	auipc	ra,0x0
    80001754:	91e080e7          	jalr	-1762(ra) # 8000106e <walkaddr>
    if(pa0 == 0)
    80001758:	cd01                	beqz	a0,80001770 <copyin+0x72>
    n = PGSIZE - (srcva - va0);
    8000175a:	418904b3          	sub	s1,s2,s8
    8000175e:	94d6                	add	s1,s1,s5
    if(n > len)
    80001760:	fc99f3e3          	bgeu	s3,s1,80001726 <copyin+0x28>
    80001764:	84ce                	mv	s1,s3
    80001766:	b7c1                	j	80001726 <copyin+0x28>
  }
  return 0;
    80001768:	4501                	li	a0,0
    8000176a:	a021                	j	80001772 <copyin+0x74>
    8000176c:	4501                	li	a0,0
}
    8000176e:	8082                	ret
      return -1;
    80001770:	557d                	li	a0,-1
}
    80001772:	60a6                	ld	ra,72(sp)
    80001774:	6406                	ld	s0,64(sp)
    80001776:	74e2                	ld	s1,56(sp)
    80001778:	7942                	ld	s2,48(sp)
    8000177a:	79a2                	ld	s3,40(sp)
    8000177c:	7a02                	ld	s4,32(sp)
    8000177e:	6ae2                	ld	s5,24(sp)
    80001780:	6b42                	ld	s6,16(sp)
    80001782:	6ba2                	ld	s7,8(sp)
    80001784:	6c02                	ld	s8,0(sp)
    80001786:	6161                	addi	sp,sp,80
    80001788:	8082                	ret

000000008000178a <copyinstr>:
copyinstr(pagetable_t pagetable, char *dst, uint64 srcva, uint64 max)
{
  uint64 n, va0, pa0;
  int got_null = 0;

  while(got_null == 0 && max > 0){
    8000178a:	c6c5                	beqz	a3,80001832 <copyinstr+0xa8>
{
    8000178c:	715d                	addi	sp,sp,-80
    8000178e:	e486                	sd	ra,72(sp)
    80001790:	e0a2                	sd	s0,64(sp)
    80001792:	fc26                	sd	s1,56(sp)
    80001794:	f84a                	sd	s2,48(sp)
    80001796:	f44e                	sd	s3,40(sp)
    80001798:	f052                	sd	s4,32(sp)
    8000179a:	ec56                	sd	s5,24(sp)
    8000179c:	e85a                	sd	s6,16(sp)
    8000179e:	e45e                	sd	s7,8(sp)
    800017a0:	0880                	addi	s0,sp,80
    800017a2:	8a2a                	mv	s4,a0
    800017a4:	8b2e                	mv	s6,a1
    800017a6:	8bb2                	mv	s7,a2
    800017a8:	84b6                	mv	s1,a3
    va0 = PGROUNDDOWN(srcva);
    800017aa:	7afd                	lui	s5,0xfffff
    pa0 = walkaddr(pagetable, va0);
    if(pa0 == 0)
      return -1;
    n = PGSIZE - (srcva - va0);
    800017ac:	6985                	lui	s3,0x1
    800017ae:	a035                	j	800017da <copyinstr+0x50>
      n = max;

    char *p = (char *) (pa0 + (srcva - va0));
    while(n > 0){
      if(*p == '\0'){
        *dst = '\0';
    800017b0:	00078023          	sb	zero,0(a5) # 1000 <_entry-0x7ffff000>
    800017b4:	4785                	li	a5,1
      dst++;
    }

    srcva = va0 + PGSIZE;
  }
  if(got_null){
    800017b6:	0017b793          	seqz	a5,a5
    800017ba:	40f00533          	neg	a0,a5
    return 0;
  } else {
    return -1;
  }
}
    800017be:	60a6                	ld	ra,72(sp)
    800017c0:	6406                	ld	s0,64(sp)
    800017c2:	74e2                	ld	s1,56(sp)
    800017c4:	7942                	ld	s2,48(sp)
    800017c6:	79a2                	ld	s3,40(sp)
    800017c8:	7a02                	ld	s4,32(sp)
    800017ca:	6ae2                	ld	s5,24(sp)
    800017cc:	6b42                	ld	s6,16(sp)
    800017ce:	6ba2                	ld	s7,8(sp)
    800017d0:	6161                	addi	sp,sp,80
    800017d2:	8082                	ret
    srcva = va0 + PGSIZE;
    800017d4:	01390bb3          	add	s7,s2,s3
  while(got_null == 0 && max > 0){
    800017d8:	c8a9                	beqz	s1,8000182a <copyinstr+0xa0>
    va0 = PGROUNDDOWN(srcva);
    800017da:	015bf933          	and	s2,s7,s5
    pa0 = walkaddr(pagetable, va0);
    800017de:	85ca                	mv	a1,s2
    800017e0:	8552                	mv	a0,s4
    800017e2:	00000097          	auipc	ra,0x0
    800017e6:	88c080e7          	jalr	-1908(ra) # 8000106e <walkaddr>
    if(pa0 == 0)
    800017ea:	c131                	beqz	a0,8000182e <copyinstr+0xa4>
    n = PGSIZE - (srcva - va0);
    800017ec:	41790833          	sub	a6,s2,s7
    800017f0:	984e                	add	a6,a6,s3
    if(n > max)
    800017f2:	0104f363          	bgeu	s1,a6,800017f8 <copyinstr+0x6e>
    800017f6:	8826                	mv	a6,s1
    char *p = (char *) (pa0 + (srcva - va0));
    800017f8:	955e                	add	a0,a0,s7
    800017fa:	41250533          	sub	a0,a0,s2
    while(n > 0){
    800017fe:	fc080be3          	beqz	a6,800017d4 <copyinstr+0x4a>
    80001802:	985a                	add	a6,a6,s6
    80001804:	87da                	mv	a5,s6
      if(*p == '\0'){
    80001806:	41650633          	sub	a2,a0,s6
    8000180a:	14fd                	addi	s1,s1,-1
    8000180c:	9b26                	add	s6,s6,s1
    8000180e:	00f60733          	add	a4,a2,a5
    80001812:	00074703          	lbu	a4,0(a4)
    80001816:	df49                	beqz	a4,800017b0 <copyinstr+0x26>
        *dst = *p;
    80001818:	00e78023          	sb	a4,0(a5)
      --max;
    8000181c:	40fb04b3          	sub	s1,s6,a5
      dst++;
    80001820:	0785                	addi	a5,a5,1
    while(n > 0){
    80001822:	ff0796e3          	bne	a5,a6,8000180e <copyinstr+0x84>
      dst++;
    80001826:	8b42                	mv	s6,a6
    80001828:	b775                	j	800017d4 <copyinstr+0x4a>
    8000182a:	4781                	li	a5,0
    8000182c:	b769                	j	800017b6 <copyinstr+0x2c>
      return -1;
    8000182e:	557d                	li	a0,-1
    80001830:	b779                	j	800017be <copyinstr+0x34>
  int got_null = 0;
    80001832:	4781                	li	a5,0
  if(got_null){
    80001834:	0017b793          	seqz	a5,a5
    80001838:	40f00533          	neg	a0,a5
}
    8000183c:	8082                	ret

000000008000183e <find_index>:

// A method to find the index of a process
// inside the process table
int
find_index(int pid)
{
    8000183e:	7179                	addi	sp,sp,-48
    80001840:	f406                	sd	ra,40(sp)
    80001842:	f022                	sd	s0,32(sp)
    80001844:	ec26                	sd	s1,24(sp)
    80001846:	e84a                	sd	s2,16(sp)
    80001848:	e44e                	sd	s3,8(sp)
    8000184a:	1800                	addi	s0,sp,48
    8000184c:	89aa                	mv	s3,a0
  int i = 0;
  struct proc *p;

  for(p = proc; proc < &proc[NPROC]; p++){
    8000184e:	00010497          	auipc	s1,0x10
    80001852:	f4248493          	addi	s1,s1,-190 # 80011790 <proc>
  int i = 0;
    80001856:	4901                	li	s2,0
    acquire(&p->lock);
    80001858:	8526                	mv	a0,s1
    8000185a:	fffff097          	auipc	ra,0xfffff
    8000185e:	38a080e7          	jalr	906(ra) # 80000be4 <acquire>
    if(p->pid == pid){
    80001862:	589c                	lw	a5,48(s1)
    80001864:	01378b63          	beq	a5,s3,8000187a <find_index+0x3c>
      release(&p->lock);
      return i;
    }
    else{
      i+=1;
    80001868:	2905                	addiw	s2,s2,1
    }
    release(&p->lock);
    8000186a:	8526                	mv	a0,s1
    8000186c:	fffff097          	auipc	ra,0xfffff
    80001870:	42c080e7          	jalr	1068(ra) # 80000c98 <release>
  for(p = proc; proc < &proc[NPROC]; p++){
    80001874:	17048493          	addi	s1,s1,368
    acquire(&p->lock);
    80001878:	b7c5                	j	80001858 <find_index+0x1a>
      release(&p->lock);
    8000187a:	8526                	mv	a0,s1
    8000187c:	fffff097          	auipc	ra,0xfffff
    80001880:	41c080e7          	jalr	1052(ra) # 80000c98 <release>
  }
  return -1;
}
    80001884:	854a                	mv	a0,s2
    80001886:	70a2                	ld	ra,40(sp)
    80001888:	7402                	ld	s0,32(sp)
    8000188a:	64e2                	ld	s1,24(sp)
    8000188c:	6942                	ld	s2,16(sp)
    8000188e:	69a2                	ld	s3,8(sp)
    80001890:	6145                	addi	sp,sp,48
    80001892:	8082                	ret

0000000080001894 <add_to_queue_helper>:

// Helper function for queue addition
// deals with inner nodes alone and not with the headlock or the first node
int
add_to_queue_helper(int prev_proc, int curr_proc, int index)
{
    80001894:	715d                	addi	sp,sp,-80
    80001896:	e486                	sd	ra,72(sp)
    80001898:	e0a2                	sd	s0,64(sp)
    8000189a:	fc26                	sd	s1,56(sp)
    8000189c:	f84a                	sd	s2,48(sp)
    8000189e:	f44e                	sd	s3,40(sp)
    800018a0:	f052                	sd	s4,32(sp)
    800018a2:	ec56                	sd	s5,24(sp)
    800018a4:	e85a                	sd	s6,16(sp)
    800018a6:	e45e                	sd	s7,8(sp)
    800018a8:	0880                	addi	s0,sp,80
    800018aa:	8b32                	mv	s6,a2
  struct proc *curr_p = &proc[curr_proc];
    800018ac:	17000793          	li	a5,368
    800018b0:	02f585b3          	mul	a1,a1,a5
    800018b4:	00010997          	auipc	s3,0x10
    800018b8:	edc98993          	addi	s3,s3,-292 # 80011790 <proc>
    800018bc:	01358933          	add	s2,a1,s3
  struct proc *prev_p = &proc[prev_proc];
    800018c0:	02f50533          	mul	a0,a0,a5
    800018c4:	99aa                	add	s3,s3,a0
  
  acquire(&prev_p->lock);
    800018c6:	854e                	mv	a0,s3
    800018c8:	fffff097          	auipc	ra,0xfffff
    800018cc:	31c080e7          	jalr	796(ra) # 80000be4 <acquire>
  acquire(&curr_p->lock);
    800018d0:	854a                	mv	a0,s2
    800018d2:	fffff097          	auipc	ra,0xfffff
    800018d6:	312080e7          	jalr	786(ra) # 80000be4 <acquire>

  while(curr_p->next_node != -1){ // exit the while if we found the last node
    800018da:	03492703          	lw	a4,52(s2) # 1034 <_entry-0x7fffefcc>
    800018de:	57fd                	li	a5,-1
    800018e0:	06f70663          	beq	a4,a5,8000194c <add_to_queue_helper+0xb8>
    // update (curr to be prev) and (next to be curr)
    release(&prev_p->lock);
    prev_p = curr_p;
    curr_p = &proc[prev_p->next_node];
    800018e4:	17000b93          	li	s7,368
    800018e8:	00010a97          	auipc	s5,0x10
    800018ec:	ea8a8a93          	addi	s5,s5,-344 # 80011790 <proc>
  while(curr_p->next_node != -1){ // exit the while if we found the last node
    800018f0:	5a7d                	li	s4,-1
    800018f2:	a011                	j	800018f6 <add_to_queue_helper+0x62>
    curr_p = &proc[prev_p->next_node];
    800018f4:	8926                	mv	s2,s1
    release(&prev_p->lock);
    800018f6:	854e                	mv	a0,s3
    800018f8:	fffff097          	auipc	ra,0xfffff
    800018fc:	3a0080e7          	jalr	928(ra) # 80000c98 <release>
    curr_p = &proc[prev_p->next_node];
    80001900:	03492483          	lw	s1,52(s2)
    80001904:	037484b3          	mul	s1,s1,s7
    80001908:	94d6                	add	s1,s1,s5
    acquire(&curr_p->lock);
    8000190a:	8526                	mv	a0,s1
    8000190c:	fffff097          	auipc	ra,0xfffff
    80001910:	2d8080e7          	jalr	728(ra) # 80000be4 <acquire>
  while(curr_p->next_node != -1){ // exit the while if we found the last node
    80001914:	89ca                	mv	s3,s2
    80001916:	58dc                	lw	a5,52(s1)
    80001918:	fd479ee3          	bne	a5,s4,800018f4 <add_to_queue_helper+0x60>
  }

  // Add the proc to the end of the list.
  curr_p->next_node = index;
    8000191c:	0364aa23          	sw	s6,52(s1)

  release(&prev_p->lock);
    80001920:	854a                	mv	a0,s2
    80001922:	fffff097          	auipc	ra,0xfffff
    80001926:	376080e7          	jalr	886(ra) # 80000c98 <release>
  release(&curr_p->lock);
    8000192a:	8526                	mv	a0,s1
    8000192c:	fffff097          	auipc	ra,0xfffff
    80001930:	36c080e7          	jalr	876(ra) # 80000c98 <release>

  return 1;
}
    80001934:	4505                	li	a0,1
    80001936:	60a6                	ld	ra,72(sp)
    80001938:	6406                	ld	s0,64(sp)
    8000193a:	74e2                	ld	s1,56(sp)
    8000193c:	7942                	ld	s2,48(sp)
    8000193e:	79a2                	ld	s3,40(sp)
    80001940:	7a02                	ld	s4,32(sp)
    80001942:	6ae2                	ld	s5,24(sp)
    80001944:	6b42                	ld	s6,16(sp)
    80001946:	6ba2                	ld	s7,8(sp)
    80001948:	6161                	addi	sp,sp,80
    8000194a:	8082                	ret
  struct proc *curr_p = &proc[curr_proc];
    8000194c:	84ca                	mv	s1,s2
  struct proc *prev_p = &proc[prev_proc];
    8000194e:	894e                	mv	s2,s3
    80001950:	b7f1                	j	8000191c <add_to_queue_helper+0x88>

0000000080001952 <add_to_queue>:
// into a specific linked list a.k.a queue
// int head: the index of the CPU to capture it's lock
// int index: The index of the process inside proc[NPROC]
int
add_to_queue(int head, int index)
{
    80001952:	7179                	addi	sp,sp,-48
    80001954:	f406                	sd	ra,40(sp)
    80001956:	f022                	sd	s0,32(sp)
    80001958:	ec26                	sd	s1,24(sp)
    8000195a:	e84a                	sd	s2,16(sp)
    8000195c:	e44e                	sd	s3,8(sp)
    8000195e:	e052                	sd	s4,0(sp)
    80001960:	1800                	addi	s0,sp,48
    80001962:	84aa                	mv	s1,a0
    80001964:	892e                	mv	s2,a1
  acquire(&cpu_queue_locks[head]);
    80001966:	00151793          	slli	a5,a0,0x1
    8000196a:	97aa                	add	a5,a5,a0
    8000196c:	078e                	slli	a5,a5,0x3
    8000196e:	00010997          	auipc	s3,0x10
    80001972:	93298993          	addi	s3,s3,-1742 # 800112a0 <cpu_queue_locks>
    80001976:	99be                	add	s3,s3,a5
    80001978:	854e                	mv	a0,s3
    8000197a:	fffff097          	auipc	ra,0xfffff
    8000197e:	26a080e7          	jalr	618(ra) # 80000be4 <acquire>

  // If the queue is empty, just update the head pointer
  if(cpus_ready_list[head] == -1){
    80001982:	00249713          	slli	a4,s1,0x2
    80001986:	00007797          	auipc	a5,0x7
    8000198a:	f1a78793          	addi	a5,a5,-230 # 800088a0 <cpus_ready_list>
    8000198e:	97ba                	add	a5,a5,a4
    80001990:	0007aa03          	lw	s4,0(a5)
    80001994:	57fd                	li	a5,-1
    80001996:	06fa0863          	beq	s4,a5,80001a06 <add_to_queue+0xb4>
    return 1;
  }
  else {
    int first_node = cpus_ready_list[head];
    struct proc *p = &proc[first_node];
    acquire(&p->lock);
    8000199a:	17000493          	li	s1,368
    8000199e:	029a04b3          	mul	s1,s4,s1
    800019a2:	00010797          	auipc	a5,0x10
    800019a6:	dee78793          	addi	a5,a5,-530 # 80011790 <proc>
    800019aa:	94be                	add	s1,s1,a5
    800019ac:	8526                	mv	a0,s1
    800019ae:	fffff097          	auipc	ra,0xfffff
    800019b2:	236080e7          	jalr	566(ra) # 80000be4 <acquire>

    // if there is only 1 node, then update it's next_node release all and return
    if(p->next_node == -1){
    800019b6:	58d8                	lw	a4,52(s1)
    800019b8:	57fd                	li	a5,-1
    800019ba:	06f70563          	beq	a4,a5,80001a24 <add_to_queue+0xd2>
    } 
    // else there are 2 nodes or more,
    // then use add_to_queue with prev_node function.
    // for concurrency reasons
    else { 
      release(&cpu_queue_locks[head]);
    800019be:	854e                	mv	a0,s3
    800019c0:	fffff097          	auipc	ra,0xfffff
    800019c4:	2d8080e7          	jalr	728(ra) # 80000c98 <release>
      int next_node = p->next_node;
    800019c8:	17000793          	li	a5,368
    800019cc:	02fa0733          	mul	a4,s4,a5
    800019d0:	00010797          	auipc	a5,0x10
    800019d4:	dc078793          	addi	a5,a5,-576 # 80011790 <proc>
    800019d8:	97ba                	add	a5,a5,a4
    800019da:	0347a983          	lw	s3,52(a5)
      release(&p->lock);
    800019de:	8526                	mv	a0,s1
    800019e0:	fffff097          	auipc	ra,0xfffff
    800019e4:	2b8080e7          	jalr	696(ra) # 80000c98 <release>
      return add_to_queue_helper(first_node, next_node, index);
    800019e8:	864a                	mv	a2,s2
    800019ea:	85ce                	mv	a1,s3
    800019ec:	8552                	mv	a0,s4
    800019ee:	00000097          	auipc	ra,0x0
    800019f2:	ea6080e7          	jalr	-346(ra) # 80001894 <add_to_queue_helper>
    }
  }
  release(&cpu_queue_locks[head]);
  return 0;
}
    800019f6:	70a2                	ld	ra,40(sp)
    800019f8:	7402                	ld	s0,32(sp)
    800019fa:	64e2                	ld	s1,24(sp)
    800019fc:	6942                	ld	s2,16(sp)
    800019fe:	69a2                	ld	s3,8(sp)
    80001a00:	6a02                	ld	s4,0(sp)
    80001a02:	6145                	addi	sp,sp,48
    80001a04:	8082                	ret
    cpus_ready_list[head] = index;
    80001a06:	00007797          	auipc	a5,0x7
    80001a0a:	e9a78793          	addi	a5,a5,-358 # 800088a0 <cpus_ready_list>
    80001a0e:	00e784b3          	add	s1,a5,a4
    80001a12:	0124a023          	sw	s2,0(s1)
    release(&cpu_queue_locks[head]);
    80001a16:	854e                	mv	a0,s3
    80001a18:	fffff097          	auipc	ra,0xfffff
    80001a1c:	280080e7          	jalr	640(ra) # 80000c98 <release>
    return 1;
    80001a20:	4505                	li	a0,1
    80001a22:	bfd1                	j	800019f6 <add_to_queue+0xa4>
      p->next_node = index;
    80001a24:	0324aa23          	sw	s2,52(s1)
      release(&cpu_queue_locks[head]);
    80001a28:	854e                	mv	a0,s3
    80001a2a:	fffff097          	auipc	ra,0xfffff
    80001a2e:	26e080e7          	jalr	622(ra) # 80000c98 <release>
      release(&p->lock);
    80001a32:	8526                	mv	a0,s1
    80001a34:	fffff097          	auipc	ra,0xfffff
    80001a38:	264080e7          	jalr	612(ra) # 80000c98 <release>
      return 1;
    80001a3c:	4505                	li	a0,1
    80001a3e:	bf65                	j	800019f6 <add_to_queue+0xa4>

0000000080001a40 <delete_from_queue_helper>:

// Helper function for queue deletion
// deals with inner nodes alone and not with the head-lock or the first node
int
delete_from_queue_helper(int prev_proc, int curr_proc, int pid)
{
    80001a40:	7139                	addi	sp,sp,-64
    80001a42:	fc06                	sd	ra,56(sp)
    80001a44:	f822                	sd	s0,48(sp)
    80001a46:	f426                	sd	s1,40(sp)
    80001a48:	f04a                	sd	s2,32(sp)
    80001a4a:	ec4e                	sd	s3,24(sp)
    80001a4c:	e852                	sd	s4,16(sp)
    80001a4e:	e456                	sd	s5,8(sp)
    80001a50:	e05a                	sd	s6,0(sp)
    80001a52:	0080                	addi	s0,sp,64
    80001a54:	8a32                	mv	s4,a2
  struct proc *curr_p = &proc[curr_proc];
    80001a56:	17000793          	li	a5,368
    80001a5a:	02f585b3          	mul	a1,a1,a5
    80001a5e:	00010997          	auipc	s3,0x10
    80001a62:	d3298993          	addi	s3,s3,-718 # 80011790 <proc>
    80001a66:	01358933          	add	s2,a1,s3
  struct proc *prev_p = &proc[prev_proc];
    80001a6a:	02f50533          	mul	a0,a0,a5
    80001a6e:	99aa                	add	s3,s3,a0
  
  acquire(&prev_p->lock);
    80001a70:	854e                	mv	a0,s3
    80001a72:	fffff097          	auipc	ra,0xfffff
    80001a76:	172080e7          	jalr	370(ra) # 80000be4 <acquire>
  acquire(&curr_p->lock);
    80001a7a:	854a                	mv	a0,s2
    80001a7c:	fffff097          	auipc	ra,0xfffff
    80001a80:	168080e7          	jalr	360(ra) # 80000be4 <acquire>

  while(curr_p->pid != pid){ // exit the while if we found the node with the desired PID
    80001a84:	03092783          	lw	a5,48(s2)
    80001a88:	07478763          	beq	a5,s4,80001af6 <delete_from_queue_helper+0xb6>
    // update (curr to be prev) and (next to be curr)
    release(&prev_p->lock);
    prev_p = curr_p;
    curr_p = &proc[prev_p->next_node];
    80001a8c:	17000b13          	li	s6,368
    80001a90:	00010a97          	auipc	s5,0x10
    80001a94:	d00a8a93          	addi	s5,s5,-768 # 80011790 <proc>
    80001a98:	a011                	j	80001a9c <delete_from_queue_helper+0x5c>
    80001a9a:	8926                	mv	s2,s1
    release(&prev_p->lock);
    80001a9c:	854e                	mv	a0,s3
    80001a9e:	fffff097          	auipc	ra,0xfffff
    80001aa2:	1fa080e7          	jalr	506(ra) # 80000c98 <release>
    curr_p = &proc[prev_p->next_node];
    80001aa6:	03492483          	lw	s1,52(s2)
    80001aaa:	036484b3          	mul	s1,s1,s6
    80001aae:	94d6                	add	s1,s1,s5
    acquire(&curr_p->lock);
    80001ab0:	8526                	mv	a0,s1
    80001ab2:	fffff097          	auipc	ra,0xfffff
    80001ab6:	132080e7          	jalr	306(ra) # 80000be4 <acquire>
  while(curr_p->pid != pid){ // exit the while if we found the node with the desired PID
    80001aba:	89ca                	mv	s3,s2
    80001abc:	589c                	lw	a5,48(s1)
    80001abe:	fd479ee3          	bne	a5,s4,80001a9a <delete_from_queue_helper+0x5a>
  }

  // Add the proc to the end of the list.
  prev_p->next_node = curr_p->next_node; 
    80001ac2:	58dc                	lw	a5,52(s1)
    80001ac4:	02f92a23          	sw	a5,52(s2)
  curr_p->next_node = -1;
    80001ac8:	57fd                	li	a5,-1
    80001aca:	d8dc                	sw	a5,52(s1)

  release(&prev_p->lock);
    80001acc:	854a                	mv	a0,s2
    80001ace:	fffff097          	auipc	ra,0xfffff
    80001ad2:	1ca080e7          	jalr	458(ra) # 80000c98 <release>
  release(&curr_p->lock);
    80001ad6:	8526                	mv	a0,s1
    80001ad8:	fffff097          	auipc	ra,0xfffff
    80001adc:	1c0080e7          	jalr	448(ra) # 80000c98 <release>

  return 1;
}
    80001ae0:	4505                	li	a0,1
    80001ae2:	70e2                	ld	ra,56(sp)
    80001ae4:	7442                	ld	s0,48(sp)
    80001ae6:	74a2                	ld	s1,40(sp)
    80001ae8:	7902                	ld	s2,32(sp)
    80001aea:	69e2                	ld	s3,24(sp)
    80001aec:	6a42                	ld	s4,16(sp)
    80001aee:	6aa2                	ld	s5,8(sp)
    80001af0:	6b02                	ld	s6,0(sp)
    80001af2:	6121                	addi	sp,sp,64
    80001af4:	8082                	ret
  struct proc *curr_p = &proc[curr_proc];
    80001af6:	84ca                	mv	s1,s2
  struct proc *prev_p = &proc[prev_proc];
    80001af8:	894e                	mv	s2,s3
    80001afa:	b7e1                	j	80001ac2 <delete_from_queue_helper+0x82>

0000000080001afc <delete_from_queue>:

// A function to delete a process from a linked-list 
int
delete_from_queue(int head, int pid)
{
    80001afc:	7139                	addi	sp,sp,-64
    80001afe:	fc06                	sd	ra,56(sp)
    80001b00:	f822                	sd	s0,48(sp)
    80001b02:	f426                	sd	s1,40(sp)
    80001b04:	f04a                	sd	s2,32(sp)
    80001b06:	ec4e                	sd	s3,24(sp)
    80001b08:	e852                	sd	s4,16(sp)
    80001b0a:	e456                	sd	s5,8(sp)
    80001b0c:	0080                	addi	s0,sp,64
    80001b0e:	84aa                	mv	s1,a0
    80001b10:	8a2e                	mv	s4,a1
  acquire(&cpu_queue_locks[head]);
    80001b12:	00151793          	slli	a5,a0,0x1
    80001b16:	97aa                	add	a5,a5,a0
    80001b18:	078e                	slli	a5,a5,0x3
    80001b1a:	0000f997          	auipc	s3,0xf
    80001b1e:	78698993          	addi	s3,s3,1926 # 800112a0 <cpu_queue_locks>
    80001b22:	99be                	add	s3,s3,a5
    80001b24:	854e                	mv	a0,s3
    80001b26:	fffff097          	auipc	ra,0xfffff
    80001b2a:	0be080e7          	jalr	190(ra) # 80000be4 <acquire>

  // If the queue is empty, return false - no element to remove.
  if(cpus_ready_list[head] == -1){
    80001b2e:	00249713          	slli	a4,s1,0x2
    80001b32:	00007797          	auipc	a5,0x7
    80001b36:	d6e78793          	addi	a5,a5,-658 # 800088a0 <cpus_ready_list>
    80001b3a:	97ba                	add	a5,a5,a4
    80001b3c:	0007aa83          	lw	s5,0(a5)
    80001b40:	57fd                	li	a5,-1
    80001b42:	06fa8863          	beq	s5,a5,80001bb2 <delete_from_queue+0xb6>
    return 0;
  }
  else {
    int first_node_index = cpus_ready_list[head];
    struct proc *p = &proc[first_node_index];
    acquire(&p->lock);
    80001b46:	17000913          	li	s2,368
    80001b4a:	032a8933          	mul	s2,s5,s2
    80001b4e:	00010797          	auipc	a5,0x10
    80001b52:	c4278793          	addi	a5,a5,-958 # 80011790 <proc>
    80001b56:	993e                	add	s2,s2,a5
    80001b58:	854a                	mv	a0,s2
    80001b5a:	fffff097          	auipc	ra,0xfffff
    80001b5e:	08a080e7          	jalr	138(ra) # 80000be4 <acquire>

    // if the first node is the 1 to be deleted
    if(p->pid == pid){
    80001b62:	03092783          	lw	a5,48(s2)
    80001b66:	05478d63          	beq	a5,s4,80001bc0 <delete_from_queue+0xc4>
    } 
    // else there are 2 nodes or more,
    // then use add_to_queue with prev_node function.
    // for concurrency reasons
    else { 
      release(&cpu_queue_locks[head]);
    80001b6a:	854e                	mv	a0,s3
    80001b6c:	fffff097          	auipc	ra,0xfffff
    80001b70:	12c080e7          	jalr	300(ra) # 80000c98 <release>
      int next_node = p->next_node;
    80001b74:	17000793          	li	a5,368
    80001b78:	02fa8733          	mul	a4,s5,a5
    80001b7c:	00010797          	auipc	a5,0x10
    80001b80:	c1478793          	addi	a5,a5,-1004 # 80011790 <proc>
    80001b84:	97ba                	add	a5,a5,a4
    80001b86:	5bc4                	lw	s1,52(a5)
      release(&p->lock);
    80001b88:	854a                	mv	a0,s2
    80001b8a:	fffff097          	auipc	ra,0xfffff
    80001b8e:	10e080e7          	jalr	270(ra) # 80000c98 <release>
      return delete_from_queue_helper(first_node_index, next_node, pid);
    80001b92:	8652                	mv	a2,s4
    80001b94:	85a6                	mv	a1,s1
    80001b96:	8556                	mv	a0,s5
    80001b98:	00000097          	auipc	ra,0x0
    80001b9c:	ea8080e7          	jalr	-344(ra) # 80001a40 <delete_from_queue_helper>
    }
  }
  release(&cpu_queue_locks[head]);
  return 0;
}
    80001ba0:	70e2                	ld	ra,56(sp)
    80001ba2:	7442                	ld	s0,48(sp)
    80001ba4:	74a2                	ld	s1,40(sp)
    80001ba6:	7902                	ld	s2,32(sp)
    80001ba8:	69e2                	ld	s3,24(sp)
    80001baa:	6a42                	ld	s4,16(sp)
    80001bac:	6aa2                	ld	s5,8(sp)
    80001bae:	6121                	addi	sp,sp,64
    80001bb0:	8082                	ret
    release(&cpu_queue_locks[head]);
    80001bb2:	854e                	mv	a0,s3
    80001bb4:	fffff097          	auipc	ra,0xfffff
    80001bb8:	0e4080e7          	jalr	228(ra) # 80000c98 <release>
    return 0;
    80001bbc:	4501                	li	a0,0
    80001bbe:	b7cd                	j	80001ba0 <delete_from_queue+0xa4>
      cpus_ready_list[head] = p->next_node;
    80001bc0:	048a                	slli	s1,s1,0x2
    80001bc2:	00007517          	auipc	a0,0x7
    80001bc6:	cde50513          	addi	a0,a0,-802 # 800088a0 <cpus_ready_list>
    80001bca:	94aa                	add	s1,s1,a0
    80001bcc:	03492783          	lw	a5,52(s2)
    80001bd0:	c09c                	sw	a5,0(s1)
      p->next_node = -1;
    80001bd2:	57fd                	li	a5,-1
    80001bd4:	02f92a23          	sw	a5,52(s2)
      release(&cpu_queue_locks[head]);
    80001bd8:	854e                	mv	a0,s3
    80001bda:	fffff097          	auipc	ra,0xfffff
    80001bde:	0be080e7          	jalr	190(ra) # 80000c98 <release>
      release(&p->lock);
    80001be2:	854a                	mv	a0,s2
    80001be4:	fffff097          	auipc	ra,0xfffff
    80001be8:	0b4080e7          	jalr	180(ra) # 80000c98 <release>
      return 1;
    80001bec:	4505                	li	a0,1
    80001bee:	bf4d                	j	80001ba0 <delete_from_queue+0xa4>

0000000080001bf0 <proc_mapstacks>:

// Allocate a page for each process's kernel stack.
// Map it high in memory, followed by an invalid
// guard page.
void
proc_mapstacks(pagetable_t kpgtbl) {
    80001bf0:	7139                	addi	sp,sp,-64
    80001bf2:	fc06                	sd	ra,56(sp)
    80001bf4:	f822                	sd	s0,48(sp)
    80001bf6:	f426                	sd	s1,40(sp)
    80001bf8:	f04a                	sd	s2,32(sp)
    80001bfa:	ec4e                	sd	s3,24(sp)
    80001bfc:	e852                	sd	s4,16(sp)
    80001bfe:	e456                	sd	s5,8(sp)
    80001c00:	e05a                	sd	s6,0(sp)
    80001c02:	0080                	addi	s0,sp,64
    80001c04:	89aa                	mv	s3,a0
  struct proc *p;
  
  for(p = proc; p < &proc[NPROC]; p++) {
    80001c06:	00010497          	auipc	s1,0x10
    80001c0a:	b8a48493          	addi	s1,s1,-1142 # 80011790 <proc>
    char *pa = kalloc();
    if(pa == 0)
      panic("kalloc");
    uint64 va = KSTACK((int) (p - proc));
    80001c0e:	8b26                	mv	s6,s1
    80001c10:	00006a97          	auipc	s5,0x6
    80001c14:	3f0a8a93          	addi	s5,s5,1008 # 80008000 <etext>
    80001c18:	04000937          	lui	s2,0x4000
    80001c1c:	197d                	addi	s2,s2,-1
    80001c1e:	0932                	slli	s2,s2,0xc
  for(p = proc; p < &proc[NPROC]; p++) {
    80001c20:	00015a17          	auipc	s4,0x15
    80001c24:	770a0a13          	addi	s4,s4,1904 # 80017390 <tickslock>
    char *pa = kalloc();
    80001c28:	fffff097          	auipc	ra,0xfffff
    80001c2c:	ecc080e7          	jalr	-308(ra) # 80000af4 <kalloc>
    80001c30:	862a                	mv	a2,a0
    if(pa == 0)
    80001c32:	c131                	beqz	a0,80001c76 <proc_mapstacks+0x86>
    uint64 va = KSTACK((int) (p - proc));
    80001c34:	416485b3          	sub	a1,s1,s6
    80001c38:	8591                	srai	a1,a1,0x4
    80001c3a:	000ab783          	ld	a5,0(s5)
    80001c3e:	02f585b3          	mul	a1,a1,a5
    80001c42:	2585                	addiw	a1,a1,1
    80001c44:	00d5959b          	slliw	a1,a1,0xd
    kvmmap(kpgtbl, va, (uint64)pa, PGSIZE, PTE_R | PTE_W);
    80001c48:	4719                	li	a4,6
    80001c4a:	6685                	lui	a3,0x1
    80001c4c:	40b905b3          	sub	a1,s2,a1
    80001c50:	854e                	mv	a0,s3
    80001c52:	fffff097          	auipc	ra,0xfffff
    80001c56:	4fe080e7          	jalr	1278(ra) # 80001150 <kvmmap>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001c5a:	17048493          	addi	s1,s1,368
    80001c5e:	fd4495e3          	bne	s1,s4,80001c28 <proc_mapstacks+0x38>
  }
}
    80001c62:	70e2                	ld	ra,56(sp)
    80001c64:	7442                	ld	s0,48(sp)
    80001c66:	74a2                	ld	s1,40(sp)
    80001c68:	7902                	ld	s2,32(sp)
    80001c6a:	69e2                	ld	s3,24(sp)
    80001c6c:	6a42                	ld	s4,16(sp)
    80001c6e:	6aa2                	ld	s5,8(sp)
    80001c70:	6b02                	ld	s6,0(sp)
    80001c72:	6121                	addi	sp,sp,64
    80001c74:	8082                	ret
      panic("kalloc");
    80001c76:	00006517          	auipc	a0,0x6
    80001c7a:	56250513          	addi	a0,a0,1378 # 800081d8 <digits+0x198>
    80001c7e:	fffff097          	auipc	ra,0xfffff
    80001c82:	8c0080e7          	jalr	-1856(ra) # 8000053e <panic>

0000000080001c86 <procinit>:

// initialize the proc table at boot time.
void
procinit(void)
{
    80001c86:	7139                	addi	sp,sp,-64
    80001c88:	fc06                	sd	ra,56(sp)
    80001c8a:	f822                	sd	s0,48(sp)
    80001c8c:	f426                	sd	s1,40(sp)
    80001c8e:	f04a                	sd	s2,32(sp)
    80001c90:	ec4e                	sd	s3,24(sp)
    80001c92:	e852                	sd	s4,16(sp)
    80001c94:	e456                	sd	s5,8(sp)
    80001c96:	e05a                	sd	s6,0(sp)
    80001c98:	0080                	addi	s0,sp,64
  struct proc *p;
  struct spinlock *l;
  
  initlock(&pid_lock, "nextpid");
    80001c9a:	00006597          	auipc	a1,0x6
    80001c9e:	54658593          	addi	a1,a1,1350 # 800081e0 <digits+0x1a0>
    80001ca2:	0000f517          	auipc	a0,0xf
    80001ca6:	6be50513          	addi	a0,a0,1726 # 80011360 <pid_lock>
    80001caa:	fffff097          	auipc	ra,0xfffff
    80001cae:	eaa080e7          	jalr	-342(ra) # 80000b54 <initlock>
  initlock(&wait_lock, "wait_lock");
    80001cb2:	00006597          	auipc	a1,0x6
    80001cb6:	53658593          	addi	a1,a1,1334 # 800081e8 <digits+0x1a8>
    80001cba:	0000f517          	auipc	a0,0xf
    80001cbe:	6be50513          	addi	a0,a0,1726 # 80011378 <wait_lock>
    80001cc2:	fffff097          	auipc	ra,0xfffff
    80001cc6:	e92080e7          	jalr	-366(ra) # 80000b54 <initlock>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001cca:	00010497          	auipc	s1,0x10
    80001cce:	ac648493          	addi	s1,s1,-1338 # 80011790 <proc>
      initlock(&p->lock, "proc");
    80001cd2:	00006b17          	auipc	s6,0x6
    80001cd6:	526b0b13          	addi	s6,s6,1318 # 800081f8 <digits+0x1b8>
      p->kstack = KSTACK((int) (p - proc));
    80001cda:	8aa6                	mv	s5,s1
    80001cdc:	00006a17          	auipc	s4,0x6
    80001ce0:	324a0a13          	addi	s4,s4,804 # 80008000 <etext>
    80001ce4:	04000937          	lui	s2,0x4000
    80001ce8:	197d                	addi	s2,s2,-1
    80001cea:	0932                	slli	s2,s2,0xc
  for(p = proc; p < &proc[NPROC]; p++) {
    80001cec:	00015997          	auipc	s3,0x15
    80001cf0:	6a498993          	addi	s3,s3,1700 # 80017390 <tickslock>
      initlock(&p->lock, "proc");
    80001cf4:	85da                	mv	a1,s6
    80001cf6:	8526                	mv	a0,s1
    80001cf8:	fffff097          	auipc	ra,0xfffff
    80001cfc:	e5c080e7          	jalr	-420(ra) # 80000b54 <initlock>
      p->kstack = KSTACK((int) (p - proc));
    80001d00:	415487b3          	sub	a5,s1,s5
    80001d04:	8791                	srai	a5,a5,0x4
    80001d06:	000a3703          	ld	a4,0(s4)
    80001d0a:	02e787b3          	mul	a5,a5,a4
    80001d0e:	2785                	addiw	a5,a5,1
    80001d10:	00d7979b          	slliw	a5,a5,0xd
    80001d14:	40f907b3          	sub	a5,s2,a5
    80001d18:	e4bc                	sd	a5,72(s1)
  for(p = proc; p < &proc[NPROC]; p++) {
    80001d1a:	17048493          	addi	s1,s1,368
    80001d1e:	fd349be3          	bne	s1,s3,80001cf4 <procinit+0x6e>
  }

  // Our addition - initiation of the queues lock
  for(l = cpu_queue_locks; l < &cpu_queue_locks[NCPU]; l++) {
    80001d22:	0000f497          	auipc	s1,0xf
    80001d26:	57e48493          	addi	s1,s1,1406 # 800112a0 <cpu_queue_locks>
      initlock(l, "ready_queue");
    80001d2a:	00006997          	auipc	s3,0x6
    80001d2e:	4d698993          	addi	s3,s3,1238 # 80008200 <digits+0x1c0>
  for(l = cpu_queue_locks; l < &cpu_queue_locks[NCPU]; l++) {
    80001d32:	0000f917          	auipc	s2,0xf
    80001d36:	62e90913          	addi	s2,s2,1582 # 80011360 <pid_lock>
      initlock(l, "ready_queue");
    80001d3a:	85ce                	mv	a1,s3
    80001d3c:	8526                	mv	a0,s1
    80001d3e:	fffff097          	auipc	ra,0xfffff
    80001d42:	e16080e7          	jalr	-490(ra) # 80000b54 <initlock>
  for(l = cpu_queue_locks; l < &cpu_queue_locks[NCPU]; l++) {
    80001d46:	04e1                	addi	s1,s1,24
    80001d48:	ff2499e3          	bne	s1,s2,80001d3a <procinit+0xb4>
  }

  // End of our addition
}
    80001d4c:	70e2                	ld	ra,56(sp)
    80001d4e:	7442                	ld	s0,48(sp)
    80001d50:	74a2                	ld	s1,40(sp)
    80001d52:	7902                	ld	s2,32(sp)
    80001d54:	69e2                	ld	s3,24(sp)
    80001d56:	6a42                	ld	s4,16(sp)
    80001d58:	6aa2                	ld	s5,8(sp)
    80001d5a:	6b02                	ld	s6,0(sp)
    80001d5c:	6121                	addi	sp,sp,64
    80001d5e:	8082                	ret

0000000080001d60 <cpuid>:
// Must be called with interrupts disabled,
// to prevent race with process being moved
// to a different CPU.
int
cpuid()
{
    80001d60:	1141                	addi	sp,sp,-16
    80001d62:	e422                	sd	s0,8(sp)
    80001d64:	0800                	addi	s0,sp,16
  asm volatile("mv %0, tp" : "=r" (x) );
    80001d66:	8512                	mv	a0,tp
  int id = r_tp();
  return id;
}
    80001d68:	2501                	sext.w	a0,a0
    80001d6a:	6422                	ld	s0,8(sp)
    80001d6c:	0141                	addi	sp,sp,16
    80001d6e:	8082                	ret

0000000080001d70 <mycpu>:

// Return this CPU's cpu struct.
// Interrupts must be disabled.
struct cpu*
mycpu(void) {
    80001d70:	1141                	addi	sp,sp,-16
    80001d72:	e422                	sd	s0,8(sp)
    80001d74:	0800                	addi	s0,sp,16
    80001d76:	8792                	mv	a5,tp
  int id = cpuid();
  struct cpu *c = &cpus[id];
    80001d78:	2781                	sext.w	a5,a5
    80001d7a:	079e                	slli	a5,a5,0x7
  return c;
}
    80001d7c:	0000f517          	auipc	a0,0xf
    80001d80:	61450513          	addi	a0,a0,1556 # 80011390 <cpus>
    80001d84:	953e                	add	a0,a0,a5
    80001d86:	6422                	ld	s0,8(sp)
    80001d88:	0141                	addi	sp,sp,16
    80001d8a:	8082                	ret

0000000080001d8c <myproc>:

// Return the current struct proc *, or zero if none.
struct proc*
myproc(void) {
    80001d8c:	1101                	addi	sp,sp,-32
    80001d8e:	ec06                	sd	ra,24(sp)
    80001d90:	e822                	sd	s0,16(sp)
    80001d92:	e426                	sd	s1,8(sp)
    80001d94:	1000                	addi	s0,sp,32
  push_off();
    80001d96:	fffff097          	auipc	ra,0xfffff
    80001d9a:	e02080e7          	jalr	-510(ra) # 80000b98 <push_off>
    80001d9e:	8792                	mv	a5,tp
  struct cpu *c = mycpu();
  struct proc *p = c->proc;
    80001da0:	2781                	sext.w	a5,a5
    80001da2:	079e                	slli	a5,a5,0x7
    80001da4:	0000f717          	auipc	a4,0xf
    80001da8:	4fc70713          	addi	a4,a4,1276 # 800112a0 <cpu_queue_locks>
    80001dac:	97ba                	add	a5,a5,a4
    80001dae:	7be4                	ld	s1,240(a5)
  pop_off();
    80001db0:	fffff097          	auipc	ra,0xfffff
    80001db4:	e88080e7          	jalr	-376(ra) # 80000c38 <pop_off>
  return p;
}
    80001db8:	8526                	mv	a0,s1
    80001dba:	60e2                	ld	ra,24(sp)
    80001dbc:	6442                	ld	s0,16(sp)
    80001dbe:	64a2                	ld	s1,8(sp)
    80001dc0:	6105                	addi	sp,sp,32
    80001dc2:	8082                	ret

0000000080001dc4 <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch to forkret.
void
forkret(void)
{
    80001dc4:	1141                	addi	sp,sp,-16
    80001dc6:	e406                	sd	ra,8(sp)
    80001dc8:	e022                	sd	s0,0(sp)
    80001dca:	0800                	addi	s0,sp,16
  static int first = 1;

  // Still holding p->lock from scheduler.
  release(&myproc()->lock);
    80001dcc:	00000097          	auipc	ra,0x0
    80001dd0:	fc0080e7          	jalr	-64(ra) # 80001d8c <myproc>
    80001dd4:	fffff097          	auipc	ra,0xfffff
    80001dd8:	ec4080e7          	jalr	-316(ra) # 80000c98 <release>

  if (first) {
    80001ddc:	00007797          	auipc	a5,0x7
    80001de0:	aa47a783          	lw	a5,-1372(a5) # 80008880 <first.1732>
    80001de4:	eb89                	bnez	a5,80001df6 <forkret+0x32>
    // be run from main().
    first = 0;
    fsinit(ROOTDEV);
  }

  usertrapret();
    80001de6:	00001097          	auipc	ra,0x1
    80001dea:	c96080e7          	jalr	-874(ra) # 80002a7c <usertrapret>
}
    80001dee:	60a2                	ld	ra,8(sp)
    80001df0:	6402                	ld	s0,0(sp)
    80001df2:	0141                	addi	sp,sp,16
    80001df4:	8082                	ret
    first = 0;
    80001df6:	00007797          	auipc	a5,0x7
    80001dfa:	a807a523          	sw	zero,-1398(a5) # 80008880 <first.1732>
    fsinit(ROOTDEV);
    80001dfe:	4505                	li	a0,1
    80001e00:	00002097          	auipc	ra,0x2
    80001e04:	9d6080e7          	jalr	-1578(ra) # 800037d6 <fsinit>
    80001e08:	bff9                	j	80001de6 <forkret+0x22>

0000000080001e0a <allocpid>:
allocpid() {
    80001e0a:	7179                	addi	sp,sp,-48
    80001e0c:	f406                	sd	ra,40(sp)
    80001e0e:	f022                	sd	s0,32(sp)
    80001e10:	ec26                	sd	s1,24(sp)
    80001e12:	e84a                	sd	s2,16(sp)
    80001e14:	e44e                	sd	s3,8(sp)
    80001e16:	1800                	addi	s0,sp,48
    pid = nextpid;
    80001e18:	00007917          	auipc	s2,0x7
    80001e1c:	a6c90913          	addi	s2,s2,-1428 # 80008884 <nextpid>
  } while(cas(&nextpid, nextpid, nextpid + 1 ) == 1);
    80001e20:	4985                	li	s3,1
    pid = nextpid;
    80001e22:	00092483          	lw	s1,0(s2)
  } while(cas(&nextpid, nextpid, nextpid + 1 ) == 1);
    80001e26:	0014861b          	addiw	a2,s1,1
    80001e2a:	85a6                	mv	a1,s1
    80001e2c:	854a                	mv	a0,s2
    80001e2e:	00004097          	auipc	ra,0x4
    80001e32:	7a8080e7          	jalr	1960(ra) # 800065d6 <cas>
    80001e36:	ff3506e3          	beq	a0,s3,80001e22 <allocpid+0x18>
}
    80001e3a:	8526                	mv	a0,s1
    80001e3c:	70a2                	ld	ra,40(sp)
    80001e3e:	7402                	ld	s0,32(sp)
    80001e40:	64e2                	ld	s1,24(sp)
    80001e42:	6942                	ld	s2,16(sp)
    80001e44:	69a2                	ld	s3,8(sp)
    80001e46:	6145                	addi	sp,sp,48
    80001e48:	8082                	ret

0000000080001e4a <proc_pagetable>:
{
    80001e4a:	1101                	addi	sp,sp,-32
    80001e4c:	ec06                	sd	ra,24(sp)
    80001e4e:	e822                	sd	s0,16(sp)
    80001e50:	e426                	sd	s1,8(sp)
    80001e52:	e04a                	sd	s2,0(sp)
    80001e54:	1000                	addi	s0,sp,32
    80001e56:	892a                	mv	s2,a0
  pagetable = uvmcreate();
    80001e58:	fffff097          	auipc	ra,0xfffff
    80001e5c:	4e2080e7          	jalr	1250(ra) # 8000133a <uvmcreate>
    80001e60:	84aa                	mv	s1,a0
  if(pagetable == 0)
    80001e62:	c121                	beqz	a0,80001ea2 <proc_pagetable+0x58>
  if(mappages(pagetable, TRAMPOLINE, PGSIZE,
    80001e64:	4729                	li	a4,10
    80001e66:	00005697          	auipc	a3,0x5
    80001e6a:	19a68693          	addi	a3,a3,410 # 80007000 <_trampoline>
    80001e6e:	6605                	lui	a2,0x1
    80001e70:	040005b7          	lui	a1,0x4000
    80001e74:	15fd                	addi	a1,a1,-1
    80001e76:	05b2                	slli	a1,a1,0xc
    80001e78:	fffff097          	auipc	ra,0xfffff
    80001e7c:	238080e7          	jalr	568(ra) # 800010b0 <mappages>
    80001e80:	02054863          	bltz	a0,80001eb0 <proc_pagetable+0x66>
  if(mappages(pagetable, TRAPFRAME, PGSIZE,
    80001e84:	4719                	li	a4,6
    80001e86:	06093683          	ld	a3,96(s2)
    80001e8a:	6605                	lui	a2,0x1
    80001e8c:	020005b7          	lui	a1,0x2000
    80001e90:	15fd                	addi	a1,a1,-1
    80001e92:	05b6                	slli	a1,a1,0xd
    80001e94:	8526                	mv	a0,s1
    80001e96:	fffff097          	auipc	ra,0xfffff
    80001e9a:	21a080e7          	jalr	538(ra) # 800010b0 <mappages>
    80001e9e:	02054163          	bltz	a0,80001ec0 <proc_pagetable+0x76>
}
    80001ea2:	8526                	mv	a0,s1
    80001ea4:	60e2                	ld	ra,24(sp)
    80001ea6:	6442                	ld	s0,16(sp)
    80001ea8:	64a2                	ld	s1,8(sp)
    80001eaa:	6902                	ld	s2,0(sp)
    80001eac:	6105                	addi	sp,sp,32
    80001eae:	8082                	ret
    uvmfree(pagetable, 0);
    80001eb0:	4581                	li	a1,0
    80001eb2:	8526                	mv	a0,s1
    80001eb4:	fffff097          	auipc	ra,0xfffff
    80001eb8:	682080e7          	jalr	1666(ra) # 80001536 <uvmfree>
    return 0;
    80001ebc:	4481                	li	s1,0
    80001ebe:	b7d5                	j	80001ea2 <proc_pagetable+0x58>
    uvmunmap(pagetable, TRAMPOLINE, 1, 0);
    80001ec0:	4681                	li	a3,0
    80001ec2:	4605                	li	a2,1
    80001ec4:	040005b7          	lui	a1,0x4000
    80001ec8:	15fd                	addi	a1,a1,-1
    80001eca:	05b2                	slli	a1,a1,0xc
    80001ecc:	8526                	mv	a0,s1
    80001ece:	fffff097          	auipc	ra,0xfffff
    80001ed2:	3a8080e7          	jalr	936(ra) # 80001276 <uvmunmap>
    uvmfree(pagetable, 0);
    80001ed6:	4581                	li	a1,0
    80001ed8:	8526                	mv	a0,s1
    80001eda:	fffff097          	auipc	ra,0xfffff
    80001ede:	65c080e7          	jalr	1628(ra) # 80001536 <uvmfree>
    return 0;
    80001ee2:	4481                	li	s1,0
    80001ee4:	bf7d                	j	80001ea2 <proc_pagetable+0x58>

0000000080001ee6 <proc_freepagetable>:
{
    80001ee6:	1101                	addi	sp,sp,-32
    80001ee8:	ec06                	sd	ra,24(sp)
    80001eea:	e822                	sd	s0,16(sp)
    80001eec:	e426                	sd	s1,8(sp)
    80001eee:	e04a                	sd	s2,0(sp)
    80001ef0:	1000                	addi	s0,sp,32
    80001ef2:	84aa                	mv	s1,a0
    80001ef4:	892e                	mv	s2,a1
  uvmunmap(pagetable, TRAMPOLINE, 1, 0);
    80001ef6:	4681                	li	a3,0
    80001ef8:	4605                	li	a2,1
    80001efa:	040005b7          	lui	a1,0x4000
    80001efe:	15fd                	addi	a1,a1,-1
    80001f00:	05b2                	slli	a1,a1,0xc
    80001f02:	fffff097          	auipc	ra,0xfffff
    80001f06:	374080e7          	jalr	884(ra) # 80001276 <uvmunmap>
  uvmunmap(pagetable, TRAPFRAME, 1, 0);
    80001f0a:	4681                	li	a3,0
    80001f0c:	4605                	li	a2,1
    80001f0e:	020005b7          	lui	a1,0x2000
    80001f12:	15fd                	addi	a1,a1,-1
    80001f14:	05b6                	slli	a1,a1,0xd
    80001f16:	8526                	mv	a0,s1
    80001f18:	fffff097          	auipc	ra,0xfffff
    80001f1c:	35e080e7          	jalr	862(ra) # 80001276 <uvmunmap>
  uvmfree(pagetable, sz);
    80001f20:	85ca                	mv	a1,s2
    80001f22:	8526                	mv	a0,s1
    80001f24:	fffff097          	auipc	ra,0xfffff
    80001f28:	612080e7          	jalr	1554(ra) # 80001536 <uvmfree>
}
    80001f2c:	60e2                	ld	ra,24(sp)
    80001f2e:	6442                	ld	s0,16(sp)
    80001f30:	64a2                	ld	s1,8(sp)
    80001f32:	6902                	ld	s2,0(sp)
    80001f34:	6105                	addi	sp,sp,32
    80001f36:	8082                	ret

0000000080001f38 <freeproc>:
{
    80001f38:	1101                	addi	sp,sp,-32
    80001f3a:	ec06                	sd	ra,24(sp)
    80001f3c:	e822                	sd	s0,16(sp)
    80001f3e:	e426                	sd	s1,8(sp)
    80001f40:	1000                	addi	s0,sp,32
    80001f42:	84aa                	mv	s1,a0
  if(p->trapframe)
    80001f44:	7128                	ld	a0,96(a0)
    80001f46:	c509                	beqz	a0,80001f50 <freeproc+0x18>
    kfree((void*)p->trapframe);
    80001f48:	fffff097          	auipc	ra,0xfffff
    80001f4c:	ab0080e7          	jalr	-1360(ra) # 800009f8 <kfree>
  p->trapframe = 0;
    80001f50:	0604b023          	sd	zero,96(s1)
  if(p->pagetable)
    80001f54:	6ca8                	ld	a0,88(s1)
    80001f56:	c511                	beqz	a0,80001f62 <freeproc+0x2a>
    proc_freepagetable(p->pagetable, p->sz);
    80001f58:	68ac                	ld	a1,80(s1)
    80001f5a:	00000097          	auipc	ra,0x0
    80001f5e:	f8c080e7          	jalr	-116(ra) # 80001ee6 <proc_freepagetable>
  p->pagetable = 0;
    80001f62:	0404bc23          	sd	zero,88(s1)
  p->sz = 0;
    80001f66:	0404b823          	sd	zero,80(s1)
  p->pid = 0;
    80001f6a:	0204a823          	sw	zero,48(s1)
  p->parent = 0;
    80001f6e:	0404b023          	sd	zero,64(s1)
  p->name[0] = 0;
    80001f72:	16048023          	sb	zero,352(s1)
  p->chan = 0;
    80001f76:	0204b023          	sd	zero,32(s1)
  p->killed = 0;
    80001f7a:	0204a423          	sw	zero,40(s1)
  p->xstate = 0;
    80001f7e:	0204a623          	sw	zero,44(s1)
  p->state = UNUSED;
    80001f82:	0004ac23          	sw	zero,24(s1)
}
    80001f86:	60e2                	ld	ra,24(sp)
    80001f88:	6442                	ld	s0,16(sp)
    80001f8a:	64a2                	ld	s1,8(sp)
    80001f8c:	6105                	addi	sp,sp,32
    80001f8e:	8082                	ret

0000000080001f90 <allocproc>:
{
    80001f90:	1101                	addi	sp,sp,-32
    80001f92:	ec06                	sd	ra,24(sp)
    80001f94:	e822                	sd	s0,16(sp)
    80001f96:	e426                	sd	s1,8(sp)
    80001f98:	e04a                	sd	s2,0(sp)
    80001f9a:	1000                	addi	s0,sp,32
  for(p = proc; p < &proc[NPROC]; p++) {
    80001f9c:	0000f497          	auipc	s1,0xf
    80001fa0:	7f448493          	addi	s1,s1,2036 # 80011790 <proc>
    80001fa4:	00015917          	auipc	s2,0x15
    80001fa8:	3ec90913          	addi	s2,s2,1004 # 80017390 <tickslock>
    acquire(&p->lock);
    80001fac:	8526                	mv	a0,s1
    80001fae:	fffff097          	auipc	ra,0xfffff
    80001fb2:	c36080e7          	jalr	-970(ra) # 80000be4 <acquire>
    if(p->state == UNUSED) {
    80001fb6:	4c9c                	lw	a5,24(s1)
    80001fb8:	cf81                	beqz	a5,80001fd0 <allocproc+0x40>
      release(&p->lock);
    80001fba:	8526                	mv	a0,s1
    80001fbc:	fffff097          	auipc	ra,0xfffff
    80001fc0:	cdc080e7          	jalr	-804(ra) # 80000c98 <release>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001fc4:	17048493          	addi	s1,s1,368
    80001fc8:	ff2492e3          	bne	s1,s2,80001fac <allocproc+0x1c>
  return 0;
    80001fcc:	4481                	li	s1,0
    80001fce:	a889                	j	80002020 <allocproc+0x90>
  p->pid = allocpid();
    80001fd0:	00000097          	auipc	ra,0x0
    80001fd4:	e3a080e7          	jalr	-454(ra) # 80001e0a <allocpid>
    80001fd8:	d888                	sw	a0,48(s1)
  p->state = USED;
    80001fda:	4785                	li	a5,1
    80001fdc:	cc9c                	sw	a5,24(s1)
  if((p->trapframe = (struct trapframe *)kalloc()) == 0){
    80001fde:	fffff097          	auipc	ra,0xfffff
    80001fe2:	b16080e7          	jalr	-1258(ra) # 80000af4 <kalloc>
    80001fe6:	892a                	mv	s2,a0
    80001fe8:	f0a8                	sd	a0,96(s1)
    80001fea:	c131                	beqz	a0,8000202e <allocproc+0x9e>
  p->pagetable = proc_pagetable(p);
    80001fec:	8526                	mv	a0,s1
    80001fee:	00000097          	auipc	ra,0x0
    80001ff2:	e5c080e7          	jalr	-420(ra) # 80001e4a <proc_pagetable>
    80001ff6:	892a                	mv	s2,a0
    80001ff8:	eca8                	sd	a0,88(s1)
  if(p->pagetable == 0){
    80001ffa:	c531                	beqz	a0,80002046 <allocproc+0xb6>
  memset(&p->context, 0, sizeof(p->context));
    80001ffc:	07000613          	li	a2,112
    80002000:	4581                	li	a1,0
    80002002:	06848513          	addi	a0,s1,104
    80002006:	fffff097          	auipc	ra,0xfffff
    8000200a:	cda080e7          	jalr	-806(ra) # 80000ce0 <memset>
  p->context.ra = (uint64)forkret;
    8000200e:	00000797          	auipc	a5,0x0
    80002012:	db678793          	addi	a5,a5,-586 # 80001dc4 <forkret>
    80002016:	f4bc                	sd	a5,104(s1)
  p->context.sp = p->kstack + PGSIZE;
    80002018:	64bc                	ld	a5,72(s1)
    8000201a:	6705                	lui	a4,0x1
    8000201c:	97ba                	add	a5,a5,a4
    8000201e:	f8bc                	sd	a5,112(s1)
}
    80002020:	8526                	mv	a0,s1
    80002022:	60e2                	ld	ra,24(sp)
    80002024:	6442                	ld	s0,16(sp)
    80002026:	64a2                	ld	s1,8(sp)
    80002028:	6902                	ld	s2,0(sp)
    8000202a:	6105                	addi	sp,sp,32
    8000202c:	8082                	ret
    freeproc(p);
    8000202e:	8526                	mv	a0,s1
    80002030:	00000097          	auipc	ra,0x0
    80002034:	f08080e7          	jalr	-248(ra) # 80001f38 <freeproc>
    release(&p->lock);
    80002038:	8526                	mv	a0,s1
    8000203a:	fffff097          	auipc	ra,0xfffff
    8000203e:	c5e080e7          	jalr	-930(ra) # 80000c98 <release>
    return 0;
    80002042:	84ca                	mv	s1,s2
    80002044:	bff1                	j	80002020 <allocproc+0x90>
    freeproc(p);
    80002046:	8526                	mv	a0,s1
    80002048:	00000097          	auipc	ra,0x0
    8000204c:	ef0080e7          	jalr	-272(ra) # 80001f38 <freeproc>
    release(&p->lock);
    80002050:	8526                	mv	a0,s1
    80002052:	fffff097          	auipc	ra,0xfffff
    80002056:	c46080e7          	jalr	-954(ra) # 80000c98 <release>
    return 0;
    8000205a:	84ca                	mv	s1,s2
    8000205c:	b7d1                	j	80002020 <allocproc+0x90>

000000008000205e <userinit>:
{
    8000205e:	1101                	addi	sp,sp,-32
    80002060:	ec06                	sd	ra,24(sp)
    80002062:	e822                	sd	s0,16(sp)
    80002064:	e426                	sd	s1,8(sp)
    80002066:	1000                	addi	s0,sp,32
  p = allocproc();
    80002068:	00000097          	auipc	ra,0x0
    8000206c:	f28080e7          	jalr	-216(ra) # 80001f90 <allocproc>
    80002070:	84aa                	mv	s1,a0
  initproc = p;
    80002072:	00007797          	auipc	a5,0x7
    80002076:	faa7bb23          	sd	a0,-74(a5) # 80009028 <initproc>
  uvminit(p->pagetable, initcode, sizeof(initcode));
    8000207a:	03400613          	li	a2,52
    8000207e:	00007597          	auipc	a1,0x7
    80002082:	84258593          	addi	a1,a1,-1982 # 800088c0 <initcode>
    80002086:	6d28                	ld	a0,88(a0)
    80002088:	fffff097          	auipc	ra,0xfffff
    8000208c:	2e0080e7          	jalr	736(ra) # 80001368 <uvminit>
  p->sz = PGSIZE;
    80002090:	6785                	lui	a5,0x1
    80002092:	e8bc                	sd	a5,80(s1)
  p->trapframe->epc = 0;      // user program counter
    80002094:	70b8                	ld	a4,96(s1)
    80002096:	00073c23          	sd	zero,24(a4) # 1018 <_entry-0x7fffefe8>
  p->trapframe->sp = PGSIZE;  // user stack pointer
    8000209a:	70b8                	ld	a4,96(s1)
    8000209c:	fb1c                	sd	a5,48(a4)
  safestrcpy(p->name, "initcode", sizeof(p->name));
    8000209e:	4641                	li	a2,16
    800020a0:	00006597          	auipc	a1,0x6
    800020a4:	17058593          	addi	a1,a1,368 # 80008210 <digits+0x1d0>
    800020a8:	16048513          	addi	a0,s1,352
    800020ac:	fffff097          	auipc	ra,0xfffff
    800020b0:	d86080e7          	jalr	-634(ra) # 80000e32 <safestrcpy>
  p->cwd = namei("/");
    800020b4:	00006517          	auipc	a0,0x6
    800020b8:	16c50513          	addi	a0,a0,364 # 80008220 <digits+0x1e0>
    800020bc:	00002097          	auipc	ra,0x2
    800020c0:	148080e7          	jalr	328(ra) # 80004204 <namei>
    800020c4:	14a4bc23          	sd	a0,344(s1)
  p->state = RUNNABLE;
    800020c8:	478d                	li	a5,3
    800020ca:	cc9c                	sw	a5,24(s1)
  release(&p->lock);
    800020cc:	8526                	mv	a0,s1
    800020ce:	fffff097          	auipc	ra,0xfffff
    800020d2:	bca080e7          	jalr	-1078(ra) # 80000c98 <release>
}
    800020d6:	60e2                	ld	ra,24(sp)
    800020d8:	6442                	ld	s0,16(sp)
    800020da:	64a2                	ld	s1,8(sp)
    800020dc:	6105                	addi	sp,sp,32
    800020de:	8082                	ret

00000000800020e0 <growproc>:
{
    800020e0:	1101                	addi	sp,sp,-32
    800020e2:	ec06                	sd	ra,24(sp)
    800020e4:	e822                	sd	s0,16(sp)
    800020e6:	e426                	sd	s1,8(sp)
    800020e8:	e04a                	sd	s2,0(sp)
    800020ea:	1000                	addi	s0,sp,32
    800020ec:	84aa                	mv	s1,a0
  struct proc *p = myproc();
    800020ee:	00000097          	auipc	ra,0x0
    800020f2:	c9e080e7          	jalr	-866(ra) # 80001d8c <myproc>
    800020f6:	892a                	mv	s2,a0
  sz = p->sz;
    800020f8:	692c                	ld	a1,80(a0)
    800020fa:	0005861b          	sext.w	a2,a1
  if(n > 0){
    800020fe:	00904f63          	bgtz	s1,8000211c <growproc+0x3c>
  } else if(n < 0){
    80002102:	0204cc63          	bltz	s1,8000213a <growproc+0x5a>
  p->sz = sz;
    80002106:	1602                	slli	a2,a2,0x20
    80002108:	9201                	srli	a2,a2,0x20
    8000210a:	04c93823          	sd	a2,80(s2)
  return 0;
    8000210e:	4501                	li	a0,0
}
    80002110:	60e2                	ld	ra,24(sp)
    80002112:	6442                	ld	s0,16(sp)
    80002114:	64a2                	ld	s1,8(sp)
    80002116:	6902                	ld	s2,0(sp)
    80002118:	6105                	addi	sp,sp,32
    8000211a:	8082                	ret
    if((sz = uvmalloc(p->pagetable, sz, sz + n)) == 0) {
    8000211c:	9e25                	addw	a2,a2,s1
    8000211e:	1602                	slli	a2,a2,0x20
    80002120:	9201                	srli	a2,a2,0x20
    80002122:	1582                	slli	a1,a1,0x20
    80002124:	9181                	srli	a1,a1,0x20
    80002126:	6d28                	ld	a0,88(a0)
    80002128:	fffff097          	auipc	ra,0xfffff
    8000212c:	2fa080e7          	jalr	762(ra) # 80001422 <uvmalloc>
    80002130:	0005061b          	sext.w	a2,a0
    80002134:	fa69                	bnez	a2,80002106 <growproc+0x26>
      return -1;
    80002136:	557d                	li	a0,-1
    80002138:	bfe1                	j	80002110 <growproc+0x30>
    sz = uvmdealloc(p->pagetable, sz, sz + n);
    8000213a:	9e25                	addw	a2,a2,s1
    8000213c:	1602                	slli	a2,a2,0x20
    8000213e:	9201                	srli	a2,a2,0x20
    80002140:	1582                	slli	a1,a1,0x20
    80002142:	9181                	srli	a1,a1,0x20
    80002144:	6d28                	ld	a0,88(a0)
    80002146:	fffff097          	auipc	ra,0xfffff
    8000214a:	294080e7          	jalr	660(ra) # 800013da <uvmdealloc>
    8000214e:	0005061b          	sext.w	a2,a0
    80002152:	bf55                	j	80002106 <growproc+0x26>

0000000080002154 <fork>:
{
    80002154:	7179                	addi	sp,sp,-48
    80002156:	f406                	sd	ra,40(sp)
    80002158:	f022                	sd	s0,32(sp)
    8000215a:	ec26                	sd	s1,24(sp)
    8000215c:	e84a                	sd	s2,16(sp)
    8000215e:	e44e                	sd	s3,8(sp)
    80002160:	e052                	sd	s4,0(sp)
    80002162:	1800                	addi	s0,sp,48
  struct proc *p = myproc();
    80002164:	00000097          	auipc	ra,0x0
    80002168:	c28080e7          	jalr	-984(ra) # 80001d8c <myproc>
    8000216c:	892a                	mv	s2,a0
  if((np = allocproc()) == 0){
    8000216e:	00000097          	auipc	ra,0x0
    80002172:	e22080e7          	jalr	-478(ra) # 80001f90 <allocproc>
    80002176:	10050f63          	beqz	a0,80002294 <fork+0x140>
    8000217a:	89aa                	mv	s3,a0
  if(uvmcopy(p->pagetable, np->pagetable, p->sz) < 0){
    8000217c:	05093603          	ld	a2,80(s2)
    80002180:	6d2c                	ld	a1,88(a0)
    80002182:	05893503          	ld	a0,88(s2)
    80002186:	fffff097          	auipc	ra,0xfffff
    8000218a:	3e8080e7          	jalr	1000(ra) # 8000156e <uvmcopy>
    8000218e:	04054663          	bltz	a0,800021da <fork+0x86>
  np->sz = p->sz;
    80002192:	05093783          	ld	a5,80(s2)
    80002196:	04f9b823          	sd	a5,80(s3)
  *(np->trapframe) = *(p->trapframe);
    8000219a:	06093683          	ld	a3,96(s2)
    8000219e:	87b6                	mv	a5,a3
    800021a0:	0609b703          	ld	a4,96(s3)
    800021a4:	12068693          	addi	a3,a3,288
    800021a8:	0007b803          	ld	a6,0(a5) # 1000 <_entry-0x7ffff000>
    800021ac:	6788                	ld	a0,8(a5)
    800021ae:	6b8c                	ld	a1,16(a5)
    800021b0:	6f90                	ld	a2,24(a5)
    800021b2:	01073023          	sd	a6,0(a4)
    800021b6:	e708                	sd	a0,8(a4)
    800021b8:	eb0c                	sd	a1,16(a4)
    800021ba:	ef10                	sd	a2,24(a4)
    800021bc:	02078793          	addi	a5,a5,32
    800021c0:	02070713          	addi	a4,a4,32
    800021c4:	fed792e3          	bne	a5,a3,800021a8 <fork+0x54>
  np->trapframe->a0 = 0;
    800021c8:	0609b783          	ld	a5,96(s3)
    800021cc:	0607b823          	sd	zero,112(a5)
    800021d0:	0d800493          	li	s1,216
  for(i = 0; i < NOFILE; i++)
    800021d4:	15800a13          	li	s4,344
    800021d8:	a03d                	j	80002206 <fork+0xb2>
    freeproc(np);
    800021da:	854e                	mv	a0,s3
    800021dc:	00000097          	auipc	ra,0x0
    800021e0:	d5c080e7          	jalr	-676(ra) # 80001f38 <freeproc>
    release(&np->lock);
    800021e4:	854e                	mv	a0,s3
    800021e6:	fffff097          	auipc	ra,0xfffff
    800021ea:	ab2080e7          	jalr	-1358(ra) # 80000c98 <release>
    return -1;
    800021ee:	5a7d                	li	s4,-1
    800021f0:	a849                	j	80002282 <fork+0x12e>
      np->ofile[i] = filedup(p->ofile[i]);
    800021f2:	00002097          	auipc	ra,0x2
    800021f6:	6a8080e7          	jalr	1704(ra) # 8000489a <filedup>
    800021fa:	009987b3          	add	a5,s3,s1
    800021fe:	e388                	sd	a0,0(a5)
  for(i = 0; i < NOFILE; i++)
    80002200:	04a1                	addi	s1,s1,8
    80002202:	01448763          	beq	s1,s4,80002210 <fork+0xbc>
    if(p->ofile[i])
    80002206:	009907b3          	add	a5,s2,s1
    8000220a:	6388                	ld	a0,0(a5)
    8000220c:	f17d                	bnez	a0,800021f2 <fork+0x9e>
    8000220e:	bfcd                	j	80002200 <fork+0xac>
  np->cwd = idup(p->cwd);
    80002210:	15893503          	ld	a0,344(s2)
    80002214:	00001097          	auipc	ra,0x1
    80002218:	7fc080e7          	jalr	2044(ra) # 80003a10 <idup>
    8000221c:	14a9bc23          	sd	a0,344(s3)
  safestrcpy(np->name, p->name, sizeof(p->name));
    80002220:	4641                	li	a2,16
    80002222:	16090593          	addi	a1,s2,352
    80002226:	16098513          	addi	a0,s3,352
    8000222a:	fffff097          	auipc	ra,0xfffff
    8000222e:	c08080e7          	jalr	-1016(ra) # 80000e32 <safestrcpy>
  pid = np->pid;
    80002232:	0309aa03          	lw	s4,48(s3)
  release(&np->lock);
    80002236:	854e                	mv	a0,s3
    80002238:	fffff097          	auipc	ra,0xfffff
    8000223c:	a60080e7          	jalr	-1440(ra) # 80000c98 <release>
  acquire(&wait_lock);
    80002240:	0000f497          	auipc	s1,0xf
    80002244:	13848493          	addi	s1,s1,312 # 80011378 <wait_lock>
    80002248:	8526                	mv	a0,s1
    8000224a:	fffff097          	auipc	ra,0xfffff
    8000224e:	99a080e7          	jalr	-1638(ra) # 80000be4 <acquire>
  np->parent = p;
    80002252:	0529b023          	sd	s2,64(s3)
  np->cpu_num = p->cpu_num;
    80002256:	03892783          	lw	a5,56(s2)
    8000225a:	02f9ac23          	sw	a5,56(s3)
  release(&wait_lock);
    8000225e:	8526                	mv	a0,s1
    80002260:	fffff097          	auipc	ra,0xfffff
    80002264:	a38080e7          	jalr	-1480(ra) # 80000c98 <release>
  acquire(&np->lock);
    80002268:	854e                	mv	a0,s3
    8000226a:	fffff097          	auipc	ra,0xfffff
    8000226e:	97a080e7          	jalr	-1670(ra) # 80000be4 <acquire>
  np->state = RUNNABLE;
    80002272:	478d                	li	a5,3
    80002274:	00f9ac23          	sw	a5,24(s3)
  release(&np->lock);
    80002278:	854e                	mv	a0,s3
    8000227a:	fffff097          	auipc	ra,0xfffff
    8000227e:	a1e080e7          	jalr	-1506(ra) # 80000c98 <release>
}
    80002282:	8552                	mv	a0,s4
    80002284:	70a2                	ld	ra,40(sp)
    80002286:	7402                	ld	s0,32(sp)
    80002288:	64e2                	ld	s1,24(sp)
    8000228a:	6942                	ld	s2,16(sp)
    8000228c:	69a2                	ld	s3,8(sp)
    8000228e:	6a02                	ld	s4,0(sp)
    80002290:	6145                	addi	sp,sp,48
    80002292:	8082                	ret
    return -1;
    80002294:	5a7d                	li	s4,-1
    80002296:	b7f5                	j	80002282 <fork+0x12e>

0000000080002298 <scheduler>:
{
    80002298:	7139                	addi	sp,sp,-64
    8000229a:	fc06                	sd	ra,56(sp)
    8000229c:	f822                	sd	s0,48(sp)
    8000229e:	f426                	sd	s1,40(sp)
    800022a0:	f04a                	sd	s2,32(sp)
    800022a2:	ec4e                	sd	s3,24(sp)
    800022a4:	e852                	sd	s4,16(sp)
    800022a6:	e456                	sd	s5,8(sp)
    800022a8:	e05a                	sd	s6,0(sp)
    800022aa:	0080                	addi	s0,sp,64
    800022ac:	8792                	mv	a5,tp
  int id = r_tp();
    800022ae:	2781                	sext.w	a5,a5
  c->proc = 0;
    800022b0:	00779a93          	slli	s5,a5,0x7
    800022b4:	0000f717          	auipc	a4,0xf
    800022b8:	fec70713          	addi	a4,a4,-20 # 800112a0 <cpu_queue_locks>
    800022bc:	9756                	add	a4,a4,s5
    800022be:	0e073823          	sd	zero,240(a4)
        swtch(&c->context, &p->context);
    800022c2:	0000f717          	auipc	a4,0xf
    800022c6:	0d670713          	addi	a4,a4,214 # 80011398 <cpus+0x8>
    800022ca:	9aba                	add	s5,s5,a4
      if(p->state == RUNNABLE) {
    800022cc:	498d                	li	s3,3
        p->state = RUNNING;
    800022ce:	4b11                	li	s6,4
        c->proc = p;
    800022d0:	079e                	slli	a5,a5,0x7
    800022d2:	0000fa17          	auipc	s4,0xf
    800022d6:	fcea0a13          	addi	s4,s4,-50 # 800112a0 <cpu_queue_locks>
    800022da:	9a3e                	add	s4,s4,a5
    for(p = proc; p < &proc[NPROC]; p++) {
    800022dc:	00015917          	auipc	s2,0x15
    800022e0:	0b490913          	addi	s2,s2,180 # 80017390 <tickslock>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    800022e4:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    800022e8:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    800022ec:	10079073          	csrw	sstatus,a5
    800022f0:	0000f497          	auipc	s1,0xf
    800022f4:	4a048493          	addi	s1,s1,1184 # 80011790 <proc>
    800022f8:	a03d                	j	80002326 <scheduler+0x8e>
        p->state = RUNNING;
    800022fa:	0164ac23          	sw	s6,24(s1)
        c->proc = p;
    800022fe:	0e9a3823          	sd	s1,240(s4)
        swtch(&c->context, &p->context);
    80002302:	06848593          	addi	a1,s1,104
    80002306:	8556                	mv	a0,s5
    80002308:	00000097          	auipc	ra,0x0
    8000230c:	6ca080e7          	jalr	1738(ra) # 800029d2 <swtch>
        c->proc = 0;
    80002310:	0e0a3823          	sd	zero,240(s4)
      release(&p->lock);
    80002314:	8526                	mv	a0,s1
    80002316:	fffff097          	auipc	ra,0xfffff
    8000231a:	982080e7          	jalr	-1662(ra) # 80000c98 <release>
    for(p = proc; p < &proc[NPROC]; p++) {
    8000231e:	17048493          	addi	s1,s1,368
    80002322:	fd2481e3          	beq	s1,s2,800022e4 <scheduler+0x4c>
      acquire(&p->lock);
    80002326:	8526                	mv	a0,s1
    80002328:	fffff097          	auipc	ra,0xfffff
    8000232c:	8bc080e7          	jalr	-1860(ra) # 80000be4 <acquire>
      if(p->state == RUNNABLE) {
    80002330:	4c9c                	lw	a5,24(s1)
    80002332:	ff3791e3          	bne	a5,s3,80002314 <scheduler+0x7c>
    80002336:	b7d1                	j	800022fa <scheduler+0x62>

0000000080002338 <sched>:
{
    80002338:	7179                	addi	sp,sp,-48
    8000233a:	f406                	sd	ra,40(sp)
    8000233c:	f022                	sd	s0,32(sp)
    8000233e:	ec26                	sd	s1,24(sp)
    80002340:	e84a                	sd	s2,16(sp)
    80002342:	e44e                	sd	s3,8(sp)
    80002344:	1800                	addi	s0,sp,48
  struct proc *p = myproc();
    80002346:	00000097          	auipc	ra,0x0
    8000234a:	a46080e7          	jalr	-1466(ra) # 80001d8c <myproc>
    8000234e:	84aa                	mv	s1,a0
  if(!holding(&p->lock))
    80002350:	fffff097          	auipc	ra,0xfffff
    80002354:	81a080e7          	jalr	-2022(ra) # 80000b6a <holding>
    80002358:	c93d                	beqz	a0,800023ce <sched+0x96>
  asm volatile("mv %0, tp" : "=r" (x) );
    8000235a:	8792                	mv	a5,tp
  if(mycpu()->noff != 1)
    8000235c:	2781                	sext.w	a5,a5
    8000235e:	079e                	slli	a5,a5,0x7
    80002360:	0000f717          	auipc	a4,0xf
    80002364:	f4070713          	addi	a4,a4,-192 # 800112a0 <cpu_queue_locks>
    80002368:	97ba                	add	a5,a5,a4
    8000236a:	1687a703          	lw	a4,360(a5)
    8000236e:	4785                	li	a5,1
    80002370:	06f71763          	bne	a4,a5,800023de <sched+0xa6>
  if(p->state == RUNNING)
    80002374:	4c98                	lw	a4,24(s1)
    80002376:	4791                	li	a5,4
    80002378:	06f70b63          	beq	a4,a5,800023ee <sched+0xb6>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    8000237c:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    80002380:	8b89                	andi	a5,a5,2
  if(intr_get())
    80002382:	efb5                	bnez	a5,800023fe <sched+0xc6>
  asm volatile("mv %0, tp" : "=r" (x) );
    80002384:	8792                	mv	a5,tp
  intena = mycpu()->intena;
    80002386:	0000f917          	auipc	s2,0xf
    8000238a:	f1a90913          	addi	s2,s2,-230 # 800112a0 <cpu_queue_locks>
    8000238e:	2781                	sext.w	a5,a5
    80002390:	079e                	slli	a5,a5,0x7
    80002392:	97ca                	add	a5,a5,s2
    80002394:	16c7a983          	lw	s3,364(a5)
    80002398:	8792                	mv	a5,tp
  swtch(&p->context, &mycpu()->context);
    8000239a:	2781                	sext.w	a5,a5
    8000239c:	079e                	slli	a5,a5,0x7
    8000239e:	0000f597          	auipc	a1,0xf
    800023a2:	ffa58593          	addi	a1,a1,-6 # 80011398 <cpus+0x8>
    800023a6:	95be                	add	a1,a1,a5
    800023a8:	06848513          	addi	a0,s1,104
    800023ac:	00000097          	auipc	ra,0x0
    800023b0:	626080e7          	jalr	1574(ra) # 800029d2 <swtch>
    800023b4:	8792                	mv	a5,tp
  mycpu()->intena = intena;
    800023b6:	2781                	sext.w	a5,a5
    800023b8:	079e                	slli	a5,a5,0x7
    800023ba:	97ca                	add	a5,a5,s2
    800023bc:	1737a623          	sw	s3,364(a5)
}
    800023c0:	70a2                	ld	ra,40(sp)
    800023c2:	7402                	ld	s0,32(sp)
    800023c4:	64e2                	ld	s1,24(sp)
    800023c6:	6942                	ld	s2,16(sp)
    800023c8:	69a2                	ld	s3,8(sp)
    800023ca:	6145                	addi	sp,sp,48
    800023cc:	8082                	ret
    panic("sched p->lock");
    800023ce:	00006517          	auipc	a0,0x6
    800023d2:	e5a50513          	addi	a0,a0,-422 # 80008228 <digits+0x1e8>
    800023d6:	ffffe097          	auipc	ra,0xffffe
    800023da:	168080e7          	jalr	360(ra) # 8000053e <panic>
    panic("sched locks");
    800023de:	00006517          	auipc	a0,0x6
    800023e2:	e5a50513          	addi	a0,a0,-422 # 80008238 <digits+0x1f8>
    800023e6:	ffffe097          	auipc	ra,0xffffe
    800023ea:	158080e7          	jalr	344(ra) # 8000053e <panic>
    panic("sched running");
    800023ee:	00006517          	auipc	a0,0x6
    800023f2:	e5a50513          	addi	a0,a0,-422 # 80008248 <digits+0x208>
    800023f6:	ffffe097          	auipc	ra,0xffffe
    800023fa:	148080e7          	jalr	328(ra) # 8000053e <panic>
    panic("sched interruptible");
    800023fe:	00006517          	auipc	a0,0x6
    80002402:	e5a50513          	addi	a0,a0,-422 # 80008258 <digits+0x218>
    80002406:	ffffe097          	auipc	ra,0xffffe
    8000240a:	138080e7          	jalr	312(ra) # 8000053e <panic>

000000008000240e <yield>:
{
    8000240e:	1101                	addi	sp,sp,-32
    80002410:	ec06                	sd	ra,24(sp)
    80002412:	e822                	sd	s0,16(sp)
    80002414:	e426                	sd	s1,8(sp)
    80002416:	1000                	addi	s0,sp,32
  struct proc *p = myproc();
    80002418:	00000097          	auipc	ra,0x0
    8000241c:	974080e7          	jalr	-1676(ra) # 80001d8c <myproc>
    80002420:	84aa                	mv	s1,a0
  acquire(&p->lock);
    80002422:	ffffe097          	auipc	ra,0xffffe
    80002426:	7c2080e7          	jalr	1986(ra) # 80000be4 <acquire>
  p->state = RUNNABLE;
    8000242a:	478d                	li	a5,3
    8000242c:	cc9c                	sw	a5,24(s1)
  sched();
    8000242e:	00000097          	auipc	ra,0x0
    80002432:	f0a080e7          	jalr	-246(ra) # 80002338 <sched>
  release(&p->lock);
    80002436:	8526                	mv	a0,s1
    80002438:	fffff097          	auipc	ra,0xfffff
    8000243c:	860080e7          	jalr	-1952(ra) # 80000c98 <release>
}
    80002440:	60e2                	ld	ra,24(sp)
    80002442:	6442                	ld	s0,16(sp)
    80002444:	64a2                	ld	s1,8(sp)
    80002446:	6105                	addi	sp,sp,32
    80002448:	8082                	ret

000000008000244a <sleep>:

// Atomically release lock and sleep on chan.
// Reacquires lock when awakened.
void
sleep(void *chan, struct spinlock *lk)
{
    8000244a:	7179                	addi	sp,sp,-48
    8000244c:	f406                	sd	ra,40(sp)
    8000244e:	f022                	sd	s0,32(sp)
    80002450:	ec26                	sd	s1,24(sp)
    80002452:	e84a                	sd	s2,16(sp)
    80002454:	e44e                	sd	s3,8(sp)
    80002456:	1800                	addi	s0,sp,48
    80002458:	89aa                	mv	s3,a0
    8000245a:	892e                	mv	s2,a1
  struct proc *p = myproc();
    8000245c:	00000097          	auipc	ra,0x0
    80002460:	930080e7          	jalr	-1744(ra) # 80001d8c <myproc>
    80002464:	84aa                	mv	s1,a0
  // Once we hold p->lock, we can be
  // guaranteed that we won't miss any wakeup
  // (wakeup locks p->lock),
  // so it's okay to release lk.

  acquire(&p->lock);  //DOC: sleeplock1
    80002466:	ffffe097          	auipc	ra,0xffffe
    8000246a:	77e080e7          	jalr	1918(ra) # 80000be4 <acquire>
  release(lk);
    8000246e:	854a                	mv	a0,s2
    80002470:	fffff097          	auipc	ra,0xfffff
    80002474:	828080e7          	jalr	-2008(ra) # 80000c98 <release>

  // Go to sleep.
  p->chan = chan;
    80002478:	0334b023          	sd	s3,32(s1)
  p->state = SLEEPING;
    8000247c:	4789                	li	a5,2
    8000247e:	cc9c                	sw	a5,24(s1)

  sched();
    80002480:	00000097          	auipc	ra,0x0
    80002484:	eb8080e7          	jalr	-328(ra) # 80002338 <sched>

  // Tidy up.
  p->chan = 0;
    80002488:	0204b023          	sd	zero,32(s1)

  // Reacquire original lock.
  release(&p->lock);
    8000248c:	8526                	mv	a0,s1
    8000248e:	fffff097          	auipc	ra,0xfffff
    80002492:	80a080e7          	jalr	-2038(ra) # 80000c98 <release>
  acquire(lk);
    80002496:	854a                	mv	a0,s2
    80002498:	ffffe097          	auipc	ra,0xffffe
    8000249c:	74c080e7          	jalr	1868(ra) # 80000be4 <acquire>
}
    800024a0:	70a2                	ld	ra,40(sp)
    800024a2:	7402                	ld	s0,32(sp)
    800024a4:	64e2                	ld	s1,24(sp)
    800024a6:	6942                	ld	s2,16(sp)
    800024a8:	69a2                	ld	s3,8(sp)
    800024aa:	6145                	addi	sp,sp,48
    800024ac:	8082                	ret

00000000800024ae <wait>:
{
    800024ae:	715d                	addi	sp,sp,-80
    800024b0:	e486                	sd	ra,72(sp)
    800024b2:	e0a2                	sd	s0,64(sp)
    800024b4:	fc26                	sd	s1,56(sp)
    800024b6:	f84a                	sd	s2,48(sp)
    800024b8:	f44e                	sd	s3,40(sp)
    800024ba:	f052                	sd	s4,32(sp)
    800024bc:	ec56                	sd	s5,24(sp)
    800024be:	e85a                	sd	s6,16(sp)
    800024c0:	e45e                	sd	s7,8(sp)
    800024c2:	e062                	sd	s8,0(sp)
    800024c4:	0880                	addi	s0,sp,80
    800024c6:	8b2a                	mv	s6,a0
  struct proc *p = myproc();
    800024c8:	00000097          	auipc	ra,0x0
    800024cc:	8c4080e7          	jalr	-1852(ra) # 80001d8c <myproc>
    800024d0:	892a                	mv	s2,a0
  acquire(&wait_lock);
    800024d2:	0000f517          	auipc	a0,0xf
    800024d6:	ea650513          	addi	a0,a0,-346 # 80011378 <wait_lock>
    800024da:	ffffe097          	auipc	ra,0xffffe
    800024de:	70a080e7          	jalr	1802(ra) # 80000be4 <acquire>
    havekids = 0;
    800024e2:	4b81                	li	s7,0
        if(np->state == ZOMBIE){
    800024e4:	4a15                	li	s4,5
    for(np = proc; np < &proc[NPROC]; np++){
    800024e6:	00015997          	auipc	s3,0x15
    800024ea:	eaa98993          	addi	s3,s3,-342 # 80017390 <tickslock>
        havekids = 1;
    800024ee:	4a85                	li	s5,1
    sleep(p, &wait_lock);  //DOC: wait-sleep
    800024f0:	0000fc17          	auipc	s8,0xf
    800024f4:	e88c0c13          	addi	s8,s8,-376 # 80011378 <wait_lock>
    havekids = 0;
    800024f8:	875e                	mv	a4,s7
    for(np = proc; np < &proc[NPROC]; np++){
    800024fa:	0000f497          	auipc	s1,0xf
    800024fe:	29648493          	addi	s1,s1,662 # 80011790 <proc>
    80002502:	a0bd                	j	80002570 <wait+0xc2>
          pid = np->pid;
    80002504:	0304a983          	lw	s3,48(s1)
          if(addr != 0 && copyout(p->pagetable, addr, (char *)&np->xstate,
    80002508:	000b0e63          	beqz	s6,80002524 <wait+0x76>
    8000250c:	4691                	li	a3,4
    8000250e:	02c48613          	addi	a2,s1,44
    80002512:	85da                	mv	a1,s6
    80002514:	05893503          	ld	a0,88(s2)
    80002518:	fffff097          	auipc	ra,0xfffff
    8000251c:	15a080e7          	jalr	346(ra) # 80001672 <copyout>
    80002520:	02054563          	bltz	a0,8000254a <wait+0x9c>
          freeproc(np);
    80002524:	8526                	mv	a0,s1
    80002526:	00000097          	auipc	ra,0x0
    8000252a:	a12080e7          	jalr	-1518(ra) # 80001f38 <freeproc>
          release(&np->lock);
    8000252e:	8526                	mv	a0,s1
    80002530:	ffffe097          	auipc	ra,0xffffe
    80002534:	768080e7          	jalr	1896(ra) # 80000c98 <release>
          release(&wait_lock);
    80002538:	0000f517          	auipc	a0,0xf
    8000253c:	e4050513          	addi	a0,a0,-448 # 80011378 <wait_lock>
    80002540:	ffffe097          	auipc	ra,0xffffe
    80002544:	758080e7          	jalr	1880(ra) # 80000c98 <release>
          return pid;
    80002548:	a09d                	j	800025ae <wait+0x100>
            release(&np->lock);
    8000254a:	8526                	mv	a0,s1
    8000254c:	ffffe097          	auipc	ra,0xffffe
    80002550:	74c080e7          	jalr	1868(ra) # 80000c98 <release>
            release(&wait_lock);
    80002554:	0000f517          	auipc	a0,0xf
    80002558:	e2450513          	addi	a0,a0,-476 # 80011378 <wait_lock>
    8000255c:	ffffe097          	auipc	ra,0xffffe
    80002560:	73c080e7          	jalr	1852(ra) # 80000c98 <release>
            return -1;
    80002564:	59fd                	li	s3,-1
    80002566:	a0a1                	j	800025ae <wait+0x100>
    for(np = proc; np < &proc[NPROC]; np++){
    80002568:	17048493          	addi	s1,s1,368
    8000256c:	03348463          	beq	s1,s3,80002594 <wait+0xe6>
      if(np->parent == p){
    80002570:	60bc                	ld	a5,64(s1)
    80002572:	ff279be3          	bne	a5,s2,80002568 <wait+0xba>
        acquire(&np->lock);
    80002576:	8526                	mv	a0,s1
    80002578:	ffffe097          	auipc	ra,0xffffe
    8000257c:	66c080e7          	jalr	1644(ra) # 80000be4 <acquire>
        if(np->state == ZOMBIE){
    80002580:	4c9c                	lw	a5,24(s1)
    80002582:	f94781e3          	beq	a5,s4,80002504 <wait+0x56>
        release(&np->lock);
    80002586:	8526                	mv	a0,s1
    80002588:	ffffe097          	auipc	ra,0xffffe
    8000258c:	710080e7          	jalr	1808(ra) # 80000c98 <release>
        havekids = 1;
    80002590:	8756                	mv	a4,s5
    80002592:	bfd9                	j	80002568 <wait+0xba>
    if(!havekids || p->killed){
    80002594:	c701                	beqz	a4,8000259c <wait+0xee>
    80002596:	02892783          	lw	a5,40(s2)
    8000259a:	c79d                	beqz	a5,800025c8 <wait+0x11a>
      release(&wait_lock);
    8000259c:	0000f517          	auipc	a0,0xf
    800025a0:	ddc50513          	addi	a0,a0,-548 # 80011378 <wait_lock>
    800025a4:	ffffe097          	auipc	ra,0xffffe
    800025a8:	6f4080e7          	jalr	1780(ra) # 80000c98 <release>
      return -1;
    800025ac:	59fd                	li	s3,-1
}
    800025ae:	854e                	mv	a0,s3
    800025b0:	60a6                	ld	ra,72(sp)
    800025b2:	6406                	ld	s0,64(sp)
    800025b4:	74e2                	ld	s1,56(sp)
    800025b6:	7942                	ld	s2,48(sp)
    800025b8:	79a2                	ld	s3,40(sp)
    800025ba:	7a02                	ld	s4,32(sp)
    800025bc:	6ae2                	ld	s5,24(sp)
    800025be:	6b42                	ld	s6,16(sp)
    800025c0:	6ba2                	ld	s7,8(sp)
    800025c2:	6c02                	ld	s8,0(sp)
    800025c4:	6161                	addi	sp,sp,80
    800025c6:	8082                	ret
    sleep(p, &wait_lock);  //DOC: wait-sleep
    800025c8:	85e2                	mv	a1,s8
    800025ca:	854a                	mv	a0,s2
    800025cc:	00000097          	auipc	ra,0x0
    800025d0:	e7e080e7          	jalr	-386(ra) # 8000244a <sleep>
    havekids = 0;
    800025d4:	b715                	j	800024f8 <wait+0x4a>

00000000800025d6 <wakeup>:

// Wake up all processes sleeping on chan.
// Must be called without any p->lock.
void
wakeup(void *chan)
{
    800025d6:	7139                	addi	sp,sp,-64
    800025d8:	fc06                	sd	ra,56(sp)
    800025da:	f822                	sd	s0,48(sp)
    800025dc:	f426                	sd	s1,40(sp)
    800025de:	f04a                	sd	s2,32(sp)
    800025e0:	ec4e                	sd	s3,24(sp)
    800025e2:	e852                	sd	s4,16(sp)
    800025e4:	e456                	sd	s5,8(sp)
    800025e6:	0080                	addi	s0,sp,64
    800025e8:	8a2a                	mv	s4,a0
  struct proc *p;

  for(p = proc; p < &proc[NPROC]; p++) {
    800025ea:	0000f497          	auipc	s1,0xf
    800025ee:	1a648493          	addi	s1,s1,422 # 80011790 <proc>
    if(p != myproc()){
      acquire(&p->lock);
      if(p->state == SLEEPING && p->chan == chan) {
    800025f2:	4989                	li	s3,2
        p->state = RUNNABLE;
    800025f4:	4a8d                	li	s5,3
  for(p = proc; p < &proc[NPROC]; p++) {
    800025f6:	00015917          	auipc	s2,0x15
    800025fa:	d9a90913          	addi	s2,s2,-614 # 80017390 <tickslock>
    800025fe:	a821                	j	80002616 <wakeup+0x40>
        p->state = RUNNABLE;
    80002600:	0154ac23          	sw	s5,24(s1)
      }
      release(&p->lock);
    80002604:	8526                	mv	a0,s1
    80002606:	ffffe097          	auipc	ra,0xffffe
    8000260a:	692080e7          	jalr	1682(ra) # 80000c98 <release>
  for(p = proc; p < &proc[NPROC]; p++) {
    8000260e:	17048493          	addi	s1,s1,368
    80002612:	03248463          	beq	s1,s2,8000263a <wakeup+0x64>
    if(p != myproc()){
    80002616:	fffff097          	auipc	ra,0xfffff
    8000261a:	776080e7          	jalr	1910(ra) # 80001d8c <myproc>
    8000261e:	fea488e3          	beq	s1,a0,8000260e <wakeup+0x38>
      acquire(&p->lock);
    80002622:	8526                	mv	a0,s1
    80002624:	ffffe097          	auipc	ra,0xffffe
    80002628:	5c0080e7          	jalr	1472(ra) # 80000be4 <acquire>
      if(p->state == SLEEPING && p->chan == chan) {
    8000262c:	4c9c                	lw	a5,24(s1)
    8000262e:	fd379be3          	bne	a5,s3,80002604 <wakeup+0x2e>
    80002632:	709c                	ld	a5,32(s1)
    80002634:	fd4798e3          	bne	a5,s4,80002604 <wakeup+0x2e>
    80002638:	b7e1                	j	80002600 <wakeup+0x2a>
    }
  }
}
    8000263a:	70e2                	ld	ra,56(sp)
    8000263c:	7442                	ld	s0,48(sp)
    8000263e:	74a2                	ld	s1,40(sp)
    80002640:	7902                	ld	s2,32(sp)
    80002642:	69e2                	ld	s3,24(sp)
    80002644:	6a42                	ld	s4,16(sp)
    80002646:	6aa2                	ld	s5,8(sp)
    80002648:	6121                	addi	sp,sp,64
    8000264a:	8082                	ret

000000008000264c <reparent>:
{
    8000264c:	7179                	addi	sp,sp,-48
    8000264e:	f406                	sd	ra,40(sp)
    80002650:	f022                	sd	s0,32(sp)
    80002652:	ec26                	sd	s1,24(sp)
    80002654:	e84a                	sd	s2,16(sp)
    80002656:	e44e                	sd	s3,8(sp)
    80002658:	e052                	sd	s4,0(sp)
    8000265a:	1800                	addi	s0,sp,48
    8000265c:	892a                	mv	s2,a0
  for(pp = proc; pp < &proc[NPROC]; pp++){
    8000265e:	0000f497          	auipc	s1,0xf
    80002662:	13248493          	addi	s1,s1,306 # 80011790 <proc>
      pp->parent = initproc;
    80002666:	00007a17          	auipc	s4,0x7
    8000266a:	9c2a0a13          	addi	s4,s4,-1598 # 80009028 <initproc>
  for(pp = proc; pp < &proc[NPROC]; pp++){
    8000266e:	00015997          	auipc	s3,0x15
    80002672:	d2298993          	addi	s3,s3,-734 # 80017390 <tickslock>
    80002676:	a029                	j	80002680 <reparent+0x34>
    80002678:	17048493          	addi	s1,s1,368
    8000267c:	01348d63          	beq	s1,s3,80002696 <reparent+0x4a>
    if(pp->parent == p){
    80002680:	60bc                	ld	a5,64(s1)
    80002682:	ff279be3          	bne	a5,s2,80002678 <reparent+0x2c>
      pp->parent = initproc;
    80002686:	000a3503          	ld	a0,0(s4)
    8000268a:	e0a8                	sd	a0,64(s1)
      wakeup(initproc);
    8000268c:	00000097          	auipc	ra,0x0
    80002690:	f4a080e7          	jalr	-182(ra) # 800025d6 <wakeup>
    80002694:	b7d5                	j	80002678 <reparent+0x2c>
}
    80002696:	70a2                	ld	ra,40(sp)
    80002698:	7402                	ld	s0,32(sp)
    8000269a:	64e2                	ld	s1,24(sp)
    8000269c:	6942                	ld	s2,16(sp)
    8000269e:	69a2                	ld	s3,8(sp)
    800026a0:	6a02                	ld	s4,0(sp)
    800026a2:	6145                	addi	sp,sp,48
    800026a4:	8082                	ret

00000000800026a6 <exit>:
{
    800026a6:	7179                	addi	sp,sp,-48
    800026a8:	f406                	sd	ra,40(sp)
    800026aa:	f022                	sd	s0,32(sp)
    800026ac:	ec26                	sd	s1,24(sp)
    800026ae:	e84a                	sd	s2,16(sp)
    800026b0:	e44e                	sd	s3,8(sp)
    800026b2:	e052                	sd	s4,0(sp)
    800026b4:	1800                	addi	s0,sp,48
    800026b6:	8a2a                	mv	s4,a0
  struct proc *p = myproc();
    800026b8:	fffff097          	auipc	ra,0xfffff
    800026bc:	6d4080e7          	jalr	1748(ra) # 80001d8c <myproc>
    800026c0:	89aa                	mv	s3,a0
  if(p == initproc)
    800026c2:	00007797          	auipc	a5,0x7
    800026c6:	9667b783          	ld	a5,-1690(a5) # 80009028 <initproc>
    800026ca:	0d850493          	addi	s1,a0,216
    800026ce:	15850913          	addi	s2,a0,344
    800026d2:	02a79363          	bne	a5,a0,800026f8 <exit+0x52>
    panic("init exiting");
    800026d6:	00006517          	auipc	a0,0x6
    800026da:	b9a50513          	addi	a0,a0,-1126 # 80008270 <digits+0x230>
    800026de:	ffffe097          	auipc	ra,0xffffe
    800026e2:	e60080e7          	jalr	-416(ra) # 8000053e <panic>
      fileclose(f);
    800026e6:	00002097          	auipc	ra,0x2
    800026ea:	206080e7          	jalr	518(ra) # 800048ec <fileclose>
      p->ofile[fd] = 0;
    800026ee:	0004b023          	sd	zero,0(s1)
  for(int fd = 0; fd < NOFILE; fd++){
    800026f2:	04a1                	addi	s1,s1,8
    800026f4:	01248563          	beq	s1,s2,800026fe <exit+0x58>
    if(p->ofile[fd]){
    800026f8:	6088                	ld	a0,0(s1)
    800026fa:	f575                	bnez	a0,800026e6 <exit+0x40>
    800026fc:	bfdd                	j	800026f2 <exit+0x4c>
  begin_op();
    800026fe:	00002097          	auipc	ra,0x2
    80002702:	d22080e7          	jalr	-734(ra) # 80004420 <begin_op>
  iput(p->cwd);
    80002706:	1589b503          	ld	a0,344(s3)
    8000270a:	00001097          	auipc	ra,0x1
    8000270e:	4fe080e7          	jalr	1278(ra) # 80003c08 <iput>
  end_op();
    80002712:	00002097          	auipc	ra,0x2
    80002716:	d8e080e7          	jalr	-626(ra) # 800044a0 <end_op>
  p->cwd = 0;
    8000271a:	1409bc23          	sd	zero,344(s3)
  acquire(&wait_lock);
    8000271e:	0000f497          	auipc	s1,0xf
    80002722:	c5a48493          	addi	s1,s1,-934 # 80011378 <wait_lock>
    80002726:	8526                	mv	a0,s1
    80002728:	ffffe097          	auipc	ra,0xffffe
    8000272c:	4bc080e7          	jalr	1212(ra) # 80000be4 <acquire>
  reparent(p);
    80002730:	854e                	mv	a0,s3
    80002732:	00000097          	auipc	ra,0x0
    80002736:	f1a080e7          	jalr	-230(ra) # 8000264c <reparent>
  wakeup(p->parent);
    8000273a:	0409b503          	ld	a0,64(s3)
    8000273e:	00000097          	auipc	ra,0x0
    80002742:	e98080e7          	jalr	-360(ra) # 800025d6 <wakeup>
  acquire(&p->lock);
    80002746:	854e                	mv	a0,s3
    80002748:	ffffe097          	auipc	ra,0xffffe
    8000274c:	49c080e7          	jalr	1180(ra) # 80000be4 <acquire>
  p->xstate = status;
    80002750:	0349a623          	sw	s4,44(s3)
  p->state = ZOMBIE;
    80002754:	4795                	li	a5,5
    80002756:	00f9ac23          	sw	a5,24(s3)
  release(&wait_lock);
    8000275a:	8526                	mv	a0,s1
    8000275c:	ffffe097          	auipc	ra,0xffffe
    80002760:	53c080e7          	jalr	1340(ra) # 80000c98 <release>
  sched();
    80002764:	00000097          	auipc	ra,0x0
    80002768:	bd4080e7          	jalr	-1068(ra) # 80002338 <sched>
  panic("zombie exit");
    8000276c:	00006517          	auipc	a0,0x6
    80002770:	b1450513          	addi	a0,a0,-1260 # 80008280 <digits+0x240>
    80002774:	ffffe097          	auipc	ra,0xffffe
    80002778:	dca080e7          	jalr	-566(ra) # 8000053e <panic>

000000008000277c <print_pids>:

// printing processes IDs
// helps to check the vadility of CAS for nextpid
int
print_pids(void){
    8000277c:	7179                	addi	sp,sp,-48
    8000277e:	f406                	sd	ra,40(sp)
    80002780:	f022                	sd	s0,32(sp)
    80002782:	ec26                	sd	s1,24(sp)
    80002784:	e84a                	sd	s2,16(sp)
    80002786:	e44e                	sd	s3,8(sp)
    80002788:	1800                	addi	s0,sp,48
  struct proc *p;
  printf("Processes PIDs: \n\n");
    8000278a:	00006517          	auipc	a0,0x6
    8000278e:	b0650513          	addi	a0,a0,-1274 # 80008290 <digits+0x250>
    80002792:	ffffe097          	auipc	ra,0xffffe
    80002796:	df6080e7          	jalr	-522(ra) # 80000588 <printf>
  for(p = proc; p < &proc[NPROC]; p++){
    8000279a:	0000f497          	auipc	s1,0xf
    8000279e:	ff648493          	addi	s1,s1,-10 # 80011790 <proc>
    acquire(&p->lock);
    if(p->state != UNUSED){
      printf("Process %s has PID %d\n", p->name, p->pid);
    800027a2:	00006997          	auipc	s3,0x6
    800027a6:	b0698993          	addi	s3,s3,-1274 # 800082a8 <digits+0x268>
  for(p = proc; p < &proc[NPROC]; p++){
    800027aa:	00015917          	auipc	s2,0x15
    800027ae:	be690913          	addi	s2,s2,-1050 # 80017390 <tickslock>
    800027b2:	a015                	j	800027d6 <print_pids+0x5a>
      printf("Process %s has PID %d\n", p->name, p->pid);
    800027b4:	5890                	lw	a2,48(s1)
    800027b6:	16048593          	addi	a1,s1,352
    800027ba:	854e                	mv	a0,s3
    800027bc:	ffffe097          	auipc	ra,0xffffe
    800027c0:	dcc080e7          	jalr	-564(ra) # 80000588 <printf>
    }
    release(&p->lock);
    800027c4:	8526                	mv	a0,s1
    800027c6:	ffffe097          	auipc	ra,0xffffe
    800027ca:	4d2080e7          	jalr	1234(ra) # 80000c98 <release>
  for(p = proc; p < &proc[NPROC]; p++){
    800027ce:	17048493          	addi	s1,s1,368
    800027d2:	01248a63          	beq	s1,s2,800027e6 <print_pids+0x6a>
    acquire(&p->lock);
    800027d6:	8526                	mv	a0,s1
    800027d8:	ffffe097          	auipc	ra,0xffffe
    800027dc:	40c080e7          	jalr	1036(ra) # 80000be4 <acquire>
    if(p->state != UNUSED){
    800027e0:	4c9c                	lw	a5,24(s1)
    800027e2:	d3ed                	beqz	a5,800027c4 <print_pids+0x48>
    800027e4:	bfc1                	j	800027b4 <print_pids+0x38>
  }
  printf("Finished printing all PIDs\n\n");
    800027e6:	00006517          	auipc	a0,0x6
    800027ea:	ada50513          	addi	a0,a0,-1318 # 800082c0 <digits+0x280>
    800027ee:	ffffe097          	auipc	ra,0xffffe
    800027f2:	d9a080e7          	jalr	-614(ra) # 80000588 <printf>
  return 0;
}
    800027f6:	4501                	li	a0,0
    800027f8:	70a2                	ld	ra,40(sp)
    800027fa:	7402                	ld	s0,32(sp)
    800027fc:	64e2                	ld	s1,24(sp)
    800027fe:	6942                	ld	s2,16(sp)
    80002800:	69a2                	ld	s3,8(sp)
    80002802:	6145                	addi	sp,sp,48
    80002804:	8082                	ret

0000000080002806 <kill>:
// Kill the process with the given pid.
// The victim won't exit until it tries to return
// to user space (see usertrap() in trap.c).
int
kill(int pid)
{
    80002806:	7179                	addi	sp,sp,-48
    80002808:	f406                	sd	ra,40(sp)
    8000280a:	f022                	sd	s0,32(sp)
    8000280c:	ec26                	sd	s1,24(sp)
    8000280e:	e84a                	sd	s2,16(sp)
    80002810:	e44e                	sd	s3,8(sp)
    80002812:	1800                	addi	s0,sp,48
    80002814:	892a                	mv	s2,a0
  struct proc *p;

  for(p = proc; p < &proc[NPROC]; p++){
    80002816:	0000f497          	auipc	s1,0xf
    8000281a:	f7a48493          	addi	s1,s1,-134 # 80011790 <proc>
    8000281e:	00015997          	auipc	s3,0x15
    80002822:	b7298993          	addi	s3,s3,-1166 # 80017390 <tickslock>
    acquire(&p->lock);
    80002826:	8526                	mv	a0,s1
    80002828:	ffffe097          	auipc	ra,0xffffe
    8000282c:	3bc080e7          	jalr	956(ra) # 80000be4 <acquire>
    if(p->pid == pid){
    80002830:	589c                	lw	a5,48(s1)
    80002832:	01278d63          	beq	a5,s2,8000284c <kill+0x46>
        p->state = RUNNABLE;
      }
      release(&p->lock);
      return 0;
    }
    release(&p->lock);
    80002836:	8526                	mv	a0,s1
    80002838:	ffffe097          	auipc	ra,0xffffe
    8000283c:	460080e7          	jalr	1120(ra) # 80000c98 <release>
  for(p = proc; p < &proc[NPROC]; p++){
    80002840:	17048493          	addi	s1,s1,368
    80002844:	ff3491e3          	bne	s1,s3,80002826 <kill+0x20>
  }
  return -1;
    80002848:	557d                	li	a0,-1
    8000284a:	a829                	j	80002864 <kill+0x5e>
      p->killed = 1;
    8000284c:	4785                	li	a5,1
    8000284e:	d49c                	sw	a5,40(s1)
      if(p->state == SLEEPING){
    80002850:	4c98                	lw	a4,24(s1)
    80002852:	4789                	li	a5,2
    80002854:	00f70f63          	beq	a4,a5,80002872 <kill+0x6c>
      release(&p->lock);
    80002858:	8526                	mv	a0,s1
    8000285a:	ffffe097          	auipc	ra,0xffffe
    8000285e:	43e080e7          	jalr	1086(ra) # 80000c98 <release>
      return 0;
    80002862:	4501                	li	a0,0
}
    80002864:	70a2                	ld	ra,40(sp)
    80002866:	7402                	ld	s0,32(sp)
    80002868:	64e2                	ld	s1,24(sp)
    8000286a:	6942                	ld	s2,16(sp)
    8000286c:	69a2                	ld	s3,8(sp)
    8000286e:	6145                	addi	sp,sp,48
    80002870:	8082                	ret
        p->state = RUNNABLE;
    80002872:	478d                	li	a5,3
    80002874:	cc9c                	sw	a5,24(s1)
    80002876:	b7cd                	j	80002858 <kill+0x52>

0000000080002878 <either_copyout>:
// Copy to either a user address, or kernel address,
// depending on usr_dst.
// Returns 0 on success, -1 on error.
int
either_copyout(int user_dst, uint64 dst, void *src, uint64 len)
{
    80002878:	7179                	addi	sp,sp,-48
    8000287a:	f406                	sd	ra,40(sp)
    8000287c:	f022                	sd	s0,32(sp)
    8000287e:	ec26                	sd	s1,24(sp)
    80002880:	e84a                	sd	s2,16(sp)
    80002882:	e44e                	sd	s3,8(sp)
    80002884:	e052                	sd	s4,0(sp)
    80002886:	1800                	addi	s0,sp,48
    80002888:	84aa                	mv	s1,a0
    8000288a:	892e                	mv	s2,a1
    8000288c:	89b2                	mv	s3,a2
    8000288e:	8a36                	mv	s4,a3
  struct proc *p = myproc();
    80002890:	fffff097          	auipc	ra,0xfffff
    80002894:	4fc080e7          	jalr	1276(ra) # 80001d8c <myproc>
  if(user_dst){
    80002898:	c08d                	beqz	s1,800028ba <either_copyout+0x42>
    return copyout(p->pagetable, dst, src, len);
    8000289a:	86d2                	mv	a3,s4
    8000289c:	864e                	mv	a2,s3
    8000289e:	85ca                	mv	a1,s2
    800028a0:	6d28                	ld	a0,88(a0)
    800028a2:	fffff097          	auipc	ra,0xfffff
    800028a6:	dd0080e7          	jalr	-560(ra) # 80001672 <copyout>
  } else {
    memmove((char *)dst, src, len);
    return 0;
  }
}
    800028aa:	70a2                	ld	ra,40(sp)
    800028ac:	7402                	ld	s0,32(sp)
    800028ae:	64e2                	ld	s1,24(sp)
    800028b0:	6942                	ld	s2,16(sp)
    800028b2:	69a2                	ld	s3,8(sp)
    800028b4:	6a02                	ld	s4,0(sp)
    800028b6:	6145                	addi	sp,sp,48
    800028b8:	8082                	ret
    memmove((char *)dst, src, len);
    800028ba:	000a061b          	sext.w	a2,s4
    800028be:	85ce                	mv	a1,s3
    800028c0:	854a                	mv	a0,s2
    800028c2:	ffffe097          	auipc	ra,0xffffe
    800028c6:	47e080e7          	jalr	1150(ra) # 80000d40 <memmove>
    return 0;
    800028ca:	8526                	mv	a0,s1
    800028cc:	bff9                	j	800028aa <either_copyout+0x32>

00000000800028ce <either_copyin>:
// Copy from either a user address, or kernel address,
// depending on usr_src.
// Returns 0 on success, -1 on error.
int
either_copyin(void *dst, int user_src, uint64 src, uint64 len)
{
    800028ce:	7179                	addi	sp,sp,-48
    800028d0:	f406                	sd	ra,40(sp)
    800028d2:	f022                	sd	s0,32(sp)
    800028d4:	ec26                	sd	s1,24(sp)
    800028d6:	e84a                	sd	s2,16(sp)
    800028d8:	e44e                	sd	s3,8(sp)
    800028da:	e052                	sd	s4,0(sp)
    800028dc:	1800                	addi	s0,sp,48
    800028de:	892a                	mv	s2,a0
    800028e0:	84ae                	mv	s1,a1
    800028e2:	89b2                	mv	s3,a2
    800028e4:	8a36                	mv	s4,a3
  struct proc *p = myproc();
    800028e6:	fffff097          	auipc	ra,0xfffff
    800028ea:	4a6080e7          	jalr	1190(ra) # 80001d8c <myproc>
  if(user_src){
    800028ee:	c08d                	beqz	s1,80002910 <either_copyin+0x42>
    return copyin(p->pagetable, dst, src, len);
    800028f0:	86d2                	mv	a3,s4
    800028f2:	864e                	mv	a2,s3
    800028f4:	85ca                	mv	a1,s2
    800028f6:	6d28                	ld	a0,88(a0)
    800028f8:	fffff097          	auipc	ra,0xfffff
    800028fc:	e06080e7          	jalr	-506(ra) # 800016fe <copyin>
  } else {
    memmove(dst, (char*)src, len);
    return 0;
  }
}
    80002900:	70a2                	ld	ra,40(sp)
    80002902:	7402                	ld	s0,32(sp)
    80002904:	64e2                	ld	s1,24(sp)
    80002906:	6942                	ld	s2,16(sp)
    80002908:	69a2                	ld	s3,8(sp)
    8000290a:	6a02                	ld	s4,0(sp)
    8000290c:	6145                	addi	sp,sp,48
    8000290e:	8082                	ret
    memmove(dst, (char*)src, len);
    80002910:	000a061b          	sext.w	a2,s4
    80002914:	85ce                	mv	a1,s3
    80002916:	854a                	mv	a0,s2
    80002918:	ffffe097          	auipc	ra,0xffffe
    8000291c:	428080e7          	jalr	1064(ra) # 80000d40 <memmove>
    return 0;
    80002920:	8526                	mv	a0,s1
    80002922:	bff9                	j	80002900 <either_copyin+0x32>

0000000080002924 <procdump>:
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void
procdump(void)
{
    80002924:	715d                	addi	sp,sp,-80
    80002926:	e486                	sd	ra,72(sp)
    80002928:	e0a2                	sd	s0,64(sp)
    8000292a:	fc26                	sd	s1,56(sp)
    8000292c:	f84a                	sd	s2,48(sp)
    8000292e:	f44e                	sd	s3,40(sp)
    80002930:	f052                	sd	s4,32(sp)
    80002932:	ec56                	sd	s5,24(sp)
    80002934:	e85a                	sd	s6,16(sp)
    80002936:	e45e                	sd	s7,8(sp)
    80002938:	0880                	addi	s0,sp,80
  [ZOMBIE]    "zombie"
  };
  struct proc *p;
  char *state;

  printf("\n");
    8000293a:	00005517          	auipc	a0,0x5
    8000293e:	78e50513          	addi	a0,a0,1934 # 800080c8 <digits+0x88>
    80002942:	ffffe097          	auipc	ra,0xffffe
    80002946:	c46080e7          	jalr	-954(ra) # 80000588 <printf>
  for(p = proc; p < &proc[NPROC]; p++){
    8000294a:	0000f497          	auipc	s1,0xf
    8000294e:	fa648493          	addi	s1,s1,-90 # 800118f0 <proc+0x160>
    80002952:	00015917          	auipc	s2,0x15
    80002956:	b9e90913          	addi	s2,s2,-1122 # 800174f0 <bcache+0x148>
    if(p->state == UNUSED)
      continue;
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    8000295a:	4b15                	li	s6,5
      state = states[p->state];
    else
      state = "???";
    8000295c:	00006997          	auipc	s3,0x6
    80002960:	98498993          	addi	s3,s3,-1660 # 800082e0 <digits+0x2a0>
    printf("%d %s %s", p->pid, state, p->name);
    80002964:	00006a97          	auipc	s5,0x6
    80002968:	984a8a93          	addi	s5,s5,-1660 # 800082e8 <digits+0x2a8>
    printf("\n");
    8000296c:	00005a17          	auipc	s4,0x5
    80002970:	75ca0a13          	addi	s4,s4,1884 # 800080c8 <digits+0x88>
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    80002974:	00006b97          	auipc	s7,0x6
    80002978:	9acb8b93          	addi	s7,s7,-1620 # 80008320 <states.1776>
    8000297c:	a00d                	j	8000299e <procdump+0x7a>
    printf("%d %s %s", p->pid, state, p->name);
    8000297e:	ed06a583          	lw	a1,-304(a3)
    80002982:	8556                	mv	a0,s5
    80002984:	ffffe097          	auipc	ra,0xffffe
    80002988:	c04080e7          	jalr	-1020(ra) # 80000588 <printf>
    printf("\n");
    8000298c:	8552                	mv	a0,s4
    8000298e:	ffffe097          	auipc	ra,0xffffe
    80002992:	bfa080e7          	jalr	-1030(ra) # 80000588 <printf>
  for(p = proc; p < &proc[NPROC]; p++){
    80002996:	17048493          	addi	s1,s1,368
    8000299a:	03248163          	beq	s1,s2,800029bc <procdump+0x98>
    if(p->state == UNUSED)
    8000299e:	86a6                	mv	a3,s1
    800029a0:	eb84a783          	lw	a5,-328(s1)
    800029a4:	dbed                	beqz	a5,80002996 <procdump+0x72>
      state = "???";
    800029a6:	864e                	mv	a2,s3
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    800029a8:	fcfb6be3          	bltu	s6,a5,8000297e <procdump+0x5a>
    800029ac:	1782                	slli	a5,a5,0x20
    800029ae:	9381                	srli	a5,a5,0x20
    800029b0:	078e                	slli	a5,a5,0x3
    800029b2:	97de                	add	a5,a5,s7
    800029b4:	6390                	ld	a2,0(a5)
    800029b6:	f661                	bnez	a2,8000297e <procdump+0x5a>
      state = "???";
    800029b8:	864e                	mv	a2,s3
    800029ba:	b7d1                	j	8000297e <procdump+0x5a>
  }
}
    800029bc:	60a6                	ld	ra,72(sp)
    800029be:	6406                	ld	s0,64(sp)
    800029c0:	74e2                	ld	s1,56(sp)
    800029c2:	7942                	ld	s2,48(sp)
    800029c4:	79a2                	ld	s3,40(sp)
    800029c6:	7a02                	ld	s4,32(sp)
    800029c8:	6ae2                	ld	s5,24(sp)
    800029ca:	6b42                	ld	s6,16(sp)
    800029cc:	6ba2                	ld	s7,8(sp)
    800029ce:	6161                	addi	sp,sp,80
    800029d0:	8082                	ret

00000000800029d2 <swtch>:
    800029d2:	00153023          	sd	ra,0(a0)
    800029d6:	00253423          	sd	sp,8(a0)
    800029da:	e900                	sd	s0,16(a0)
    800029dc:	ed04                	sd	s1,24(a0)
    800029de:	03253023          	sd	s2,32(a0)
    800029e2:	03353423          	sd	s3,40(a0)
    800029e6:	03453823          	sd	s4,48(a0)
    800029ea:	03553c23          	sd	s5,56(a0)
    800029ee:	05653023          	sd	s6,64(a0)
    800029f2:	05753423          	sd	s7,72(a0)
    800029f6:	05853823          	sd	s8,80(a0)
    800029fa:	05953c23          	sd	s9,88(a0)
    800029fe:	07a53023          	sd	s10,96(a0)
    80002a02:	07b53423          	sd	s11,104(a0)
    80002a06:	0005b083          	ld	ra,0(a1)
    80002a0a:	0085b103          	ld	sp,8(a1)
    80002a0e:	6980                	ld	s0,16(a1)
    80002a10:	6d84                	ld	s1,24(a1)
    80002a12:	0205b903          	ld	s2,32(a1)
    80002a16:	0285b983          	ld	s3,40(a1)
    80002a1a:	0305ba03          	ld	s4,48(a1)
    80002a1e:	0385ba83          	ld	s5,56(a1)
    80002a22:	0405bb03          	ld	s6,64(a1)
    80002a26:	0485bb83          	ld	s7,72(a1)
    80002a2a:	0505bc03          	ld	s8,80(a1)
    80002a2e:	0585bc83          	ld	s9,88(a1)
    80002a32:	0605bd03          	ld	s10,96(a1)
    80002a36:	0685bd83          	ld	s11,104(a1)
    80002a3a:	8082                	ret

0000000080002a3c <trapinit>:

extern int devintr();

void
trapinit(void)
{
    80002a3c:	1141                	addi	sp,sp,-16
    80002a3e:	e406                	sd	ra,8(sp)
    80002a40:	e022                	sd	s0,0(sp)
    80002a42:	0800                	addi	s0,sp,16
  initlock(&tickslock, "time");
    80002a44:	00006597          	auipc	a1,0x6
    80002a48:	90c58593          	addi	a1,a1,-1780 # 80008350 <states.1776+0x30>
    80002a4c:	00015517          	auipc	a0,0x15
    80002a50:	94450513          	addi	a0,a0,-1724 # 80017390 <tickslock>
    80002a54:	ffffe097          	auipc	ra,0xffffe
    80002a58:	100080e7          	jalr	256(ra) # 80000b54 <initlock>
}
    80002a5c:	60a2                	ld	ra,8(sp)
    80002a5e:	6402                	ld	s0,0(sp)
    80002a60:	0141                	addi	sp,sp,16
    80002a62:	8082                	ret

0000000080002a64 <trapinithart>:

// set up to take exceptions and traps while in the kernel.
void
trapinithart(void)
{
    80002a64:	1141                	addi	sp,sp,-16
    80002a66:	e422                	sd	s0,8(sp)
    80002a68:	0800                	addi	s0,sp,16
  asm volatile("csrw stvec, %0" : : "r" (x));
    80002a6a:	00003797          	auipc	a5,0x3
    80002a6e:	49678793          	addi	a5,a5,1174 # 80005f00 <kernelvec>
    80002a72:	10579073          	csrw	stvec,a5
  w_stvec((uint64)kernelvec);
}
    80002a76:	6422                	ld	s0,8(sp)
    80002a78:	0141                	addi	sp,sp,16
    80002a7a:	8082                	ret

0000000080002a7c <usertrapret>:
//
// return to user space
//
void
usertrapret(void)
{
    80002a7c:	1141                	addi	sp,sp,-16
    80002a7e:	e406                	sd	ra,8(sp)
    80002a80:	e022                	sd	s0,0(sp)
    80002a82:	0800                	addi	s0,sp,16
  struct proc *p = myproc();
    80002a84:	fffff097          	auipc	ra,0xfffff
    80002a88:	308080e7          	jalr	776(ra) # 80001d8c <myproc>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002a8c:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() & ~SSTATUS_SIE);
    80002a90:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002a92:	10079073          	csrw	sstatus,a5
  // kerneltrap() to usertrap(), so turn off interrupts until
  // we're back in user space, where usertrap() is correct.
  intr_off();

  // send syscalls, interrupts, and exceptions to trampoline.S
  w_stvec(TRAMPOLINE + (uservec - trampoline));
    80002a96:	00004617          	auipc	a2,0x4
    80002a9a:	56a60613          	addi	a2,a2,1386 # 80007000 <_trampoline>
    80002a9e:	00004697          	auipc	a3,0x4
    80002aa2:	56268693          	addi	a3,a3,1378 # 80007000 <_trampoline>
    80002aa6:	8e91                	sub	a3,a3,a2
    80002aa8:	040007b7          	lui	a5,0x4000
    80002aac:	17fd                	addi	a5,a5,-1
    80002aae:	07b2                	slli	a5,a5,0xc
    80002ab0:	96be                	add	a3,a3,a5
  asm volatile("csrw stvec, %0" : : "r" (x));
    80002ab2:	10569073          	csrw	stvec,a3

  // set up trapframe values that uservec will need when
  // the process next re-enters the kernel.
  p->trapframe->kernel_satp = r_satp();         // kernel page table
    80002ab6:	7138                	ld	a4,96(a0)
  asm volatile("csrr %0, satp" : "=r" (x) );
    80002ab8:	180026f3          	csrr	a3,satp
    80002abc:	e314                	sd	a3,0(a4)
  p->trapframe->kernel_sp = p->kstack + PGSIZE; // process's kernel stack
    80002abe:	7138                	ld	a4,96(a0)
    80002ac0:	6534                	ld	a3,72(a0)
    80002ac2:	6585                	lui	a1,0x1
    80002ac4:	96ae                	add	a3,a3,a1
    80002ac6:	e714                	sd	a3,8(a4)
  p->trapframe->kernel_trap = (uint64)usertrap;
    80002ac8:	7138                	ld	a4,96(a0)
    80002aca:	00000697          	auipc	a3,0x0
    80002ace:	13868693          	addi	a3,a3,312 # 80002c02 <usertrap>
    80002ad2:	eb14                	sd	a3,16(a4)
  p->trapframe->kernel_hartid = r_tp();         // hartid for cpuid()
    80002ad4:	7138                	ld	a4,96(a0)
  asm volatile("mv %0, tp" : "=r" (x) );
    80002ad6:	8692                	mv	a3,tp
    80002ad8:	f314                	sd	a3,32(a4)
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002ada:	100026f3          	csrr	a3,sstatus
  // set up the registers that trampoline.S's sret will use
  // to get to user space.
  
  // set S Previous Privilege mode to User.
  unsigned long x = r_sstatus();
  x &= ~SSTATUS_SPP; // clear SPP to 0 for user mode
    80002ade:	eff6f693          	andi	a3,a3,-257
  x |= SSTATUS_SPIE; // enable interrupts in user mode
    80002ae2:	0206e693          	ori	a3,a3,32
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002ae6:	10069073          	csrw	sstatus,a3
  w_sstatus(x);

  // set S Exception Program Counter to the saved user pc.
  w_sepc(p->trapframe->epc);
    80002aea:	7138                	ld	a4,96(a0)
  asm volatile("csrw sepc, %0" : : "r" (x));
    80002aec:	6f18                	ld	a4,24(a4)
    80002aee:	14171073          	csrw	sepc,a4

  // tell trampoline.S the user page table to switch to.
  uint64 satp = MAKE_SATP(p->pagetable);
    80002af2:	6d2c                	ld	a1,88(a0)
    80002af4:	81b1                	srli	a1,a1,0xc

  // jump to trampoline.S at the top of memory, which 
  // switches to the user page table, restores user registers,
  // and switches to user mode with sret.
  uint64 fn = TRAMPOLINE + (userret - trampoline);
    80002af6:	00004717          	auipc	a4,0x4
    80002afa:	59a70713          	addi	a4,a4,1434 # 80007090 <userret>
    80002afe:	8f11                	sub	a4,a4,a2
    80002b00:	97ba                	add	a5,a5,a4
  ((void (*)(uint64,uint64))fn)(TRAPFRAME, satp);
    80002b02:	577d                	li	a4,-1
    80002b04:	177e                	slli	a4,a4,0x3f
    80002b06:	8dd9                	or	a1,a1,a4
    80002b08:	02000537          	lui	a0,0x2000
    80002b0c:	157d                	addi	a0,a0,-1
    80002b0e:	0536                	slli	a0,a0,0xd
    80002b10:	9782                	jalr	a5
}
    80002b12:	60a2                	ld	ra,8(sp)
    80002b14:	6402                	ld	s0,0(sp)
    80002b16:	0141                	addi	sp,sp,16
    80002b18:	8082                	ret

0000000080002b1a <clockintr>:
  w_sstatus(sstatus);
}

void
clockintr()
{
    80002b1a:	1101                	addi	sp,sp,-32
    80002b1c:	ec06                	sd	ra,24(sp)
    80002b1e:	e822                	sd	s0,16(sp)
    80002b20:	e426                	sd	s1,8(sp)
    80002b22:	1000                	addi	s0,sp,32
  acquire(&tickslock);
    80002b24:	00015497          	auipc	s1,0x15
    80002b28:	86c48493          	addi	s1,s1,-1940 # 80017390 <tickslock>
    80002b2c:	8526                	mv	a0,s1
    80002b2e:	ffffe097          	auipc	ra,0xffffe
    80002b32:	0b6080e7          	jalr	182(ra) # 80000be4 <acquire>
  ticks++;
    80002b36:	00006517          	auipc	a0,0x6
    80002b3a:	4fa50513          	addi	a0,a0,1274 # 80009030 <ticks>
    80002b3e:	411c                	lw	a5,0(a0)
    80002b40:	2785                	addiw	a5,a5,1
    80002b42:	c11c                	sw	a5,0(a0)
  wakeup(&ticks);
    80002b44:	00000097          	auipc	ra,0x0
    80002b48:	a92080e7          	jalr	-1390(ra) # 800025d6 <wakeup>
  release(&tickslock);
    80002b4c:	8526                	mv	a0,s1
    80002b4e:	ffffe097          	auipc	ra,0xffffe
    80002b52:	14a080e7          	jalr	330(ra) # 80000c98 <release>
}
    80002b56:	60e2                	ld	ra,24(sp)
    80002b58:	6442                	ld	s0,16(sp)
    80002b5a:	64a2                	ld	s1,8(sp)
    80002b5c:	6105                	addi	sp,sp,32
    80002b5e:	8082                	ret

0000000080002b60 <devintr>:
// returns 2 if timer interrupt,
// 1 if other device,
// 0 if not recognized.
int
devintr()
{
    80002b60:	1101                	addi	sp,sp,-32
    80002b62:	ec06                	sd	ra,24(sp)
    80002b64:	e822                	sd	s0,16(sp)
    80002b66:	e426                	sd	s1,8(sp)
    80002b68:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002b6a:	14202773          	csrr	a4,scause
  uint64 scause = r_scause();

  if((scause & 0x8000000000000000L) &&
    80002b6e:	00074d63          	bltz	a4,80002b88 <devintr+0x28>
    // now allowed to interrupt again.
    if(irq)
      plic_complete(irq);

    return 1;
  } else if(scause == 0x8000000000000001L){
    80002b72:	57fd                	li	a5,-1
    80002b74:	17fe                	slli	a5,a5,0x3f
    80002b76:	0785                	addi	a5,a5,1
    // the SSIP bit in sip.
    w_sip(r_sip() & ~2);

    return 2;
  } else {
    return 0;
    80002b78:	4501                	li	a0,0
  } else if(scause == 0x8000000000000001L){
    80002b7a:	06f70363          	beq	a4,a5,80002be0 <devintr+0x80>
  }
}
    80002b7e:	60e2                	ld	ra,24(sp)
    80002b80:	6442                	ld	s0,16(sp)
    80002b82:	64a2                	ld	s1,8(sp)
    80002b84:	6105                	addi	sp,sp,32
    80002b86:	8082                	ret
     (scause & 0xff) == 9){
    80002b88:	0ff77793          	andi	a5,a4,255
  if((scause & 0x8000000000000000L) &&
    80002b8c:	46a5                	li	a3,9
    80002b8e:	fed792e3          	bne	a5,a3,80002b72 <devintr+0x12>
    int irq = plic_claim();
    80002b92:	00003097          	auipc	ra,0x3
    80002b96:	476080e7          	jalr	1142(ra) # 80006008 <plic_claim>
    80002b9a:	84aa                	mv	s1,a0
    if(irq == UART0_IRQ){
    80002b9c:	47a9                	li	a5,10
    80002b9e:	02f50763          	beq	a0,a5,80002bcc <devintr+0x6c>
    } else if(irq == VIRTIO0_IRQ){
    80002ba2:	4785                	li	a5,1
    80002ba4:	02f50963          	beq	a0,a5,80002bd6 <devintr+0x76>
    return 1;
    80002ba8:	4505                	li	a0,1
    } else if(irq){
    80002baa:	d8f1                	beqz	s1,80002b7e <devintr+0x1e>
      printf("unexpected interrupt irq=%d\n", irq);
    80002bac:	85a6                	mv	a1,s1
    80002bae:	00005517          	auipc	a0,0x5
    80002bb2:	7aa50513          	addi	a0,a0,1962 # 80008358 <states.1776+0x38>
    80002bb6:	ffffe097          	auipc	ra,0xffffe
    80002bba:	9d2080e7          	jalr	-1582(ra) # 80000588 <printf>
      plic_complete(irq);
    80002bbe:	8526                	mv	a0,s1
    80002bc0:	00003097          	auipc	ra,0x3
    80002bc4:	46c080e7          	jalr	1132(ra) # 8000602c <plic_complete>
    return 1;
    80002bc8:	4505                	li	a0,1
    80002bca:	bf55                	j	80002b7e <devintr+0x1e>
      uartintr();
    80002bcc:	ffffe097          	auipc	ra,0xffffe
    80002bd0:	ddc080e7          	jalr	-548(ra) # 800009a8 <uartintr>
    80002bd4:	b7ed                	j	80002bbe <devintr+0x5e>
      virtio_disk_intr();
    80002bd6:	00004097          	auipc	ra,0x4
    80002bda:	936080e7          	jalr	-1738(ra) # 8000650c <virtio_disk_intr>
    80002bde:	b7c5                	j	80002bbe <devintr+0x5e>
    if(cpuid() == 0){
    80002be0:	fffff097          	auipc	ra,0xfffff
    80002be4:	180080e7          	jalr	384(ra) # 80001d60 <cpuid>
    80002be8:	c901                	beqz	a0,80002bf8 <devintr+0x98>
  asm volatile("csrr %0, sip" : "=r" (x) );
    80002bea:	144027f3          	csrr	a5,sip
    w_sip(r_sip() & ~2);
    80002bee:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sip, %0" : : "r" (x));
    80002bf0:	14479073          	csrw	sip,a5
    return 2;
    80002bf4:	4509                	li	a0,2
    80002bf6:	b761                	j	80002b7e <devintr+0x1e>
      clockintr();
    80002bf8:	00000097          	auipc	ra,0x0
    80002bfc:	f22080e7          	jalr	-222(ra) # 80002b1a <clockintr>
    80002c00:	b7ed                	j	80002bea <devintr+0x8a>

0000000080002c02 <usertrap>:
{
    80002c02:	1101                	addi	sp,sp,-32
    80002c04:	ec06                	sd	ra,24(sp)
    80002c06:	e822                	sd	s0,16(sp)
    80002c08:	e426                	sd	s1,8(sp)
    80002c0a:	e04a                	sd	s2,0(sp)
    80002c0c:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002c0e:	100027f3          	csrr	a5,sstatus
  if((r_sstatus() & SSTATUS_SPP) != 0)
    80002c12:	1007f793          	andi	a5,a5,256
    80002c16:	e3ad                	bnez	a5,80002c78 <usertrap+0x76>
  asm volatile("csrw stvec, %0" : : "r" (x));
    80002c18:	00003797          	auipc	a5,0x3
    80002c1c:	2e878793          	addi	a5,a5,744 # 80005f00 <kernelvec>
    80002c20:	10579073          	csrw	stvec,a5
  struct proc *p = myproc();
    80002c24:	fffff097          	auipc	ra,0xfffff
    80002c28:	168080e7          	jalr	360(ra) # 80001d8c <myproc>
    80002c2c:	84aa                	mv	s1,a0
  p->trapframe->epc = r_sepc();
    80002c2e:	713c                	ld	a5,96(a0)
  asm volatile("csrr %0, sepc" : "=r" (x) );
    80002c30:	14102773          	csrr	a4,sepc
    80002c34:	ef98                	sd	a4,24(a5)
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002c36:	14202773          	csrr	a4,scause
  if(r_scause() == 8){
    80002c3a:	47a1                	li	a5,8
    80002c3c:	04f71c63          	bne	a4,a5,80002c94 <usertrap+0x92>
    if(p->killed)
    80002c40:	551c                	lw	a5,40(a0)
    80002c42:	e3b9                	bnez	a5,80002c88 <usertrap+0x86>
    p->trapframe->epc += 4;
    80002c44:	70b8                	ld	a4,96(s1)
    80002c46:	6f1c                	ld	a5,24(a4)
    80002c48:	0791                	addi	a5,a5,4
    80002c4a:	ef1c                	sd	a5,24(a4)
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002c4c:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80002c50:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002c54:	10079073          	csrw	sstatus,a5
    syscall();
    80002c58:	00000097          	auipc	ra,0x0
    80002c5c:	2e0080e7          	jalr	736(ra) # 80002f38 <syscall>
  if(p->killed)
    80002c60:	549c                	lw	a5,40(s1)
    80002c62:	ebc1                	bnez	a5,80002cf2 <usertrap+0xf0>
  usertrapret();
    80002c64:	00000097          	auipc	ra,0x0
    80002c68:	e18080e7          	jalr	-488(ra) # 80002a7c <usertrapret>
}
    80002c6c:	60e2                	ld	ra,24(sp)
    80002c6e:	6442                	ld	s0,16(sp)
    80002c70:	64a2                	ld	s1,8(sp)
    80002c72:	6902                	ld	s2,0(sp)
    80002c74:	6105                	addi	sp,sp,32
    80002c76:	8082                	ret
    panic("usertrap: not from user mode");
    80002c78:	00005517          	auipc	a0,0x5
    80002c7c:	70050513          	addi	a0,a0,1792 # 80008378 <states.1776+0x58>
    80002c80:	ffffe097          	auipc	ra,0xffffe
    80002c84:	8be080e7          	jalr	-1858(ra) # 8000053e <panic>
      exit(-1);
    80002c88:	557d                	li	a0,-1
    80002c8a:	00000097          	auipc	ra,0x0
    80002c8e:	a1c080e7          	jalr	-1508(ra) # 800026a6 <exit>
    80002c92:	bf4d                	j	80002c44 <usertrap+0x42>
  } else if((which_dev = devintr()) != 0){
    80002c94:	00000097          	auipc	ra,0x0
    80002c98:	ecc080e7          	jalr	-308(ra) # 80002b60 <devintr>
    80002c9c:	892a                	mv	s2,a0
    80002c9e:	c501                	beqz	a0,80002ca6 <usertrap+0xa4>
  if(p->killed)
    80002ca0:	549c                	lw	a5,40(s1)
    80002ca2:	c3a1                	beqz	a5,80002ce2 <usertrap+0xe0>
    80002ca4:	a815                	j	80002cd8 <usertrap+0xd6>
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002ca6:	142025f3          	csrr	a1,scause
    printf("usertrap(): unexpected scause %p pid=%d\n", r_scause(), p->pid);
    80002caa:	5890                	lw	a2,48(s1)
    80002cac:	00005517          	auipc	a0,0x5
    80002cb0:	6ec50513          	addi	a0,a0,1772 # 80008398 <states.1776+0x78>
    80002cb4:	ffffe097          	auipc	ra,0xffffe
    80002cb8:	8d4080e7          	jalr	-1836(ra) # 80000588 <printf>
  asm volatile("csrr %0, sepc" : "=r" (x) );
    80002cbc:	141025f3          	csrr	a1,sepc
  asm volatile("csrr %0, stval" : "=r" (x) );
    80002cc0:	14302673          	csrr	a2,stval
    printf("            sepc=%p stval=%p\n", r_sepc(), r_stval());
    80002cc4:	00005517          	auipc	a0,0x5
    80002cc8:	70450513          	addi	a0,a0,1796 # 800083c8 <states.1776+0xa8>
    80002ccc:	ffffe097          	auipc	ra,0xffffe
    80002cd0:	8bc080e7          	jalr	-1860(ra) # 80000588 <printf>
    p->killed = 1;
    80002cd4:	4785                	li	a5,1
    80002cd6:	d49c                	sw	a5,40(s1)
    exit(-1);
    80002cd8:	557d                	li	a0,-1
    80002cda:	00000097          	auipc	ra,0x0
    80002cde:	9cc080e7          	jalr	-1588(ra) # 800026a6 <exit>
  if(which_dev == 2)
    80002ce2:	4789                	li	a5,2
    80002ce4:	f8f910e3          	bne	s2,a5,80002c64 <usertrap+0x62>
    yield();
    80002ce8:	fffff097          	auipc	ra,0xfffff
    80002cec:	726080e7          	jalr	1830(ra) # 8000240e <yield>
    80002cf0:	bf95                	j	80002c64 <usertrap+0x62>
  int which_dev = 0;
    80002cf2:	4901                	li	s2,0
    80002cf4:	b7d5                	j	80002cd8 <usertrap+0xd6>

0000000080002cf6 <kerneltrap>:
{
    80002cf6:	7179                	addi	sp,sp,-48
    80002cf8:	f406                	sd	ra,40(sp)
    80002cfa:	f022                	sd	s0,32(sp)
    80002cfc:	ec26                	sd	s1,24(sp)
    80002cfe:	e84a                	sd	s2,16(sp)
    80002d00:	e44e                	sd	s3,8(sp)
    80002d02:	1800                	addi	s0,sp,48
  asm volatile("csrr %0, sepc" : "=r" (x) );
    80002d04:	14102973          	csrr	s2,sepc
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002d08:	100024f3          	csrr	s1,sstatus
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002d0c:	142029f3          	csrr	s3,scause
  if((sstatus & SSTATUS_SPP) == 0)
    80002d10:	1004f793          	andi	a5,s1,256
    80002d14:	cb85                	beqz	a5,80002d44 <kerneltrap+0x4e>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002d16:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    80002d1a:	8b89                	andi	a5,a5,2
  if(intr_get() != 0)
    80002d1c:	ef85                	bnez	a5,80002d54 <kerneltrap+0x5e>
  if((which_dev = devintr()) == 0){
    80002d1e:	00000097          	auipc	ra,0x0
    80002d22:	e42080e7          	jalr	-446(ra) # 80002b60 <devintr>
    80002d26:	cd1d                	beqz	a0,80002d64 <kerneltrap+0x6e>
  if(which_dev == 2 && myproc() != 0 && myproc()->state == RUNNING)
    80002d28:	4789                	li	a5,2
    80002d2a:	06f50a63          	beq	a0,a5,80002d9e <kerneltrap+0xa8>
  asm volatile("csrw sepc, %0" : : "r" (x));
    80002d2e:	14191073          	csrw	sepc,s2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002d32:	10049073          	csrw	sstatus,s1
}
    80002d36:	70a2                	ld	ra,40(sp)
    80002d38:	7402                	ld	s0,32(sp)
    80002d3a:	64e2                	ld	s1,24(sp)
    80002d3c:	6942                	ld	s2,16(sp)
    80002d3e:	69a2                	ld	s3,8(sp)
    80002d40:	6145                	addi	sp,sp,48
    80002d42:	8082                	ret
    panic("kerneltrap: not from supervisor mode");
    80002d44:	00005517          	auipc	a0,0x5
    80002d48:	6a450513          	addi	a0,a0,1700 # 800083e8 <states.1776+0xc8>
    80002d4c:	ffffd097          	auipc	ra,0xffffd
    80002d50:	7f2080e7          	jalr	2034(ra) # 8000053e <panic>
    panic("kerneltrap: interrupts enabled");
    80002d54:	00005517          	auipc	a0,0x5
    80002d58:	6bc50513          	addi	a0,a0,1724 # 80008410 <states.1776+0xf0>
    80002d5c:	ffffd097          	auipc	ra,0xffffd
    80002d60:	7e2080e7          	jalr	2018(ra) # 8000053e <panic>
    printf("scause %p\n", scause);
    80002d64:	85ce                	mv	a1,s3
    80002d66:	00005517          	auipc	a0,0x5
    80002d6a:	6ca50513          	addi	a0,a0,1738 # 80008430 <states.1776+0x110>
    80002d6e:	ffffe097          	auipc	ra,0xffffe
    80002d72:	81a080e7          	jalr	-2022(ra) # 80000588 <printf>
  asm volatile("csrr %0, sepc" : "=r" (x) );
    80002d76:	141025f3          	csrr	a1,sepc
  asm volatile("csrr %0, stval" : "=r" (x) );
    80002d7a:	14302673          	csrr	a2,stval
    printf("sepc=%p stval=%p\n", r_sepc(), r_stval());
    80002d7e:	00005517          	auipc	a0,0x5
    80002d82:	6c250513          	addi	a0,a0,1730 # 80008440 <states.1776+0x120>
    80002d86:	ffffe097          	auipc	ra,0xffffe
    80002d8a:	802080e7          	jalr	-2046(ra) # 80000588 <printf>
    panic("kerneltrap");
    80002d8e:	00005517          	auipc	a0,0x5
    80002d92:	6ca50513          	addi	a0,a0,1738 # 80008458 <states.1776+0x138>
    80002d96:	ffffd097          	auipc	ra,0xffffd
    80002d9a:	7a8080e7          	jalr	1960(ra) # 8000053e <panic>
  if(which_dev == 2 && myproc() != 0 && myproc()->state == RUNNING)
    80002d9e:	fffff097          	auipc	ra,0xfffff
    80002da2:	fee080e7          	jalr	-18(ra) # 80001d8c <myproc>
    80002da6:	d541                	beqz	a0,80002d2e <kerneltrap+0x38>
    80002da8:	fffff097          	auipc	ra,0xfffff
    80002dac:	fe4080e7          	jalr	-28(ra) # 80001d8c <myproc>
    80002db0:	4d18                	lw	a4,24(a0)
    80002db2:	4791                	li	a5,4
    80002db4:	f6f71de3          	bne	a4,a5,80002d2e <kerneltrap+0x38>
    yield();
    80002db8:	fffff097          	auipc	ra,0xfffff
    80002dbc:	656080e7          	jalr	1622(ra) # 8000240e <yield>
    80002dc0:	b7bd                	j	80002d2e <kerneltrap+0x38>

0000000080002dc2 <argraw>:
  return strlen(buf);
}

static uint64
argraw(int n)
{
    80002dc2:	1101                	addi	sp,sp,-32
    80002dc4:	ec06                	sd	ra,24(sp)
    80002dc6:	e822                	sd	s0,16(sp)
    80002dc8:	e426                	sd	s1,8(sp)
    80002dca:	1000                	addi	s0,sp,32
    80002dcc:	84aa                	mv	s1,a0
  struct proc *p = myproc();
    80002dce:	fffff097          	auipc	ra,0xfffff
    80002dd2:	fbe080e7          	jalr	-66(ra) # 80001d8c <myproc>
  switch (n) {
    80002dd6:	4795                	li	a5,5
    80002dd8:	0497e163          	bltu	a5,s1,80002e1a <argraw+0x58>
    80002ddc:	048a                	slli	s1,s1,0x2
    80002dde:	00005717          	auipc	a4,0x5
    80002de2:	6b270713          	addi	a4,a4,1714 # 80008490 <states.1776+0x170>
    80002de6:	94ba                	add	s1,s1,a4
    80002de8:	409c                	lw	a5,0(s1)
    80002dea:	97ba                	add	a5,a5,a4
    80002dec:	8782                	jr	a5
  case 0:
    return p->trapframe->a0;
    80002dee:	713c                	ld	a5,96(a0)
    80002df0:	7ba8                	ld	a0,112(a5)
  case 5:
    return p->trapframe->a5;
  }
  panic("argraw");
  return -1;
}
    80002df2:	60e2                	ld	ra,24(sp)
    80002df4:	6442                	ld	s0,16(sp)
    80002df6:	64a2                	ld	s1,8(sp)
    80002df8:	6105                	addi	sp,sp,32
    80002dfa:	8082                	ret
    return p->trapframe->a1;
    80002dfc:	713c                	ld	a5,96(a0)
    80002dfe:	7fa8                	ld	a0,120(a5)
    80002e00:	bfcd                	j	80002df2 <argraw+0x30>
    return p->trapframe->a2;
    80002e02:	713c                	ld	a5,96(a0)
    80002e04:	63c8                	ld	a0,128(a5)
    80002e06:	b7f5                	j	80002df2 <argraw+0x30>
    return p->trapframe->a3;
    80002e08:	713c                	ld	a5,96(a0)
    80002e0a:	67c8                	ld	a0,136(a5)
    80002e0c:	b7dd                	j	80002df2 <argraw+0x30>
    return p->trapframe->a4;
    80002e0e:	713c                	ld	a5,96(a0)
    80002e10:	6bc8                	ld	a0,144(a5)
    80002e12:	b7c5                	j	80002df2 <argraw+0x30>
    return p->trapframe->a5;
    80002e14:	713c                	ld	a5,96(a0)
    80002e16:	6fc8                	ld	a0,152(a5)
    80002e18:	bfe9                	j	80002df2 <argraw+0x30>
  panic("argraw");
    80002e1a:	00005517          	auipc	a0,0x5
    80002e1e:	64e50513          	addi	a0,a0,1614 # 80008468 <states.1776+0x148>
    80002e22:	ffffd097          	auipc	ra,0xffffd
    80002e26:	71c080e7          	jalr	1820(ra) # 8000053e <panic>

0000000080002e2a <fetchaddr>:
{
    80002e2a:	1101                	addi	sp,sp,-32
    80002e2c:	ec06                	sd	ra,24(sp)
    80002e2e:	e822                	sd	s0,16(sp)
    80002e30:	e426                	sd	s1,8(sp)
    80002e32:	e04a                	sd	s2,0(sp)
    80002e34:	1000                	addi	s0,sp,32
    80002e36:	84aa                	mv	s1,a0
    80002e38:	892e                	mv	s2,a1
  struct proc *p = myproc();
    80002e3a:	fffff097          	auipc	ra,0xfffff
    80002e3e:	f52080e7          	jalr	-174(ra) # 80001d8c <myproc>
  if(addr >= p->sz || addr+sizeof(uint64) > p->sz)
    80002e42:	693c                	ld	a5,80(a0)
    80002e44:	02f4f863          	bgeu	s1,a5,80002e74 <fetchaddr+0x4a>
    80002e48:	00848713          	addi	a4,s1,8
    80002e4c:	02e7e663          	bltu	a5,a4,80002e78 <fetchaddr+0x4e>
  if(copyin(p->pagetable, (char *)ip, addr, sizeof(*ip)) != 0)
    80002e50:	46a1                	li	a3,8
    80002e52:	8626                	mv	a2,s1
    80002e54:	85ca                	mv	a1,s2
    80002e56:	6d28                	ld	a0,88(a0)
    80002e58:	fffff097          	auipc	ra,0xfffff
    80002e5c:	8a6080e7          	jalr	-1882(ra) # 800016fe <copyin>
    80002e60:	00a03533          	snez	a0,a0
    80002e64:	40a00533          	neg	a0,a0
}
    80002e68:	60e2                	ld	ra,24(sp)
    80002e6a:	6442                	ld	s0,16(sp)
    80002e6c:	64a2                	ld	s1,8(sp)
    80002e6e:	6902                	ld	s2,0(sp)
    80002e70:	6105                	addi	sp,sp,32
    80002e72:	8082                	ret
    return -1;
    80002e74:	557d                	li	a0,-1
    80002e76:	bfcd                	j	80002e68 <fetchaddr+0x3e>
    80002e78:	557d                	li	a0,-1
    80002e7a:	b7fd                	j	80002e68 <fetchaddr+0x3e>

0000000080002e7c <fetchstr>:
{
    80002e7c:	7179                	addi	sp,sp,-48
    80002e7e:	f406                	sd	ra,40(sp)
    80002e80:	f022                	sd	s0,32(sp)
    80002e82:	ec26                	sd	s1,24(sp)
    80002e84:	e84a                	sd	s2,16(sp)
    80002e86:	e44e                	sd	s3,8(sp)
    80002e88:	1800                	addi	s0,sp,48
    80002e8a:	892a                	mv	s2,a0
    80002e8c:	84ae                	mv	s1,a1
    80002e8e:	89b2                	mv	s3,a2
  struct proc *p = myproc();
    80002e90:	fffff097          	auipc	ra,0xfffff
    80002e94:	efc080e7          	jalr	-260(ra) # 80001d8c <myproc>
  int err = copyinstr(p->pagetable, buf, addr, max);
    80002e98:	86ce                	mv	a3,s3
    80002e9a:	864a                	mv	a2,s2
    80002e9c:	85a6                	mv	a1,s1
    80002e9e:	6d28                	ld	a0,88(a0)
    80002ea0:	fffff097          	auipc	ra,0xfffff
    80002ea4:	8ea080e7          	jalr	-1814(ra) # 8000178a <copyinstr>
  if(err < 0)
    80002ea8:	00054763          	bltz	a0,80002eb6 <fetchstr+0x3a>
  return strlen(buf);
    80002eac:	8526                	mv	a0,s1
    80002eae:	ffffe097          	auipc	ra,0xffffe
    80002eb2:	fb6080e7          	jalr	-74(ra) # 80000e64 <strlen>
}
    80002eb6:	70a2                	ld	ra,40(sp)
    80002eb8:	7402                	ld	s0,32(sp)
    80002eba:	64e2                	ld	s1,24(sp)
    80002ebc:	6942                	ld	s2,16(sp)
    80002ebe:	69a2                	ld	s3,8(sp)
    80002ec0:	6145                	addi	sp,sp,48
    80002ec2:	8082                	ret

0000000080002ec4 <argint>:

// Fetch the nth 32-bit system call argument.
int
argint(int n, int *ip)
{
    80002ec4:	1101                	addi	sp,sp,-32
    80002ec6:	ec06                	sd	ra,24(sp)
    80002ec8:	e822                	sd	s0,16(sp)
    80002eca:	e426                	sd	s1,8(sp)
    80002ecc:	1000                	addi	s0,sp,32
    80002ece:	84ae                	mv	s1,a1
  *ip = argraw(n);
    80002ed0:	00000097          	auipc	ra,0x0
    80002ed4:	ef2080e7          	jalr	-270(ra) # 80002dc2 <argraw>
    80002ed8:	c088                	sw	a0,0(s1)
  return 0;
}
    80002eda:	4501                	li	a0,0
    80002edc:	60e2                	ld	ra,24(sp)
    80002ede:	6442                	ld	s0,16(sp)
    80002ee0:	64a2                	ld	s1,8(sp)
    80002ee2:	6105                	addi	sp,sp,32
    80002ee4:	8082                	ret

0000000080002ee6 <argaddr>:
// Retrieve an argument as a pointer.
// Doesn't check for legality, since
// copyin/copyout will do that.
int
argaddr(int n, uint64 *ip)
{
    80002ee6:	1101                	addi	sp,sp,-32
    80002ee8:	ec06                	sd	ra,24(sp)
    80002eea:	e822                	sd	s0,16(sp)
    80002eec:	e426                	sd	s1,8(sp)
    80002eee:	1000                	addi	s0,sp,32
    80002ef0:	84ae                	mv	s1,a1
  *ip = argraw(n);
    80002ef2:	00000097          	auipc	ra,0x0
    80002ef6:	ed0080e7          	jalr	-304(ra) # 80002dc2 <argraw>
    80002efa:	e088                	sd	a0,0(s1)
  return 0;
}
    80002efc:	4501                	li	a0,0
    80002efe:	60e2                	ld	ra,24(sp)
    80002f00:	6442                	ld	s0,16(sp)
    80002f02:	64a2                	ld	s1,8(sp)
    80002f04:	6105                	addi	sp,sp,32
    80002f06:	8082                	ret

0000000080002f08 <argstr>:
// Fetch the nth word-sized system call argument as a null-terminated string.
// Copies into buf, at most max.
// Returns string length if OK (including nul), -1 if error.
int
argstr(int n, char *buf, int max)
{
    80002f08:	1101                	addi	sp,sp,-32
    80002f0a:	ec06                	sd	ra,24(sp)
    80002f0c:	e822                	sd	s0,16(sp)
    80002f0e:	e426                	sd	s1,8(sp)
    80002f10:	e04a                	sd	s2,0(sp)
    80002f12:	1000                	addi	s0,sp,32
    80002f14:	84ae                	mv	s1,a1
    80002f16:	8932                	mv	s2,a2
  *ip = argraw(n);
    80002f18:	00000097          	auipc	ra,0x0
    80002f1c:	eaa080e7          	jalr	-342(ra) # 80002dc2 <argraw>
  uint64 addr;
  if(argaddr(n, &addr) < 0)
    return -1;
  return fetchstr(addr, buf, max);
    80002f20:	864a                	mv	a2,s2
    80002f22:	85a6                	mv	a1,s1
    80002f24:	00000097          	auipc	ra,0x0
    80002f28:	f58080e7          	jalr	-168(ra) # 80002e7c <fetchstr>
}
    80002f2c:	60e2                	ld	ra,24(sp)
    80002f2e:	6442                	ld	s0,16(sp)
    80002f30:	64a2                	ld	s1,8(sp)
    80002f32:	6902                	ld	s2,0(sp)
    80002f34:	6105                	addi	sp,sp,32
    80002f36:	8082                	ret

0000000080002f38 <syscall>:
[SYS_print_pids] sys_print_pids,
};

void
syscall(void)
{
    80002f38:	1101                	addi	sp,sp,-32
    80002f3a:	ec06                	sd	ra,24(sp)
    80002f3c:	e822                	sd	s0,16(sp)
    80002f3e:	e426                	sd	s1,8(sp)
    80002f40:	e04a                	sd	s2,0(sp)
    80002f42:	1000                	addi	s0,sp,32
  int num;
  struct proc *p = myproc();
    80002f44:	fffff097          	auipc	ra,0xfffff
    80002f48:	e48080e7          	jalr	-440(ra) # 80001d8c <myproc>
    80002f4c:	84aa                	mv	s1,a0

  num = p->trapframe->a7;
    80002f4e:	06053903          	ld	s2,96(a0)
    80002f52:	0a893783          	ld	a5,168(s2)
    80002f56:	0007869b          	sext.w	a3,a5
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    80002f5a:	37fd                	addiw	a5,a5,-1
    80002f5c:	4755                	li	a4,21
    80002f5e:	00f76f63          	bltu	a4,a5,80002f7c <syscall+0x44>
    80002f62:	00369713          	slli	a4,a3,0x3
    80002f66:	00005797          	auipc	a5,0x5
    80002f6a:	54278793          	addi	a5,a5,1346 # 800084a8 <syscalls>
    80002f6e:	97ba                	add	a5,a5,a4
    80002f70:	639c                	ld	a5,0(a5)
    80002f72:	c789                	beqz	a5,80002f7c <syscall+0x44>
    p->trapframe->a0 = syscalls[num]();
    80002f74:	9782                	jalr	a5
    80002f76:	06a93823          	sd	a0,112(s2)
    80002f7a:	a839                	j	80002f98 <syscall+0x60>
  } else {
    printf("%d %s: unknown sys call %d\n",
    80002f7c:	16048613          	addi	a2,s1,352
    80002f80:	588c                	lw	a1,48(s1)
    80002f82:	00005517          	auipc	a0,0x5
    80002f86:	4ee50513          	addi	a0,a0,1262 # 80008470 <states.1776+0x150>
    80002f8a:	ffffd097          	auipc	ra,0xffffd
    80002f8e:	5fe080e7          	jalr	1534(ra) # 80000588 <printf>
            p->pid, p->name, num);
    p->trapframe->a0 = -1;
    80002f92:	70bc                	ld	a5,96(s1)
    80002f94:	577d                	li	a4,-1
    80002f96:	fbb8                	sd	a4,112(a5)
  }
}
    80002f98:	60e2                	ld	ra,24(sp)
    80002f9a:	6442                	ld	s0,16(sp)
    80002f9c:	64a2                	ld	s1,8(sp)
    80002f9e:	6902                	ld	s2,0(sp)
    80002fa0:	6105                	addi	sp,sp,32
    80002fa2:	8082                	ret

0000000080002fa4 <sys_print_pids>:


// Adding print_pids
uint64
sys_print_pids(void)
{
    80002fa4:	1141                	addi	sp,sp,-16
    80002fa6:	e406                	sd	ra,8(sp)
    80002fa8:	e022                	sd	s0,0(sp)
    80002faa:	0800                	addi	s0,sp,16
  return print_pids();
    80002fac:	fffff097          	auipc	ra,0xfffff
    80002fb0:	7d0080e7          	jalr	2000(ra) # 8000277c <print_pids>
}
    80002fb4:	60a2                	ld	ra,8(sp)
    80002fb6:	6402                	ld	s0,0(sp)
    80002fb8:	0141                	addi	sp,sp,16
    80002fba:	8082                	ret

0000000080002fbc <sys_exit>:

uint64
sys_exit(void)
{
    80002fbc:	1101                	addi	sp,sp,-32
    80002fbe:	ec06                	sd	ra,24(sp)
    80002fc0:	e822                	sd	s0,16(sp)
    80002fc2:	1000                	addi	s0,sp,32
  int n;
  if(argint(0, &n) < 0)
    80002fc4:	fec40593          	addi	a1,s0,-20
    80002fc8:	4501                	li	a0,0
    80002fca:	00000097          	auipc	ra,0x0
    80002fce:	efa080e7          	jalr	-262(ra) # 80002ec4 <argint>
    return -1;
    80002fd2:	57fd                	li	a5,-1
  if(argint(0, &n) < 0)
    80002fd4:	00054963          	bltz	a0,80002fe6 <sys_exit+0x2a>
  exit(n);
    80002fd8:	fec42503          	lw	a0,-20(s0)
    80002fdc:	fffff097          	auipc	ra,0xfffff
    80002fe0:	6ca080e7          	jalr	1738(ra) # 800026a6 <exit>
  return 0;  // not reached
    80002fe4:	4781                	li	a5,0
}
    80002fe6:	853e                	mv	a0,a5
    80002fe8:	60e2                	ld	ra,24(sp)
    80002fea:	6442                	ld	s0,16(sp)
    80002fec:	6105                	addi	sp,sp,32
    80002fee:	8082                	ret

0000000080002ff0 <sys_getpid>:

uint64
sys_getpid(void)
{
    80002ff0:	1141                	addi	sp,sp,-16
    80002ff2:	e406                	sd	ra,8(sp)
    80002ff4:	e022                	sd	s0,0(sp)
    80002ff6:	0800                	addi	s0,sp,16
  return myproc()->pid;
    80002ff8:	fffff097          	auipc	ra,0xfffff
    80002ffc:	d94080e7          	jalr	-620(ra) # 80001d8c <myproc>
}
    80003000:	5908                	lw	a0,48(a0)
    80003002:	60a2                	ld	ra,8(sp)
    80003004:	6402                	ld	s0,0(sp)
    80003006:	0141                	addi	sp,sp,16
    80003008:	8082                	ret

000000008000300a <sys_fork>:

uint64
sys_fork(void)
{
    8000300a:	1141                	addi	sp,sp,-16
    8000300c:	e406                	sd	ra,8(sp)
    8000300e:	e022                	sd	s0,0(sp)
    80003010:	0800                	addi	s0,sp,16
  return fork();
    80003012:	fffff097          	auipc	ra,0xfffff
    80003016:	142080e7          	jalr	322(ra) # 80002154 <fork>
}
    8000301a:	60a2                	ld	ra,8(sp)
    8000301c:	6402                	ld	s0,0(sp)
    8000301e:	0141                	addi	sp,sp,16
    80003020:	8082                	ret

0000000080003022 <sys_wait>:

uint64
sys_wait(void)
{
    80003022:	1101                	addi	sp,sp,-32
    80003024:	ec06                	sd	ra,24(sp)
    80003026:	e822                	sd	s0,16(sp)
    80003028:	1000                	addi	s0,sp,32
  uint64 p;
  if(argaddr(0, &p) < 0)
    8000302a:	fe840593          	addi	a1,s0,-24
    8000302e:	4501                	li	a0,0
    80003030:	00000097          	auipc	ra,0x0
    80003034:	eb6080e7          	jalr	-330(ra) # 80002ee6 <argaddr>
    80003038:	87aa                	mv	a5,a0
    return -1;
    8000303a:	557d                	li	a0,-1
  if(argaddr(0, &p) < 0)
    8000303c:	0007c863          	bltz	a5,8000304c <sys_wait+0x2a>
  return wait(p);
    80003040:	fe843503          	ld	a0,-24(s0)
    80003044:	fffff097          	auipc	ra,0xfffff
    80003048:	46a080e7          	jalr	1130(ra) # 800024ae <wait>
}
    8000304c:	60e2                	ld	ra,24(sp)
    8000304e:	6442                	ld	s0,16(sp)
    80003050:	6105                	addi	sp,sp,32
    80003052:	8082                	ret

0000000080003054 <sys_sbrk>:

uint64
sys_sbrk(void)
{
    80003054:	7179                	addi	sp,sp,-48
    80003056:	f406                	sd	ra,40(sp)
    80003058:	f022                	sd	s0,32(sp)
    8000305a:	ec26                	sd	s1,24(sp)
    8000305c:	1800                	addi	s0,sp,48
  int addr;
  int n;

  if(argint(0, &n) < 0)
    8000305e:	fdc40593          	addi	a1,s0,-36
    80003062:	4501                	li	a0,0
    80003064:	00000097          	auipc	ra,0x0
    80003068:	e60080e7          	jalr	-416(ra) # 80002ec4 <argint>
    8000306c:	87aa                	mv	a5,a0
    return -1;
    8000306e:	557d                	li	a0,-1
  if(argint(0, &n) < 0)
    80003070:	0207c063          	bltz	a5,80003090 <sys_sbrk+0x3c>
  addr = myproc()->sz;
    80003074:	fffff097          	auipc	ra,0xfffff
    80003078:	d18080e7          	jalr	-744(ra) # 80001d8c <myproc>
    8000307c:	4924                	lw	s1,80(a0)
  if(growproc(n) < 0)
    8000307e:	fdc42503          	lw	a0,-36(s0)
    80003082:	fffff097          	auipc	ra,0xfffff
    80003086:	05e080e7          	jalr	94(ra) # 800020e0 <growproc>
    8000308a:	00054863          	bltz	a0,8000309a <sys_sbrk+0x46>
    return -1;
  return addr;
    8000308e:	8526                	mv	a0,s1
}
    80003090:	70a2                	ld	ra,40(sp)
    80003092:	7402                	ld	s0,32(sp)
    80003094:	64e2                	ld	s1,24(sp)
    80003096:	6145                	addi	sp,sp,48
    80003098:	8082                	ret
    return -1;
    8000309a:	557d                	li	a0,-1
    8000309c:	bfd5                	j	80003090 <sys_sbrk+0x3c>

000000008000309e <sys_sleep>:

uint64
sys_sleep(void)
{
    8000309e:	7139                	addi	sp,sp,-64
    800030a0:	fc06                	sd	ra,56(sp)
    800030a2:	f822                	sd	s0,48(sp)
    800030a4:	f426                	sd	s1,40(sp)
    800030a6:	f04a                	sd	s2,32(sp)
    800030a8:	ec4e                	sd	s3,24(sp)
    800030aa:	0080                	addi	s0,sp,64
  int n;
  uint ticks0;

  if(argint(0, &n) < 0)
    800030ac:	fcc40593          	addi	a1,s0,-52
    800030b0:	4501                	li	a0,0
    800030b2:	00000097          	auipc	ra,0x0
    800030b6:	e12080e7          	jalr	-494(ra) # 80002ec4 <argint>
    return -1;
    800030ba:	57fd                	li	a5,-1
  if(argint(0, &n) < 0)
    800030bc:	06054563          	bltz	a0,80003126 <sys_sleep+0x88>
  acquire(&tickslock);
    800030c0:	00014517          	auipc	a0,0x14
    800030c4:	2d050513          	addi	a0,a0,720 # 80017390 <tickslock>
    800030c8:	ffffe097          	auipc	ra,0xffffe
    800030cc:	b1c080e7          	jalr	-1252(ra) # 80000be4 <acquire>
  ticks0 = ticks;
    800030d0:	00006917          	auipc	s2,0x6
    800030d4:	f6092903          	lw	s2,-160(s2) # 80009030 <ticks>
  while(ticks - ticks0 < n){
    800030d8:	fcc42783          	lw	a5,-52(s0)
    800030dc:	cf85                	beqz	a5,80003114 <sys_sleep+0x76>
    if(myproc()->killed){
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
    800030de:	00014997          	auipc	s3,0x14
    800030e2:	2b298993          	addi	s3,s3,690 # 80017390 <tickslock>
    800030e6:	00006497          	auipc	s1,0x6
    800030ea:	f4a48493          	addi	s1,s1,-182 # 80009030 <ticks>
    if(myproc()->killed){
    800030ee:	fffff097          	auipc	ra,0xfffff
    800030f2:	c9e080e7          	jalr	-866(ra) # 80001d8c <myproc>
    800030f6:	551c                	lw	a5,40(a0)
    800030f8:	ef9d                	bnez	a5,80003136 <sys_sleep+0x98>
    sleep(&ticks, &tickslock);
    800030fa:	85ce                	mv	a1,s3
    800030fc:	8526                	mv	a0,s1
    800030fe:	fffff097          	auipc	ra,0xfffff
    80003102:	34c080e7          	jalr	844(ra) # 8000244a <sleep>
  while(ticks - ticks0 < n){
    80003106:	409c                	lw	a5,0(s1)
    80003108:	412787bb          	subw	a5,a5,s2
    8000310c:	fcc42703          	lw	a4,-52(s0)
    80003110:	fce7efe3          	bltu	a5,a4,800030ee <sys_sleep+0x50>
  }
  release(&tickslock);
    80003114:	00014517          	auipc	a0,0x14
    80003118:	27c50513          	addi	a0,a0,636 # 80017390 <tickslock>
    8000311c:	ffffe097          	auipc	ra,0xffffe
    80003120:	b7c080e7          	jalr	-1156(ra) # 80000c98 <release>
  return 0;
    80003124:	4781                	li	a5,0
}
    80003126:	853e                	mv	a0,a5
    80003128:	70e2                	ld	ra,56(sp)
    8000312a:	7442                	ld	s0,48(sp)
    8000312c:	74a2                	ld	s1,40(sp)
    8000312e:	7902                	ld	s2,32(sp)
    80003130:	69e2                	ld	s3,24(sp)
    80003132:	6121                	addi	sp,sp,64
    80003134:	8082                	ret
      release(&tickslock);
    80003136:	00014517          	auipc	a0,0x14
    8000313a:	25a50513          	addi	a0,a0,602 # 80017390 <tickslock>
    8000313e:	ffffe097          	auipc	ra,0xffffe
    80003142:	b5a080e7          	jalr	-1190(ra) # 80000c98 <release>
      return -1;
    80003146:	57fd                	li	a5,-1
    80003148:	bff9                	j	80003126 <sys_sleep+0x88>

000000008000314a <sys_kill>:

uint64
sys_kill(void)
{
    8000314a:	1101                	addi	sp,sp,-32
    8000314c:	ec06                	sd	ra,24(sp)
    8000314e:	e822                	sd	s0,16(sp)
    80003150:	1000                	addi	s0,sp,32
  int pid;

  if(argint(0, &pid) < 0)
    80003152:	fec40593          	addi	a1,s0,-20
    80003156:	4501                	li	a0,0
    80003158:	00000097          	auipc	ra,0x0
    8000315c:	d6c080e7          	jalr	-660(ra) # 80002ec4 <argint>
    80003160:	87aa                	mv	a5,a0
    return -1;
    80003162:	557d                	li	a0,-1
  if(argint(0, &pid) < 0)
    80003164:	0007c863          	bltz	a5,80003174 <sys_kill+0x2a>
  return kill(pid);
    80003168:	fec42503          	lw	a0,-20(s0)
    8000316c:	fffff097          	auipc	ra,0xfffff
    80003170:	69a080e7          	jalr	1690(ra) # 80002806 <kill>
}
    80003174:	60e2                	ld	ra,24(sp)
    80003176:	6442                	ld	s0,16(sp)
    80003178:	6105                	addi	sp,sp,32
    8000317a:	8082                	ret

000000008000317c <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
uint64
sys_uptime(void)
{
    8000317c:	1101                	addi	sp,sp,-32
    8000317e:	ec06                	sd	ra,24(sp)
    80003180:	e822                	sd	s0,16(sp)
    80003182:	e426                	sd	s1,8(sp)
    80003184:	1000                	addi	s0,sp,32
  uint xticks;

  acquire(&tickslock);
    80003186:	00014517          	auipc	a0,0x14
    8000318a:	20a50513          	addi	a0,a0,522 # 80017390 <tickslock>
    8000318e:	ffffe097          	auipc	ra,0xffffe
    80003192:	a56080e7          	jalr	-1450(ra) # 80000be4 <acquire>
  xticks = ticks;
    80003196:	00006497          	auipc	s1,0x6
    8000319a:	e9a4a483          	lw	s1,-358(s1) # 80009030 <ticks>
  release(&tickslock);
    8000319e:	00014517          	auipc	a0,0x14
    800031a2:	1f250513          	addi	a0,a0,498 # 80017390 <tickslock>
    800031a6:	ffffe097          	auipc	ra,0xffffe
    800031aa:	af2080e7          	jalr	-1294(ra) # 80000c98 <release>
  return xticks;
}
    800031ae:	02049513          	slli	a0,s1,0x20
    800031b2:	9101                	srli	a0,a0,0x20
    800031b4:	60e2                	ld	ra,24(sp)
    800031b6:	6442                	ld	s0,16(sp)
    800031b8:	64a2                	ld	s1,8(sp)
    800031ba:	6105                	addi	sp,sp,32
    800031bc:	8082                	ret

00000000800031be <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
    800031be:	7179                	addi	sp,sp,-48
    800031c0:	f406                	sd	ra,40(sp)
    800031c2:	f022                	sd	s0,32(sp)
    800031c4:	ec26                	sd	s1,24(sp)
    800031c6:	e84a                	sd	s2,16(sp)
    800031c8:	e44e                	sd	s3,8(sp)
    800031ca:	e052                	sd	s4,0(sp)
    800031cc:	1800                	addi	s0,sp,48
  struct buf *b;

  initlock(&bcache.lock, "bcache");
    800031ce:	00005597          	auipc	a1,0x5
    800031d2:	39258593          	addi	a1,a1,914 # 80008560 <syscalls+0xb8>
    800031d6:	00014517          	auipc	a0,0x14
    800031da:	1d250513          	addi	a0,a0,466 # 800173a8 <bcache>
    800031de:	ffffe097          	auipc	ra,0xffffe
    800031e2:	976080e7          	jalr	-1674(ra) # 80000b54 <initlock>

  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
    800031e6:	0001c797          	auipc	a5,0x1c
    800031ea:	1c278793          	addi	a5,a5,450 # 8001f3a8 <bcache+0x8000>
    800031ee:	0001c717          	auipc	a4,0x1c
    800031f2:	42270713          	addi	a4,a4,1058 # 8001f610 <bcache+0x8268>
    800031f6:	2ae7b823          	sd	a4,688(a5)
  bcache.head.next = &bcache.head;
    800031fa:	2ae7bc23          	sd	a4,696(a5)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    800031fe:	00014497          	auipc	s1,0x14
    80003202:	1c248493          	addi	s1,s1,450 # 800173c0 <bcache+0x18>
    b->next = bcache.head.next;
    80003206:	893e                	mv	s2,a5
    b->prev = &bcache.head;
    80003208:	89ba                	mv	s3,a4
    initsleeplock(&b->lock, "buffer");
    8000320a:	00005a17          	auipc	s4,0x5
    8000320e:	35ea0a13          	addi	s4,s4,862 # 80008568 <syscalls+0xc0>
    b->next = bcache.head.next;
    80003212:	2b893783          	ld	a5,696(s2)
    80003216:	e8bc                	sd	a5,80(s1)
    b->prev = &bcache.head;
    80003218:	0534b423          	sd	s3,72(s1)
    initsleeplock(&b->lock, "buffer");
    8000321c:	85d2                	mv	a1,s4
    8000321e:	01048513          	addi	a0,s1,16
    80003222:	00001097          	auipc	ra,0x1
    80003226:	4bc080e7          	jalr	1212(ra) # 800046de <initsleeplock>
    bcache.head.next->prev = b;
    8000322a:	2b893783          	ld	a5,696(s2)
    8000322e:	e7a4                	sd	s1,72(a5)
    bcache.head.next = b;
    80003230:	2a993c23          	sd	s1,696(s2)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    80003234:	45848493          	addi	s1,s1,1112
    80003238:	fd349de3          	bne	s1,s3,80003212 <binit+0x54>
  }
}
    8000323c:	70a2                	ld	ra,40(sp)
    8000323e:	7402                	ld	s0,32(sp)
    80003240:	64e2                	ld	s1,24(sp)
    80003242:	6942                	ld	s2,16(sp)
    80003244:	69a2                	ld	s3,8(sp)
    80003246:	6a02                	ld	s4,0(sp)
    80003248:	6145                	addi	sp,sp,48
    8000324a:	8082                	ret

000000008000324c <bread>:
}

// Return a locked buf with the contents of the indicated block.
struct buf*
bread(uint dev, uint blockno)
{
    8000324c:	7179                	addi	sp,sp,-48
    8000324e:	f406                	sd	ra,40(sp)
    80003250:	f022                	sd	s0,32(sp)
    80003252:	ec26                	sd	s1,24(sp)
    80003254:	e84a                	sd	s2,16(sp)
    80003256:	e44e                	sd	s3,8(sp)
    80003258:	1800                	addi	s0,sp,48
    8000325a:	89aa                	mv	s3,a0
    8000325c:	892e                	mv	s2,a1
  acquire(&bcache.lock);
    8000325e:	00014517          	auipc	a0,0x14
    80003262:	14a50513          	addi	a0,a0,330 # 800173a8 <bcache>
    80003266:	ffffe097          	auipc	ra,0xffffe
    8000326a:	97e080e7          	jalr	-1666(ra) # 80000be4 <acquire>
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
    8000326e:	0001c497          	auipc	s1,0x1c
    80003272:	3f24b483          	ld	s1,1010(s1) # 8001f660 <bcache+0x82b8>
    80003276:	0001c797          	auipc	a5,0x1c
    8000327a:	39a78793          	addi	a5,a5,922 # 8001f610 <bcache+0x8268>
    8000327e:	02f48f63          	beq	s1,a5,800032bc <bread+0x70>
    80003282:	873e                	mv	a4,a5
    80003284:	a021                	j	8000328c <bread+0x40>
    80003286:	68a4                	ld	s1,80(s1)
    80003288:	02e48a63          	beq	s1,a4,800032bc <bread+0x70>
    if(b->dev == dev && b->blockno == blockno){
    8000328c:	449c                	lw	a5,8(s1)
    8000328e:	ff379ce3          	bne	a5,s3,80003286 <bread+0x3a>
    80003292:	44dc                	lw	a5,12(s1)
    80003294:	ff2799e3          	bne	a5,s2,80003286 <bread+0x3a>
      b->refcnt++;
    80003298:	40bc                	lw	a5,64(s1)
    8000329a:	2785                	addiw	a5,a5,1
    8000329c:	c0bc                	sw	a5,64(s1)
      release(&bcache.lock);
    8000329e:	00014517          	auipc	a0,0x14
    800032a2:	10a50513          	addi	a0,a0,266 # 800173a8 <bcache>
    800032a6:	ffffe097          	auipc	ra,0xffffe
    800032aa:	9f2080e7          	jalr	-1550(ra) # 80000c98 <release>
      acquiresleep(&b->lock);
    800032ae:	01048513          	addi	a0,s1,16
    800032b2:	00001097          	auipc	ra,0x1
    800032b6:	466080e7          	jalr	1126(ra) # 80004718 <acquiresleep>
      return b;
    800032ba:	a8b9                	j	80003318 <bread+0xcc>
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    800032bc:	0001c497          	auipc	s1,0x1c
    800032c0:	39c4b483          	ld	s1,924(s1) # 8001f658 <bcache+0x82b0>
    800032c4:	0001c797          	auipc	a5,0x1c
    800032c8:	34c78793          	addi	a5,a5,844 # 8001f610 <bcache+0x8268>
    800032cc:	00f48863          	beq	s1,a5,800032dc <bread+0x90>
    800032d0:	873e                	mv	a4,a5
    if(b->refcnt == 0) {
    800032d2:	40bc                	lw	a5,64(s1)
    800032d4:	cf81                	beqz	a5,800032ec <bread+0xa0>
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    800032d6:	64a4                	ld	s1,72(s1)
    800032d8:	fee49de3          	bne	s1,a4,800032d2 <bread+0x86>
  panic("bget: no buffers");
    800032dc:	00005517          	auipc	a0,0x5
    800032e0:	29450513          	addi	a0,a0,660 # 80008570 <syscalls+0xc8>
    800032e4:	ffffd097          	auipc	ra,0xffffd
    800032e8:	25a080e7          	jalr	602(ra) # 8000053e <panic>
      b->dev = dev;
    800032ec:	0134a423          	sw	s3,8(s1)
      b->blockno = blockno;
    800032f0:	0124a623          	sw	s2,12(s1)
      b->valid = 0;
    800032f4:	0004a023          	sw	zero,0(s1)
      b->refcnt = 1;
    800032f8:	4785                	li	a5,1
    800032fa:	c0bc                	sw	a5,64(s1)
      release(&bcache.lock);
    800032fc:	00014517          	auipc	a0,0x14
    80003300:	0ac50513          	addi	a0,a0,172 # 800173a8 <bcache>
    80003304:	ffffe097          	auipc	ra,0xffffe
    80003308:	994080e7          	jalr	-1644(ra) # 80000c98 <release>
      acquiresleep(&b->lock);
    8000330c:	01048513          	addi	a0,s1,16
    80003310:	00001097          	auipc	ra,0x1
    80003314:	408080e7          	jalr	1032(ra) # 80004718 <acquiresleep>
  struct buf *b;

  b = bget(dev, blockno);
  if(!b->valid) {
    80003318:	409c                	lw	a5,0(s1)
    8000331a:	cb89                	beqz	a5,8000332c <bread+0xe0>
    virtio_disk_rw(b, 0);
    b->valid = 1;
  }
  return b;
}
    8000331c:	8526                	mv	a0,s1
    8000331e:	70a2                	ld	ra,40(sp)
    80003320:	7402                	ld	s0,32(sp)
    80003322:	64e2                	ld	s1,24(sp)
    80003324:	6942                	ld	s2,16(sp)
    80003326:	69a2                	ld	s3,8(sp)
    80003328:	6145                	addi	sp,sp,48
    8000332a:	8082                	ret
    virtio_disk_rw(b, 0);
    8000332c:	4581                	li	a1,0
    8000332e:	8526                	mv	a0,s1
    80003330:	00003097          	auipc	ra,0x3
    80003334:	f06080e7          	jalr	-250(ra) # 80006236 <virtio_disk_rw>
    b->valid = 1;
    80003338:	4785                	li	a5,1
    8000333a:	c09c                	sw	a5,0(s1)
  return b;
    8000333c:	b7c5                	j	8000331c <bread+0xd0>

000000008000333e <bwrite>:

// Write b's contents to disk.  Must be locked.
void
bwrite(struct buf *b)
{
    8000333e:	1101                	addi	sp,sp,-32
    80003340:	ec06                	sd	ra,24(sp)
    80003342:	e822                	sd	s0,16(sp)
    80003344:	e426                	sd	s1,8(sp)
    80003346:	1000                	addi	s0,sp,32
    80003348:	84aa                	mv	s1,a0
  if(!holdingsleep(&b->lock))
    8000334a:	0541                	addi	a0,a0,16
    8000334c:	00001097          	auipc	ra,0x1
    80003350:	466080e7          	jalr	1126(ra) # 800047b2 <holdingsleep>
    80003354:	cd01                	beqz	a0,8000336c <bwrite+0x2e>
    panic("bwrite");
  virtio_disk_rw(b, 1);
    80003356:	4585                	li	a1,1
    80003358:	8526                	mv	a0,s1
    8000335a:	00003097          	auipc	ra,0x3
    8000335e:	edc080e7          	jalr	-292(ra) # 80006236 <virtio_disk_rw>
}
    80003362:	60e2                	ld	ra,24(sp)
    80003364:	6442                	ld	s0,16(sp)
    80003366:	64a2                	ld	s1,8(sp)
    80003368:	6105                	addi	sp,sp,32
    8000336a:	8082                	ret
    panic("bwrite");
    8000336c:	00005517          	auipc	a0,0x5
    80003370:	21c50513          	addi	a0,a0,540 # 80008588 <syscalls+0xe0>
    80003374:	ffffd097          	auipc	ra,0xffffd
    80003378:	1ca080e7          	jalr	458(ra) # 8000053e <panic>

000000008000337c <brelse>:

// Release a locked buffer.
// Move to the head of the most-recently-used list.
void
brelse(struct buf *b)
{
    8000337c:	1101                	addi	sp,sp,-32
    8000337e:	ec06                	sd	ra,24(sp)
    80003380:	e822                	sd	s0,16(sp)
    80003382:	e426                	sd	s1,8(sp)
    80003384:	e04a                	sd	s2,0(sp)
    80003386:	1000                	addi	s0,sp,32
    80003388:	84aa                	mv	s1,a0
  if(!holdingsleep(&b->lock))
    8000338a:	01050913          	addi	s2,a0,16
    8000338e:	854a                	mv	a0,s2
    80003390:	00001097          	auipc	ra,0x1
    80003394:	422080e7          	jalr	1058(ra) # 800047b2 <holdingsleep>
    80003398:	c92d                	beqz	a0,8000340a <brelse+0x8e>
    panic("brelse");

  releasesleep(&b->lock);
    8000339a:	854a                	mv	a0,s2
    8000339c:	00001097          	auipc	ra,0x1
    800033a0:	3d2080e7          	jalr	978(ra) # 8000476e <releasesleep>

  acquire(&bcache.lock);
    800033a4:	00014517          	auipc	a0,0x14
    800033a8:	00450513          	addi	a0,a0,4 # 800173a8 <bcache>
    800033ac:	ffffe097          	auipc	ra,0xffffe
    800033b0:	838080e7          	jalr	-1992(ra) # 80000be4 <acquire>
  b->refcnt--;
    800033b4:	40bc                	lw	a5,64(s1)
    800033b6:	37fd                	addiw	a5,a5,-1
    800033b8:	0007871b          	sext.w	a4,a5
    800033bc:	c0bc                	sw	a5,64(s1)
  if (b->refcnt == 0) {
    800033be:	eb05                	bnez	a4,800033ee <brelse+0x72>
    // no one is waiting for it.
    b->next->prev = b->prev;
    800033c0:	68bc                	ld	a5,80(s1)
    800033c2:	64b8                	ld	a4,72(s1)
    800033c4:	e7b8                	sd	a4,72(a5)
    b->prev->next = b->next;
    800033c6:	64bc                	ld	a5,72(s1)
    800033c8:	68b8                	ld	a4,80(s1)
    800033ca:	ebb8                	sd	a4,80(a5)
    b->next = bcache.head.next;
    800033cc:	0001c797          	auipc	a5,0x1c
    800033d0:	fdc78793          	addi	a5,a5,-36 # 8001f3a8 <bcache+0x8000>
    800033d4:	2b87b703          	ld	a4,696(a5)
    800033d8:	e8b8                	sd	a4,80(s1)
    b->prev = &bcache.head;
    800033da:	0001c717          	auipc	a4,0x1c
    800033de:	23670713          	addi	a4,a4,566 # 8001f610 <bcache+0x8268>
    800033e2:	e4b8                	sd	a4,72(s1)
    bcache.head.next->prev = b;
    800033e4:	2b87b703          	ld	a4,696(a5)
    800033e8:	e724                	sd	s1,72(a4)
    bcache.head.next = b;
    800033ea:	2a97bc23          	sd	s1,696(a5)
  }
  
  release(&bcache.lock);
    800033ee:	00014517          	auipc	a0,0x14
    800033f2:	fba50513          	addi	a0,a0,-70 # 800173a8 <bcache>
    800033f6:	ffffe097          	auipc	ra,0xffffe
    800033fa:	8a2080e7          	jalr	-1886(ra) # 80000c98 <release>
}
    800033fe:	60e2                	ld	ra,24(sp)
    80003400:	6442                	ld	s0,16(sp)
    80003402:	64a2                	ld	s1,8(sp)
    80003404:	6902                	ld	s2,0(sp)
    80003406:	6105                	addi	sp,sp,32
    80003408:	8082                	ret
    panic("brelse");
    8000340a:	00005517          	auipc	a0,0x5
    8000340e:	18650513          	addi	a0,a0,390 # 80008590 <syscalls+0xe8>
    80003412:	ffffd097          	auipc	ra,0xffffd
    80003416:	12c080e7          	jalr	300(ra) # 8000053e <panic>

000000008000341a <bpin>:

void
bpin(struct buf *b) {
    8000341a:	1101                	addi	sp,sp,-32
    8000341c:	ec06                	sd	ra,24(sp)
    8000341e:	e822                	sd	s0,16(sp)
    80003420:	e426                	sd	s1,8(sp)
    80003422:	1000                	addi	s0,sp,32
    80003424:	84aa                	mv	s1,a0
  acquire(&bcache.lock);
    80003426:	00014517          	auipc	a0,0x14
    8000342a:	f8250513          	addi	a0,a0,-126 # 800173a8 <bcache>
    8000342e:	ffffd097          	auipc	ra,0xffffd
    80003432:	7b6080e7          	jalr	1974(ra) # 80000be4 <acquire>
  b->refcnt++;
    80003436:	40bc                	lw	a5,64(s1)
    80003438:	2785                	addiw	a5,a5,1
    8000343a:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    8000343c:	00014517          	auipc	a0,0x14
    80003440:	f6c50513          	addi	a0,a0,-148 # 800173a8 <bcache>
    80003444:	ffffe097          	auipc	ra,0xffffe
    80003448:	854080e7          	jalr	-1964(ra) # 80000c98 <release>
}
    8000344c:	60e2                	ld	ra,24(sp)
    8000344e:	6442                	ld	s0,16(sp)
    80003450:	64a2                	ld	s1,8(sp)
    80003452:	6105                	addi	sp,sp,32
    80003454:	8082                	ret

0000000080003456 <bunpin>:

void
bunpin(struct buf *b) {
    80003456:	1101                	addi	sp,sp,-32
    80003458:	ec06                	sd	ra,24(sp)
    8000345a:	e822                	sd	s0,16(sp)
    8000345c:	e426                	sd	s1,8(sp)
    8000345e:	1000                	addi	s0,sp,32
    80003460:	84aa                	mv	s1,a0
  acquire(&bcache.lock);
    80003462:	00014517          	auipc	a0,0x14
    80003466:	f4650513          	addi	a0,a0,-186 # 800173a8 <bcache>
    8000346a:	ffffd097          	auipc	ra,0xffffd
    8000346e:	77a080e7          	jalr	1914(ra) # 80000be4 <acquire>
  b->refcnt--;
    80003472:	40bc                	lw	a5,64(s1)
    80003474:	37fd                	addiw	a5,a5,-1
    80003476:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    80003478:	00014517          	auipc	a0,0x14
    8000347c:	f3050513          	addi	a0,a0,-208 # 800173a8 <bcache>
    80003480:	ffffe097          	auipc	ra,0xffffe
    80003484:	818080e7          	jalr	-2024(ra) # 80000c98 <release>
}
    80003488:	60e2                	ld	ra,24(sp)
    8000348a:	6442                	ld	s0,16(sp)
    8000348c:	64a2                	ld	s1,8(sp)
    8000348e:	6105                	addi	sp,sp,32
    80003490:	8082                	ret

0000000080003492 <bfree>:
}

// Free a disk block.
static void
bfree(int dev, uint b)
{
    80003492:	1101                	addi	sp,sp,-32
    80003494:	ec06                	sd	ra,24(sp)
    80003496:	e822                	sd	s0,16(sp)
    80003498:	e426                	sd	s1,8(sp)
    8000349a:	e04a                	sd	s2,0(sp)
    8000349c:	1000                	addi	s0,sp,32
    8000349e:	84ae                	mv	s1,a1
  struct buf *bp;
  int bi, m;

  bp = bread(dev, BBLOCK(b, sb));
    800034a0:	00d5d59b          	srliw	a1,a1,0xd
    800034a4:	0001c797          	auipc	a5,0x1c
    800034a8:	5e07a783          	lw	a5,1504(a5) # 8001fa84 <sb+0x1c>
    800034ac:	9dbd                	addw	a1,a1,a5
    800034ae:	00000097          	auipc	ra,0x0
    800034b2:	d9e080e7          	jalr	-610(ra) # 8000324c <bread>
  bi = b % BPB;
  m = 1 << (bi % 8);
    800034b6:	0074f713          	andi	a4,s1,7
    800034ba:	4785                	li	a5,1
    800034bc:	00e797bb          	sllw	a5,a5,a4
  if((bp->data[bi/8] & m) == 0)
    800034c0:	14ce                	slli	s1,s1,0x33
    800034c2:	90d9                	srli	s1,s1,0x36
    800034c4:	00950733          	add	a4,a0,s1
    800034c8:	05874703          	lbu	a4,88(a4)
    800034cc:	00e7f6b3          	and	a3,a5,a4
    800034d0:	c69d                	beqz	a3,800034fe <bfree+0x6c>
    800034d2:	892a                	mv	s2,a0
    panic("freeing free block");
  bp->data[bi/8] &= ~m;
    800034d4:	94aa                	add	s1,s1,a0
    800034d6:	fff7c793          	not	a5,a5
    800034da:	8ff9                	and	a5,a5,a4
    800034dc:	04f48c23          	sb	a5,88(s1)
  log_write(bp);
    800034e0:	00001097          	auipc	ra,0x1
    800034e4:	118080e7          	jalr	280(ra) # 800045f8 <log_write>
  brelse(bp);
    800034e8:	854a                	mv	a0,s2
    800034ea:	00000097          	auipc	ra,0x0
    800034ee:	e92080e7          	jalr	-366(ra) # 8000337c <brelse>
}
    800034f2:	60e2                	ld	ra,24(sp)
    800034f4:	6442                	ld	s0,16(sp)
    800034f6:	64a2                	ld	s1,8(sp)
    800034f8:	6902                	ld	s2,0(sp)
    800034fa:	6105                	addi	sp,sp,32
    800034fc:	8082                	ret
    panic("freeing free block");
    800034fe:	00005517          	auipc	a0,0x5
    80003502:	09a50513          	addi	a0,a0,154 # 80008598 <syscalls+0xf0>
    80003506:	ffffd097          	auipc	ra,0xffffd
    8000350a:	038080e7          	jalr	56(ra) # 8000053e <panic>

000000008000350e <balloc>:
{
    8000350e:	711d                	addi	sp,sp,-96
    80003510:	ec86                	sd	ra,88(sp)
    80003512:	e8a2                	sd	s0,80(sp)
    80003514:	e4a6                	sd	s1,72(sp)
    80003516:	e0ca                	sd	s2,64(sp)
    80003518:	fc4e                	sd	s3,56(sp)
    8000351a:	f852                	sd	s4,48(sp)
    8000351c:	f456                	sd	s5,40(sp)
    8000351e:	f05a                	sd	s6,32(sp)
    80003520:	ec5e                	sd	s7,24(sp)
    80003522:	e862                	sd	s8,16(sp)
    80003524:	e466                	sd	s9,8(sp)
    80003526:	1080                	addi	s0,sp,96
  for(b = 0; b < sb.size; b += BPB){
    80003528:	0001c797          	auipc	a5,0x1c
    8000352c:	5447a783          	lw	a5,1348(a5) # 8001fa6c <sb+0x4>
    80003530:	cbd1                	beqz	a5,800035c4 <balloc+0xb6>
    80003532:	8baa                	mv	s7,a0
    80003534:	4a81                	li	s5,0
    bp = bread(dev, BBLOCK(b, sb));
    80003536:	0001cb17          	auipc	s6,0x1c
    8000353a:	532b0b13          	addi	s6,s6,1330 # 8001fa68 <sb>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    8000353e:	4c01                	li	s8,0
      m = 1 << (bi % 8);
    80003540:	4985                	li	s3,1
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80003542:	6a09                	lui	s4,0x2
  for(b = 0; b < sb.size; b += BPB){
    80003544:	6c89                	lui	s9,0x2
    80003546:	a831                	j	80003562 <balloc+0x54>
    brelse(bp);
    80003548:	854a                	mv	a0,s2
    8000354a:	00000097          	auipc	ra,0x0
    8000354e:	e32080e7          	jalr	-462(ra) # 8000337c <brelse>
  for(b = 0; b < sb.size; b += BPB){
    80003552:	015c87bb          	addw	a5,s9,s5
    80003556:	00078a9b          	sext.w	s5,a5
    8000355a:	004b2703          	lw	a4,4(s6)
    8000355e:	06eaf363          	bgeu	s5,a4,800035c4 <balloc+0xb6>
    bp = bread(dev, BBLOCK(b, sb));
    80003562:	41fad79b          	sraiw	a5,s5,0x1f
    80003566:	0137d79b          	srliw	a5,a5,0x13
    8000356a:	015787bb          	addw	a5,a5,s5
    8000356e:	40d7d79b          	sraiw	a5,a5,0xd
    80003572:	01cb2583          	lw	a1,28(s6)
    80003576:	9dbd                	addw	a1,a1,a5
    80003578:	855e                	mv	a0,s7
    8000357a:	00000097          	auipc	ra,0x0
    8000357e:	cd2080e7          	jalr	-814(ra) # 8000324c <bread>
    80003582:	892a                	mv	s2,a0
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80003584:	004b2503          	lw	a0,4(s6)
    80003588:	000a849b          	sext.w	s1,s5
    8000358c:	8662                	mv	a2,s8
    8000358e:	faa4fde3          	bgeu	s1,a0,80003548 <balloc+0x3a>
      m = 1 << (bi % 8);
    80003592:	41f6579b          	sraiw	a5,a2,0x1f
    80003596:	01d7d69b          	srliw	a3,a5,0x1d
    8000359a:	00c6873b          	addw	a4,a3,a2
    8000359e:	00777793          	andi	a5,a4,7
    800035a2:	9f95                	subw	a5,a5,a3
    800035a4:	00f997bb          	sllw	a5,s3,a5
      if((bp->data[bi/8] & m) == 0){  // Is block free?
    800035a8:	4037571b          	sraiw	a4,a4,0x3
    800035ac:	00e906b3          	add	a3,s2,a4
    800035b0:	0586c683          	lbu	a3,88(a3)
    800035b4:	00d7f5b3          	and	a1,a5,a3
    800035b8:	cd91                	beqz	a1,800035d4 <balloc+0xc6>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    800035ba:	2605                	addiw	a2,a2,1
    800035bc:	2485                	addiw	s1,s1,1
    800035be:	fd4618e3          	bne	a2,s4,8000358e <balloc+0x80>
    800035c2:	b759                	j	80003548 <balloc+0x3a>
  panic("balloc: out of blocks");
    800035c4:	00005517          	auipc	a0,0x5
    800035c8:	fec50513          	addi	a0,a0,-20 # 800085b0 <syscalls+0x108>
    800035cc:	ffffd097          	auipc	ra,0xffffd
    800035d0:	f72080e7          	jalr	-142(ra) # 8000053e <panic>
        bp->data[bi/8] |= m;  // Mark block in use.
    800035d4:	974a                	add	a4,a4,s2
    800035d6:	8fd5                	or	a5,a5,a3
    800035d8:	04f70c23          	sb	a5,88(a4)
        log_write(bp);
    800035dc:	854a                	mv	a0,s2
    800035de:	00001097          	auipc	ra,0x1
    800035e2:	01a080e7          	jalr	26(ra) # 800045f8 <log_write>
        brelse(bp);
    800035e6:	854a                	mv	a0,s2
    800035e8:	00000097          	auipc	ra,0x0
    800035ec:	d94080e7          	jalr	-620(ra) # 8000337c <brelse>
  bp = bread(dev, bno);
    800035f0:	85a6                	mv	a1,s1
    800035f2:	855e                	mv	a0,s7
    800035f4:	00000097          	auipc	ra,0x0
    800035f8:	c58080e7          	jalr	-936(ra) # 8000324c <bread>
    800035fc:	892a                	mv	s2,a0
  memset(bp->data, 0, BSIZE);
    800035fe:	40000613          	li	a2,1024
    80003602:	4581                	li	a1,0
    80003604:	05850513          	addi	a0,a0,88
    80003608:	ffffd097          	auipc	ra,0xffffd
    8000360c:	6d8080e7          	jalr	1752(ra) # 80000ce0 <memset>
  log_write(bp);
    80003610:	854a                	mv	a0,s2
    80003612:	00001097          	auipc	ra,0x1
    80003616:	fe6080e7          	jalr	-26(ra) # 800045f8 <log_write>
  brelse(bp);
    8000361a:	854a                	mv	a0,s2
    8000361c:	00000097          	auipc	ra,0x0
    80003620:	d60080e7          	jalr	-672(ra) # 8000337c <brelse>
}
    80003624:	8526                	mv	a0,s1
    80003626:	60e6                	ld	ra,88(sp)
    80003628:	6446                	ld	s0,80(sp)
    8000362a:	64a6                	ld	s1,72(sp)
    8000362c:	6906                	ld	s2,64(sp)
    8000362e:	79e2                	ld	s3,56(sp)
    80003630:	7a42                	ld	s4,48(sp)
    80003632:	7aa2                	ld	s5,40(sp)
    80003634:	7b02                	ld	s6,32(sp)
    80003636:	6be2                	ld	s7,24(sp)
    80003638:	6c42                	ld	s8,16(sp)
    8000363a:	6ca2                	ld	s9,8(sp)
    8000363c:	6125                	addi	sp,sp,96
    8000363e:	8082                	ret

0000000080003640 <bmap>:

// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
static uint
bmap(struct inode *ip, uint bn)
{
    80003640:	7179                	addi	sp,sp,-48
    80003642:	f406                	sd	ra,40(sp)
    80003644:	f022                	sd	s0,32(sp)
    80003646:	ec26                	sd	s1,24(sp)
    80003648:	e84a                	sd	s2,16(sp)
    8000364a:	e44e                	sd	s3,8(sp)
    8000364c:	e052                	sd	s4,0(sp)
    8000364e:	1800                	addi	s0,sp,48
    80003650:	892a                	mv	s2,a0
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
    80003652:	47ad                	li	a5,11
    80003654:	04b7fe63          	bgeu	a5,a1,800036b0 <bmap+0x70>
    if((addr = ip->addrs[bn]) == 0)
      ip->addrs[bn] = addr = balloc(ip->dev);
    return addr;
  }
  bn -= NDIRECT;
    80003658:	ff45849b          	addiw	s1,a1,-12
    8000365c:	0004871b          	sext.w	a4,s1

  if(bn < NINDIRECT){
    80003660:	0ff00793          	li	a5,255
    80003664:	0ae7e363          	bltu	a5,a4,8000370a <bmap+0xca>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0)
    80003668:	08052583          	lw	a1,128(a0)
    8000366c:	c5ad                	beqz	a1,800036d6 <bmap+0x96>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
    bp = bread(ip->dev, addr);
    8000366e:	00092503          	lw	a0,0(s2)
    80003672:	00000097          	auipc	ra,0x0
    80003676:	bda080e7          	jalr	-1062(ra) # 8000324c <bread>
    8000367a:	8a2a                	mv	s4,a0
    a = (uint*)bp->data;
    8000367c:	05850793          	addi	a5,a0,88
    if((addr = a[bn]) == 0){
    80003680:	02049593          	slli	a1,s1,0x20
    80003684:	9181                	srli	a1,a1,0x20
    80003686:	058a                	slli	a1,a1,0x2
    80003688:	00b784b3          	add	s1,a5,a1
    8000368c:	0004a983          	lw	s3,0(s1)
    80003690:	04098d63          	beqz	s3,800036ea <bmap+0xaa>
      a[bn] = addr = balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);
    80003694:	8552                	mv	a0,s4
    80003696:	00000097          	auipc	ra,0x0
    8000369a:	ce6080e7          	jalr	-794(ra) # 8000337c <brelse>
    return addr;
  }

  panic("bmap: out of range");
}
    8000369e:	854e                	mv	a0,s3
    800036a0:	70a2                	ld	ra,40(sp)
    800036a2:	7402                	ld	s0,32(sp)
    800036a4:	64e2                	ld	s1,24(sp)
    800036a6:	6942                	ld	s2,16(sp)
    800036a8:	69a2                	ld	s3,8(sp)
    800036aa:	6a02                	ld	s4,0(sp)
    800036ac:	6145                	addi	sp,sp,48
    800036ae:	8082                	ret
    if((addr = ip->addrs[bn]) == 0)
    800036b0:	02059493          	slli	s1,a1,0x20
    800036b4:	9081                	srli	s1,s1,0x20
    800036b6:	048a                	slli	s1,s1,0x2
    800036b8:	94aa                	add	s1,s1,a0
    800036ba:	0504a983          	lw	s3,80(s1)
    800036be:	fe0990e3          	bnez	s3,8000369e <bmap+0x5e>
      ip->addrs[bn] = addr = balloc(ip->dev);
    800036c2:	4108                	lw	a0,0(a0)
    800036c4:	00000097          	auipc	ra,0x0
    800036c8:	e4a080e7          	jalr	-438(ra) # 8000350e <balloc>
    800036cc:	0005099b          	sext.w	s3,a0
    800036d0:	0534a823          	sw	s3,80(s1)
    800036d4:	b7e9                	j	8000369e <bmap+0x5e>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
    800036d6:	4108                	lw	a0,0(a0)
    800036d8:	00000097          	auipc	ra,0x0
    800036dc:	e36080e7          	jalr	-458(ra) # 8000350e <balloc>
    800036e0:	0005059b          	sext.w	a1,a0
    800036e4:	08b92023          	sw	a1,128(s2)
    800036e8:	b759                	j	8000366e <bmap+0x2e>
      a[bn] = addr = balloc(ip->dev);
    800036ea:	00092503          	lw	a0,0(s2)
    800036ee:	00000097          	auipc	ra,0x0
    800036f2:	e20080e7          	jalr	-480(ra) # 8000350e <balloc>
    800036f6:	0005099b          	sext.w	s3,a0
    800036fa:	0134a023          	sw	s3,0(s1)
      log_write(bp);
    800036fe:	8552                	mv	a0,s4
    80003700:	00001097          	auipc	ra,0x1
    80003704:	ef8080e7          	jalr	-264(ra) # 800045f8 <log_write>
    80003708:	b771                	j	80003694 <bmap+0x54>
  panic("bmap: out of range");
    8000370a:	00005517          	auipc	a0,0x5
    8000370e:	ebe50513          	addi	a0,a0,-322 # 800085c8 <syscalls+0x120>
    80003712:	ffffd097          	auipc	ra,0xffffd
    80003716:	e2c080e7          	jalr	-468(ra) # 8000053e <panic>

000000008000371a <iget>:
{
    8000371a:	7179                	addi	sp,sp,-48
    8000371c:	f406                	sd	ra,40(sp)
    8000371e:	f022                	sd	s0,32(sp)
    80003720:	ec26                	sd	s1,24(sp)
    80003722:	e84a                	sd	s2,16(sp)
    80003724:	e44e                	sd	s3,8(sp)
    80003726:	e052                	sd	s4,0(sp)
    80003728:	1800                	addi	s0,sp,48
    8000372a:	89aa                	mv	s3,a0
    8000372c:	8a2e                	mv	s4,a1
  acquire(&itable.lock);
    8000372e:	0001c517          	auipc	a0,0x1c
    80003732:	35a50513          	addi	a0,a0,858 # 8001fa88 <itable>
    80003736:	ffffd097          	auipc	ra,0xffffd
    8000373a:	4ae080e7          	jalr	1198(ra) # 80000be4 <acquire>
  empty = 0;
    8000373e:	4901                	li	s2,0
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    80003740:	0001c497          	auipc	s1,0x1c
    80003744:	36048493          	addi	s1,s1,864 # 8001faa0 <itable+0x18>
    80003748:	0001e697          	auipc	a3,0x1e
    8000374c:	de868693          	addi	a3,a3,-536 # 80021530 <log>
    80003750:	a039                	j	8000375e <iget+0x44>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    80003752:	02090b63          	beqz	s2,80003788 <iget+0x6e>
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    80003756:	08848493          	addi	s1,s1,136
    8000375a:	02d48a63          	beq	s1,a3,8000378e <iget+0x74>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
    8000375e:	449c                	lw	a5,8(s1)
    80003760:	fef059e3          	blez	a5,80003752 <iget+0x38>
    80003764:	4098                	lw	a4,0(s1)
    80003766:	ff3716e3          	bne	a4,s3,80003752 <iget+0x38>
    8000376a:	40d8                	lw	a4,4(s1)
    8000376c:	ff4713e3          	bne	a4,s4,80003752 <iget+0x38>
      ip->ref++;
    80003770:	2785                	addiw	a5,a5,1
    80003772:	c49c                	sw	a5,8(s1)
      release(&itable.lock);
    80003774:	0001c517          	auipc	a0,0x1c
    80003778:	31450513          	addi	a0,a0,788 # 8001fa88 <itable>
    8000377c:	ffffd097          	auipc	ra,0xffffd
    80003780:	51c080e7          	jalr	1308(ra) # 80000c98 <release>
      return ip;
    80003784:	8926                	mv	s2,s1
    80003786:	a03d                	j	800037b4 <iget+0x9a>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    80003788:	f7f9                	bnez	a5,80003756 <iget+0x3c>
    8000378a:	8926                	mv	s2,s1
    8000378c:	b7e9                	j	80003756 <iget+0x3c>
  if(empty == 0)
    8000378e:	02090c63          	beqz	s2,800037c6 <iget+0xac>
  ip->dev = dev;
    80003792:	01392023          	sw	s3,0(s2)
  ip->inum = inum;
    80003796:	01492223          	sw	s4,4(s2)
  ip->ref = 1;
    8000379a:	4785                	li	a5,1
    8000379c:	00f92423          	sw	a5,8(s2)
  ip->valid = 0;
    800037a0:	04092023          	sw	zero,64(s2)
  release(&itable.lock);
    800037a4:	0001c517          	auipc	a0,0x1c
    800037a8:	2e450513          	addi	a0,a0,740 # 8001fa88 <itable>
    800037ac:	ffffd097          	auipc	ra,0xffffd
    800037b0:	4ec080e7          	jalr	1260(ra) # 80000c98 <release>
}
    800037b4:	854a                	mv	a0,s2
    800037b6:	70a2                	ld	ra,40(sp)
    800037b8:	7402                	ld	s0,32(sp)
    800037ba:	64e2                	ld	s1,24(sp)
    800037bc:	6942                	ld	s2,16(sp)
    800037be:	69a2                	ld	s3,8(sp)
    800037c0:	6a02                	ld	s4,0(sp)
    800037c2:	6145                	addi	sp,sp,48
    800037c4:	8082                	ret
    panic("iget: no inodes");
    800037c6:	00005517          	auipc	a0,0x5
    800037ca:	e1a50513          	addi	a0,a0,-486 # 800085e0 <syscalls+0x138>
    800037ce:	ffffd097          	auipc	ra,0xffffd
    800037d2:	d70080e7          	jalr	-656(ra) # 8000053e <panic>

00000000800037d6 <fsinit>:
fsinit(int dev) {
    800037d6:	7179                	addi	sp,sp,-48
    800037d8:	f406                	sd	ra,40(sp)
    800037da:	f022                	sd	s0,32(sp)
    800037dc:	ec26                	sd	s1,24(sp)
    800037de:	e84a                	sd	s2,16(sp)
    800037e0:	e44e                	sd	s3,8(sp)
    800037e2:	1800                	addi	s0,sp,48
    800037e4:	892a                	mv	s2,a0
  bp = bread(dev, 1);
    800037e6:	4585                	li	a1,1
    800037e8:	00000097          	auipc	ra,0x0
    800037ec:	a64080e7          	jalr	-1436(ra) # 8000324c <bread>
    800037f0:	84aa                	mv	s1,a0
  memmove(sb, bp->data, sizeof(*sb));
    800037f2:	0001c997          	auipc	s3,0x1c
    800037f6:	27698993          	addi	s3,s3,630 # 8001fa68 <sb>
    800037fa:	02000613          	li	a2,32
    800037fe:	05850593          	addi	a1,a0,88
    80003802:	854e                	mv	a0,s3
    80003804:	ffffd097          	auipc	ra,0xffffd
    80003808:	53c080e7          	jalr	1340(ra) # 80000d40 <memmove>
  brelse(bp);
    8000380c:	8526                	mv	a0,s1
    8000380e:	00000097          	auipc	ra,0x0
    80003812:	b6e080e7          	jalr	-1170(ra) # 8000337c <brelse>
  if(sb.magic != FSMAGIC)
    80003816:	0009a703          	lw	a4,0(s3)
    8000381a:	102037b7          	lui	a5,0x10203
    8000381e:	04078793          	addi	a5,a5,64 # 10203040 <_entry-0x6fdfcfc0>
    80003822:	02f71263          	bne	a4,a5,80003846 <fsinit+0x70>
  initlog(dev, &sb);
    80003826:	0001c597          	auipc	a1,0x1c
    8000382a:	24258593          	addi	a1,a1,578 # 8001fa68 <sb>
    8000382e:	854a                	mv	a0,s2
    80003830:	00001097          	auipc	ra,0x1
    80003834:	b4c080e7          	jalr	-1204(ra) # 8000437c <initlog>
}
    80003838:	70a2                	ld	ra,40(sp)
    8000383a:	7402                	ld	s0,32(sp)
    8000383c:	64e2                	ld	s1,24(sp)
    8000383e:	6942                	ld	s2,16(sp)
    80003840:	69a2                	ld	s3,8(sp)
    80003842:	6145                	addi	sp,sp,48
    80003844:	8082                	ret
    panic("invalid file system");
    80003846:	00005517          	auipc	a0,0x5
    8000384a:	daa50513          	addi	a0,a0,-598 # 800085f0 <syscalls+0x148>
    8000384e:	ffffd097          	auipc	ra,0xffffd
    80003852:	cf0080e7          	jalr	-784(ra) # 8000053e <panic>

0000000080003856 <iinit>:
{
    80003856:	7179                	addi	sp,sp,-48
    80003858:	f406                	sd	ra,40(sp)
    8000385a:	f022                	sd	s0,32(sp)
    8000385c:	ec26                	sd	s1,24(sp)
    8000385e:	e84a                	sd	s2,16(sp)
    80003860:	e44e                	sd	s3,8(sp)
    80003862:	1800                	addi	s0,sp,48
  initlock(&itable.lock, "itable");
    80003864:	00005597          	auipc	a1,0x5
    80003868:	da458593          	addi	a1,a1,-604 # 80008608 <syscalls+0x160>
    8000386c:	0001c517          	auipc	a0,0x1c
    80003870:	21c50513          	addi	a0,a0,540 # 8001fa88 <itable>
    80003874:	ffffd097          	auipc	ra,0xffffd
    80003878:	2e0080e7          	jalr	736(ra) # 80000b54 <initlock>
  for(i = 0; i < NINODE; i++) {
    8000387c:	0001c497          	auipc	s1,0x1c
    80003880:	23448493          	addi	s1,s1,564 # 8001fab0 <itable+0x28>
    80003884:	0001e997          	auipc	s3,0x1e
    80003888:	cbc98993          	addi	s3,s3,-836 # 80021540 <log+0x10>
    initsleeplock(&itable.inode[i].lock, "inode");
    8000388c:	00005917          	auipc	s2,0x5
    80003890:	d8490913          	addi	s2,s2,-636 # 80008610 <syscalls+0x168>
    80003894:	85ca                	mv	a1,s2
    80003896:	8526                	mv	a0,s1
    80003898:	00001097          	auipc	ra,0x1
    8000389c:	e46080e7          	jalr	-442(ra) # 800046de <initsleeplock>
  for(i = 0; i < NINODE; i++) {
    800038a0:	08848493          	addi	s1,s1,136
    800038a4:	ff3498e3          	bne	s1,s3,80003894 <iinit+0x3e>
}
    800038a8:	70a2                	ld	ra,40(sp)
    800038aa:	7402                	ld	s0,32(sp)
    800038ac:	64e2                	ld	s1,24(sp)
    800038ae:	6942                	ld	s2,16(sp)
    800038b0:	69a2                	ld	s3,8(sp)
    800038b2:	6145                	addi	sp,sp,48
    800038b4:	8082                	ret

00000000800038b6 <ialloc>:
{
    800038b6:	715d                	addi	sp,sp,-80
    800038b8:	e486                	sd	ra,72(sp)
    800038ba:	e0a2                	sd	s0,64(sp)
    800038bc:	fc26                	sd	s1,56(sp)
    800038be:	f84a                	sd	s2,48(sp)
    800038c0:	f44e                	sd	s3,40(sp)
    800038c2:	f052                	sd	s4,32(sp)
    800038c4:	ec56                	sd	s5,24(sp)
    800038c6:	e85a                	sd	s6,16(sp)
    800038c8:	e45e                	sd	s7,8(sp)
    800038ca:	0880                	addi	s0,sp,80
  for(inum = 1; inum < sb.ninodes; inum++){
    800038cc:	0001c717          	auipc	a4,0x1c
    800038d0:	1a872703          	lw	a4,424(a4) # 8001fa74 <sb+0xc>
    800038d4:	4785                	li	a5,1
    800038d6:	04e7fa63          	bgeu	a5,a4,8000392a <ialloc+0x74>
    800038da:	8aaa                	mv	s5,a0
    800038dc:	8bae                	mv	s7,a1
    800038de:	4485                	li	s1,1
    bp = bread(dev, IBLOCK(inum, sb));
    800038e0:	0001ca17          	auipc	s4,0x1c
    800038e4:	188a0a13          	addi	s4,s4,392 # 8001fa68 <sb>
    800038e8:	00048b1b          	sext.w	s6,s1
    800038ec:	0044d593          	srli	a1,s1,0x4
    800038f0:	018a2783          	lw	a5,24(s4)
    800038f4:	9dbd                	addw	a1,a1,a5
    800038f6:	8556                	mv	a0,s5
    800038f8:	00000097          	auipc	ra,0x0
    800038fc:	954080e7          	jalr	-1708(ra) # 8000324c <bread>
    80003900:	892a                	mv	s2,a0
    dip = (struct dinode*)bp->data + inum%IPB;
    80003902:	05850993          	addi	s3,a0,88
    80003906:	00f4f793          	andi	a5,s1,15
    8000390a:	079a                	slli	a5,a5,0x6
    8000390c:	99be                	add	s3,s3,a5
    if(dip->type == 0){  // a free inode
    8000390e:	00099783          	lh	a5,0(s3)
    80003912:	c785                	beqz	a5,8000393a <ialloc+0x84>
    brelse(bp);
    80003914:	00000097          	auipc	ra,0x0
    80003918:	a68080e7          	jalr	-1432(ra) # 8000337c <brelse>
  for(inum = 1; inum < sb.ninodes; inum++){
    8000391c:	0485                	addi	s1,s1,1
    8000391e:	00ca2703          	lw	a4,12(s4)
    80003922:	0004879b          	sext.w	a5,s1
    80003926:	fce7e1e3          	bltu	a5,a4,800038e8 <ialloc+0x32>
  panic("ialloc: no inodes");
    8000392a:	00005517          	auipc	a0,0x5
    8000392e:	cee50513          	addi	a0,a0,-786 # 80008618 <syscalls+0x170>
    80003932:	ffffd097          	auipc	ra,0xffffd
    80003936:	c0c080e7          	jalr	-1012(ra) # 8000053e <panic>
      memset(dip, 0, sizeof(*dip));
    8000393a:	04000613          	li	a2,64
    8000393e:	4581                	li	a1,0
    80003940:	854e                	mv	a0,s3
    80003942:	ffffd097          	auipc	ra,0xffffd
    80003946:	39e080e7          	jalr	926(ra) # 80000ce0 <memset>
      dip->type = type;
    8000394a:	01799023          	sh	s7,0(s3)
      log_write(bp);   // mark it allocated on the disk
    8000394e:	854a                	mv	a0,s2
    80003950:	00001097          	auipc	ra,0x1
    80003954:	ca8080e7          	jalr	-856(ra) # 800045f8 <log_write>
      brelse(bp);
    80003958:	854a                	mv	a0,s2
    8000395a:	00000097          	auipc	ra,0x0
    8000395e:	a22080e7          	jalr	-1502(ra) # 8000337c <brelse>
      return iget(dev, inum);
    80003962:	85da                	mv	a1,s6
    80003964:	8556                	mv	a0,s5
    80003966:	00000097          	auipc	ra,0x0
    8000396a:	db4080e7          	jalr	-588(ra) # 8000371a <iget>
}
    8000396e:	60a6                	ld	ra,72(sp)
    80003970:	6406                	ld	s0,64(sp)
    80003972:	74e2                	ld	s1,56(sp)
    80003974:	7942                	ld	s2,48(sp)
    80003976:	79a2                	ld	s3,40(sp)
    80003978:	7a02                	ld	s4,32(sp)
    8000397a:	6ae2                	ld	s5,24(sp)
    8000397c:	6b42                	ld	s6,16(sp)
    8000397e:	6ba2                	ld	s7,8(sp)
    80003980:	6161                	addi	sp,sp,80
    80003982:	8082                	ret

0000000080003984 <iupdate>:
{
    80003984:	1101                	addi	sp,sp,-32
    80003986:	ec06                	sd	ra,24(sp)
    80003988:	e822                	sd	s0,16(sp)
    8000398a:	e426                	sd	s1,8(sp)
    8000398c:	e04a                	sd	s2,0(sp)
    8000398e:	1000                	addi	s0,sp,32
    80003990:	84aa                	mv	s1,a0
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    80003992:	415c                	lw	a5,4(a0)
    80003994:	0047d79b          	srliw	a5,a5,0x4
    80003998:	0001c597          	auipc	a1,0x1c
    8000399c:	0e85a583          	lw	a1,232(a1) # 8001fa80 <sb+0x18>
    800039a0:	9dbd                	addw	a1,a1,a5
    800039a2:	4108                	lw	a0,0(a0)
    800039a4:	00000097          	auipc	ra,0x0
    800039a8:	8a8080e7          	jalr	-1880(ra) # 8000324c <bread>
    800039ac:	892a                	mv	s2,a0
  dip = (struct dinode*)bp->data + ip->inum%IPB;
    800039ae:	05850793          	addi	a5,a0,88
    800039b2:	40c8                	lw	a0,4(s1)
    800039b4:	893d                	andi	a0,a0,15
    800039b6:	051a                	slli	a0,a0,0x6
    800039b8:	953e                	add	a0,a0,a5
  dip->type = ip->type;
    800039ba:	04449703          	lh	a4,68(s1)
    800039be:	00e51023          	sh	a4,0(a0)
  dip->major = ip->major;
    800039c2:	04649703          	lh	a4,70(s1)
    800039c6:	00e51123          	sh	a4,2(a0)
  dip->minor = ip->minor;
    800039ca:	04849703          	lh	a4,72(s1)
    800039ce:	00e51223          	sh	a4,4(a0)
  dip->nlink = ip->nlink;
    800039d2:	04a49703          	lh	a4,74(s1)
    800039d6:	00e51323          	sh	a4,6(a0)
  dip->size = ip->size;
    800039da:	44f8                	lw	a4,76(s1)
    800039dc:	c518                	sw	a4,8(a0)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
    800039de:	03400613          	li	a2,52
    800039e2:	05048593          	addi	a1,s1,80
    800039e6:	0531                	addi	a0,a0,12
    800039e8:	ffffd097          	auipc	ra,0xffffd
    800039ec:	358080e7          	jalr	856(ra) # 80000d40 <memmove>
  log_write(bp);
    800039f0:	854a                	mv	a0,s2
    800039f2:	00001097          	auipc	ra,0x1
    800039f6:	c06080e7          	jalr	-1018(ra) # 800045f8 <log_write>
  brelse(bp);
    800039fa:	854a                	mv	a0,s2
    800039fc:	00000097          	auipc	ra,0x0
    80003a00:	980080e7          	jalr	-1664(ra) # 8000337c <brelse>
}
    80003a04:	60e2                	ld	ra,24(sp)
    80003a06:	6442                	ld	s0,16(sp)
    80003a08:	64a2                	ld	s1,8(sp)
    80003a0a:	6902                	ld	s2,0(sp)
    80003a0c:	6105                	addi	sp,sp,32
    80003a0e:	8082                	ret

0000000080003a10 <idup>:
{
    80003a10:	1101                	addi	sp,sp,-32
    80003a12:	ec06                	sd	ra,24(sp)
    80003a14:	e822                	sd	s0,16(sp)
    80003a16:	e426                	sd	s1,8(sp)
    80003a18:	1000                	addi	s0,sp,32
    80003a1a:	84aa                	mv	s1,a0
  acquire(&itable.lock);
    80003a1c:	0001c517          	auipc	a0,0x1c
    80003a20:	06c50513          	addi	a0,a0,108 # 8001fa88 <itable>
    80003a24:	ffffd097          	auipc	ra,0xffffd
    80003a28:	1c0080e7          	jalr	448(ra) # 80000be4 <acquire>
  ip->ref++;
    80003a2c:	449c                	lw	a5,8(s1)
    80003a2e:	2785                	addiw	a5,a5,1
    80003a30:	c49c                	sw	a5,8(s1)
  release(&itable.lock);
    80003a32:	0001c517          	auipc	a0,0x1c
    80003a36:	05650513          	addi	a0,a0,86 # 8001fa88 <itable>
    80003a3a:	ffffd097          	auipc	ra,0xffffd
    80003a3e:	25e080e7          	jalr	606(ra) # 80000c98 <release>
}
    80003a42:	8526                	mv	a0,s1
    80003a44:	60e2                	ld	ra,24(sp)
    80003a46:	6442                	ld	s0,16(sp)
    80003a48:	64a2                	ld	s1,8(sp)
    80003a4a:	6105                	addi	sp,sp,32
    80003a4c:	8082                	ret

0000000080003a4e <ilock>:
{
    80003a4e:	1101                	addi	sp,sp,-32
    80003a50:	ec06                	sd	ra,24(sp)
    80003a52:	e822                	sd	s0,16(sp)
    80003a54:	e426                	sd	s1,8(sp)
    80003a56:	e04a                	sd	s2,0(sp)
    80003a58:	1000                	addi	s0,sp,32
  if(ip == 0 || ip->ref < 1)
    80003a5a:	c115                	beqz	a0,80003a7e <ilock+0x30>
    80003a5c:	84aa                	mv	s1,a0
    80003a5e:	451c                	lw	a5,8(a0)
    80003a60:	00f05f63          	blez	a5,80003a7e <ilock+0x30>
  acquiresleep(&ip->lock);
    80003a64:	0541                	addi	a0,a0,16
    80003a66:	00001097          	auipc	ra,0x1
    80003a6a:	cb2080e7          	jalr	-846(ra) # 80004718 <acquiresleep>
  if(ip->valid == 0){
    80003a6e:	40bc                	lw	a5,64(s1)
    80003a70:	cf99                	beqz	a5,80003a8e <ilock+0x40>
}
    80003a72:	60e2                	ld	ra,24(sp)
    80003a74:	6442                	ld	s0,16(sp)
    80003a76:	64a2                	ld	s1,8(sp)
    80003a78:	6902                	ld	s2,0(sp)
    80003a7a:	6105                	addi	sp,sp,32
    80003a7c:	8082                	ret
    panic("ilock");
    80003a7e:	00005517          	auipc	a0,0x5
    80003a82:	bb250513          	addi	a0,a0,-1102 # 80008630 <syscalls+0x188>
    80003a86:	ffffd097          	auipc	ra,0xffffd
    80003a8a:	ab8080e7          	jalr	-1352(ra) # 8000053e <panic>
    bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    80003a8e:	40dc                	lw	a5,4(s1)
    80003a90:	0047d79b          	srliw	a5,a5,0x4
    80003a94:	0001c597          	auipc	a1,0x1c
    80003a98:	fec5a583          	lw	a1,-20(a1) # 8001fa80 <sb+0x18>
    80003a9c:	9dbd                	addw	a1,a1,a5
    80003a9e:	4088                	lw	a0,0(s1)
    80003aa0:	fffff097          	auipc	ra,0xfffff
    80003aa4:	7ac080e7          	jalr	1964(ra) # 8000324c <bread>
    80003aa8:	892a                	mv	s2,a0
    dip = (struct dinode*)bp->data + ip->inum%IPB;
    80003aaa:	05850593          	addi	a1,a0,88
    80003aae:	40dc                	lw	a5,4(s1)
    80003ab0:	8bbd                	andi	a5,a5,15
    80003ab2:	079a                	slli	a5,a5,0x6
    80003ab4:	95be                	add	a1,a1,a5
    ip->type = dip->type;
    80003ab6:	00059783          	lh	a5,0(a1)
    80003aba:	04f49223          	sh	a5,68(s1)
    ip->major = dip->major;
    80003abe:	00259783          	lh	a5,2(a1)
    80003ac2:	04f49323          	sh	a5,70(s1)
    ip->minor = dip->minor;
    80003ac6:	00459783          	lh	a5,4(a1)
    80003aca:	04f49423          	sh	a5,72(s1)
    ip->nlink = dip->nlink;
    80003ace:	00659783          	lh	a5,6(a1)
    80003ad2:	04f49523          	sh	a5,74(s1)
    ip->size = dip->size;
    80003ad6:	459c                	lw	a5,8(a1)
    80003ad8:	c4fc                	sw	a5,76(s1)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
    80003ada:	03400613          	li	a2,52
    80003ade:	05b1                	addi	a1,a1,12
    80003ae0:	05048513          	addi	a0,s1,80
    80003ae4:	ffffd097          	auipc	ra,0xffffd
    80003ae8:	25c080e7          	jalr	604(ra) # 80000d40 <memmove>
    brelse(bp);
    80003aec:	854a                	mv	a0,s2
    80003aee:	00000097          	auipc	ra,0x0
    80003af2:	88e080e7          	jalr	-1906(ra) # 8000337c <brelse>
    ip->valid = 1;
    80003af6:	4785                	li	a5,1
    80003af8:	c0bc                	sw	a5,64(s1)
    if(ip->type == 0)
    80003afa:	04449783          	lh	a5,68(s1)
    80003afe:	fbb5                	bnez	a5,80003a72 <ilock+0x24>
      panic("ilock: no type");
    80003b00:	00005517          	auipc	a0,0x5
    80003b04:	b3850513          	addi	a0,a0,-1224 # 80008638 <syscalls+0x190>
    80003b08:	ffffd097          	auipc	ra,0xffffd
    80003b0c:	a36080e7          	jalr	-1482(ra) # 8000053e <panic>

0000000080003b10 <iunlock>:
{
    80003b10:	1101                	addi	sp,sp,-32
    80003b12:	ec06                	sd	ra,24(sp)
    80003b14:	e822                	sd	s0,16(sp)
    80003b16:	e426                	sd	s1,8(sp)
    80003b18:	e04a                	sd	s2,0(sp)
    80003b1a:	1000                	addi	s0,sp,32
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
    80003b1c:	c905                	beqz	a0,80003b4c <iunlock+0x3c>
    80003b1e:	84aa                	mv	s1,a0
    80003b20:	01050913          	addi	s2,a0,16
    80003b24:	854a                	mv	a0,s2
    80003b26:	00001097          	auipc	ra,0x1
    80003b2a:	c8c080e7          	jalr	-884(ra) # 800047b2 <holdingsleep>
    80003b2e:	cd19                	beqz	a0,80003b4c <iunlock+0x3c>
    80003b30:	449c                	lw	a5,8(s1)
    80003b32:	00f05d63          	blez	a5,80003b4c <iunlock+0x3c>
  releasesleep(&ip->lock);
    80003b36:	854a                	mv	a0,s2
    80003b38:	00001097          	auipc	ra,0x1
    80003b3c:	c36080e7          	jalr	-970(ra) # 8000476e <releasesleep>
}
    80003b40:	60e2                	ld	ra,24(sp)
    80003b42:	6442                	ld	s0,16(sp)
    80003b44:	64a2                	ld	s1,8(sp)
    80003b46:	6902                	ld	s2,0(sp)
    80003b48:	6105                	addi	sp,sp,32
    80003b4a:	8082                	ret
    panic("iunlock");
    80003b4c:	00005517          	auipc	a0,0x5
    80003b50:	afc50513          	addi	a0,a0,-1284 # 80008648 <syscalls+0x1a0>
    80003b54:	ffffd097          	auipc	ra,0xffffd
    80003b58:	9ea080e7          	jalr	-1558(ra) # 8000053e <panic>

0000000080003b5c <itrunc>:

// Truncate inode (discard contents).
// Caller must hold ip->lock.
void
itrunc(struct inode *ip)
{
    80003b5c:	7179                	addi	sp,sp,-48
    80003b5e:	f406                	sd	ra,40(sp)
    80003b60:	f022                	sd	s0,32(sp)
    80003b62:	ec26                	sd	s1,24(sp)
    80003b64:	e84a                	sd	s2,16(sp)
    80003b66:	e44e                	sd	s3,8(sp)
    80003b68:	e052                	sd	s4,0(sp)
    80003b6a:	1800                	addi	s0,sp,48
    80003b6c:	89aa                	mv	s3,a0
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
    80003b6e:	05050493          	addi	s1,a0,80
    80003b72:	08050913          	addi	s2,a0,128
    80003b76:	a021                	j	80003b7e <itrunc+0x22>
    80003b78:	0491                	addi	s1,s1,4
    80003b7a:	01248d63          	beq	s1,s2,80003b94 <itrunc+0x38>
    if(ip->addrs[i]){
    80003b7e:	408c                	lw	a1,0(s1)
    80003b80:	dde5                	beqz	a1,80003b78 <itrunc+0x1c>
      bfree(ip->dev, ip->addrs[i]);
    80003b82:	0009a503          	lw	a0,0(s3)
    80003b86:	00000097          	auipc	ra,0x0
    80003b8a:	90c080e7          	jalr	-1780(ra) # 80003492 <bfree>
      ip->addrs[i] = 0;
    80003b8e:	0004a023          	sw	zero,0(s1)
    80003b92:	b7dd                	j	80003b78 <itrunc+0x1c>
    }
  }

  if(ip->addrs[NDIRECT]){
    80003b94:	0809a583          	lw	a1,128(s3)
    80003b98:	e185                	bnez	a1,80003bb8 <itrunc+0x5c>
    brelse(bp);
    bfree(ip->dev, ip->addrs[NDIRECT]);
    ip->addrs[NDIRECT] = 0;
  }

  ip->size = 0;
    80003b9a:	0409a623          	sw	zero,76(s3)
  iupdate(ip);
    80003b9e:	854e                	mv	a0,s3
    80003ba0:	00000097          	auipc	ra,0x0
    80003ba4:	de4080e7          	jalr	-540(ra) # 80003984 <iupdate>
}
    80003ba8:	70a2                	ld	ra,40(sp)
    80003baa:	7402                	ld	s0,32(sp)
    80003bac:	64e2                	ld	s1,24(sp)
    80003bae:	6942                	ld	s2,16(sp)
    80003bb0:	69a2                	ld	s3,8(sp)
    80003bb2:	6a02                	ld	s4,0(sp)
    80003bb4:	6145                	addi	sp,sp,48
    80003bb6:	8082                	ret
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    80003bb8:	0009a503          	lw	a0,0(s3)
    80003bbc:	fffff097          	auipc	ra,0xfffff
    80003bc0:	690080e7          	jalr	1680(ra) # 8000324c <bread>
    80003bc4:	8a2a                	mv	s4,a0
    for(j = 0; j < NINDIRECT; j++){
    80003bc6:	05850493          	addi	s1,a0,88
    80003bca:	45850913          	addi	s2,a0,1112
    80003bce:	a811                	j	80003be2 <itrunc+0x86>
        bfree(ip->dev, a[j]);
    80003bd0:	0009a503          	lw	a0,0(s3)
    80003bd4:	00000097          	auipc	ra,0x0
    80003bd8:	8be080e7          	jalr	-1858(ra) # 80003492 <bfree>
    for(j = 0; j < NINDIRECT; j++){
    80003bdc:	0491                	addi	s1,s1,4
    80003bde:	01248563          	beq	s1,s2,80003be8 <itrunc+0x8c>
      if(a[j])
    80003be2:	408c                	lw	a1,0(s1)
    80003be4:	dde5                	beqz	a1,80003bdc <itrunc+0x80>
    80003be6:	b7ed                	j	80003bd0 <itrunc+0x74>
    brelse(bp);
    80003be8:	8552                	mv	a0,s4
    80003bea:	fffff097          	auipc	ra,0xfffff
    80003bee:	792080e7          	jalr	1938(ra) # 8000337c <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
    80003bf2:	0809a583          	lw	a1,128(s3)
    80003bf6:	0009a503          	lw	a0,0(s3)
    80003bfa:	00000097          	auipc	ra,0x0
    80003bfe:	898080e7          	jalr	-1896(ra) # 80003492 <bfree>
    ip->addrs[NDIRECT] = 0;
    80003c02:	0809a023          	sw	zero,128(s3)
    80003c06:	bf51                	j	80003b9a <itrunc+0x3e>

0000000080003c08 <iput>:
{
    80003c08:	1101                	addi	sp,sp,-32
    80003c0a:	ec06                	sd	ra,24(sp)
    80003c0c:	e822                	sd	s0,16(sp)
    80003c0e:	e426                	sd	s1,8(sp)
    80003c10:	e04a                	sd	s2,0(sp)
    80003c12:	1000                	addi	s0,sp,32
    80003c14:	84aa                	mv	s1,a0
  acquire(&itable.lock);
    80003c16:	0001c517          	auipc	a0,0x1c
    80003c1a:	e7250513          	addi	a0,a0,-398 # 8001fa88 <itable>
    80003c1e:	ffffd097          	auipc	ra,0xffffd
    80003c22:	fc6080e7          	jalr	-58(ra) # 80000be4 <acquire>
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    80003c26:	4498                	lw	a4,8(s1)
    80003c28:	4785                	li	a5,1
    80003c2a:	02f70363          	beq	a4,a5,80003c50 <iput+0x48>
  ip->ref--;
    80003c2e:	449c                	lw	a5,8(s1)
    80003c30:	37fd                	addiw	a5,a5,-1
    80003c32:	c49c                	sw	a5,8(s1)
  release(&itable.lock);
    80003c34:	0001c517          	auipc	a0,0x1c
    80003c38:	e5450513          	addi	a0,a0,-428 # 8001fa88 <itable>
    80003c3c:	ffffd097          	auipc	ra,0xffffd
    80003c40:	05c080e7          	jalr	92(ra) # 80000c98 <release>
}
    80003c44:	60e2                	ld	ra,24(sp)
    80003c46:	6442                	ld	s0,16(sp)
    80003c48:	64a2                	ld	s1,8(sp)
    80003c4a:	6902                	ld	s2,0(sp)
    80003c4c:	6105                	addi	sp,sp,32
    80003c4e:	8082                	ret
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    80003c50:	40bc                	lw	a5,64(s1)
    80003c52:	dff1                	beqz	a5,80003c2e <iput+0x26>
    80003c54:	04a49783          	lh	a5,74(s1)
    80003c58:	fbf9                	bnez	a5,80003c2e <iput+0x26>
    acquiresleep(&ip->lock);
    80003c5a:	01048913          	addi	s2,s1,16
    80003c5e:	854a                	mv	a0,s2
    80003c60:	00001097          	auipc	ra,0x1
    80003c64:	ab8080e7          	jalr	-1352(ra) # 80004718 <acquiresleep>
    release(&itable.lock);
    80003c68:	0001c517          	auipc	a0,0x1c
    80003c6c:	e2050513          	addi	a0,a0,-480 # 8001fa88 <itable>
    80003c70:	ffffd097          	auipc	ra,0xffffd
    80003c74:	028080e7          	jalr	40(ra) # 80000c98 <release>
    itrunc(ip);
    80003c78:	8526                	mv	a0,s1
    80003c7a:	00000097          	auipc	ra,0x0
    80003c7e:	ee2080e7          	jalr	-286(ra) # 80003b5c <itrunc>
    ip->type = 0;
    80003c82:	04049223          	sh	zero,68(s1)
    iupdate(ip);
    80003c86:	8526                	mv	a0,s1
    80003c88:	00000097          	auipc	ra,0x0
    80003c8c:	cfc080e7          	jalr	-772(ra) # 80003984 <iupdate>
    ip->valid = 0;
    80003c90:	0404a023          	sw	zero,64(s1)
    releasesleep(&ip->lock);
    80003c94:	854a                	mv	a0,s2
    80003c96:	00001097          	auipc	ra,0x1
    80003c9a:	ad8080e7          	jalr	-1320(ra) # 8000476e <releasesleep>
    acquire(&itable.lock);
    80003c9e:	0001c517          	auipc	a0,0x1c
    80003ca2:	dea50513          	addi	a0,a0,-534 # 8001fa88 <itable>
    80003ca6:	ffffd097          	auipc	ra,0xffffd
    80003caa:	f3e080e7          	jalr	-194(ra) # 80000be4 <acquire>
    80003cae:	b741                	j	80003c2e <iput+0x26>

0000000080003cb0 <iunlockput>:
{
    80003cb0:	1101                	addi	sp,sp,-32
    80003cb2:	ec06                	sd	ra,24(sp)
    80003cb4:	e822                	sd	s0,16(sp)
    80003cb6:	e426                	sd	s1,8(sp)
    80003cb8:	1000                	addi	s0,sp,32
    80003cba:	84aa                	mv	s1,a0
  iunlock(ip);
    80003cbc:	00000097          	auipc	ra,0x0
    80003cc0:	e54080e7          	jalr	-428(ra) # 80003b10 <iunlock>
  iput(ip);
    80003cc4:	8526                	mv	a0,s1
    80003cc6:	00000097          	auipc	ra,0x0
    80003cca:	f42080e7          	jalr	-190(ra) # 80003c08 <iput>
}
    80003cce:	60e2                	ld	ra,24(sp)
    80003cd0:	6442                	ld	s0,16(sp)
    80003cd2:	64a2                	ld	s1,8(sp)
    80003cd4:	6105                	addi	sp,sp,32
    80003cd6:	8082                	ret

0000000080003cd8 <stati>:

// Copy stat information from inode.
// Caller must hold ip->lock.
void
stati(struct inode *ip, struct stat *st)
{
    80003cd8:	1141                	addi	sp,sp,-16
    80003cda:	e422                	sd	s0,8(sp)
    80003cdc:	0800                	addi	s0,sp,16
  st->dev = ip->dev;
    80003cde:	411c                	lw	a5,0(a0)
    80003ce0:	c19c                	sw	a5,0(a1)
  st->ino = ip->inum;
    80003ce2:	415c                	lw	a5,4(a0)
    80003ce4:	c1dc                	sw	a5,4(a1)
  st->type = ip->type;
    80003ce6:	04451783          	lh	a5,68(a0)
    80003cea:	00f59423          	sh	a5,8(a1)
  st->nlink = ip->nlink;
    80003cee:	04a51783          	lh	a5,74(a0)
    80003cf2:	00f59523          	sh	a5,10(a1)
  st->size = ip->size;
    80003cf6:	04c56783          	lwu	a5,76(a0)
    80003cfa:	e99c                	sd	a5,16(a1)
}
    80003cfc:	6422                	ld	s0,8(sp)
    80003cfe:	0141                	addi	sp,sp,16
    80003d00:	8082                	ret

0000000080003d02 <readi>:
readi(struct inode *ip, int user_dst, uint64 dst, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;

  if(off > ip->size || off + n < off)
    80003d02:	457c                	lw	a5,76(a0)
    80003d04:	0ed7e963          	bltu	a5,a3,80003df6 <readi+0xf4>
{
    80003d08:	7159                	addi	sp,sp,-112
    80003d0a:	f486                	sd	ra,104(sp)
    80003d0c:	f0a2                	sd	s0,96(sp)
    80003d0e:	eca6                	sd	s1,88(sp)
    80003d10:	e8ca                	sd	s2,80(sp)
    80003d12:	e4ce                	sd	s3,72(sp)
    80003d14:	e0d2                	sd	s4,64(sp)
    80003d16:	fc56                	sd	s5,56(sp)
    80003d18:	f85a                	sd	s6,48(sp)
    80003d1a:	f45e                	sd	s7,40(sp)
    80003d1c:	f062                	sd	s8,32(sp)
    80003d1e:	ec66                	sd	s9,24(sp)
    80003d20:	e86a                	sd	s10,16(sp)
    80003d22:	e46e                	sd	s11,8(sp)
    80003d24:	1880                	addi	s0,sp,112
    80003d26:	8baa                	mv	s7,a0
    80003d28:	8c2e                	mv	s8,a1
    80003d2a:	8ab2                	mv	s5,a2
    80003d2c:	84b6                	mv	s1,a3
    80003d2e:	8b3a                	mv	s6,a4
  if(off > ip->size || off + n < off)
    80003d30:	9f35                	addw	a4,a4,a3
    return 0;
    80003d32:	4501                	li	a0,0
  if(off > ip->size || off + n < off)
    80003d34:	0ad76063          	bltu	a4,a3,80003dd4 <readi+0xd2>
  if(off + n > ip->size)
    80003d38:	00e7f463          	bgeu	a5,a4,80003d40 <readi+0x3e>
    n = ip->size - off;
    80003d3c:	40d78b3b          	subw	s6,a5,a3

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    80003d40:	0a0b0963          	beqz	s6,80003df2 <readi+0xf0>
    80003d44:	4981                	li	s3,0
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    m = min(n - tot, BSIZE - off%BSIZE);
    80003d46:	40000d13          	li	s10,1024
    if(either_copyout(user_dst, dst, bp->data + (off % BSIZE), m) == -1) {
    80003d4a:	5cfd                	li	s9,-1
    80003d4c:	a82d                	j	80003d86 <readi+0x84>
    80003d4e:	020a1d93          	slli	s11,s4,0x20
    80003d52:	020ddd93          	srli	s11,s11,0x20
    80003d56:	05890613          	addi	a2,s2,88
    80003d5a:	86ee                	mv	a3,s11
    80003d5c:	963a                	add	a2,a2,a4
    80003d5e:	85d6                	mv	a1,s5
    80003d60:	8562                	mv	a0,s8
    80003d62:	fffff097          	auipc	ra,0xfffff
    80003d66:	b16080e7          	jalr	-1258(ra) # 80002878 <either_copyout>
    80003d6a:	05950d63          	beq	a0,s9,80003dc4 <readi+0xc2>
      brelse(bp);
      tot = -1;
      break;
    }
    brelse(bp);
    80003d6e:	854a                	mv	a0,s2
    80003d70:	fffff097          	auipc	ra,0xfffff
    80003d74:	60c080e7          	jalr	1548(ra) # 8000337c <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    80003d78:	013a09bb          	addw	s3,s4,s3
    80003d7c:	009a04bb          	addw	s1,s4,s1
    80003d80:	9aee                	add	s5,s5,s11
    80003d82:	0569f763          	bgeu	s3,s6,80003dd0 <readi+0xce>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    80003d86:	000ba903          	lw	s2,0(s7)
    80003d8a:	00a4d59b          	srliw	a1,s1,0xa
    80003d8e:	855e                	mv	a0,s7
    80003d90:	00000097          	auipc	ra,0x0
    80003d94:	8b0080e7          	jalr	-1872(ra) # 80003640 <bmap>
    80003d98:	0005059b          	sext.w	a1,a0
    80003d9c:	854a                	mv	a0,s2
    80003d9e:	fffff097          	auipc	ra,0xfffff
    80003da2:	4ae080e7          	jalr	1198(ra) # 8000324c <bread>
    80003da6:	892a                	mv	s2,a0
    m = min(n - tot, BSIZE - off%BSIZE);
    80003da8:	3ff4f713          	andi	a4,s1,1023
    80003dac:	40ed07bb          	subw	a5,s10,a4
    80003db0:	413b06bb          	subw	a3,s6,s3
    80003db4:	8a3e                	mv	s4,a5
    80003db6:	2781                	sext.w	a5,a5
    80003db8:	0006861b          	sext.w	a2,a3
    80003dbc:	f8f679e3          	bgeu	a2,a5,80003d4e <readi+0x4c>
    80003dc0:	8a36                	mv	s4,a3
    80003dc2:	b771                	j	80003d4e <readi+0x4c>
      brelse(bp);
    80003dc4:	854a                	mv	a0,s2
    80003dc6:	fffff097          	auipc	ra,0xfffff
    80003dca:	5b6080e7          	jalr	1462(ra) # 8000337c <brelse>
      tot = -1;
    80003dce:	59fd                	li	s3,-1
  }
  return tot;
    80003dd0:	0009851b          	sext.w	a0,s3
}
    80003dd4:	70a6                	ld	ra,104(sp)
    80003dd6:	7406                	ld	s0,96(sp)
    80003dd8:	64e6                	ld	s1,88(sp)
    80003dda:	6946                	ld	s2,80(sp)
    80003ddc:	69a6                	ld	s3,72(sp)
    80003dde:	6a06                	ld	s4,64(sp)
    80003de0:	7ae2                	ld	s5,56(sp)
    80003de2:	7b42                	ld	s6,48(sp)
    80003de4:	7ba2                	ld	s7,40(sp)
    80003de6:	7c02                	ld	s8,32(sp)
    80003de8:	6ce2                	ld	s9,24(sp)
    80003dea:	6d42                	ld	s10,16(sp)
    80003dec:	6da2                	ld	s11,8(sp)
    80003dee:	6165                	addi	sp,sp,112
    80003df0:	8082                	ret
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    80003df2:	89da                	mv	s3,s6
    80003df4:	bff1                	j	80003dd0 <readi+0xce>
    return 0;
    80003df6:	4501                	li	a0,0
}
    80003df8:	8082                	ret

0000000080003dfa <writei>:
writei(struct inode *ip, int user_src, uint64 src, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;

  if(off > ip->size || off + n < off)
    80003dfa:	457c                	lw	a5,76(a0)
    80003dfc:	10d7e863          	bltu	a5,a3,80003f0c <writei+0x112>
{
    80003e00:	7159                	addi	sp,sp,-112
    80003e02:	f486                	sd	ra,104(sp)
    80003e04:	f0a2                	sd	s0,96(sp)
    80003e06:	eca6                	sd	s1,88(sp)
    80003e08:	e8ca                	sd	s2,80(sp)
    80003e0a:	e4ce                	sd	s3,72(sp)
    80003e0c:	e0d2                	sd	s4,64(sp)
    80003e0e:	fc56                	sd	s5,56(sp)
    80003e10:	f85a                	sd	s6,48(sp)
    80003e12:	f45e                	sd	s7,40(sp)
    80003e14:	f062                	sd	s8,32(sp)
    80003e16:	ec66                	sd	s9,24(sp)
    80003e18:	e86a                	sd	s10,16(sp)
    80003e1a:	e46e                	sd	s11,8(sp)
    80003e1c:	1880                	addi	s0,sp,112
    80003e1e:	8b2a                	mv	s6,a0
    80003e20:	8c2e                	mv	s8,a1
    80003e22:	8ab2                	mv	s5,a2
    80003e24:	8936                	mv	s2,a3
    80003e26:	8bba                	mv	s7,a4
  if(off > ip->size || off + n < off)
    80003e28:	00e687bb          	addw	a5,a3,a4
    80003e2c:	0ed7e263          	bltu	a5,a3,80003f10 <writei+0x116>
    return -1;
  if(off + n > MAXFILE*BSIZE)
    80003e30:	00043737          	lui	a4,0x43
    80003e34:	0ef76063          	bltu	a4,a5,80003f14 <writei+0x11a>
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    80003e38:	0c0b8863          	beqz	s7,80003f08 <writei+0x10e>
    80003e3c:	4a01                	li	s4,0
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    m = min(n - tot, BSIZE - off%BSIZE);
    80003e3e:	40000d13          	li	s10,1024
    if(either_copyin(bp->data + (off % BSIZE), user_src, src, m) == -1) {
    80003e42:	5cfd                	li	s9,-1
    80003e44:	a091                	j	80003e88 <writei+0x8e>
    80003e46:	02099d93          	slli	s11,s3,0x20
    80003e4a:	020ddd93          	srli	s11,s11,0x20
    80003e4e:	05848513          	addi	a0,s1,88
    80003e52:	86ee                	mv	a3,s11
    80003e54:	8656                	mv	a2,s5
    80003e56:	85e2                	mv	a1,s8
    80003e58:	953a                	add	a0,a0,a4
    80003e5a:	fffff097          	auipc	ra,0xfffff
    80003e5e:	a74080e7          	jalr	-1420(ra) # 800028ce <either_copyin>
    80003e62:	07950263          	beq	a0,s9,80003ec6 <writei+0xcc>
      brelse(bp);
      break;
    }
    log_write(bp);
    80003e66:	8526                	mv	a0,s1
    80003e68:	00000097          	auipc	ra,0x0
    80003e6c:	790080e7          	jalr	1936(ra) # 800045f8 <log_write>
    brelse(bp);
    80003e70:	8526                	mv	a0,s1
    80003e72:	fffff097          	auipc	ra,0xfffff
    80003e76:	50a080e7          	jalr	1290(ra) # 8000337c <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    80003e7a:	01498a3b          	addw	s4,s3,s4
    80003e7e:	0129893b          	addw	s2,s3,s2
    80003e82:	9aee                	add	s5,s5,s11
    80003e84:	057a7663          	bgeu	s4,s7,80003ed0 <writei+0xd6>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    80003e88:	000b2483          	lw	s1,0(s6)
    80003e8c:	00a9559b          	srliw	a1,s2,0xa
    80003e90:	855a                	mv	a0,s6
    80003e92:	fffff097          	auipc	ra,0xfffff
    80003e96:	7ae080e7          	jalr	1966(ra) # 80003640 <bmap>
    80003e9a:	0005059b          	sext.w	a1,a0
    80003e9e:	8526                	mv	a0,s1
    80003ea0:	fffff097          	auipc	ra,0xfffff
    80003ea4:	3ac080e7          	jalr	940(ra) # 8000324c <bread>
    80003ea8:	84aa                	mv	s1,a0
    m = min(n - tot, BSIZE - off%BSIZE);
    80003eaa:	3ff97713          	andi	a4,s2,1023
    80003eae:	40ed07bb          	subw	a5,s10,a4
    80003eb2:	414b86bb          	subw	a3,s7,s4
    80003eb6:	89be                	mv	s3,a5
    80003eb8:	2781                	sext.w	a5,a5
    80003eba:	0006861b          	sext.w	a2,a3
    80003ebe:	f8f674e3          	bgeu	a2,a5,80003e46 <writei+0x4c>
    80003ec2:	89b6                	mv	s3,a3
    80003ec4:	b749                	j	80003e46 <writei+0x4c>
      brelse(bp);
    80003ec6:	8526                	mv	a0,s1
    80003ec8:	fffff097          	auipc	ra,0xfffff
    80003ecc:	4b4080e7          	jalr	1204(ra) # 8000337c <brelse>
  }

  if(off > ip->size)
    80003ed0:	04cb2783          	lw	a5,76(s6)
    80003ed4:	0127f463          	bgeu	a5,s2,80003edc <writei+0xe2>
    ip->size = off;
    80003ed8:	052b2623          	sw	s2,76(s6)

  // write the i-node back to disk even if the size didn't change
  // because the loop above might have called bmap() and added a new
  // block to ip->addrs[].
  iupdate(ip);
    80003edc:	855a                	mv	a0,s6
    80003ede:	00000097          	auipc	ra,0x0
    80003ee2:	aa6080e7          	jalr	-1370(ra) # 80003984 <iupdate>

  return tot;
    80003ee6:	000a051b          	sext.w	a0,s4
}
    80003eea:	70a6                	ld	ra,104(sp)
    80003eec:	7406                	ld	s0,96(sp)
    80003eee:	64e6                	ld	s1,88(sp)
    80003ef0:	6946                	ld	s2,80(sp)
    80003ef2:	69a6                	ld	s3,72(sp)
    80003ef4:	6a06                	ld	s4,64(sp)
    80003ef6:	7ae2                	ld	s5,56(sp)
    80003ef8:	7b42                	ld	s6,48(sp)
    80003efa:	7ba2                	ld	s7,40(sp)
    80003efc:	7c02                	ld	s8,32(sp)
    80003efe:	6ce2                	ld	s9,24(sp)
    80003f00:	6d42                	ld	s10,16(sp)
    80003f02:	6da2                	ld	s11,8(sp)
    80003f04:	6165                	addi	sp,sp,112
    80003f06:	8082                	ret
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    80003f08:	8a5e                	mv	s4,s7
    80003f0a:	bfc9                	j	80003edc <writei+0xe2>
    return -1;
    80003f0c:	557d                	li	a0,-1
}
    80003f0e:	8082                	ret
    return -1;
    80003f10:	557d                	li	a0,-1
    80003f12:	bfe1                	j	80003eea <writei+0xf0>
    return -1;
    80003f14:	557d                	li	a0,-1
    80003f16:	bfd1                	j	80003eea <writei+0xf0>

0000000080003f18 <namecmp>:

// Directories

int
namecmp(const char *s, const char *t)
{
    80003f18:	1141                	addi	sp,sp,-16
    80003f1a:	e406                	sd	ra,8(sp)
    80003f1c:	e022                	sd	s0,0(sp)
    80003f1e:	0800                	addi	s0,sp,16
  return strncmp(s, t, DIRSIZ);
    80003f20:	4639                	li	a2,14
    80003f22:	ffffd097          	auipc	ra,0xffffd
    80003f26:	e96080e7          	jalr	-362(ra) # 80000db8 <strncmp>
}
    80003f2a:	60a2                	ld	ra,8(sp)
    80003f2c:	6402                	ld	s0,0(sp)
    80003f2e:	0141                	addi	sp,sp,16
    80003f30:	8082                	ret

0000000080003f32 <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
    80003f32:	7139                	addi	sp,sp,-64
    80003f34:	fc06                	sd	ra,56(sp)
    80003f36:	f822                	sd	s0,48(sp)
    80003f38:	f426                	sd	s1,40(sp)
    80003f3a:	f04a                	sd	s2,32(sp)
    80003f3c:	ec4e                	sd	s3,24(sp)
    80003f3e:	e852                	sd	s4,16(sp)
    80003f40:	0080                	addi	s0,sp,64
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
    80003f42:	04451703          	lh	a4,68(a0)
    80003f46:	4785                	li	a5,1
    80003f48:	00f71a63          	bne	a4,a5,80003f5c <dirlookup+0x2a>
    80003f4c:	892a                	mv	s2,a0
    80003f4e:	89ae                	mv	s3,a1
    80003f50:	8a32                	mv	s4,a2
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
    80003f52:	457c                	lw	a5,76(a0)
    80003f54:	4481                	li	s1,0
      inum = de.inum;
      return iget(dp->dev, inum);
    }
  }

  return 0;
    80003f56:	4501                	li	a0,0
  for(off = 0; off < dp->size; off += sizeof(de)){
    80003f58:	e79d                	bnez	a5,80003f86 <dirlookup+0x54>
    80003f5a:	a8a5                	j	80003fd2 <dirlookup+0xa0>
    panic("dirlookup not DIR");
    80003f5c:	00004517          	auipc	a0,0x4
    80003f60:	6f450513          	addi	a0,a0,1780 # 80008650 <syscalls+0x1a8>
    80003f64:	ffffc097          	auipc	ra,0xffffc
    80003f68:	5da080e7          	jalr	1498(ra) # 8000053e <panic>
      panic("dirlookup read");
    80003f6c:	00004517          	auipc	a0,0x4
    80003f70:	6fc50513          	addi	a0,a0,1788 # 80008668 <syscalls+0x1c0>
    80003f74:	ffffc097          	auipc	ra,0xffffc
    80003f78:	5ca080e7          	jalr	1482(ra) # 8000053e <panic>
  for(off = 0; off < dp->size; off += sizeof(de)){
    80003f7c:	24c1                	addiw	s1,s1,16
    80003f7e:	04c92783          	lw	a5,76(s2)
    80003f82:	04f4f763          	bgeu	s1,a5,80003fd0 <dirlookup+0x9e>
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80003f86:	4741                	li	a4,16
    80003f88:	86a6                	mv	a3,s1
    80003f8a:	fc040613          	addi	a2,s0,-64
    80003f8e:	4581                	li	a1,0
    80003f90:	854a                	mv	a0,s2
    80003f92:	00000097          	auipc	ra,0x0
    80003f96:	d70080e7          	jalr	-656(ra) # 80003d02 <readi>
    80003f9a:	47c1                	li	a5,16
    80003f9c:	fcf518e3          	bne	a0,a5,80003f6c <dirlookup+0x3a>
    if(de.inum == 0)
    80003fa0:	fc045783          	lhu	a5,-64(s0)
    80003fa4:	dfe1                	beqz	a5,80003f7c <dirlookup+0x4a>
    if(namecmp(name, de.name) == 0){
    80003fa6:	fc240593          	addi	a1,s0,-62
    80003faa:	854e                	mv	a0,s3
    80003fac:	00000097          	auipc	ra,0x0
    80003fb0:	f6c080e7          	jalr	-148(ra) # 80003f18 <namecmp>
    80003fb4:	f561                	bnez	a0,80003f7c <dirlookup+0x4a>
      if(poff)
    80003fb6:	000a0463          	beqz	s4,80003fbe <dirlookup+0x8c>
        *poff = off;
    80003fba:	009a2023          	sw	s1,0(s4)
      return iget(dp->dev, inum);
    80003fbe:	fc045583          	lhu	a1,-64(s0)
    80003fc2:	00092503          	lw	a0,0(s2)
    80003fc6:	fffff097          	auipc	ra,0xfffff
    80003fca:	754080e7          	jalr	1876(ra) # 8000371a <iget>
    80003fce:	a011                	j	80003fd2 <dirlookup+0xa0>
  return 0;
    80003fd0:	4501                	li	a0,0
}
    80003fd2:	70e2                	ld	ra,56(sp)
    80003fd4:	7442                	ld	s0,48(sp)
    80003fd6:	74a2                	ld	s1,40(sp)
    80003fd8:	7902                	ld	s2,32(sp)
    80003fda:	69e2                	ld	s3,24(sp)
    80003fdc:	6a42                	ld	s4,16(sp)
    80003fde:	6121                	addi	sp,sp,64
    80003fe0:	8082                	ret

0000000080003fe2 <namex>:
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
// Must be called inside a transaction since it calls iput().
static struct inode*
namex(char *path, int nameiparent, char *name)
{
    80003fe2:	711d                	addi	sp,sp,-96
    80003fe4:	ec86                	sd	ra,88(sp)
    80003fe6:	e8a2                	sd	s0,80(sp)
    80003fe8:	e4a6                	sd	s1,72(sp)
    80003fea:	e0ca                	sd	s2,64(sp)
    80003fec:	fc4e                	sd	s3,56(sp)
    80003fee:	f852                	sd	s4,48(sp)
    80003ff0:	f456                	sd	s5,40(sp)
    80003ff2:	f05a                	sd	s6,32(sp)
    80003ff4:	ec5e                	sd	s7,24(sp)
    80003ff6:	e862                	sd	s8,16(sp)
    80003ff8:	e466                	sd	s9,8(sp)
    80003ffa:	1080                	addi	s0,sp,96
    80003ffc:	84aa                	mv	s1,a0
    80003ffe:	8b2e                	mv	s6,a1
    80004000:	8ab2                	mv	s5,a2
  struct inode *ip, *next;

  if(*path == '/')
    80004002:	00054703          	lbu	a4,0(a0)
    80004006:	02f00793          	li	a5,47
    8000400a:	02f70363          	beq	a4,a5,80004030 <namex+0x4e>
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(myproc()->cwd);
    8000400e:	ffffe097          	auipc	ra,0xffffe
    80004012:	d7e080e7          	jalr	-642(ra) # 80001d8c <myproc>
    80004016:	15853503          	ld	a0,344(a0)
    8000401a:	00000097          	auipc	ra,0x0
    8000401e:	9f6080e7          	jalr	-1546(ra) # 80003a10 <idup>
    80004022:	89aa                	mv	s3,a0
  while(*path == '/')
    80004024:	02f00913          	li	s2,47
  len = path - s;
    80004028:	4b81                	li	s7,0
  if(len >= DIRSIZ)
    8000402a:	4cb5                	li	s9,13

  while((path = skipelem(path, name)) != 0){
    ilock(ip);
    if(ip->type != T_DIR){
    8000402c:	4c05                	li	s8,1
    8000402e:	a865                	j	800040e6 <namex+0x104>
    ip = iget(ROOTDEV, ROOTINO);
    80004030:	4585                	li	a1,1
    80004032:	4505                	li	a0,1
    80004034:	fffff097          	auipc	ra,0xfffff
    80004038:	6e6080e7          	jalr	1766(ra) # 8000371a <iget>
    8000403c:	89aa                	mv	s3,a0
    8000403e:	b7dd                	j	80004024 <namex+0x42>
      iunlockput(ip);
    80004040:	854e                	mv	a0,s3
    80004042:	00000097          	auipc	ra,0x0
    80004046:	c6e080e7          	jalr	-914(ra) # 80003cb0 <iunlockput>
      return 0;
    8000404a:	4981                	li	s3,0
  if(nameiparent){
    iput(ip);
    return 0;
  }
  return ip;
}
    8000404c:	854e                	mv	a0,s3
    8000404e:	60e6                	ld	ra,88(sp)
    80004050:	6446                	ld	s0,80(sp)
    80004052:	64a6                	ld	s1,72(sp)
    80004054:	6906                	ld	s2,64(sp)
    80004056:	79e2                	ld	s3,56(sp)
    80004058:	7a42                	ld	s4,48(sp)
    8000405a:	7aa2                	ld	s5,40(sp)
    8000405c:	7b02                	ld	s6,32(sp)
    8000405e:	6be2                	ld	s7,24(sp)
    80004060:	6c42                	ld	s8,16(sp)
    80004062:	6ca2                	ld	s9,8(sp)
    80004064:	6125                	addi	sp,sp,96
    80004066:	8082                	ret
      iunlock(ip);
    80004068:	854e                	mv	a0,s3
    8000406a:	00000097          	auipc	ra,0x0
    8000406e:	aa6080e7          	jalr	-1370(ra) # 80003b10 <iunlock>
      return ip;
    80004072:	bfe9                	j	8000404c <namex+0x6a>
      iunlockput(ip);
    80004074:	854e                	mv	a0,s3
    80004076:	00000097          	auipc	ra,0x0
    8000407a:	c3a080e7          	jalr	-966(ra) # 80003cb0 <iunlockput>
      return 0;
    8000407e:	89d2                	mv	s3,s4
    80004080:	b7f1                	j	8000404c <namex+0x6a>
  len = path - s;
    80004082:	40b48633          	sub	a2,s1,a1
    80004086:	00060a1b          	sext.w	s4,a2
  if(len >= DIRSIZ)
    8000408a:	094cd463          	bge	s9,s4,80004112 <namex+0x130>
    memmove(name, s, DIRSIZ);
    8000408e:	4639                	li	a2,14
    80004090:	8556                	mv	a0,s5
    80004092:	ffffd097          	auipc	ra,0xffffd
    80004096:	cae080e7          	jalr	-850(ra) # 80000d40 <memmove>
  while(*path == '/')
    8000409a:	0004c783          	lbu	a5,0(s1)
    8000409e:	01279763          	bne	a5,s2,800040ac <namex+0xca>
    path++;
    800040a2:	0485                	addi	s1,s1,1
  while(*path == '/')
    800040a4:	0004c783          	lbu	a5,0(s1)
    800040a8:	ff278de3          	beq	a5,s2,800040a2 <namex+0xc0>
    ilock(ip);
    800040ac:	854e                	mv	a0,s3
    800040ae:	00000097          	auipc	ra,0x0
    800040b2:	9a0080e7          	jalr	-1632(ra) # 80003a4e <ilock>
    if(ip->type != T_DIR){
    800040b6:	04499783          	lh	a5,68(s3)
    800040ba:	f98793e3          	bne	a5,s8,80004040 <namex+0x5e>
    if(nameiparent && *path == '\0'){
    800040be:	000b0563          	beqz	s6,800040c8 <namex+0xe6>
    800040c2:	0004c783          	lbu	a5,0(s1)
    800040c6:	d3cd                	beqz	a5,80004068 <namex+0x86>
    if((next = dirlookup(ip, name, 0)) == 0){
    800040c8:	865e                	mv	a2,s7
    800040ca:	85d6                	mv	a1,s5
    800040cc:	854e                	mv	a0,s3
    800040ce:	00000097          	auipc	ra,0x0
    800040d2:	e64080e7          	jalr	-412(ra) # 80003f32 <dirlookup>
    800040d6:	8a2a                	mv	s4,a0
    800040d8:	dd51                	beqz	a0,80004074 <namex+0x92>
    iunlockput(ip);
    800040da:	854e                	mv	a0,s3
    800040dc:	00000097          	auipc	ra,0x0
    800040e0:	bd4080e7          	jalr	-1068(ra) # 80003cb0 <iunlockput>
    ip = next;
    800040e4:	89d2                	mv	s3,s4
  while(*path == '/')
    800040e6:	0004c783          	lbu	a5,0(s1)
    800040ea:	05279763          	bne	a5,s2,80004138 <namex+0x156>
    path++;
    800040ee:	0485                	addi	s1,s1,1
  while(*path == '/')
    800040f0:	0004c783          	lbu	a5,0(s1)
    800040f4:	ff278de3          	beq	a5,s2,800040ee <namex+0x10c>
  if(*path == 0)
    800040f8:	c79d                	beqz	a5,80004126 <namex+0x144>
    path++;
    800040fa:	85a6                	mv	a1,s1
  len = path - s;
    800040fc:	8a5e                	mv	s4,s7
    800040fe:	865e                	mv	a2,s7
  while(*path != '/' && *path != 0)
    80004100:	01278963          	beq	a5,s2,80004112 <namex+0x130>
    80004104:	dfbd                	beqz	a5,80004082 <namex+0xa0>
    path++;
    80004106:	0485                	addi	s1,s1,1
  while(*path != '/' && *path != 0)
    80004108:	0004c783          	lbu	a5,0(s1)
    8000410c:	ff279ce3          	bne	a5,s2,80004104 <namex+0x122>
    80004110:	bf8d                	j	80004082 <namex+0xa0>
    memmove(name, s, len);
    80004112:	2601                	sext.w	a2,a2
    80004114:	8556                	mv	a0,s5
    80004116:	ffffd097          	auipc	ra,0xffffd
    8000411a:	c2a080e7          	jalr	-982(ra) # 80000d40 <memmove>
    name[len] = 0;
    8000411e:	9a56                	add	s4,s4,s5
    80004120:	000a0023          	sb	zero,0(s4)
    80004124:	bf9d                	j	8000409a <namex+0xb8>
  if(nameiparent){
    80004126:	f20b03e3          	beqz	s6,8000404c <namex+0x6a>
    iput(ip);
    8000412a:	854e                	mv	a0,s3
    8000412c:	00000097          	auipc	ra,0x0
    80004130:	adc080e7          	jalr	-1316(ra) # 80003c08 <iput>
    return 0;
    80004134:	4981                	li	s3,0
    80004136:	bf19                	j	8000404c <namex+0x6a>
  if(*path == 0)
    80004138:	d7fd                	beqz	a5,80004126 <namex+0x144>
  while(*path != '/' && *path != 0)
    8000413a:	0004c783          	lbu	a5,0(s1)
    8000413e:	85a6                	mv	a1,s1
    80004140:	b7d1                	j	80004104 <namex+0x122>

0000000080004142 <dirlink>:
{
    80004142:	7139                	addi	sp,sp,-64
    80004144:	fc06                	sd	ra,56(sp)
    80004146:	f822                	sd	s0,48(sp)
    80004148:	f426                	sd	s1,40(sp)
    8000414a:	f04a                	sd	s2,32(sp)
    8000414c:	ec4e                	sd	s3,24(sp)
    8000414e:	e852                	sd	s4,16(sp)
    80004150:	0080                	addi	s0,sp,64
    80004152:	892a                	mv	s2,a0
    80004154:	8a2e                	mv	s4,a1
    80004156:	89b2                	mv	s3,a2
  if((ip = dirlookup(dp, name, 0)) != 0){
    80004158:	4601                	li	a2,0
    8000415a:	00000097          	auipc	ra,0x0
    8000415e:	dd8080e7          	jalr	-552(ra) # 80003f32 <dirlookup>
    80004162:	e93d                	bnez	a0,800041d8 <dirlink+0x96>
  for(off = 0; off < dp->size; off += sizeof(de)){
    80004164:	04c92483          	lw	s1,76(s2)
    80004168:	c49d                	beqz	s1,80004196 <dirlink+0x54>
    8000416a:	4481                	li	s1,0
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    8000416c:	4741                	li	a4,16
    8000416e:	86a6                	mv	a3,s1
    80004170:	fc040613          	addi	a2,s0,-64
    80004174:	4581                	li	a1,0
    80004176:	854a                	mv	a0,s2
    80004178:	00000097          	auipc	ra,0x0
    8000417c:	b8a080e7          	jalr	-1142(ra) # 80003d02 <readi>
    80004180:	47c1                	li	a5,16
    80004182:	06f51163          	bne	a0,a5,800041e4 <dirlink+0xa2>
    if(de.inum == 0)
    80004186:	fc045783          	lhu	a5,-64(s0)
    8000418a:	c791                	beqz	a5,80004196 <dirlink+0x54>
  for(off = 0; off < dp->size; off += sizeof(de)){
    8000418c:	24c1                	addiw	s1,s1,16
    8000418e:	04c92783          	lw	a5,76(s2)
    80004192:	fcf4ede3          	bltu	s1,a5,8000416c <dirlink+0x2a>
  strncpy(de.name, name, DIRSIZ);
    80004196:	4639                	li	a2,14
    80004198:	85d2                	mv	a1,s4
    8000419a:	fc240513          	addi	a0,s0,-62
    8000419e:	ffffd097          	auipc	ra,0xffffd
    800041a2:	c56080e7          	jalr	-938(ra) # 80000df4 <strncpy>
  de.inum = inum;
    800041a6:	fd341023          	sh	s3,-64(s0)
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    800041aa:	4741                	li	a4,16
    800041ac:	86a6                	mv	a3,s1
    800041ae:	fc040613          	addi	a2,s0,-64
    800041b2:	4581                	li	a1,0
    800041b4:	854a                	mv	a0,s2
    800041b6:	00000097          	auipc	ra,0x0
    800041ba:	c44080e7          	jalr	-956(ra) # 80003dfa <writei>
    800041be:	872a                	mv	a4,a0
    800041c0:	47c1                	li	a5,16
  return 0;
    800041c2:	4501                	li	a0,0
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    800041c4:	02f71863          	bne	a4,a5,800041f4 <dirlink+0xb2>
}
    800041c8:	70e2                	ld	ra,56(sp)
    800041ca:	7442                	ld	s0,48(sp)
    800041cc:	74a2                	ld	s1,40(sp)
    800041ce:	7902                	ld	s2,32(sp)
    800041d0:	69e2                	ld	s3,24(sp)
    800041d2:	6a42                	ld	s4,16(sp)
    800041d4:	6121                	addi	sp,sp,64
    800041d6:	8082                	ret
    iput(ip);
    800041d8:	00000097          	auipc	ra,0x0
    800041dc:	a30080e7          	jalr	-1488(ra) # 80003c08 <iput>
    return -1;
    800041e0:	557d                	li	a0,-1
    800041e2:	b7dd                	j	800041c8 <dirlink+0x86>
      panic("dirlink read");
    800041e4:	00004517          	auipc	a0,0x4
    800041e8:	49450513          	addi	a0,a0,1172 # 80008678 <syscalls+0x1d0>
    800041ec:	ffffc097          	auipc	ra,0xffffc
    800041f0:	352080e7          	jalr	850(ra) # 8000053e <panic>
    panic("dirlink");
    800041f4:	00004517          	auipc	a0,0x4
    800041f8:	59450513          	addi	a0,a0,1428 # 80008788 <syscalls+0x2e0>
    800041fc:	ffffc097          	auipc	ra,0xffffc
    80004200:	342080e7          	jalr	834(ra) # 8000053e <panic>

0000000080004204 <namei>:

struct inode*
namei(char *path)
{
    80004204:	1101                	addi	sp,sp,-32
    80004206:	ec06                	sd	ra,24(sp)
    80004208:	e822                	sd	s0,16(sp)
    8000420a:	1000                	addi	s0,sp,32
  char name[DIRSIZ];
  return namex(path, 0, name);
    8000420c:	fe040613          	addi	a2,s0,-32
    80004210:	4581                	li	a1,0
    80004212:	00000097          	auipc	ra,0x0
    80004216:	dd0080e7          	jalr	-560(ra) # 80003fe2 <namex>
}
    8000421a:	60e2                	ld	ra,24(sp)
    8000421c:	6442                	ld	s0,16(sp)
    8000421e:	6105                	addi	sp,sp,32
    80004220:	8082                	ret

0000000080004222 <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
    80004222:	1141                	addi	sp,sp,-16
    80004224:	e406                	sd	ra,8(sp)
    80004226:	e022                	sd	s0,0(sp)
    80004228:	0800                	addi	s0,sp,16
    8000422a:	862e                	mv	a2,a1
  return namex(path, 1, name);
    8000422c:	4585                	li	a1,1
    8000422e:	00000097          	auipc	ra,0x0
    80004232:	db4080e7          	jalr	-588(ra) # 80003fe2 <namex>
}
    80004236:	60a2                	ld	ra,8(sp)
    80004238:	6402                	ld	s0,0(sp)
    8000423a:	0141                	addi	sp,sp,16
    8000423c:	8082                	ret

000000008000423e <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
    8000423e:	1101                	addi	sp,sp,-32
    80004240:	ec06                	sd	ra,24(sp)
    80004242:	e822                	sd	s0,16(sp)
    80004244:	e426                	sd	s1,8(sp)
    80004246:	e04a                	sd	s2,0(sp)
    80004248:	1000                	addi	s0,sp,32
  struct buf *buf = bread(log.dev, log.start);
    8000424a:	0001d917          	auipc	s2,0x1d
    8000424e:	2e690913          	addi	s2,s2,742 # 80021530 <log>
    80004252:	01892583          	lw	a1,24(s2)
    80004256:	02892503          	lw	a0,40(s2)
    8000425a:	fffff097          	auipc	ra,0xfffff
    8000425e:	ff2080e7          	jalr	-14(ra) # 8000324c <bread>
    80004262:	84aa                	mv	s1,a0
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
    80004264:	02c92683          	lw	a3,44(s2)
    80004268:	cd34                	sw	a3,88(a0)
  for (i = 0; i < log.lh.n; i++) {
    8000426a:	02d05763          	blez	a3,80004298 <write_head+0x5a>
    8000426e:	0001d797          	auipc	a5,0x1d
    80004272:	2f278793          	addi	a5,a5,754 # 80021560 <log+0x30>
    80004276:	05c50713          	addi	a4,a0,92
    8000427a:	36fd                	addiw	a3,a3,-1
    8000427c:	1682                	slli	a3,a3,0x20
    8000427e:	9281                	srli	a3,a3,0x20
    80004280:	068a                	slli	a3,a3,0x2
    80004282:	0001d617          	auipc	a2,0x1d
    80004286:	2e260613          	addi	a2,a2,738 # 80021564 <log+0x34>
    8000428a:	96b2                	add	a3,a3,a2
    hb->block[i] = log.lh.block[i];
    8000428c:	4390                	lw	a2,0(a5)
    8000428e:	c310                	sw	a2,0(a4)
  for (i = 0; i < log.lh.n; i++) {
    80004290:	0791                	addi	a5,a5,4
    80004292:	0711                	addi	a4,a4,4
    80004294:	fed79ce3          	bne	a5,a3,8000428c <write_head+0x4e>
  }
  bwrite(buf);
    80004298:	8526                	mv	a0,s1
    8000429a:	fffff097          	auipc	ra,0xfffff
    8000429e:	0a4080e7          	jalr	164(ra) # 8000333e <bwrite>
  brelse(buf);
    800042a2:	8526                	mv	a0,s1
    800042a4:	fffff097          	auipc	ra,0xfffff
    800042a8:	0d8080e7          	jalr	216(ra) # 8000337c <brelse>
}
    800042ac:	60e2                	ld	ra,24(sp)
    800042ae:	6442                	ld	s0,16(sp)
    800042b0:	64a2                	ld	s1,8(sp)
    800042b2:	6902                	ld	s2,0(sp)
    800042b4:	6105                	addi	sp,sp,32
    800042b6:	8082                	ret

00000000800042b8 <install_trans>:
  for (tail = 0; tail < log.lh.n; tail++) {
    800042b8:	0001d797          	auipc	a5,0x1d
    800042bc:	2a47a783          	lw	a5,676(a5) # 8002155c <log+0x2c>
    800042c0:	0af05d63          	blez	a5,8000437a <install_trans+0xc2>
{
    800042c4:	7139                	addi	sp,sp,-64
    800042c6:	fc06                	sd	ra,56(sp)
    800042c8:	f822                	sd	s0,48(sp)
    800042ca:	f426                	sd	s1,40(sp)
    800042cc:	f04a                	sd	s2,32(sp)
    800042ce:	ec4e                	sd	s3,24(sp)
    800042d0:	e852                	sd	s4,16(sp)
    800042d2:	e456                	sd	s5,8(sp)
    800042d4:	e05a                	sd	s6,0(sp)
    800042d6:	0080                	addi	s0,sp,64
    800042d8:	8b2a                	mv	s6,a0
    800042da:	0001da97          	auipc	s5,0x1d
    800042de:	286a8a93          	addi	s5,s5,646 # 80021560 <log+0x30>
  for (tail = 0; tail < log.lh.n; tail++) {
    800042e2:	4a01                	li	s4,0
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    800042e4:	0001d997          	auipc	s3,0x1d
    800042e8:	24c98993          	addi	s3,s3,588 # 80021530 <log>
    800042ec:	a035                	j	80004318 <install_trans+0x60>
      bunpin(dbuf);
    800042ee:	8526                	mv	a0,s1
    800042f0:	fffff097          	auipc	ra,0xfffff
    800042f4:	166080e7          	jalr	358(ra) # 80003456 <bunpin>
    brelse(lbuf);
    800042f8:	854a                	mv	a0,s2
    800042fa:	fffff097          	auipc	ra,0xfffff
    800042fe:	082080e7          	jalr	130(ra) # 8000337c <brelse>
    brelse(dbuf);
    80004302:	8526                	mv	a0,s1
    80004304:	fffff097          	auipc	ra,0xfffff
    80004308:	078080e7          	jalr	120(ra) # 8000337c <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    8000430c:	2a05                	addiw	s4,s4,1
    8000430e:	0a91                	addi	s5,s5,4
    80004310:	02c9a783          	lw	a5,44(s3)
    80004314:	04fa5963          	bge	s4,a5,80004366 <install_trans+0xae>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    80004318:	0189a583          	lw	a1,24(s3)
    8000431c:	014585bb          	addw	a1,a1,s4
    80004320:	2585                	addiw	a1,a1,1
    80004322:	0289a503          	lw	a0,40(s3)
    80004326:	fffff097          	auipc	ra,0xfffff
    8000432a:	f26080e7          	jalr	-218(ra) # 8000324c <bread>
    8000432e:	892a                	mv	s2,a0
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
    80004330:	000aa583          	lw	a1,0(s5)
    80004334:	0289a503          	lw	a0,40(s3)
    80004338:	fffff097          	auipc	ra,0xfffff
    8000433c:	f14080e7          	jalr	-236(ra) # 8000324c <bread>
    80004340:	84aa                	mv	s1,a0
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    80004342:	40000613          	li	a2,1024
    80004346:	05890593          	addi	a1,s2,88
    8000434a:	05850513          	addi	a0,a0,88
    8000434e:	ffffd097          	auipc	ra,0xffffd
    80004352:	9f2080e7          	jalr	-1550(ra) # 80000d40 <memmove>
    bwrite(dbuf);  // write dst to disk
    80004356:	8526                	mv	a0,s1
    80004358:	fffff097          	auipc	ra,0xfffff
    8000435c:	fe6080e7          	jalr	-26(ra) # 8000333e <bwrite>
    if(recovering == 0)
    80004360:	f80b1ce3          	bnez	s6,800042f8 <install_trans+0x40>
    80004364:	b769                	j	800042ee <install_trans+0x36>
}
    80004366:	70e2                	ld	ra,56(sp)
    80004368:	7442                	ld	s0,48(sp)
    8000436a:	74a2                	ld	s1,40(sp)
    8000436c:	7902                	ld	s2,32(sp)
    8000436e:	69e2                	ld	s3,24(sp)
    80004370:	6a42                	ld	s4,16(sp)
    80004372:	6aa2                	ld	s5,8(sp)
    80004374:	6b02                	ld	s6,0(sp)
    80004376:	6121                	addi	sp,sp,64
    80004378:	8082                	ret
    8000437a:	8082                	ret

000000008000437c <initlog>:
{
    8000437c:	7179                	addi	sp,sp,-48
    8000437e:	f406                	sd	ra,40(sp)
    80004380:	f022                	sd	s0,32(sp)
    80004382:	ec26                	sd	s1,24(sp)
    80004384:	e84a                	sd	s2,16(sp)
    80004386:	e44e                	sd	s3,8(sp)
    80004388:	1800                	addi	s0,sp,48
    8000438a:	892a                	mv	s2,a0
    8000438c:	89ae                	mv	s3,a1
  initlock(&log.lock, "log");
    8000438e:	0001d497          	auipc	s1,0x1d
    80004392:	1a248493          	addi	s1,s1,418 # 80021530 <log>
    80004396:	00004597          	auipc	a1,0x4
    8000439a:	2f258593          	addi	a1,a1,754 # 80008688 <syscalls+0x1e0>
    8000439e:	8526                	mv	a0,s1
    800043a0:	ffffc097          	auipc	ra,0xffffc
    800043a4:	7b4080e7          	jalr	1972(ra) # 80000b54 <initlock>
  log.start = sb->logstart;
    800043a8:	0149a583          	lw	a1,20(s3)
    800043ac:	cc8c                	sw	a1,24(s1)
  log.size = sb->nlog;
    800043ae:	0109a783          	lw	a5,16(s3)
    800043b2:	ccdc                	sw	a5,28(s1)
  log.dev = dev;
    800043b4:	0324a423          	sw	s2,40(s1)
  struct buf *buf = bread(log.dev, log.start);
    800043b8:	854a                	mv	a0,s2
    800043ba:	fffff097          	auipc	ra,0xfffff
    800043be:	e92080e7          	jalr	-366(ra) # 8000324c <bread>
  log.lh.n = lh->n;
    800043c2:	4d3c                	lw	a5,88(a0)
    800043c4:	d4dc                	sw	a5,44(s1)
  for (i = 0; i < log.lh.n; i++) {
    800043c6:	02f05563          	blez	a5,800043f0 <initlog+0x74>
    800043ca:	05c50713          	addi	a4,a0,92
    800043ce:	0001d697          	auipc	a3,0x1d
    800043d2:	19268693          	addi	a3,a3,402 # 80021560 <log+0x30>
    800043d6:	37fd                	addiw	a5,a5,-1
    800043d8:	1782                	slli	a5,a5,0x20
    800043da:	9381                	srli	a5,a5,0x20
    800043dc:	078a                	slli	a5,a5,0x2
    800043de:	06050613          	addi	a2,a0,96
    800043e2:	97b2                	add	a5,a5,a2
    log.lh.block[i] = lh->block[i];
    800043e4:	4310                	lw	a2,0(a4)
    800043e6:	c290                	sw	a2,0(a3)
  for (i = 0; i < log.lh.n; i++) {
    800043e8:	0711                	addi	a4,a4,4
    800043ea:	0691                	addi	a3,a3,4
    800043ec:	fef71ce3          	bne	a4,a5,800043e4 <initlog+0x68>
  brelse(buf);
    800043f0:	fffff097          	auipc	ra,0xfffff
    800043f4:	f8c080e7          	jalr	-116(ra) # 8000337c <brelse>

static void
recover_from_log(void)
{
  read_head();
  install_trans(1); // if committed, copy from log to disk
    800043f8:	4505                	li	a0,1
    800043fa:	00000097          	auipc	ra,0x0
    800043fe:	ebe080e7          	jalr	-322(ra) # 800042b8 <install_trans>
  log.lh.n = 0;
    80004402:	0001d797          	auipc	a5,0x1d
    80004406:	1407ad23          	sw	zero,346(a5) # 8002155c <log+0x2c>
  write_head(); // clear the log
    8000440a:	00000097          	auipc	ra,0x0
    8000440e:	e34080e7          	jalr	-460(ra) # 8000423e <write_head>
}
    80004412:	70a2                	ld	ra,40(sp)
    80004414:	7402                	ld	s0,32(sp)
    80004416:	64e2                	ld	s1,24(sp)
    80004418:	6942                	ld	s2,16(sp)
    8000441a:	69a2                	ld	s3,8(sp)
    8000441c:	6145                	addi	sp,sp,48
    8000441e:	8082                	ret

0000000080004420 <begin_op>:
}

// called at the start of each FS system call.
void
begin_op(void)
{
    80004420:	1101                	addi	sp,sp,-32
    80004422:	ec06                	sd	ra,24(sp)
    80004424:	e822                	sd	s0,16(sp)
    80004426:	e426                	sd	s1,8(sp)
    80004428:	e04a                	sd	s2,0(sp)
    8000442a:	1000                	addi	s0,sp,32
  acquire(&log.lock);
    8000442c:	0001d517          	auipc	a0,0x1d
    80004430:	10450513          	addi	a0,a0,260 # 80021530 <log>
    80004434:	ffffc097          	auipc	ra,0xffffc
    80004438:	7b0080e7          	jalr	1968(ra) # 80000be4 <acquire>
  while(1){
    if(log.committing){
    8000443c:	0001d497          	auipc	s1,0x1d
    80004440:	0f448493          	addi	s1,s1,244 # 80021530 <log>
      sleep(&log, &log.lock);
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGSIZE){
    80004444:	4979                	li	s2,30
    80004446:	a039                	j	80004454 <begin_op+0x34>
      sleep(&log, &log.lock);
    80004448:	85a6                	mv	a1,s1
    8000444a:	8526                	mv	a0,s1
    8000444c:	ffffe097          	auipc	ra,0xffffe
    80004450:	ffe080e7          	jalr	-2(ra) # 8000244a <sleep>
    if(log.committing){
    80004454:	50dc                	lw	a5,36(s1)
    80004456:	fbed                	bnez	a5,80004448 <begin_op+0x28>
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGSIZE){
    80004458:	509c                	lw	a5,32(s1)
    8000445a:	0017871b          	addiw	a4,a5,1
    8000445e:	0007069b          	sext.w	a3,a4
    80004462:	0027179b          	slliw	a5,a4,0x2
    80004466:	9fb9                	addw	a5,a5,a4
    80004468:	0017979b          	slliw	a5,a5,0x1
    8000446c:	54d8                	lw	a4,44(s1)
    8000446e:	9fb9                	addw	a5,a5,a4
    80004470:	00f95963          	bge	s2,a5,80004482 <begin_op+0x62>
      // this op might exhaust log space; wait for commit.
      sleep(&log, &log.lock);
    80004474:	85a6                	mv	a1,s1
    80004476:	8526                	mv	a0,s1
    80004478:	ffffe097          	auipc	ra,0xffffe
    8000447c:	fd2080e7          	jalr	-46(ra) # 8000244a <sleep>
    80004480:	bfd1                	j	80004454 <begin_op+0x34>
    } else {
      log.outstanding += 1;
    80004482:	0001d517          	auipc	a0,0x1d
    80004486:	0ae50513          	addi	a0,a0,174 # 80021530 <log>
    8000448a:	d114                	sw	a3,32(a0)
      release(&log.lock);
    8000448c:	ffffd097          	auipc	ra,0xffffd
    80004490:	80c080e7          	jalr	-2036(ra) # 80000c98 <release>
      break;
    }
  }
}
    80004494:	60e2                	ld	ra,24(sp)
    80004496:	6442                	ld	s0,16(sp)
    80004498:	64a2                	ld	s1,8(sp)
    8000449a:	6902                	ld	s2,0(sp)
    8000449c:	6105                	addi	sp,sp,32
    8000449e:	8082                	ret

00000000800044a0 <end_op>:

// called at the end of each FS system call.
// commits if this was the last outstanding operation.
void
end_op(void)
{
    800044a0:	7139                	addi	sp,sp,-64
    800044a2:	fc06                	sd	ra,56(sp)
    800044a4:	f822                	sd	s0,48(sp)
    800044a6:	f426                	sd	s1,40(sp)
    800044a8:	f04a                	sd	s2,32(sp)
    800044aa:	ec4e                	sd	s3,24(sp)
    800044ac:	e852                	sd	s4,16(sp)
    800044ae:	e456                	sd	s5,8(sp)
    800044b0:	0080                	addi	s0,sp,64
  int do_commit = 0;

  acquire(&log.lock);
    800044b2:	0001d497          	auipc	s1,0x1d
    800044b6:	07e48493          	addi	s1,s1,126 # 80021530 <log>
    800044ba:	8526                	mv	a0,s1
    800044bc:	ffffc097          	auipc	ra,0xffffc
    800044c0:	728080e7          	jalr	1832(ra) # 80000be4 <acquire>
  log.outstanding -= 1;
    800044c4:	509c                	lw	a5,32(s1)
    800044c6:	37fd                	addiw	a5,a5,-1
    800044c8:	0007891b          	sext.w	s2,a5
    800044cc:	d09c                	sw	a5,32(s1)
  if(log.committing)
    800044ce:	50dc                	lw	a5,36(s1)
    800044d0:	efb9                	bnez	a5,8000452e <end_op+0x8e>
    panic("log.committing");
  if(log.outstanding == 0){
    800044d2:	06091663          	bnez	s2,8000453e <end_op+0x9e>
    do_commit = 1;
    log.committing = 1;
    800044d6:	0001d497          	auipc	s1,0x1d
    800044da:	05a48493          	addi	s1,s1,90 # 80021530 <log>
    800044de:	4785                	li	a5,1
    800044e0:	d0dc                	sw	a5,36(s1)
    // begin_op() may be waiting for log space,
    // and decrementing log.outstanding has decreased
    // the amount of reserved space.
    wakeup(&log);
  }
  release(&log.lock);
    800044e2:	8526                	mv	a0,s1
    800044e4:	ffffc097          	auipc	ra,0xffffc
    800044e8:	7b4080e7          	jalr	1972(ra) # 80000c98 <release>
}

static void
commit()
{
  if (log.lh.n > 0) {
    800044ec:	54dc                	lw	a5,44(s1)
    800044ee:	06f04763          	bgtz	a5,8000455c <end_op+0xbc>
    acquire(&log.lock);
    800044f2:	0001d497          	auipc	s1,0x1d
    800044f6:	03e48493          	addi	s1,s1,62 # 80021530 <log>
    800044fa:	8526                	mv	a0,s1
    800044fc:	ffffc097          	auipc	ra,0xffffc
    80004500:	6e8080e7          	jalr	1768(ra) # 80000be4 <acquire>
    log.committing = 0;
    80004504:	0204a223          	sw	zero,36(s1)
    wakeup(&log);
    80004508:	8526                	mv	a0,s1
    8000450a:	ffffe097          	auipc	ra,0xffffe
    8000450e:	0cc080e7          	jalr	204(ra) # 800025d6 <wakeup>
    release(&log.lock);
    80004512:	8526                	mv	a0,s1
    80004514:	ffffc097          	auipc	ra,0xffffc
    80004518:	784080e7          	jalr	1924(ra) # 80000c98 <release>
}
    8000451c:	70e2                	ld	ra,56(sp)
    8000451e:	7442                	ld	s0,48(sp)
    80004520:	74a2                	ld	s1,40(sp)
    80004522:	7902                	ld	s2,32(sp)
    80004524:	69e2                	ld	s3,24(sp)
    80004526:	6a42                	ld	s4,16(sp)
    80004528:	6aa2                	ld	s5,8(sp)
    8000452a:	6121                	addi	sp,sp,64
    8000452c:	8082                	ret
    panic("log.committing");
    8000452e:	00004517          	auipc	a0,0x4
    80004532:	16250513          	addi	a0,a0,354 # 80008690 <syscalls+0x1e8>
    80004536:	ffffc097          	auipc	ra,0xffffc
    8000453a:	008080e7          	jalr	8(ra) # 8000053e <panic>
    wakeup(&log);
    8000453e:	0001d497          	auipc	s1,0x1d
    80004542:	ff248493          	addi	s1,s1,-14 # 80021530 <log>
    80004546:	8526                	mv	a0,s1
    80004548:	ffffe097          	auipc	ra,0xffffe
    8000454c:	08e080e7          	jalr	142(ra) # 800025d6 <wakeup>
  release(&log.lock);
    80004550:	8526                	mv	a0,s1
    80004552:	ffffc097          	auipc	ra,0xffffc
    80004556:	746080e7          	jalr	1862(ra) # 80000c98 <release>
  if(do_commit){
    8000455a:	b7c9                	j	8000451c <end_op+0x7c>
  for (tail = 0; tail < log.lh.n; tail++) {
    8000455c:	0001da97          	auipc	s5,0x1d
    80004560:	004a8a93          	addi	s5,s5,4 # 80021560 <log+0x30>
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
    80004564:	0001da17          	auipc	s4,0x1d
    80004568:	fcca0a13          	addi	s4,s4,-52 # 80021530 <log>
    8000456c:	018a2583          	lw	a1,24(s4)
    80004570:	012585bb          	addw	a1,a1,s2
    80004574:	2585                	addiw	a1,a1,1
    80004576:	028a2503          	lw	a0,40(s4)
    8000457a:	fffff097          	auipc	ra,0xfffff
    8000457e:	cd2080e7          	jalr	-814(ra) # 8000324c <bread>
    80004582:	84aa                	mv	s1,a0
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
    80004584:	000aa583          	lw	a1,0(s5)
    80004588:	028a2503          	lw	a0,40(s4)
    8000458c:	fffff097          	auipc	ra,0xfffff
    80004590:	cc0080e7          	jalr	-832(ra) # 8000324c <bread>
    80004594:	89aa                	mv	s3,a0
    memmove(to->data, from->data, BSIZE);
    80004596:	40000613          	li	a2,1024
    8000459a:	05850593          	addi	a1,a0,88
    8000459e:	05848513          	addi	a0,s1,88
    800045a2:	ffffc097          	auipc	ra,0xffffc
    800045a6:	79e080e7          	jalr	1950(ra) # 80000d40 <memmove>
    bwrite(to);  // write the log
    800045aa:	8526                	mv	a0,s1
    800045ac:	fffff097          	auipc	ra,0xfffff
    800045b0:	d92080e7          	jalr	-622(ra) # 8000333e <bwrite>
    brelse(from);
    800045b4:	854e                	mv	a0,s3
    800045b6:	fffff097          	auipc	ra,0xfffff
    800045ba:	dc6080e7          	jalr	-570(ra) # 8000337c <brelse>
    brelse(to);
    800045be:	8526                	mv	a0,s1
    800045c0:	fffff097          	auipc	ra,0xfffff
    800045c4:	dbc080e7          	jalr	-580(ra) # 8000337c <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    800045c8:	2905                	addiw	s2,s2,1
    800045ca:	0a91                	addi	s5,s5,4
    800045cc:	02ca2783          	lw	a5,44(s4)
    800045d0:	f8f94ee3          	blt	s2,a5,8000456c <end_op+0xcc>
    write_log();     // Write modified blocks from cache to log
    write_head();    // Write header to disk -- the real commit
    800045d4:	00000097          	auipc	ra,0x0
    800045d8:	c6a080e7          	jalr	-918(ra) # 8000423e <write_head>
    install_trans(0); // Now install writes to home locations
    800045dc:	4501                	li	a0,0
    800045de:	00000097          	auipc	ra,0x0
    800045e2:	cda080e7          	jalr	-806(ra) # 800042b8 <install_trans>
    log.lh.n = 0;
    800045e6:	0001d797          	auipc	a5,0x1d
    800045ea:	f607ab23          	sw	zero,-138(a5) # 8002155c <log+0x2c>
    write_head();    // Erase the transaction from the log
    800045ee:	00000097          	auipc	ra,0x0
    800045f2:	c50080e7          	jalr	-944(ra) # 8000423e <write_head>
    800045f6:	bdf5                	j	800044f2 <end_op+0x52>

00000000800045f8 <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
    800045f8:	1101                	addi	sp,sp,-32
    800045fa:	ec06                	sd	ra,24(sp)
    800045fc:	e822                	sd	s0,16(sp)
    800045fe:	e426                	sd	s1,8(sp)
    80004600:	e04a                	sd	s2,0(sp)
    80004602:	1000                	addi	s0,sp,32
    80004604:	84aa                	mv	s1,a0
  int i;

  acquire(&log.lock);
    80004606:	0001d917          	auipc	s2,0x1d
    8000460a:	f2a90913          	addi	s2,s2,-214 # 80021530 <log>
    8000460e:	854a                	mv	a0,s2
    80004610:	ffffc097          	auipc	ra,0xffffc
    80004614:	5d4080e7          	jalr	1492(ra) # 80000be4 <acquire>
  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
    80004618:	02c92603          	lw	a2,44(s2)
    8000461c:	47f5                	li	a5,29
    8000461e:	06c7c563          	blt	a5,a2,80004688 <log_write+0x90>
    80004622:	0001d797          	auipc	a5,0x1d
    80004626:	f2a7a783          	lw	a5,-214(a5) # 8002154c <log+0x1c>
    8000462a:	37fd                	addiw	a5,a5,-1
    8000462c:	04f65e63          	bge	a2,a5,80004688 <log_write+0x90>
    panic("too big a transaction");
  if (log.outstanding < 1)
    80004630:	0001d797          	auipc	a5,0x1d
    80004634:	f207a783          	lw	a5,-224(a5) # 80021550 <log+0x20>
    80004638:	06f05063          	blez	a5,80004698 <log_write+0xa0>
    panic("log_write outside of trans");

  for (i = 0; i < log.lh.n; i++) {
    8000463c:	4781                	li	a5,0
    8000463e:	06c05563          	blez	a2,800046a8 <log_write+0xb0>
    if (log.lh.block[i] == b->blockno)   // log absorption
    80004642:	44cc                	lw	a1,12(s1)
    80004644:	0001d717          	auipc	a4,0x1d
    80004648:	f1c70713          	addi	a4,a4,-228 # 80021560 <log+0x30>
  for (i = 0; i < log.lh.n; i++) {
    8000464c:	4781                	li	a5,0
    if (log.lh.block[i] == b->blockno)   // log absorption
    8000464e:	4314                	lw	a3,0(a4)
    80004650:	04b68c63          	beq	a3,a1,800046a8 <log_write+0xb0>
  for (i = 0; i < log.lh.n; i++) {
    80004654:	2785                	addiw	a5,a5,1
    80004656:	0711                	addi	a4,a4,4
    80004658:	fef61be3          	bne	a2,a5,8000464e <log_write+0x56>
      break;
  }
  log.lh.block[i] = b->blockno;
    8000465c:	0621                	addi	a2,a2,8
    8000465e:	060a                	slli	a2,a2,0x2
    80004660:	0001d797          	auipc	a5,0x1d
    80004664:	ed078793          	addi	a5,a5,-304 # 80021530 <log>
    80004668:	963e                	add	a2,a2,a5
    8000466a:	44dc                	lw	a5,12(s1)
    8000466c:	ca1c                	sw	a5,16(a2)
  if (i == log.lh.n) {  // Add new block to log?
    bpin(b);
    8000466e:	8526                	mv	a0,s1
    80004670:	fffff097          	auipc	ra,0xfffff
    80004674:	daa080e7          	jalr	-598(ra) # 8000341a <bpin>
    log.lh.n++;
    80004678:	0001d717          	auipc	a4,0x1d
    8000467c:	eb870713          	addi	a4,a4,-328 # 80021530 <log>
    80004680:	575c                	lw	a5,44(a4)
    80004682:	2785                	addiw	a5,a5,1
    80004684:	d75c                	sw	a5,44(a4)
    80004686:	a835                	j	800046c2 <log_write+0xca>
    panic("too big a transaction");
    80004688:	00004517          	auipc	a0,0x4
    8000468c:	01850513          	addi	a0,a0,24 # 800086a0 <syscalls+0x1f8>
    80004690:	ffffc097          	auipc	ra,0xffffc
    80004694:	eae080e7          	jalr	-338(ra) # 8000053e <panic>
    panic("log_write outside of trans");
    80004698:	00004517          	auipc	a0,0x4
    8000469c:	02050513          	addi	a0,a0,32 # 800086b8 <syscalls+0x210>
    800046a0:	ffffc097          	auipc	ra,0xffffc
    800046a4:	e9e080e7          	jalr	-354(ra) # 8000053e <panic>
  log.lh.block[i] = b->blockno;
    800046a8:	00878713          	addi	a4,a5,8
    800046ac:	00271693          	slli	a3,a4,0x2
    800046b0:	0001d717          	auipc	a4,0x1d
    800046b4:	e8070713          	addi	a4,a4,-384 # 80021530 <log>
    800046b8:	9736                	add	a4,a4,a3
    800046ba:	44d4                	lw	a3,12(s1)
    800046bc:	cb14                	sw	a3,16(a4)
  if (i == log.lh.n) {  // Add new block to log?
    800046be:	faf608e3          	beq	a2,a5,8000466e <log_write+0x76>
  }
  release(&log.lock);
    800046c2:	0001d517          	auipc	a0,0x1d
    800046c6:	e6e50513          	addi	a0,a0,-402 # 80021530 <log>
    800046ca:	ffffc097          	auipc	ra,0xffffc
    800046ce:	5ce080e7          	jalr	1486(ra) # 80000c98 <release>
}
    800046d2:	60e2                	ld	ra,24(sp)
    800046d4:	6442                	ld	s0,16(sp)
    800046d6:	64a2                	ld	s1,8(sp)
    800046d8:	6902                	ld	s2,0(sp)
    800046da:	6105                	addi	sp,sp,32
    800046dc:	8082                	ret

00000000800046de <initsleeplock>:
#include "proc.h"
#include "sleeplock.h"

void
initsleeplock(struct sleeplock *lk, char *name)
{
    800046de:	1101                	addi	sp,sp,-32
    800046e0:	ec06                	sd	ra,24(sp)
    800046e2:	e822                	sd	s0,16(sp)
    800046e4:	e426                	sd	s1,8(sp)
    800046e6:	e04a                	sd	s2,0(sp)
    800046e8:	1000                	addi	s0,sp,32
    800046ea:	84aa                	mv	s1,a0
    800046ec:	892e                	mv	s2,a1
  initlock(&lk->lk, "sleep lock");
    800046ee:	00004597          	auipc	a1,0x4
    800046f2:	fea58593          	addi	a1,a1,-22 # 800086d8 <syscalls+0x230>
    800046f6:	0521                	addi	a0,a0,8
    800046f8:	ffffc097          	auipc	ra,0xffffc
    800046fc:	45c080e7          	jalr	1116(ra) # 80000b54 <initlock>
  lk->name = name;
    80004700:	0324b023          	sd	s2,32(s1)
  lk->locked = 0;
    80004704:	0004a023          	sw	zero,0(s1)
  lk->pid = 0;
    80004708:	0204a423          	sw	zero,40(s1)
}
    8000470c:	60e2                	ld	ra,24(sp)
    8000470e:	6442                	ld	s0,16(sp)
    80004710:	64a2                	ld	s1,8(sp)
    80004712:	6902                	ld	s2,0(sp)
    80004714:	6105                	addi	sp,sp,32
    80004716:	8082                	ret

0000000080004718 <acquiresleep>:

void
acquiresleep(struct sleeplock *lk)
{
    80004718:	1101                	addi	sp,sp,-32
    8000471a:	ec06                	sd	ra,24(sp)
    8000471c:	e822                	sd	s0,16(sp)
    8000471e:	e426                	sd	s1,8(sp)
    80004720:	e04a                	sd	s2,0(sp)
    80004722:	1000                	addi	s0,sp,32
    80004724:	84aa                	mv	s1,a0
  acquire(&lk->lk);
    80004726:	00850913          	addi	s2,a0,8
    8000472a:	854a                	mv	a0,s2
    8000472c:	ffffc097          	auipc	ra,0xffffc
    80004730:	4b8080e7          	jalr	1208(ra) # 80000be4 <acquire>
  while (lk->locked) {
    80004734:	409c                	lw	a5,0(s1)
    80004736:	cb89                	beqz	a5,80004748 <acquiresleep+0x30>
    sleep(lk, &lk->lk);
    80004738:	85ca                	mv	a1,s2
    8000473a:	8526                	mv	a0,s1
    8000473c:	ffffe097          	auipc	ra,0xffffe
    80004740:	d0e080e7          	jalr	-754(ra) # 8000244a <sleep>
  while (lk->locked) {
    80004744:	409c                	lw	a5,0(s1)
    80004746:	fbed                	bnez	a5,80004738 <acquiresleep+0x20>
  }
  lk->locked = 1;
    80004748:	4785                	li	a5,1
    8000474a:	c09c                	sw	a5,0(s1)
  lk->pid = myproc()->pid;
    8000474c:	ffffd097          	auipc	ra,0xffffd
    80004750:	640080e7          	jalr	1600(ra) # 80001d8c <myproc>
    80004754:	591c                	lw	a5,48(a0)
    80004756:	d49c                	sw	a5,40(s1)
  release(&lk->lk);
    80004758:	854a                	mv	a0,s2
    8000475a:	ffffc097          	auipc	ra,0xffffc
    8000475e:	53e080e7          	jalr	1342(ra) # 80000c98 <release>
}
    80004762:	60e2                	ld	ra,24(sp)
    80004764:	6442                	ld	s0,16(sp)
    80004766:	64a2                	ld	s1,8(sp)
    80004768:	6902                	ld	s2,0(sp)
    8000476a:	6105                	addi	sp,sp,32
    8000476c:	8082                	ret

000000008000476e <releasesleep>:

void
releasesleep(struct sleeplock *lk)
{
    8000476e:	1101                	addi	sp,sp,-32
    80004770:	ec06                	sd	ra,24(sp)
    80004772:	e822                	sd	s0,16(sp)
    80004774:	e426                	sd	s1,8(sp)
    80004776:	e04a                	sd	s2,0(sp)
    80004778:	1000                	addi	s0,sp,32
    8000477a:	84aa                	mv	s1,a0
  acquire(&lk->lk);
    8000477c:	00850913          	addi	s2,a0,8
    80004780:	854a                	mv	a0,s2
    80004782:	ffffc097          	auipc	ra,0xffffc
    80004786:	462080e7          	jalr	1122(ra) # 80000be4 <acquire>
  lk->locked = 0;
    8000478a:	0004a023          	sw	zero,0(s1)
  lk->pid = 0;
    8000478e:	0204a423          	sw	zero,40(s1)
  wakeup(lk);
    80004792:	8526                	mv	a0,s1
    80004794:	ffffe097          	auipc	ra,0xffffe
    80004798:	e42080e7          	jalr	-446(ra) # 800025d6 <wakeup>
  release(&lk->lk);
    8000479c:	854a                	mv	a0,s2
    8000479e:	ffffc097          	auipc	ra,0xffffc
    800047a2:	4fa080e7          	jalr	1274(ra) # 80000c98 <release>
}
    800047a6:	60e2                	ld	ra,24(sp)
    800047a8:	6442                	ld	s0,16(sp)
    800047aa:	64a2                	ld	s1,8(sp)
    800047ac:	6902                	ld	s2,0(sp)
    800047ae:	6105                	addi	sp,sp,32
    800047b0:	8082                	ret

00000000800047b2 <holdingsleep>:

int
holdingsleep(struct sleeplock *lk)
{
    800047b2:	7179                	addi	sp,sp,-48
    800047b4:	f406                	sd	ra,40(sp)
    800047b6:	f022                	sd	s0,32(sp)
    800047b8:	ec26                	sd	s1,24(sp)
    800047ba:	e84a                	sd	s2,16(sp)
    800047bc:	e44e                	sd	s3,8(sp)
    800047be:	1800                	addi	s0,sp,48
    800047c0:	84aa                	mv	s1,a0
  int r;
  
  acquire(&lk->lk);
    800047c2:	00850913          	addi	s2,a0,8
    800047c6:	854a                	mv	a0,s2
    800047c8:	ffffc097          	auipc	ra,0xffffc
    800047cc:	41c080e7          	jalr	1052(ra) # 80000be4 <acquire>
  r = lk->locked && (lk->pid == myproc()->pid);
    800047d0:	409c                	lw	a5,0(s1)
    800047d2:	ef99                	bnez	a5,800047f0 <holdingsleep+0x3e>
    800047d4:	4481                	li	s1,0
  release(&lk->lk);
    800047d6:	854a                	mv	a0,s2
    800047d8:	ffffc097          	auipc	ra,0xffffc
    800047dc:	4c0080e7          	jalr	1216(ra) # 80000c98 <release>
  return r;
}
    800047e0:	8526                	mv	a0,s1
    800047e2:	70a2                	ld	ra,40(sp)
    800047e4:	7402                	ld	s0,32(sp)
    800047e6:	64e2                	ld	s1,24(sp)
    800047e8:	6942                	ld	s2,16(sp)
    800047ea:	69a2                	ld	s3,8(sp)
    800047ec:	6145                	addi	sp,sp,48
    800047ee:	8082                	ret
  r = lk->locked && (lk->pid == myproc()->pid);
    800047f0:	0284a983          	lw	s3,40(s1)
    800047f4:	ffffd097          	auipc	ra,0xffffd
    800047f8:	598080e7          	jalr	1432(ra) # 80001d8c <myproc>
    800047fc:	5904                	lw	s1,48(a0)
    800047fe:	413484b3          	sub	s1,s1,s3
    80004802:	0014b493          	seqz	s1,s1
    80004806:	bfc1                	j	800047d6 <holdingsleep+0x24>

0000000080004808 <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
    80004808:	1141                	addi	sp,sp,-16
    8000480a:	e406                	sd	ra,8(sp)
    8000480c:	e022                	sd	s0,0(sp)
    8000480e:	0800                	addi	s0,sp,16
  initlock(&ftable.lock, "ftable");
    80004810:	00004597          	auipc	a1,0x4
    80004814:	ed858593          	addi	a1,a1,-296 # 800086e8 <syscalls+0x240>
    80004818:	0001d517          	auipc	a0,0x1d
    8000481c:	e6050513          	addi	a0,a0,-416 # 80021678 <ftable>
    80004820:	ffffc097          	auipc	ra,0xffffc
    80004824:	334080e7          	jalr	820(ra) # 80000b54 <initlock>
}
    80004828:	60a2                	ld	ra,8(sp)
    8000482a:	6402                	ld	s0,0(sp)
    8000482c:	0141                	addi	sp,sp,16
    8000482e:	8082                	ret

0000000080004830 <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
    80004830:	1101                	addi	sp,sp,-32
    80004832:	ec06                	sd	ra,24(sp)
    80004834:	e822                	sd	s0,16(sp)
    80004836:	e426                	sd	s1,8(sp)
    80004838:	1000                	addi	s0,sp,32
  struct file *f;

  acquire(&ftable.lock);
    8000483a:	0001d517          	auipc	a0,0x1d
    8000483e:	e3e50513          	addi	a0,a0,-450 # 80021678 <ftable>
    80004842:	ffffc097          	auipc	ra,0xffffc
    80004846:	3a2080e7          	jalr	930(ra) # 80000be4 <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    8000484a:	0001d497          	auipc	s1,0x1d
    8000484e:	e4648493          	addi	s1,s1,-442 # 80021690 <ftable+0x18>
    80004852:	0001e717          	auipc	a4,0x1e
    80004856:	dde70713          	addi	a4,a4,-546 # 80022630 <ftable+0xfb8>
    if(f->ref == 0){
    8000485a:	40dc                	lw	a5,4(s1)
    8000485c:	cf99                	beqz	a5,8000487a <filealloc+0x4a>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    8000485e:	02848493          	addi	s1,s1,40
    80004862:	fee49ce3          	bne	s1,a4,8000485a <filealloc+0x2a>
      f->ref = 1;
      release(&ftable.lock);
      return f;
    }
  }
  release(&ftable.lock);
    80004866:	0001d517          	auipc	a0,0x1d
    8000486a:	e1250513          	addi	a0,a0,-494 # 80021678 <ftable>
    8000486e:	ffffc097          	auipc	ra,0xffffc
    80004872:	42a080e7          	jalr	1066(ra) # 80000c98 <release>
  return 0;
    80004876:	4481                	li	s1,0
    80004878:	a819                	j	8000488e <filealloc+0x5e>
      f->ref = 1;
    8000487a:	4785                	li	a5,1
    8000487c:	c0dc                	sw	a5,4(s1)
      release(&ftable.lock);
    8000487e:	0001d517          	auipc	a0,0x1d
    80004882:	dfa50513          	addi	a0,a0,-518 # 80021678 <ftable>
    80004886:	ffffc097          	auipc	ra,0xffffc
    8000488a:	412080e7          	jalr	1042(ra) # 80000c98 <release>
}
    8000488e:	8526                	mv	a0,s1
    80004890:	60e2                	ld	ra,24(sp)
    80004892:	6442                	ld	s0,16(sp)
    80004894:	64a2                	ld	s1,8(sp)
    80004896:	6105                	addi	sp,sp,32
    80004898:	8082                	ret

000000008000489a <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
    8000489a:	1101                	addi	sp,sp,-32
    8000489c:	ec06                	sd	ra,24(sp)
    8000489e:	e822                	sd	s0,16(sp)
    800048a0:	e426                	sd	s1,8(sp)
    800048a2:	1000                	addi	s0,sp,32
    800048a4:	84aa                	mv	s1,a0
  acquire(&ftable.lock);
    800048a6:	0001d517          	auipc	a0,0x1d
    800048aa:	dd250513          	addi	a0,a0,-558 # 80021678 <ftable>
    800048ae:	ffffc097          	auipc	ra,0xffffc
    800048b2:	336080e7          	jalr	822(ra) # 80000be4 <acquire>
  if(f->ref < 1)
    800048b6:	40dc                	lw	a5,4(s1)
    800048b8:	02f05263          	blez	a5,800048dc <filedup+0x42>
    panic("filedup");
  f->ref++;
    800048bc:	2785                	addiw	a5,a5,1
    800048be:	c0dc                	sw	a5,4(s1)
  release(&ftable.lock);
    800048c0:	0001d517          	auipc	a0,0x1d
    800048c4:	db850513          	addi	a0,a0,-584 # 80021678 <ftable>
    800048c8:	ffffc097          	auipc	ra,0xffffc
    800048cc:	3d0080e7          	jalr	976(ra) # 80000c98 <release>
  return f;
}
    800048d0:	8526                	mv	a0,s1
    800048d2:	60e2                	ld	ra,24(sp)
    800048d4:	6442                	ld	s0,16(sp)
    800048d6:	64a2                	ld	s1,8(sp)
    800048d8:	6105                	addi	sp,sp,32
    800048da:	8082                	ret
    panic("filedup");
    800048dc:	00004517          	auipc	a0,0x4
    800048e0:	e1450513          	addi	a0,a0,-492 # 800086f0 <syscalls+0x248>
    800048e4:	ffffc097          	auipc	ra,0xffffc
    800048e8:	c5a080e7          	jalr	-934(ra) # 8000053e <panic>

00000000800048ec <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
    800048ec:	7139                	addi	sp,sp,-64
    800048ee:	fc06                	sd	ra,56(sp)
    800048f0:	f822                	sd	s0,48(sp)
    800048f2:	f426                	sd	s1,40(sp)
    800048f4:	f04a                	sd	s2,32(sp)
    800048f6:	ec4e                	sd	s3,24(sp)
    800048f8:	e852                	sd	s4,16(sp)
    800048fa:	e456                	sd	s5,8(sp)
    800048fc:	0080                	addi	s0,sp,64
    800048fe:	84aa                	mv	s1,a0
  struct file ff;

  acquire(&ftable.lock);
    80004900:	0001d517          	auipc	a0,0x1d
    80004904:	d7850513          	addi	a0,a0,-648 # 80021678 <ftable>
    80004908:	ffffc097          	auipc	ra,0xffffc
    8000490c:	2dc080e7          	jalr	732(ra) # 80000be4 <acquire>
  if(f->ref < 1)
    80004910:	40dc                	lw	a5,4(s1)
    80004912:	06f05163          	blez	a5,80004974 <fileclose+0x88>
    panic("fileclose");
  if(--f->ref > 0){
    80004916:	37fd                	addiw	a5,a5,-1
    80004918:	0007871b          	sext.w	a4,a5
    8000491c:	c0dc                	sw	a5,4(s1)
    8000491e:	06e04363          	bgtz	a4,80004984 <fileclose+0x98>
    release(&ftable.lock);
    return;
  }
  ff = *f;
    80004922:	0004a903          	lw	s2,0(s1)
    80004926:	0094ca83          	lbu	s5,9(s1)
    8000492a:	0104ba03          	ld	s4,16(s1)
    8000492e:	0184b983          	ld	s3,24(s1)
  f->ref = 0;
    80004932:	0004a223          	sw	zero,4(s1)
  f->type = FD_NONE;
    80004936:	0004a023          	sw	zero,0(s1)
  release(&ftable.lock);
    8000493a:	0001d517          	auipc	a0,0x1d
    8000493e:	d3e50513          	addi	a0,a0,-706 # 80021678 <ftable>
    80004942:	ffffc097          	auipc	ra,0xffffc
    80004946:	356080e7          	jalr	854(ra) # 80000c98 <release>

  if(ff.type == FD_PIPE){
    8000494a:	4785                	li	a5,1
    8000494c:	04f90d63          	beq	s2,a5,800049a6 <fileclose+0xba>
    pipeclose(ff.pipe, ff.writable);
  } else if(ff.type == FD_INODE || ff.type == FD_DEVICE){
    80004950:	3979                	addiw	s2,s2,-2
    80004952:	4785                	li	a5,1
    80004954:	0527e063          	bltu	a5,s2,80004994 <fileclose+0xa8>
    begin_op();
    80004958:	00000097          	auipc	ra,0x0
    8000495c:	ac8080e7          	jalr	-1336(ra) # 80004420 <begin_op>
    iput(ff.ip);
    80004960:	854e                	mv	a0,s3
    80004962:	fffff097          	auipc	ra,0xfffff
    80004966:	2a6080e7          	jalr	678(ra) # 80003c08 <iput>
    end_op();
    8000496a:	00000097          	auipc	ra,0x0
    8000496e:	b36080e7          	jalr	-1226(ra) # 800044a0 <end_op>
    80004972:	a00d                	j	80004994 <fileclose+0xa8>
    panic("fileclose");
    80004974:	00004517          	auipc	a0,0x4
    80004978:	d8450513          	addi	a0,a0,-636 # 800086f8 <syscalls+0x250>
    8000497c:	ffffc097          	auipc	ra,0xffffc
    80004980:	bc2080e7          	jalr	-1086(ra) # 8000053e <panic>
    release(&ftable.lock);
    80004984:	0001d517          	auipc	a0,0x1d
    80004988:	cf450513          	addi	a0,a0,-780 # 80021678 <ftable>
    8000498c:	ffffc097          	auipc	ra,0xffffc
    80004990:	30c080e7          	jalr	780(ra) # 80000c98 <release>
  }
}
    80004994:	70e2                	ld	ra,56(sp)
    80004996:	7442                	ld	s0,48(sp)
    80004998:	74a2                	ld	s1,40(sp)
    8000499a:	7902                	ld	s2,32(sp)
    8000499c:	69e2                	ld	s3,24(sp)
    8000499e:	6a42                	ld	s4,16(sp)
    800049a0:	6aa2                	ld	s5,8(sp)
    800049a2:	6121                	addi	sp,sp,64
    800049a4:	8082                	ret
    pipeclose(ff.pipe, ff.writable);
    800049a6:	85d6                	mv	a1,s5
    800049a8:	8552                	mv	a0,s4
    800049aa:	00000097          	auipc	ra,0x0
    800049ae:	34c080e7          	jalr	844(ra) # 80004cf6 <pipeclose>
    800049b2:	b7cd                	j	80004994 <fileclose+0xa8>

00000000800049b4 <filestat>:

// Get metadata about file f.
// addr is a user virtual address, pointing to a struct stat.
int
filestat(struct file *f, uint64 addr)
{
    800049b4:	715d                	addi	sp,sp,-80
    800049b6:	e486                	sd	ra,72(sp)
    800049b8:	e0a2                	sd	s0,64(sp)
    800049ba:	fc26                	sd	s1,56(sp)
    800049bc:	f84a                	sd	s2,48(sp)
    800049be:	f44e                	sd	s3,40(sp)
    800049c0:	0880                	addi	s0,sp,80
    800049c2:	84aa                	mv	s1,a0
    800049c4:	89ae                	mv	s3,a1
  struct proc *p = myproc();
    800049c6:	ffffd097          	auipc	ra,0xffffd
    800049ca:	3c6080e7          	jalr	966(ra) # 80001d8c <myproc>
  struct stat st;
  
  if(f->type == FD_INODE || f->type == FD_DEVICE){
    800049ce:	409c                	lw	a5,0(s1)
    800049d0:	37f9                	addiw	a5,a5,-2
    800049d2:	4705                	li	a4,1
    800049d4:	04f76763          	bltu	a4,a5,80004a22 <filestat+0x6e>
    800049d8:	892a                	mv	s2,a0
    ilock(f->ip);
    800049da:	6c88                	ld	a0,24(s1)
    800049dc:	fffff097          	auipc	ra,0xfffff
    800049e0:	072080e7          	jalr	114(ra) # 80003a4e <ilock>
    stati(f->ip, &st);
    800049e4:	fb840593          	addi	a1,s0,-72
    800049e8:	6c88                	ld	a0,24(s1)
    800049ea:	fffff097          	auipc	ra,0xfffff
    800049ee:	2ee080e7          	jalr	750(ra) # 80003cd8 <stati>
    iunlock(f->ip);
    800049f2:	6c88                	ld	a0,24(s1)
    800049f4:	fffff097          	auipc	ra,0xfffff
    800049f8:	11c080e7          	jalr	284(ra) # 80003b10 <iunlock>
    if(copyout(p->pagetable, addr, (char *)&st, sizeof(st)) < 0)
    800049fc:	46e1                	li	a3,24
    800049fe:	fb840613          	addi	a2,s0,-72
    80004a02:	85ce                	mv	a1,s3
    80004a04:	05893503          	ld	a0,88(s2)
    80004a08:	ffffd097          	auipc	ra,0xffffd
    80004a0c:	c6a080e7          	jalr	-918(ra) # 80001672 <copyout>
    80004a10:	41f5551b          	sraiw	a0,a0,0x1f
      return -1;
    return 0;
  }
  return -1;
}
    80004a14:	60a6                	ld	ra,72(sp)
    80004a16:	6406                	ld	s0,64(sp)
    80004a18:	74e2                	ld	s1,56(sp)
    80004a1a:	7942                	ld	s2,48(sp)
    80004a1c:	79a2                	ld	s3,40(sp)
    80004a1e:	6161                	addi	sp,sp,80
    80004a20:	8082                	ret
  return -1;
    80004a22:	557d                	li	a0,-1
    80004a24:	bfc5                	j	80004a14 <filestat+0x60>

0000000080004a26 <fileread>:

// Read from file f.
// addr is a user virtual address.
int
fileread(struct file *f, uint64 addr, int n)
{
    80004a26:	7179                	addi	sp,sp,-48
    80004a28:	f406                	sd	ra,40(sp)
    80004a2a:	f022                	sd	s0,32(sp)
    80004a2c:	ec26                	sd	s1,24(sp)
    80004a2e:	e84a                	sd	s2,16(sp)
    80004a30:	e44e                	sd	s3,8(sp)
    80004a32:	1800                	addi	s0,sp,48
  int r = 0;

  if(f->readable == 0)
    80004a34:	00854783          	lbu	a5,8(a0)
    80004a38:	c3d5                	beqz	a5,80004adc <fileread+0xb6>
    80004a3a:	84aa                	mv	s1,a0
    80004a3c:	89ae                	mv	s3,a1
    80004a3e:	8932                	mv	s2,a2
    return -1;

  if(f->type == FD_PIPE){
    80004a40:	411c                	lw	a5,0(a0)
    80004a42:	4705                	li	a4,1
    80004a44:	04e78963          	beq	a5,a4,80004a96 <fileread+0x70>
    r = piperead(f->pipe, addr, n);
  } else if(f->type == FD_DEVICE){
    80004a48:	470d                	li	a4,3
    80004a4a:	04e78d63          	beq	a5,a4,80004aa4 <fileread+0x7e>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].read)
      return -1;
    r = devsw[f->major].read(1, addr, n);
  } else if(f->type == FD_INODE){
    80004a4e:	4709                	li	a4,2
    80004a50:	06e79e63          	bne	a5,a4,80004acc <fileread+0xa6>
    ilock(f->ip);
    80004a54:	6d08                	ld	a0,24(a0)
    80004a56:	fffff097          	auipc	ra,0xfffff
    80004a5a:	ff8080e7          	jalr	-8(ra) # 80003a4e <ilock>
    if((r = readi(f->ip, 1, addr, f->off, n)) > 0)
    80004a5e:	874a                	mv	a4,s2
    80004a60:	5094                	lw	a3,32(s1)
    80004a62:	864e                	mv	a2,s3
    80004a64:	4585                	li	a1,1
    80004a66:	6c88                	ld	a0,24(s1)
    80004a68:	fffff097          	auipc	ra,0xfffff
    80004a6c:	29a080e7          	jalr	666(ra) # 80003d02 <readi>
    80004a70:	892a                	mv	s2,a0
    80004a72:	00a05563          	blez	a0,80004a7c <fileread+0x56>
      f->off += r;
    80004a76:	509c                	lw	a5,32(s1)
    80004a78:	9fa9                	addw	a5,a5,a0
    80004a7a:	d09c                	sw	a5,32(s1)
    iunlock(f->ip);
    80004a7c:	6c88                	ld	a0,24(s1)
    80004a7e:	fffff097          	auipc	ra,0xfffff
    80004a82:	092080e7          	jalr	146(ra) # 80003b10 <iunlock>
  } else {
    panic("fileread");
  }

  return r;
}
    80004a86:	854a                	mv	a0,s2
    80004a88:	70a2                	ld	ra,40(sp)
    80004a8a:	7402                	ld	s0,32(sp)
    80004a8c:	64e2                	ld	s1,24(sp)
    80004a8e:	6942                	ld	s2,16(sp)
    80004a90:	69a2                	ld	s3,8(sp)
    80004a92:	6145                	addi	sp,sp,48
    80004a94:	8082                	ret
    r = piperead(f->pipe, addr, n);
    80004a96:	6908                	ld	a0,16(a0)
    80004a98:	00000097          	auipc	ra,0x0
    80004a9c:	3c8080e7          	jalr	968(ra) # 80004e60 <piperead>
    80004aa0:	892a                	mv	s2,a0
    80004aa2:	b7d5                	j	80004a86 <fileread+0x60>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].read)
    80004aa4:	02451783          	lh	a5,36(a0)
    80004aa8:	03079693          	slli	a3,a5,0x30
    80004aac:	92c1                	srli	a3,a3,0x30
    80004aae:	4725                	li	a4,9
    80004ab0:	02d76863          	bltu	a4,a3,80004ae0 <fileread+0xba>
    80004ab4:	0792                	slli	a5,a5,0x4
    80004ab6:	0001d717          	auipc	a4,0x1d
    80004aba:	b2270713          	addi	a4,a4,-1246 # 800215d8 <devsw>
    80004abe:	97ba                	add	a5,a5,a4
    80004ac0:	639c                	ld	a5,0(a5)
    80004ac2:	c38d                	beqz	a5,80004ae4 <fileread+0xbe>
    r = devsw[f->major].read(1, addr, n);
    80004ac4:	4505                	li	a0,1
    80004ac6:	9782                	jalr	a5
    80004ac8:	892a                	mv	s2,a0
    80004aca:	bf75                	j	80004a86 <fileread+0x60>
    panic("fileread");
    80004acc:	00004517          	auipc	a0,0x4
    80004ad0:	c3c50513          	addi	a0,a0,-964 # 80008708 <syscalls+0x260>
    80004ad4:	ffffc097          	auipc	ra,0xffffc
    80004ad8:	a6a080e7          	jalr	-1430(ra) # 8000053e <panic>
    return -1;
    80004adc:	597d                	li	s2,-1
    80004ade:	b765                	j	80004a86 <fileread+0x60>
      return -1;
    80004ae0:	597d                	li	s2,-1
    80004ae2:	b755                	j	80004a86 <fileread+0x60>
    80004ae4:	597d                	li	s2,-1
    80004ae6:	b745                	j	80004a86 <fileread+0x60>

0000000080004ae8 <filewrite>:

// Write to file f.
// addr is a user virtual address.
int
filewrite(struct file *f, uint64 addr, int n)
{
    80004ae8:	715d                	addi	sp,sp,-80
    80004aea:	e486                	sd	ra,72(sp)
    80004aec:	e0a2                	sd	s0,64(sp)
    80004aee:	fc26                	sd	s1,56(sp)
    80004af0:	f84a                	sd	s2,48(sp)
    80004af2:	f44e                	sd	s3,40(sp)
    80004af4:	f052                	sd	s4,32(sp)
    80004af6:	ec56                	sd	s5,24(sp)
    80004af8:	e85a                	sd	s6,16(sp)
    80004afa:	e45e                	sd	s7,8(sp)
    80004afc:	e062                	sd	s8,0(sp)
    80004afe:	0880                	addi	s0,sp,80
  int r, ret = 0;

  if(f->writable == 0)
    80004b00:	00954783          	lbu	a5,9(a0)
    80004b04:	10078663          	beqz	a5,80004c10 <filewrite+0x128>
    80004b08:	892a                	mv	s2,a0
    80004b0a:	8aae                	mv	s5,a1
    80004b0c:	8a32                	mv	s4,a2
    return -1;

  if(f->type == FD_PIPE){
    80004b0e:	411c                	lw	a5,0(a0)
    80004b10:	4705                	li	a4,1
    80004b12:	02e78263          	beq	a5,a4,80004b36 <filewrite+0x4e>
    ret = pipewrite(f->pipe, addr, n);
  } else if(f->type == FD_DEVICE){
    80004b16:	470d                	li	a4,3
    80004b18:	02e78663          	beq	a5,a4,80004b44 <filewrite+0x5c>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].write)
      return -1;
    ret = devsw[f->major].write(1, addr, n);
  } else if(f->type == FD_INODE){
    80004b1c:	4709                	li	a4,2
    80004b1e:	0ee79163          	bne	a5,a4,80004c00 <filewrite+0x118>
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((MAXOPBLOCKS-1-1-2) / 2) * BSIZE;
    int i = 0;
    while(i < n){
    80004b22:	0ac05d63          	blez	a2,80004bdc <filewrite+0xf4>
    int i = 0;
    80004b26:	4981                	li	s3,0
    80004b28:	6b05                	lui	s6,0x1
    80004b2a:	c00b0b13          	addi	s6,s6,-1024 # c00 <_entry-0x7ffff400>
    80004b2e:	6b85                	lui	s7,0x1
    80004b30:	c00b8b9b          	addiw	s7,s7,-1024
    80004b34:	a861                	j	80004bcc <filewrite+0xe4>
    ret = pipewrite(f->pipe, addr, n);
    80004b36:	6908                	ld	a0,16(a0)
    80004b38:	00000097          	auipc	ra,0x0
    80004b3c:	22e080e7          	jalr	558(ra) # 80004d66 <pipewrite>
    80004b40:	8a2a                	mv	s4,a0
    80004b42:	a045                	j	80004be2 <filewrite+0xfa>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].write)
    80004b44:	02451783          	lh	a5,36(a0)
    80004b48:	03079693          	slli	a3,a5,0x30
    80004b4c:	92c1                	srli	a3,a3,0x30
    80004b4e:	4725                	li	a4,9
    80004b50:	0cd76263          	bltu	a4,a3,80004c14 <filewrite+0x12c>
    80004b54:	0792                	slli	a5,a5,0x4
    80004b56:	0001d717          	auipc	a4,0x1d
    80004b5a:	a8270713          	addi	a4,a4,-1406 # 800215d8 <devsw>
    80004b5e:	97ba                	add	a5,a5,a4
    80004b60:	679c                	ld	a5,8(a5)
    80004b62:	cbdd                	beqz	a5,80004c18 <filewrite+0x130>
    ret = devsw[f->major].write(1, addr, n);
    80004b64:	4505                	li	a0,1
    80004b66:	9782                	jalr	a5
    80004b68:	8a2a                	mv	s4,a0
    80004b6a:	a8a5                	j	80004be2 <filewrite+0xfa>
    80004b6c:	00048c1b          	sext.w	s8,s1
      int n1 = n - i;
      if(n1 > max)
        n1 = max;

      begin_op();
    80004b70:	00000097          	auipc	ra,0x0
    80004b74:	8b0080e7          	jalr	-1872(ra) # 80004420 <begin_op>
      ilock(f->ip);
    80004b78:	01893503          	ld	a0,24(s2)
    80004b7c:	fffff097          	auipc	ra,0xfffff
    80004b80:	ed2080e7          	jalr	-302(ra) # 80003a4e <ilock>
      if ((r = writei(f->ip, 1, addr + i, f->off, n1)) > 0)
    80004b84:	8762                	mv	a4,s8
    80004b86:	02092683          	lw	a3,32(s2)
    80004b8a:	01598633          	add	a2,s3,s5
    80004b8e:	4585                	li	a1,1
    80004b90:	01893503          	ld	a0,24(s2)
    80004b94:	fffff097          	auipc	ra,0xfffff
    80004b98:	266080e7          	jalr	614(ra) # 80003dfa <writei>
    80004b9c:	84aa                	mv	s1,a0
    80004b9e:	00a05763          	blez	a0,80004bac <filewrite+0xc4>
        f->off += r;
    80004ba2:	02092783          	lw	a5,32(s2)
    80004ba6:	9fa9                	addw	a5,a5,a0
    80004ba8:	02f92023          	sw	a5,32(s2)
      iunlock(f->ip);
    80004bac:	01893503          	ld	a0,24(s2)
    80004bb0:	fffff097          	auipc	ra,0xfffff
    80004bb4:	f60080e7          	jalr	-160(ra) # 80003b10 <iunlock>
      end_op();
    80004bb8:	00000097          	auipc	ra,0x0
    80004bbc:	8e8080e7          	jalr	-1816(ra) # 800044a0 <end_op>

      if(r != n1){
    80004bc0:	009c1f63          	bne	s8,s1,80004bde <filewrite+0xf6>
        // error from writei
        break;
      }
      i += r;
    80004bc4:	013489bb          	addw	s3,s1,s3
    while(i < n){
    80004bc8:	0149db63          	bge	s3,s4,80004bde <filewrite+0xf6>
      int n1 = n - i;
    80004bcc:	413a07bb          	subw	a5,s4,s3
      if(n1 > max)
    80004bd0:	84be                	mv	s1,a5
    80004bd2:	2781                	sext.w	a5,a5
    80004bd4:	f8fb5ce3          	bge	s6,a5,80004b6c <filewrite+0x84>
    80004bd8:	84de                	mv	s1,s7
    80004bda:	bf49                	j	80004b6c <filewrite+0x84>
    int i = 0;
    80004bdc:	4981                	li	s3,0
    }
    ret = (i == n ? n : -1);
    80004bde:	013a1f63          	bne	s4,s3,80004bfc <filewrite+0x114>
  } else {
    panic("filewrite");
  }

  return ret;
}
    80004be2:	8552                	mv	a0,s4
    80004be4:	60a6                	ld	ra,72(sp)
    80004be6:	6406                	ld	s0,64(sp)
    80004be8:	74e2                	ld	s1,56(sp)
    80004bea:	7942                	ld	s2,48(sp)
    80004bec:	79a2                	ld	s3,40(sp)
    80004bee:	7a02                	ld	s4,32(sp)
    80004bf0:	6ae2                	ld	s5,24(sp)
    80004bf2:	6b42                	ld	s6,16(sp)
    80004bf4:	6ba2                	ld	s7,8(sp)
    80004bf6:	6c02                	ld	s8,0(sp)
    80004bf8:	6161                	addi	sp,sp,80
    80004bfa:	8082                	ret
    ret = (i == n ? n : -1);
    80004bfc:	5a7d                	li	s4,-1
    80004bfe:	b7d5                	j	80004be2 <filewrite+0xfa>
    panic("filewrite");
    80004c00:	00004517          	auipc	a0,0x4
    80004c04:	b1850513          	addi	a0,a0,-1256 # 80008718 <syscalls+0x270>
    80004c08:	ffffc097          	auipc	ra,0xffffc
    80004c0c:	936080e7          	jalr	-1738(ra) # 8000053e <panic>
    return -1;
    80004c10:	5a7d                	li	s4,-1
    80004c12:	bfc1                	j	80004be2 <filewrite+0xfa>
      return -1;
    80004c14:	5a7d                	li	s4,-1
    80004c16:	b7f1                	j	80004be2 <filewrite+0xfa>
    80004c18:	5a7d                	li	s4,-1
    80004c1a:	b7e1                	j	80004be2 <filewrite+0xfa>

0000000080004c1c <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
    80004c1c:	7179                	addi	sp,sp,-48
    80004c1e:	f406                	sd	ra,40(sp)
    80004c20:	f022                	sd	s0,32(sp)
    80004c22:	ec26                	sd	s1,24(sp)
    80004c24:	e84a                	sd	s2,16(sp)
    80004c26:	e44e                	sd	s3,8(sp)
    80004c28:	e052                	sd	s4,0(sp)
    80004c2a:	1800                	addi	s0,sp,48
    80004c2c:	84aa                	mv	s1,a0
    80004c2e:	8a2e                	mv	s4,a1
  struct pipe *pi;

  pi = 0;
  *f0 = *f1 = 0;
    80004c30:	0005b023          	sd	zero,0(a1)
    80004c34:	00053023          	sd	zero,0(a0)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
    80004c38:	00000097          	auipc	ra,0x0
    80004c3c:	bf8080e7          	jalr	-1032(ra) # 80004830 <filealloc>
    80004c40:	e088                	sd	a0,0(s1)
    80004c42:	c551                	beqz	a0,80004cce <pipealloc+0xb2>
    80004c44:	00000097          	auipc	ra,0x0
    80004c48:	bec080e7          	jalr	-1044(ra) # 80004830 <filealloc>
    80004c4c:	00aa3023          	sd	a0,0(s4)
    80004c50:	c92d                	beqz	a0,80004cc2 <pipealloc+0xa6>
    goto bad;
  if((pi = (struct pipe*)kalloc()) == 0)
    80004c52:	ffffc097          	auipc	ra,0xffffc
    80004c56:	ea2080e7          	jalr	-350(ra) # 80000af4 <kalloc>
    80004c5a:	892a                	mv	s2,a0
    80004c5c:	c125                	beqz	a0,80004cbc <pipealloc+0xa0>
    goto bad;
  pi->readopen = 1;
    80004c5e:	4985                	li	s3,1
    80004c60:	23352023          	sw	s3,544(a0)
  pi->writeopen = 1;
    80004c64:	23352223          	sw	s3,548(a0)
  pi->nwrite = 0;
    80004c68:	20052e23          	sw	zero,540(a0)
  pi->nread = 0;
    80004c6c:	20052c23          	sw	zero,536(a0)
  initlock(&pi->lock, "pipe");
    80004c70:	00004597          	auipc	a1,0x4
    80004c74:	ab858593          	addi	a1,a1,-1352 # 80008728 <syscalls+0x280>
    80004c78:	ffffc097          	auipc	ra,0xffffc
    80004c7c:	edc080e7          	jalr	-292(ra) # 80000b54 <initlock>
  (*f0)->type = FD_PIPE;
    80004c80:	609c                	ld	a5,0(s1)
    80004c82:	0137a023          	sw	s3,0(a5)
  (*f0)->readable = 1;
    80004c86:	609c                	ld	a5,0(s1)
    80004c88:	01378423          	sb	s3,8(a5)
  (*f0)->writable = 0;
    80004c8c:	609c                	ld	a5,0(s1)
    80004c8e:	000784a3          	sb	zero,9(a5)
  (*f0)->pipe = pi;
    80004c92:	609c                	ld	a5,0(s1)
    80004c94:	0127b823          	sd	s2,16(a5)
  (*f1)->type = FD_PIPE;
    80004c98:	000a3783          	ld	a5,0(s4)
    80004c9c:	0137a023          	sw	s3,0(a5)
  (*f1)->readable = 0;
    80004ca0:	000a3783          	ld	a5,0(s4)
    80004ca4:	00078423          	sb	zero,8(a5)
  (*f1)->writable = 1;
    80004ca8:	000a3783          	ld	a5,0(s4)
    80004cac:	013784a3          	sb	s3,9(a5)
  (*f1)->pipe = pi;
    80004cb0:	000a3783          	ld	a5,0(s4)
    80004cb4:	0127b823          	sd	s2,16(a5)
  return 0;
    80004cb8:	4501                	li	a0,0
    80004cba:	a025                	j	80004ce2 <pipealloc+0xc6>

 bad:
  if(pi)
    kfree((char*)pi);
  if(*f0)
    80004cbc:	6088                	ld	a0,0(s1)
    80004cbe:	e501                	bnez	a0,80004cc6 <pipealloc+0xaa>
    80004cc0:	a039                	j	80004cce <pipealloc+0xb2>
    80004cc2:	6088                	ld	a0,0(s1)
    80004cc4:	c51d                	beqz	a0,80004cf2 <pipealloc+0xd6>
    fileclose(*f0);
    80004cc6:	00000097          	auipc	ra,0x0
    80004cca:	c26080e7          	jalr	-986(ra) # 800048ec <fileclose>
  if(*f1)
    80004cce:	000a3783          	ld	a5,0(s4)
    fileclose(*f1);
  return -1;
    80004cd2:	557d                	li	a0,-1
  if(*f1)
    80004cd4:	c799                	beqz	a5,80004ce2 <pipealloc+0xc6>
    fileclose(*f1);
    80004cd6:	853e                	mv	a0,a5
    80004cd8:	00000097          	auipc	ra,0x0
    80004cdc:	c14080e7          	jalr	-1004(ra) # 800048ec <fileclose>
  return -1;
    80004ce0:	557d                	li	a0,-1
}
    80004ce2:	70a2                	ld	ra,40(sp)
    80004ce4:	7402                	ld	s0,32(sp)
    80004ce6:	64e2                	ld	s1,24(sp)
    80004ce8:	6942                	ld	s2,16(sp)
    80004cea:	69a2                	ld	s3,8(sp)
    80004cec:	6a02                	ld	s4,0(sp)
    80004cee:	6145                	addi	sp,sp,48
    80004cf0:	8082                	ret
  return -1;
    80004cf2:	557d                	li	a0,-1
    80004cf4:	b7fd                	j	80004ce2 <pipealloc+0xc6>

0000000080004cf6 <pipeclose>:

void
pipeclose(struct pipe *pi, int writable)
{
    80004cf6:	1101                	addi	sp,sp,-32
    80004cf8:	ec06                	sd	ra,24(sp)
    80004cfa:	e822                	sd	s0,16(sp)
    80004cfc:	e426                	sd	s1,8(sp)
    80004cfe:	e04a                	sd	s2,0(sp)
    80004d00:	1000                	addi	s0,sp,32
    80004d02:	84aa                	mv	s1,a0
    80004d04:	892e                	mv	s2,a1
  acquire(&pi->lock);
    80004d06:	ffffc097          	auipc	ra,0xffffc
    80004d0a:	ede080e7          	jalr	-290(ra) # 80000be4 <acquire>
  if(writable){
    80004d0e:	02090d63          	beqz	s2,80004d48 <pipeclose+0x52>
    pi->writeopen = 0;
    80004d12:	2204a223          	sw	zero,548(s1)
    wakeup(&pi->nread);
    80004d16:	21848513          	addi	a0,s1,536
    80004d1a:	ffffe097          	auipc	ra,0xffffe
    80004d1e:	8bc080e7          	jalr	-1860(ra) # 800025d6 <wakeup>
  } else {
    pi->readopen = 0;
    wakeup(&pi->nwrite);
  }
  if(pi->readopen == 0 && pi->writeopen == 0){
    80004d22:	2204b783          	ld	a5,544(s1)
    80004d26:	eb95                	bnez	a5,80004d5a <pipeclose+0x64>
    release(&pi->lock);
    80004d28:	8526                	mv	a0,s1
    80004d2a:	ffffc097          	auipc	ra,0xffffc
    80004d2e:	f6e080e7          	jalr	-146(ra) # 80000c98 <release>
    kfree((char*)pi);
    80004d32:	8526                	mv	a0,s1
    80004d34:	ffffc097          	auipc	ra,0xffffc
    80004d38:	cc4080e7          	jalr	-828(ra) # 800009f8 <kfree>
  } else
    release(&pi->lock);
}
    80004d3c:	60e2                	ld	ra,24(sp)
    80004d3e:	6442                	ld	s0,16(sp)
    80004d40:	64a2                	ld	s1,8(sp)
    80004d42:	6902                	ld	s2,0(sp)
    80004d44:	6105                	addi	sp,sp,32
    80004d46:	8082                	ret
    pi->readopen = 0;
    80004d48:	2204a023          	sw	zero,544(s1)
    wakeup(&pi->nwrite);
    80004d4c:	21c48513          	addi	a0,s1,540
    80004d50:	ffffe097          	auipc	ra,0xffffe
    80004d54:	886080e7          	jalr	-1914(ra) # 800025d6 <wakeup>
    80004d58:	b7e9                	j	80004d22 <pipeclose+0x2c>
    release(&pi->lock);
    80004d5a:	8526                	mv	a0,s1
    80004d5c:	ffffc097          	auipc	ra,0xffffc
    80004d60:	f3c080e7          	jalr	-196(ra) # 80000c98 <release>
}
    80004d64:	bfe1                	j	80004d3c <pipeclose+0x46>

0000000080004d66 <pipewrite>:

int
pipewrite(struct pipe *pi, uint64 addr, int n)
{
    80004d66:	7159                	addi	sp,sp,-112
    80004d68:	f486                	sd	ra,104(sp)
    80004d6a:	f0a2                	sd	s0,96(sp)
    80004d6c:	eca6                	sd	s1,88(sp)
    80004d6e:	e8ca                	sd	s2,80(sp)
    80004d70:	e4ce                	sd	s3,72(sp)
    80004d72:	e0d2                	sd	s4,64(sp)
    80004d74:	fc56                	sd	s5,56(sp)
    80004d76:	f85a                	sd	s6,48(sp)
    80004d78:	f45e                	sd	s7,40(sp)
    80004d7a:	f062                	sd	s8,32(sp)
    80004d7c:	ec66                	sd	s9,24(sp)
    80004d7e:	1880                	addi	s0,sp,112
    80004d80:	84aa                	mv	s1,a0
    80004d82:	8aae                	mv	s5,a1
    80004d84:	8a32                	mv	s4,a2
  int i = 0;
  struct proc *pr = myproc();
    80004d86:	ffffd097          	auipc	ra,0xffffd
    80004d8a:	006080e7          	jalr	6(ra) # 80001d8c <myproc>
    80004d8e:	89aa                	mv	s3,a0

  acquire(&pi->lock);
    80004d90:	8526                	mv	a0,s1
    80004d92:	ffffc097          	auipc	ra,0xffffc
    80004d96:	e52080e7          	jalr	-430(ra) # 80000be4 <acquire>
  while(i < n){
    80004d9a:	0d405163          	blez	s4,80004e5c <pipewrite+0xf6>
    80004d9e:	8ba6                	mv	s7,s1
  int i = 0;
    80004da0:	4901                	li	s2,0
    if(pi->nwrite == pi->nread + PIPESIZE){ //DOC: pipewrite-full
      wakeup(&pi->nread);
      sleep(&pi->nwrite, &pi->lock);
    } else {
      char ch;
      if(copyin(pr->pagetable, &ch, addr + i, 1) == -1)
    80004da2:	5b7d                	li	s6,-1
      wakeup(&pi->nread);
    80004da4:	21848c93          	addi	s9,s1,536
      sleep(&pi->nwrite, &pi->lock);
    80004da8:	21c48c13          	addi	s8,s1,540
    80004dac:	a08d                	j	80004e0e <pipewrite+0xa8>
      release(&pi->lock);
    80004dae:	8526                	mv	a0,s1
    80004db0:	ffffc097          	auipc	ra,0xffffc
    80004db4:	ee8080e7          	jalr	-280(ra) # 80000c98 <release>
      return -1;
    80004db8:	597d                	li	s2,-1
  }
  wakeup(&pi->nread);
  release(&pi->lock);

  return i;
}
    80004dba:	854a                	mv	a0,s2
    80004dbc:	70a6                	ld	ra,104(sp)
    80004dbe:	7406                	ld	s0,96(sp)
    80004dc0:	64e6                	ld	s1,88(sp)
    80004dc2:	6946                	ld	s2,80(sp)
    80004dc4:	69a6                	ld	s3,72(sp)
    80004dc6:	6a06                	ld	s4,64(sp)
    80004dc8:	7ae2                	ld	s5,56(sp)
    80004dca:	7b42                	ld	s6,48(sp)
    80004dcc:	7ba2                	ld	s7,40(sp)
    80004dce:	7c02                	ld	s8,32(sp)
    80004dd0:	6ce2                	ld	s9,24(sp)
    80004dd2:	6165                	addi	sp,sp,112
    80004dd4:	8082                	ret
      wakeup(&pi->nread);
    80004dd6:	8566                	mv	a0,s9
    80004dd8:	ffffd097          	auipc	ra,0xffffd
    80004ddc:	7fe080e7          	jalr	2046(ra) # 800025d6 <wakeup>
      sleep(&pi->nwrite, &pi->lock);
    80004de0:	85de                	mv	a1,s7
    80004de2:	8562                	mv	a0,s8
    80004de4:	ffffd097          	auipc	ra,0xffffd
    80004de8:	666080e7          	jalr	1638(ra) # 8000244a <sleep>
    80004dec:	a839                	j	80004e0a <pipewrite+0xa4>
      pi->data[pi->nwrite++ % PIPESIZE] = ch;
    80004dee:	21c4a783          	lw	a5,540(s1)
    80004df2:	0017871b          	addiw	a4,a5,1
    80004df6:	20e4ae23          	sw	a4,540(s1)
    80004dfa:	1ff7f793          	andi	a5,a5,511
    80004dfe:	97a6                	add	a5,a5,s1
    80004e00:	f9f44703          	lbu	a4,-97(s0)
    80004e04:	00e78c23          	sb	a4,24(a5)
      i++;
    80004e08:	2905                	addiw	s2,s2,1
  while(i < n){
    80004e0a:	03495d63          	bge	s2,s4,80004e44 <pipewrite+0xde>
    if(pi->readopen == 0 || pr->killed){
    80004e0e:	2204a783          	lw	a5,544(s1)
    80004e12:	dfd1                	beqz	a5,80004dae <pipewrite+0x48>
    80004e14:	0289a783          	lw	a5,40(s3)
    80004e18:	fbd9                	bnez	a5,80004dae <pipewrite+0x48>
    if(pi->nwrite == pi->nread + PIPESIZE){ //DOC: pipewrite-full
    80004e1a:	2184a783          	lw	a5,536(s1)
    80004e1e:	21c4a703          	lw	a4,540(s1)
    80004e22:	2007879b          	addiw	a5,a5,512
    80004e26:	faf708e3          	beq	a4,a5,80004dd6 <pipewrite+0x70>
      if(copyin(pr->pagetable, &ch, addr + i, 1) == -1)
    80004e2a:	4685                	li	a3,1
    80004e2c:	01590633          	add	a2,s2,s5
    80004e30:	f9f40593          	addi	a1,s0,-97
    80004e34:	0589b503          	ld	a0,88(s3)
    80004e38:	ffffd097          	auipc	ra,0xffffd
    80004e3c:	8c6080e7          	jalr	-1850(ra) # 800016fe <copyin>
    80004e40:	fb6517e3          	bne	a0,s6,80004dee <pipewrite+0x88>
  wakeup(&pi->nread);
    80004e44:	21848513          	addi	a0,s1,536
    80004e48:	ffffd097          	auipc	ra,0xffffd
    80004e4c:	78e080e7          	jalr	1934(ra) # 800025d6 <wakeup>
  release(&pi->lock);
    80004e50:	8526                	mv	a0,s1
    80004e52:	ffffc097          	auipc	ra,0xffffc
    80004e56:	e46080e7          	jalr	-442(ra) # 80000c98 <release>
  return i;
    80004e5a:	b785                	j	80004dba <pipewrite+0x54>
  int i = 0;
    80004e5c:	4901                	li	s2,0
    80004e5e:	b7dd                	j	80004e44 <pipewrite+0xde>

0000000080004e60 <piperead>:

int
piperead(struct pipe *pi, uint64 addr, int n)
{
    80004e60:	715d                	addi	sp,sp,-80
    80004e62:	e486                	sd	ra,72(sp)
    80004e64:	e0a2                	sd	s0,64(sp)
    80004e66:	fc26                	sd	s1,56(sp)
    80004e68:	f84a                	sd	s2,48(sp)
    80004e6a:	f44e                	sd	s3,40(sp)
    80004e6c:	f052                	sd	s4,32(sp)
    80004e6e:	ec56                	sd	s5,24(sp)
    80004e70:	e85a                	sd	s6,16(sp)
    80004e72:	0880                	addi	s0,sp,80
    80004e74:	84aa                	mv	s1,a0
    80004e76:	892e                	mv	s2,a1
    80004e78:	8ab2                	mv	s5,a2
  int i;
  struct proc *pr = myproc();
    80004e7a:	ffffd097          	auipc	ra,0xffffd
    80004e7e:	f12080e7          	jalr	-238(ra) # 80001d8c <myproc>
    80004e82:	8a2a                	mv	s4,a0
  char ch;

  acquire(&pi->lock);
    80004e84:	8b26                	mv	s6,s1
    80004e86:	8526                	mv	a0,s1
    80004e88:	ffffc097          	auipc	ra,0xffffc
    80004e8c:	d5c080e7          	jalr	-676(ra) # 80000be4 <acquire>
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    80004e90:	2184a703          	lw	a4,536(s1)
    80004e94:	21c4a783          	lw	a5,540(s1)
    if(pr->killed){
      release(&pi->lock);
      return -1;
    }
    sleep(&pi->nread, &pi->lock); //DOC: piperead-sleep
    80004e98:	21848993          	addi	s3,s1,536
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    80004e9c:	02f71463          	bne	a4,a5,80004ec4 <piperead+0x64>
    80004ea0:	2244a783          	lw	a5,548(s1)
    80004ea4:	c385                	beqz	a5,80004ec4 <piperead+0x64>
    if(pr->killed){
    80004ea6:	028a2783          	lw	a5,40(s4)
    80004eaa:	ebc1                	bnez	a5,80004f3a <piperead+0xda>
    sleep(&pi->nread, &pi->lock); //DOC: piperead-sleep
    80004eac:	85da                	mv	a1,s6
    80004eae:	854e                	mv	a0,s3
    80004eb0:	ffffd097          	auipc	ra,0xffffd
    80004eb4:	59a080e7          	jalr	1434(ra) # 8000244a <sleep>
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    80004eb8:	2184a703          	lw	a4,536(s1)
    80004ebc:	21c4a783          	lw	a5,540(s1)
    80004ec0:	fef700e3          	beq	a4,a5,80004ea0 <piperead+0x40>
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    80004ec4:	09505263          	blez	s5,80004f48 <piperead+0xe8>
    80004ec8:	4981                	li	s3,0
    if(pi->nread == pi->nwrite)
      break;
    ch = pi->data[pi->nread++ % PIPESIZE];
    if(copyout(pr->pagetable, addr + i, &ch, 1) == -1)
    80004eca:	5b7d                	li	s6,-1
    if(pi->nread == pi->nwrite)
    80004ecc:	2184a783          	lw	a5,536(s1)
    80004ed0:	21c4a703          	lw	a4,540(s1)
    80004ed4:	02f70d63          	beq	a4,a5,80004f0e <piperead+0xae>
    ch = pi->data[pi->nread++ % PIPESIZE];
    80004ed8:	0017871b          	addiw	a4,a5,1
    80004edc:	20e4ac23          	sw	a4,536(s1)
    80004ee0:	1ff7f793          	andi	a5,a5,511
    80004ee4:	97a6                	add	a5,a5,s1
    80004ee6:	0187c783          	lbu	a5,24(a5)
    80004eea:	faf40fa3          	sb	a5,-65(s0)
    if(copyout(pr->pagetable, addr + i, &ch, 1) == -1)
    80004eee:	4685                	li	a3,1
    80004ef0:	fbf40613          	addi	a2,s0,-65
    80004ef4:	85ca                	mv	a1,s2
    80004ef6:	058a3503          	ld	a0,88(s4)
    80004efa:	ffffc097          	auipc	ra,0xffffc
    80004efe:	778080e7          	jalr	1912(ra) # 80001672 <copyout>
    80004f02:	01650663          	beq	a0,s6,80004f0e <piperead+0xae>
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    80004f06:	2985                	addiw	s3,s3,1
    80004f08:	0905                	addi	s2,s2,1
    80004f0a:	fd3a91e3          	bne	s5,s3,80004ecc <piperead+0x6c>
      break;
  }
  wakeup(&pi->nwrite);  //DOC: piperead-wakeup
    80004f0e:	21c48513          	addi	a0,s1,540
    80004f12:	ffffd097          	auipc	ra,0xffffd
    80004f16:	6c4080e7          	jalr	1732(ra) # 800025d6 <wakeup>
  release(&pi->lock);
    80004f1a:	8526                	mv	a0,s1
    80004f1c:	ffffc097          	auipc	ra,0xffffc
    80004f20:	d7c080e7          	jalr	-644(ra) # 80000c98 <release>
  return i;
}
    80004f24:	854e                	mv	a0,s3
    80004f26:	60a6                	ld	ra,72(sp)
    80004f28:	6406                	ld	s0,64(sp)
    80004f2a:	74e2                	ld	s1,56(sp)
    80004f2c:	7942                	ld	s2,48(sp)
    80004f2e:	79a2                	ld	s3,40(sp)
    80004f30:	7a02                	ld	s4,32(sp)
    80004f32:	6ae2                	ld	s5,24(sp)
    80004f34:	6b42                	ld	s6,16(sp)
    80004f36:	6161                	addi	sp,sp,80
    80004f38:	8082                	ret
      release(&pi->lock);
    80004f3a:	8526                	mv	a0,s1
    80004f3c:	ffffc097          	auipc	ra,0xffffc
    80004f40:	d5c080e7          	jalr	-676(ra) # 80000c98 <release>
      return -1;
    80004f44:	59fd                	li	s3,-1
    80004f46:	bff9                	j	80004f24 <piperead+0xc4>
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    80004f48:	4981                	li	s3,0
    80004f4a:	b7d1                	j	80004f0e <piperead+0xae>

0000000080004f4c <exec>:

static int loadseg(pde_t *pgdir, uint64 addr, struct inode *ip, uint offset, uint sz);

int
exec(char *path, char **argv)
{
    80004f4c:	df010113          	addi	sp,sp,-528
    80004f50:	20113423          	sd	ra,520(sp)
    80004f54:	20813023          	sd	s0,512(sp)
    80004f58:	ffa6                	sd	s1,504(sp)
    80004f5a:	fbca                	sd	s2,496(sp)
    80004f5c:	f7ce                	sd	s3,488(sp)
    80004f5e:	f3d2                	sd	s4,480(sp)
    80004f60:	efd6                	sd	s5,472(sp)
    80004f62:	ebda                	sd	s6,464(sp)
    80004f64:	e7de                	sd	s7,456(sp)
    80004f66:	e3e2                	sd	s8,448(sp)
    80004f68:	ff66                	sd	s9,440(sp)
    80004f6a:	fb6a                	sd	s10,432(sp)
    80004f6c:	f76e                	sd	s11,424(sp)
    80004f6e:	0c00                	addi	s0,sp,528
    80004f70:	84aa                	mv	s1,a0
    80004f72:	dea43c23          	sd	a0,-520(s0)
    80004f76:	e0b43023          	sd	a1,-512(s0)
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pagetable_t pagetable = 0, oldpagetable;
  struct proc *p = myproc();
    80004f7a:	ffffd097          	auipc	ra,0xffffd
    80004f7e:	e12080e7          	jalr	-494(ra) # 80001d8c <myproc>
    80004f82:	892a                	mv	s2,a0

  begin_op();
    80004f84:	fffff097          	auipc	ra,0xfffff
    80004f88:	49c080e7          	jalr	1180(ra) # 80004420 <begin_op>

  if((ip = namei(path)) == 0){
    80004f8c:	8526                	mv	a0,s1
    80004f8e:	fffff097          	auipc	ra,0xfffff
    80004f92:	276080e7          	jalr	630(ra) # 80004204 <namei>
    80004f96:	c92d                	beqz	a0,80005008 <exec+0xbc>
    80004f98:	84aa                	mv	s1,a0
    end_op();
    return -1;
  }
  ilock(ip);
    80004f9a:	fffff097          	auipc	ra,0xfffff
    80004f9e:	ab4080e7          	jalr	-1356(ra) # 80003a4e <ilock>

  // Check ELF header
  if(readi(ip, 0, (uint64)&elf, 0, sizeof(elf)) != sizeof(elf))
    80004fa2:	04000713          	li	a4,64
    80004fa6:	4681                	li	a3,0
    80004fa8:	e5040613          	addi	a2,s0,-432
    80004fac:	4581                	li	a1,0
    80004fae:	8526                	mv	a0,s1
    80004fb0:	fffff097          	auipc	ra,0xfffff
    80004fb4:	d52080e7          	jalr	-686(ra) # 80003d02 <readi>
    80004fb8:	04000793          	li	a5,64
    80004fbc:	00f51a63          	bne	a0,a5,80004fd0 <exec+0x84>
    goto bad;
  if(elf.magic != ELF_MAGIC)
    80004fc0:	e5042703          	lw	a4,-432(s0)
    80004fc4:	464c47b7          	lui	a5,0x464c4
    80004fc8:	57f78793          	addi	a5,a5,1407 # 464c457f <_entry-0x39b3ba81>
    80004fcc:	04f70463          	beq	a4,a5,80005014 <exec+0xc8>

 bad:
  if(pagetable)
    proc_freepagetable(pagetable, sz);
  if(ip){
    iunlockput(ip);
    80004fd0:	8526                	mv	a0,s1
    80004fd2:	fffff097          	auipc	ra,0xfffff
    80004fd6:	cde080e7          	jalr	-802(ra) # 80003cb0 <iunlockput>
    end_op();
    80004fda:	fffff097          	auipc	ra,0xfffff
    80004fde:	4c6080e7          	jalr	1222(ra) # 800044a0 <end_op>
  }
  return -1;
    80004fe2:	557d                	li	a0,-1
}
    80004fe4:	20813083          	ld	ra,520(sp)
    80004fe8:	20013403          	ld	s0,512(sp)
    80004fec:	74fe                	ld	s1,504(sp)
    80004fee:	795e                	ld	s2,496(sp)
    80004ff0:	79be                	ld	s3,488(sp)
    80004ff2:	7a1e                	ld	s4,480(sp)
    80004ff4:	6afe                	ld	s5,472(sp)
    80004ff6:	6b5e                	ld	s6,464(sp)
    80004ff8:	6bbe                	ld	s7,456(sp)
    80004ffa:	6c1e                	ld	s8,448(sp)
    80004ffc:	7cfa                	ld	s9,440(sp)
    80004ffe:	7d5a                	ld	s10,432(sp)
    80005000:	7dba                	ld	s11,424(sp)
    80005002:	21010113          	addi	sp,sp,528
    80005006:	8082                	ret
    end_op();
    80005008:	fffff097          	auipc	ra,0xfffff
    8000500c:	498080e7          	jalr	1176(ra) # 800044a0 <end_op>
    return -1;
    80005010:	557d                	li	a0,-1
    80005012:	bfc9                	j	80004fe4 <exec+0x98>
  if((pagetable = proc_pagetable(p)) == 0)
    80005014:	854a                	mv	a0,s2
    80005016:	ffffd097          	auipc	ra,0xffffd
    8000501a:	e34080e7          	jalr	-460(ra) # 80001e4a <proc_pagetable>
    8000501e:	8baa                	mv	s7,a0
    80005020:	d945                	beqz	a0,80004fd0 <exec+0x84>
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    80005022:	e7042983          	lw	s3,-400(s0)
    80005026:	e8845783          	lhu	a5,-376(s0)
    8000502a:	c7ad                	beqz	a5,80005094 <exec+0x148>
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
    8000502c:	4901                	li	s2,0
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    8000502e:	4b01                	li	s6,0
    if((ph.vaddr % PGSIZE) != 0)
    80005030:	6c85                	lui	s9,0x1
    80005032:	fffc8793          	addi	a5,s9,-1 # fff <_entry-0x7ffff001>
    80005036:	def43823          	sd	a5,-528(s0)
    8000503a:	a42d                	j	80005264 <exec+0x318>
  uint64 pa;

  for(i = 0; i < sz; i += PGSIZE){
    pa = walkaddr(pagetable, va + i);
    if(pa == 0)
      panic("loadseg: address should exist");
    8000503c:	00003517          	auipc	a0,0x3
    80005040:	6f450513          	addi	a0,a0,1780 # 80008730 <syscalls+0x288>
    80005044:	ffffb097          	auipc	ra,0xffffb
    80005048:	4fa080e7          	jalr	1274(ra) # 8000053e <panic>
    if(sz - i < PGSIZE)
      n = sz - i;
    else
      n = PGSIZE;
    if(readi(ip, 0, (uint64)pa, offset+i, n) != n)
    8000504c:	8756                	mv	a4,s5
    8000504e:	012d86bb          	addw	a3,s11,s2
    80005052:	4581                	li	a1,0
    80005054:	8526                	mv	a0,s1
    80005056:	fffff097          	auipc	ra,0xfffff
    8000505a:	cac080e7          	jalr	-852(ra) # 80003d02 <readi>
    8000505e:	2501                	sext.w	a0,a0
    80005060:	1aaa9963          	bne	s5,a0,80005212 <exec+0x2c6>
  for(i = 0; i < sz; i += PGSIZE){
    80005064:	6785                	lui	a5,0x1
    80005066:	0127893b          	addw	s2,a5,s2
    8000506a:	77fd                	lui	a5,0xfffff
    8000506c:	01478a3b          	addw	s4,a5,s4
    80005070:	1f897163          	bgeu	s2,s8,80005252 <exec+0x306>
    pa = walkaddr(pagetable, va + i);
    80005074:	02091593          	slli	a1,s2,0x20
    80005078:	9181                	srli	a1,a1,0x20
    8000507a:	95ea                	add	a1,a1,s10
    8000507c:	855e                	mv	a0,s7
    8000507e:	ffffc097          	auipc	ra,0xffffc
    80005082:	ff0080e7          	jalr	-16(ra) # 8000106e <walkaddr>
    80005086:	862a                	mv	a2,a0
    if(pa == 0)
    80005088:	d955                	beqz	a0,8000503c <exec+0xf0>
      n = PGSIZE;
    8000508a:	8ae6                	mv	s5,s9
    if(sz - i < PGSIZE)
    8000508c:	fd9a70e3          	bgeu	s4,s9,8000504c <exec+0x100>
      n = sz - i;
    80005090:	8ad2                	mv	s5,s4
    80005092:	bf6d                	j	8000504c <exec+0x100>
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
    80005094:	4901                	li	s2,0
  iunlockput(ip);
    80005096:	8526                	mv	a0,s1
    80005098:	fffff097          	auipc	ra,0xfffff
    8000509c:	c18080e7          	jalr	-1000(ra) # 80003cb0 <iunlockput>
  end_op();
    800050a0:	fffff097          	auipc	ra,0xfffff
    800050a4:	400080e7          	jalr	1024(ra) # 800044a0 <end_op>
  p = myproc();
    800050a8:	ffffd097          	auipc	ra,0xffffd
    800050ac:	ce4080e7          	jalr	-796(ra) # 80001d8c <myproc>
    800050b0:	8aaa                	mv	s5,a0
  uint64 oldsz = p->sz;
    800050b2:	05053d03          	ld	s10,80(a0)
  sz = PGROUNDUP(sz);
    800050b6:	6785                	lui	a5,0x1
    800050b8:	17fd                	addi	a5,a5,-1
    800050ba:	993e                	add	s2,s2,a5
    800050bc:	757d                	lui	a0,0xfffff
    800050be:	00a977b3          	and	a5,s2,a0
    800050c2:	e0f43423          	sd	a5,-504(s0)
  if((sz1 = uvmalloc(pagetable, sz, sz + 2*PGSIZE)) == 0)
    800050c6:	6609                	lui	a2,0x2
    800050c8:	963e                	add	a2,a2,a5
    800050ca:	85be                	mv	a1,a5
    800050cc:	855e                	mv	a0,s7
    800050ce:	ffffc097          	auipc	ra,0xffffc
    800050d2:	354080e7          	jalr	852(ra) # 80001422 <uvmalloc>
    800050d6:	8b2a                	mv	s6,a0
  ip = 0;
    800050d8:	4481                	li	s1,0
  if((sz1 = uvmalloc(pagetable, sz, sz + 2*PGSIZE)) == 0)
    800050da:	12050c63          	beqz	a0,80005212 <exec+0x2c6>
  uvmclear(pagetable, sz-2*PGSIZE);
    800050de:	75f9                	lui	a1,0xffffe
    800050e0:	95aa                	add	a1,a1,a0
    800050e2:	855e                	mv	a0,s7
    800050e4:	ffffc097          	auipc	ra,0xffffc
    800050e8:	55c080e7          	jalr	1372(ra) # 80001640 <uvmclear>
  stackbase = sp - PGSIZE;
    800050ec:	7c7d                	lui	s8,0xfffff
    800050ee:	9c5a                	add	s8,s8,s6
  for(argc = 0; argv[argc]; argc++) {
    800050f0:	e0043783          	ld	a5,-512(s0)
    800050f4:	6388                	ld	a0,0(a5)
    800050f6:	c535                	beqz	a0,80005162 <exec+0x216>
    800050f8:	e9040993          	addi	s3,s0,-368
    800050fc:	f9040c93          	addi	s9,s0,-112
  sp = sz;
    80005100:	895a                	mv	s2,s6
    sp -= strlen(argv[argc]) + 1;
    80005102:	ffffc097          	auipc	ra,0xffffc
    80005106:	d62080e7          	jalr	-670(ra) # 80000e64 <strlen>
    8000510a:	2505                	addiw	a0,a0,1
    8000510c:	40a90933          	sub	s2,s2,a0
    sp -= sp % 16; // riscv sp must be 16-byte aligned
    80005110:	ff097913          	andi	s2,s2,-16
    if(sp < stackbase)
    80005114:	13896363          	bltu	s2,s8,8000523a <exec+0x2ee>
    if(copyout(pagetable, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
    80005118:	e0043d83          	ld	s11,-512(s0)
    8000511c:	000dba03          	ld	s4,0(s11)
    80005120:	8552                	mv	a0,s4
    80005122:	ffffc097          	auipc	ra,0xffffc
    80005126:	d42080e7          	jalr	-702(ra) # 80000e64 <strlen>
    8000512a:	0015069b          	addiw	a3,a0,1
    8000512e:	8652                	mv	a2,s4
    80005130:	85ca                	mv	a1,s2
    80005132:	855e                	mv	a0,s7
    80005134:	ffffc097          	auipc	ra,0xffffc
    80005138:	53e080e7          	jalr	1342(ra) # 80001672 <copyout>
    8000513c:	10054363          	bltz	a0,80005242 <exec+0x2f6>
    ustack[argc] = sp;
    80005140:	0129b023          	sd	s2,0(s3)
  for(argc = 0; argv[argc]; argc++) {
    80005144:	0485                	addi	s1,s1,1
    80005146:	008d8793          	addi	a5,s11,8
    8000514a:	e0f43023          	sd	a5,-512(s0)
    8000514e:	008db503          	ld	a0,8(s11)
    80005152:	c911                	beqz	a0,80005166 <exec+0x21a>
    if(argc >= MAXARG)
    80005154:	09a1                	addi	s3,s3,8
    80005156:	fb3c96e3          	bne	s9,s3,80005102 <exec+0x1b6>
  sz = sz1;
    8000515a:	e1643423          	sd	s6,-504(s0)
  ip = 0;
    8000515e:	4481                	li	s1,0
    80005160:	a84d                	j	80005212 <exec+0x2c6>
  sp = sz;
    80005162:	895a                	mv	s2,s6
  for(argc = 0; argv[argc]; argc++) {
    80005164:	4481                	li	s1,0
  ustack[argc] = 0;
    80005166:	00349793          	slli	a5,s1,0x3
    8000516a:	f9040713          	addi	a4,s0,-112
    8000516e:	97ba                	add	a5,a5,a4
    80005170:	f007b023          	sd	zero,-256(a5) # f00 <_entry-0x7ffff100>
  sp -= (argc+1) * sizeof(uint64);
    80005174:	00148693          	addi	a3,s1,1
    80005178:	068e                	slli	a3,a3,0x3
    8000517a:	40d90933          	sub	s2,s2,a3
  sp -= sp % 16;
    8000517e:	ff097913          	andi	s2,s2,-16
  if(sp < stackbase)
    80005182:	01897663          	bgeu	s2,s8,8000518e <exec+0x242>
  sz = sz1;
    80005186:	e1643423          	sd	s6,-504(s0)
  ip = 0;
    8000518a:	4481                	li	s1,0
    8000518c:	a059                	j	80005212 <exec+0x2c6>
  if(copyout(pagetable, sp, (char *)ustack, (argc+1)*sizeof(uint64)) < 0)
    8000518e:	e9040613          	addi	a2,s0,-368
    80005192:	85ca                	mv	a1,s2
    80005194:	855e                	mv	a0,s7
    80005196:	ffffc097          	auipc	ra,0xffffc
    8000519a:	4dc080e7          	jalr	1244(ra) # 80001672 <copyout>
    8000519e:	0a054663          	bltz	a0,8000524a <exec+0x2fe>
  p->trapframe->a1 = sp;
    800051a2:	060ab783          	ld	a5,96(s5)
    800051a6:	0727bc23          	sd	s2,120(a5)
  for(last=s=path; *s; s++)
    800051aa:	df843783          	ld	a5,-520(s0)
    800051ae:	0007c703          	lbu	a4,0(a5)
    800051b2:	cf11                	beqz	a4,800051ce <exec+0x282>
    800051b4:	0785                	addi	a5,a5,1
    if(*s == '/')
    800051b6:	02f00693          	li	a3,47
    800051ba:	a039                	j	800051c8 <exec+0x27c>
      last = s+1;
    800051bc:	def43c23          	sd	a5,-520(s0)
  for(last=s=path; *s; s++)
    800051c0:	0785                	addi	a5,a5,1
    800051c2:	fff7c703          	lbu	a4,-1(a5)
    800051c6:	c701                	beqz	a4,800051ce <exec+0x282>
    if(*s == '/')
    800051c8:	fed71ce3          	bne	a4,a3,800051c0 <exec+0x274>
    800051cc:	bfc5                	j	800051bc <exec+0x270>
  safestrcpy(p->name, last, sizeof(p->name));
    800051ce:	4641                	li	a2,16
    800051d0:	df843583          	ld	a1,-520(s0)
    800051d4:	160a8513          	addi	a0,s5,352
    800051d8:	ffffc097          	auipc	ra,0xffffc
    800051dc:	c5a080e7          	jalr	-934(ra) # 80000e32 <safestrcpy>
  oldpagetable = p->pagetable;
    800051e0:	058ab503          	ld	a0,88(s5)
  p->pagetable = pagetable;
    800051e4:	057abc23          	sd	s7,88(s5)
  p->sz = sz;
    800051e8:	056ab823          	sd	s6,80(s5)
  p->trapframe->epc = elf.entry;  // initial program counter = main
    800051ec:	060ab783          	ld	a5,96(s5)
    800051f0:	e6843703          	ld	a4,-408(s0)
    800051f4:	ef98                	sd	a4,24(a5)
  p->trapframe->sp = sp; // initial stack pointer
    800051f6:	060ab783          	ld	a5,96(s5)
    800051fa:	0327b823          	sd	s2,48(a5)
  proc_freepagetable(oldpagetable, oldsz);
    800051fe:	85ea                	mv	a1,s10
    80005200:	ffffd097          	auipc	ra,0xffffd
    80005204:	ce6080e7          	jalr	-794(ra) # 80001ee6 <proc_freepagetable>
  return argc; // this ends up in a0, the first argument to main(argc, argv)
    80005208:	0004851b          	sext.w	a0,s1
    8000520c:	bbe1                	j	80004fe4 <exec+0x98>
    8000520e:	e1243423          	sd	s2,-504(s0)
    proc_freepagetable(pagetable, sz);
    80005212:	e0843583          	ld	a1,-504(s0)
    80005216:	855e                	mv	a0,s7
    80005218:	ffffd097          	auipc	ra,0xffffd
    8000521c:	cce080e7          	jalr	-818(ra) # 80001ee6 <proc_freepagetable>
  if(ip){
    80005220:	da0498e3          	bnez	s1,80004fd0 <exec+0x84>
  return -1;
    80005224:	557d                	li	a0,-1
    80005226:	bb7d                	j	80004fe4 <exec+0x98>
    80005228:	e1243423          	sd	s2,-504(s0)
    8000522c:	b7dd                	j	80005212 <exec+0x2c6>
    8000522e:	e1243423          	sd	s2,-504(s0)
    80005232:	b7c5                	j	80005212 <exec+0x2c6>
    80005234:	e1243423          	sd	s2,-504(s0)
    80005238:	bfe9                	j	80005212 <exec+0x2c6>
  sz = sz1;
    8000523a:	e1643423          	sd	s6,-504(s0)
  ip = 0;
    8000523e:	4481                	li	s1,0
    80005240:	bfc9                	j	80005212 <exec+0x2c6>
  sz = sz1;
    80005242:	e1643423          	sd	s6,-504(s0)
  ip = 0;
    80005246:	4481                	li	s1,0
    80005248:	b7e9                	j	80005212 <exec+0x2c6>
  sz = sz1;
    8000524a:	e1643423          	sd	s6,-504(s0)
  ip = 0;
    8000524e:	4481                	li	s1,0
    80005250:	b7c9                	j	80005212 <exec+0x2c6>
    if((sz1 = uvmalloc(pagetable, sz, ph.vaddr + ph.memsz)) == 0)
    80005252:	e0843903          	ld	s2,-504(s0)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    80005256:	2b05                	addiw	s6,s6,1
    80005258:	0389899b          	addiw	s3,s3,56
    8000525c:	e8845783          	lhu	a5,-376(s0)
    80005260:	e2fb5be3          	bge	s6,a5,80005096 <exec+0x14a>
    if(readi(ip, 0, (uint64)&ph, off, sizeof(ph)) != sizeof(ph))
    80005264:	2981                	sext.w	s3,s3
    80005266:	03800713          	li	a4,56
    8000526a:	86ce                	mv	a3,s3
    8000526c:	e1840613          	addi	a2,s0,-488
    80005270:	4581                	li	a1,0
    80005272:	8526                	mv	a0,s1
    80005274:	fffff097          	auipc	ra,0xfffff
    80005278:	a8e080e7          	jalr	-1394(ra) # 80003d02 <readi>
    8000527c:	03800793          	li	a5,56
    80005280:	f8f517e3          	bne	a0,a5,8000520e <exec+0x2c2>
    if(ph.type != ELF_PROG_LOAD)
    80005284:	e1842783          	lw	a5,-488(s0)
    80005288:	4705                	li	a4,1
    8000528a:	fce796e3          	bne	a5,a4,80005256 <exec+0x30a>
    if(ph.memsz < ph.filesz)
    8000528e:	e4043603          	ld	a2,-448(s0)
    80005292:	e3843783          	ld	a5,-456(s0)
    80005296:	f8f669e3          	bltu	a2,a5,80005228 <exec+0x2dc>
    if(ph.vaddr + ph.memsz < ph.vaddr)
    8000529a:	e2843783          	ld	a5,-472(s0)
    8000529e:	963e                	add	a2,a2,a5
    800052a0:	f8f667e3          	bltu	a2,a5,8000522e <exec+0x2e2>
    if((sz1 = uvmalloc(pagetable, sz, ph.vaddr + ph.memsz)) == 0)
    800052a4:	85ca                	mv	a1,s2
    800052a6:	855e                	mv	a0,s7
    800052a8:	ffffc097          	auipc	ra,0xffffc
    800052ac:	17a080e7          	jalr	378(ra) # 80001422 <uvmalloc>
    800052b0:	e0a43423          	sd	a0,-504(s0)
    800052b4:	d141                	beqz	a0,80005234 <exec+0x2e8>
    if((ph.vaddr % PGSIZE) != 0)
    800052b6:	e2843d03          	ld	s10,-472(s0)
    800052ba:	df043783          	ld	a5,-528(s0)
    800052be:	00fd77b3          	and	a5,s10,a5
    800052c2:	fba1                	bnez	a5,80005212 <exec+0x2c6>
    if(loadseg(pagetable, ph.vaddr, ip, ph.off, ph.filesz) < 0)
    800052c4:	e2042d83          	lw	s11,-480(s0)
    800052c8:	e3842c03          	lw	s8,-456(s0)
  for(i = 0; i < sz; i += PGSIZE){
    800052cc:	f80c03e3          	beqz	s8,80005252 <exec+0x306>
    800052d0:	8a62                	mv	s4,s8
    800052d2:	4901                	li	s2,0
    800052d4:	b345                	j	80005074 <exec+0x128>

00000000800052d6 <argfd>:

// Fetch the nth word-sized system call argument as a file descriptor
// and return both the descriptor and the corresponding struct file.
static int
argfd(int n, int *pfd, struct file **pf)
{
    800052d6:	7179                	addi	sp,sp,-48
    800052d8:	f406                	sd	ra,40(sp)
    800052da:	f022                	sd	s0,32(sp)
    800052dc:	ec26                	sd	s1,24(sp)
    800052de:	e84a                	sd	s2,16(sp)
    800052e0:	1800                	addi	s0,sp,48
    800052e2:	892e                	mv	s2,a1
    800052e4:	84b2                	mv	s1,a2
  int fd;
  struct file *f;

  if(argint(n, &fd) < 0)
    800052e6:	fdc40593          	addi	a1,s0,-36
    800052ea:	ffffe097          	auipc	ra,0xffffe
    800052ee:	bda080e7          	jalr	-1062(ra) # 80002ec4 <argint>
    800052f2:	04054063          	bltz	a0,80005332 <argfd+0x5c>
    return -1;
  if(fd < 0 || fd >= NOFILE || (f=myproc()->ofile[fd]) == 0)
    800052f6:	fdc42703          	lw	a4,-36(s0)
    800052fa:	47bd                	li	a5,15
    800052fc:	02e7ed63          	bltu	a5,a4,80005336 <argfd+0x60>
    80005300:	ffffd097          	auipc	ra,0xffffd
    80005304:	a8c080e7          	jalr	-1396(ra) # 80001d8c <myproc>
    80005308:	fdc42703          	lw	a4,-36(s0)
    8000530c:	01a70793          	addi	a5,a4,26
    80005310:	078e                	slli	a5,a5,0x3
    80005312:	953e                	add	a0,a0,a5
    80005314:	651c                	ld	a5,8(a0)
    80005316:	c395                	beqz	a5,8000533a <argfd+0x64>
    return -1;
  if(pfd)
    80005318:	00090463          	beqz	s2,80005320 <argfd+0x4a>
    *pfd = fd;
    8000531c:	00e92023          	sw	a4,0(s2)
  if(pf)
    *pf = f;
  return 0;
    80005320:	4501                	li	a0,0
  if(pf)
    80005322:	c091                	beqz	s1,80005326 <argfd+0x50>
    *pf = f;
    80005324:	e09c                	sd	a5,0(s1)
}
    80005326:	70a2                	ld	ra,40(sp)
    80005328:	7402                	ld	s0,32(sp)
    8000532a:	64e2                	ld	s1,24(sp)
    8000532c:	6942                	ld	s2,16(sp)
    8000532e:	6145                	addi	sp,sp,48
    80005330:	8082                	ret
    return -1;
    80005332:	557d                	li	a0,-1
    80005334:	bfcd                	j	80005326 <argfd+0x50>
    return -1;
    80005336:	557d                	li	a0,-1
    80005338:	b7fd                	j	80005326 <argfd+0x50>
    8000533a:	557d                	li	a0,-1
    8000533c:	b7ed                	j	80005326 <argfd+0x50>

000000008000533e <fdalloc>:

// Allocate a file descriptor for the given file.
// Takes over file reference from caller on success.
static int
fdalloc(struct file *f)
{
    8000533e:	1101                	addi	sp,sp,-32
    80005340:	ec06                	sd	ra,24(sp)
    80005342:	e822                	sd	s0,16(sp)
    80005344:	e426                	sd	s1,8(sp)
    80005346:	1000                	addi	s0,sp,32
    80005348:	84aa                	mv	s1,a0
  int fd;
  struct proc *p = myproc();
    8000534a:	ffffd097          	auipc	ra,0xffffd
    8000534e:	a42080e7          	jalr	-1470(ra) # 80001d8c <myproc>
    80005352:	862a                	mv	a2,a0

  for(fd = 0; fd < NOFILE; fd++){
    80005354:	0d850793          	addi	a5,a0,216 # fffffffffffff0d8 <end+0xffffffff7ffd90d8>
    80005358:	4501                	li	a0,0
    8000535a:	46c1                	li	a3,16
    if(p->ofile[fd] == 0){
    8000535c:	6398                	ld	a4,0(a5)
    8000535e:	cb19                	beqz	a4,80005374 <fdalloc+0x36>
  for(fd = 0; fd < NOFILE; fd++){
    80005360:	2505                	addiw	a0,a0,1
    80005362:	07a1                	addi	a5,a5,8
    80005364:	fed51ce3          	bne	a0,a3,8000535c <fdalloc+0x1e>
      p->ofile[fd] = f;
      return fd;
    }
  }
  return -1;
    80005368:	557d                	li	a0,-1
}
    8000536a:	60e2                	ld	ra,24(sp)
    8000536c:	6442                	ld	s0,16(sp)
    8000536e:	64a2                	ld	s1,8(sp)
    80005370:	6105                	addi	sp,sp,32
    80005372:	8082                	ret
      p->ofile[fd] = f;
    80005374:	01a50793          	addi	a5,a0,26
    80005378:	078e                	slli	a5,a5,0x3
    8000537a:	963e                	add	a2,a2,a5
    8000537c:	e604                	sd	s1,8(a2)
      return fd;
    8000537e:	b7f5                	j	8000536a <fdalloc+0x2c>

0000000080005380 <create>:
  return -1;
}

static struct inode*
create(char *path, short type, short major, short minor)
{
    80005380:	715d                	addi	sp,sp,-80
    80005382:	e486                	sd	ra,72(sp)
    80005384:	e0a2                	sd	s0,64(sp)
    80005386:	fc26                	sd	s1,56(sp)
    80005388:	f84a                	sd	s2,48(sp)
    8000538a:	f44e                	sd	s3,40(sp)
    8000538c:	f052                	sd	s4,32(sp)
    8000538e:	ec56                	sd	s5,24(sp)
    80005390:	0880                	addi	s0,sp,80
    80005392:	89ae                	mv	s3,a1
    80005394:	8ab2                	mv	s5,a2
    80005396:	8a36                	mv	s4,a3
  struct inode *ip, *dp;
  char name[DIRSIZ];

  if((dp = nameiparent(path, name)) == 0)
    80005398:	fb040593          	addi	a1,s0,-80
    8000539c:	fffff097          	auipc	ra,0xfffff
    800053a0:	e86080e7          	jalr	-378(ra) # 80004222 <nameiparent>
    800053a4:	892a                	mv	s2,a0
    800053a6:	12050f63          	beqz	a0,800054e4 <create+0x164>
    return 0;

  ilock(dp);
    800053aa:	ffffe097          	auipc	ra,0xffffe
    800053ae:	6a4080e7          	jalr	1700(ra) # 80003a4e <ilock>

  if((ip = dirlookup(dp, name, 0)) != 0){
    800053b2:	4601                	li	a2,0
    800053b4:	fb040593          	addi	a1,s0,-80
    800053b8:	854a                	mv	a0,s2
    800053ba:	fffff097          	auipc	ra,0xfffff
    800053be:	b78080e7          	jalr	-1160(ra) # 80003f32 <dirlookup>
    800053c2:	84aa                	mv	s1,a0
    800053c4:	c921                	beqz	a0,80005414 <create+0x94>
    iunlockput(dp);
    800053c6:	854a                	mv	a0,s2
    800053c8:	fffff097          	auipc	ra,0xfffff
    800053cc:	8e8080e7          	jalr	-1816(ra) # 80003cb0 <iunlockput>
    ilock(ip);
    800053d0:	8526                	mv	a0,s1
    800053d2:	ffffe097          	auipc	ra,0xffffe
    800053d6:	67c080e7          	jalr	1660(ra) # 80003a4e <ilock>
    if(type == T_FILE && (ip->type == T_FILE || ip->type == T_DEVICE))
    800053da:	2981                	sext.w	s3,s3
    800053dc:	4789                	li	a5,2
    800053de:	02f99463          	bne	s3,a5,80005406 <create+0x86>
    800053e2:	0444d783          	lhu	a5,68(s1)
    800053e6:	37f9                	addiw	a5,a5,-2
    800053e8:	17c2                	slli	a5,a5,0x30
    800053ea:	93c1                	srli	a5,a5,0x30
    800053ec:	4705                	li	a4,1
    800053ee:	00f76c63          	bltu	a4,a5,80005406 <create+0x86>
    panic("create: dirlink");

  iunlockput(dp);

  return ip;
}
    800053f2:	8526                	mv	a0,s1
    800053f4:	60a6                	ld	ra,72(sp)
    800053f6:	6406                	ld	s0,64(sp)
    800053f8:	74e2                	ld	s1,56(sp)
    800053fa:	7942                	ld	s2,48(sp)
    800053fc:	79a2                	ld	s3,40(sp)
    800053fe:	7a02                	ld	s4,32(sp)
    80005400:	6ae2                	ld	s5,24(sp)
    80005402:	6161                	addi	sp,sp,80
    80005404:	8082                	ret
    iunlockput(ip);
    80005406:	8526                	mv	a0,s1
    80005408:	fffff097          	auipc	ra,0xfffff
    8000540c:	8a8080e7          	jalr	-1880(ra) # 80003cb0 <iunlockput>
    return 0;
    80005410:	4481                	li	s1,0
    80005412:	b7c5                	j	800053f2 <create+0x72>
  if((ip = ialloc(dp->dev, type)) == 0)
    80005414:	85ce                	mv	a1,s3
    80005416:	00092503          	lw	a0,0(s2)
    8000541a:	ffffe097          	auipc	ra,0xffffe
    8000541e:	49c080e7          	jalr	1180(ra) # 800038b6 <ialloc>
    80005422:	84aa                	mv	s1,a0
    80005424:	c529                	beqz	a0,8000546e <create+0xee>
  ilock(ip);
    80005426:	ffffe097          	auipc	ra,0xffffe
    8000542a:	628080e7          	jalr	1576(ra) # 80003a4e <ilock>
  ip->major = major;
    8000542e:	05549323          	sh	s5,70(s1)
  ip->minor = minor;
    80005432:	05449423          	sh	s4,72(s1)
  ip->nlink = 1;
    80005436:	4785                	li	a5,1
    80005438:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    8000543c:	8526                	mv	a0,s1
    8000543e:	ffffe097          	auipc	ra,0xffffe
    80005442:	546080e7          	jalr	1350(ra) # 80003984 <iupdate>
  if(type == T_DIR){  // Create . and .. entries.
    80005446:	2981                	sext.w	s3,s3
    80005448:	4785                	li	a5,1
    8000544a:	02f98a63          	beq	s3,a5,8000547e <create+0xfe>
  if(dirlink(dp, name, ip->inum) < 0)
    8000544e:	40d0                	lw	a2,4(s1)
    80005450:	fb040593          	addi	a1,s0,-80
    80005454:	854a                	mv	a0,s2
    80005456:	fffff097          	auipc	ra,0xfffff
    8000545a:	cec080e7          	jalr	-788(ra) # 80004142 <dirlink>
    8000545e:	06054b63          	bltz	a0,800054d4 <create+0x154>
  iunlockput(dp);
    80005462:	854a                	mv	a0,s2
    80005464:	fffff097          	auipc	ra,0xfffff
    80005468:	84c080e7          	jalr	-1972(ra) # 80003cb0 <iunlockput>
  return ip;
    8000546c:	b759                	j	800053f2 <create+0x72>
    panic("create: ialloc");
    8000546e:	00003517          	auipc	a0,0x3
    80005472:	2e250513          	addi	a0,a0,738 # 80008750 <syscalls+0x2a8>
    80005476:	ffffb097          	auipc	ra,0xffffb
    8000547a:	0c8080e7          	jalr	200(ra) # 8000053e <panic>
    dp->nlink++;  // for ".."
    8000547e:	04a95783          	lhu	a5,74(s2)
    80005482:	2785                	addiw	a5,a5,1
    80005484:	04f91523          	sh	a5,74(s2)
    iupdate(dp);
    80005488:	854a                	mv	a0,s2
    8000548a:	ffffe097          	auipc	ra,0xffffe
    8000548e:	4fa080e7          	jalr	1274(ra) # 80003984 <iupdate>
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
    80005492:	40d0                	lw	a2,4(s1)
    80005494:	00003597          	auipc	a1,0x3
    80005498:	2cc58593          	addi	a1,a1,716 # 80008760 <syscalls+0x2b8>
    8000549c:	8526                	mv	a0,s1
    8000549e:	fffff097          	auipc	ra,0xfffff
    800054a2:	ca4080e7          	jalr	-860(ra) # 80004142 <dirlink>
    800054a6:	00054f63          	bltz	a0,800054c4 <create+0x144>
    800054aa:	00492603          	lw	a2,4(s2)
    800054ae:	00003597          	auipc	a1,0x3
    800054b2:	2ba58593          	addi	a1,a1,698 # 80008768 <syscalls+0x2c0>
    800054b6:	8526                	mv	a0,s1
    800054b8:	fffff097          	auipc	ra,0xfffff
    800054bc:	c8a080e7          	jalr	-886(ra) # 80004142 <dirlink>
    800054c0:	f80557e3          	bgez	a0,8000544e <create+0xce>
      panic("create dots");
    800054c4:	00003517          	auipc	a0,0x3
    800054c8:	2ac50513          	addi	a0,a0,684 # 80008770 <syscalls+0x2c8>
    800054cc:	ffffb097          	auipc	ra,0xffffb
    800054d0:	072080e7          	jalr	114(ra) # 8000053e <panic>
    panic("create: dirlink");
    800054d4:	00003517          	auipc	a0,0x3
    800054d8:	2ac50513          	addi	a0,a0,684 # 80008780 <syscalls+0x2d8>
    800054dc:	ffffb097          	auipc	ra,0xffffb
    800054e0:	062080e7          	jalr	98(ra) # 8000053e <panic>
    return 0;
    800054e4:	84aa                	mv	s1,a0
    800054e6:	b731                	j	800053f2 <create+0x72>

00000000800054e8 <sys_dup>:
{
    800054e8:	7179                	addi	sp,sp,-48
    800054ea:	f406                	sd	ra,40(sp)
    800054ec:	f022                	sd	s0,32(sp)
    800054ee:	ec26                	sd	s1,24(sp)
    800054f0:	1800                	addi	s0,sp,48
  if(argfd(0, 0, &f) < 0)
    800054f2:	fd840613          	addi	a2,s0,-40
    800054f6:	4581                	li	a1,0
    800054f8:	4501                	li	a0,0
    800054fa:	00000097          	auipc	ra,0x0
    800054fe:	ddc080e7          	jalr	-548(ra) # 800052d6 <argfd>
    return -1;
    80005502:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0)
    80005504:	02054363          	bltz	a0,8000552a <sys_dup+0x42>
  if((fd=fdalloc(f)) < 0)
    80005508:	fd843503          	ld	a0,-40(s0)
    8000550c:	00000097          	auipc	ra,0x0
    80005510:	e32080e7          	jalr	-462(ra) # 8000533e <fdalloc>
    80005514:	84aa                	mv	s1,a0
    return -1;
    80005516:	57fd                	li	a5,-1
  if((fd=fdalloc(f)) < 0)
    80005518:	00054963          	bltz	a0,8000552a <sys_dup+0x42>
  filedup(f);
    8000551c:	fd843503          	ld	a0,-40(s0)
    80005520:	fffff097          	auipc	ra,0xfffff
    80005524:	37a080e7          	jalr	890(ra) # 8000489a <filedup>
  return fd;
    80005528:	87a6                	mv	a5,s1
}
    8000552a:	853e                	mv	a0,a5
    8000552c:	70a2                	ld	ra,40(sp)
    8000552e:	7402                	ld	s0,32(sp)
    80005530:	64e2                	ld	s1,24(sp)
    80005532:	6145                	addi	sp,sp,48
    80005534:	8082                	ret

0000000080005536 <sys_read>:
{
    80005536:	7179                	addi	sp,sp,-48
    80005538:	f406                	sd	ra,40(sp)
    8000553a:	f022                	sd	s0,32(sp)
    8000553c:	1800                	addi	s0,sp,48
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    8000553e:	fe840613          	addi	a2,s0,-24
    80005542:	4581                	li	a1,0
    80005544:	4501                	li	a0,0
    80005546:	00000097          	auipc	ra,0x0
    8000554a:	d90080e7          	jalr	-624(ra) # 800052d6 <argfd>
    return -1;
    8000554e:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    80005550:	04054163          	bltz	a0,80005592 <sys_read+0x5c>
    80005554:	fe440593          	addi	a1,s0,-28
    80005558:	4509                	li	a0,2
    8000555a:	ffffe097          	auipc	ra,0xffffe
    8000555e:	96a080e7          	jalr	-1686(ra) # 80002ec4 <argint>
    return -1;
    80005562:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    80005564:	02054763          	bltz	a0,80005592 <sys_read+0x5c>
    80005568:	fd840593          	addi	a1,s0,-40
    8000556c:	4505                	li	a0,1
    8000556e:	ffffe097          	auipc	ra,0xffffe
    80005572:	978080e7          	jalr	-1672(ra) # 80002ee6 <argaddr>
    return -1;
    80005576:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    80005578:	00054d63          	bltz	a0,80005592 <sys_read+0x5c>
  return fileread(f, p, n);
    8000557c:	fe442603          	lw	a2,-28(s0)
    80005580:	fd843583          	ld	a1,-40(s0)
    80005584:	fe843503          	ld	a0,-24(s0)
    80005588:	fffff097          	auipc	ra,0xfffff
    8000558c:	49e080e7          	jalr	1182(ra) # 80004a26 <fileread>
    80005590:	87aa                	mv	a5,a0
}
    80005592:	853e                	mv	a0,a5
    80005594:	70a2                	ld	ra,40(sp)
    80005596:	7402                	ld	s0,32(sp)
    80005598:	6145                	addi	sp,sp,48
    8000559a:	8082                	ret

000000008000559c <sys_write>:
{
    8000559c:	7179                	addi	sp,sp,-48
    8000559e:	f406                	sd	ra,40(sp)
    800055a0:	f022                	sd	s0,32(sp)
    800055a2:	1800                	addi	s0,sp,48
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    800055a4:	fe840613          	addi	a2,s0,-24
    800055a8:	4581                	li	a1,0
    800055aa:	4501                	li	a0,0
    800055ac:	00000097          	auipc	ra,0x0
    800055b0:	d2a080e7          	jalr	-726(ra) # 800052d6 <argfd>
    return -1;
    800055b4:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    800055b6:	04054163          	bltz	a0,800055f8 <sys_write+0x5c>
    800055ba:	fe440593          	addi	a1,s0,-28
    800055be:	4509                	li	a0,2
    800055c0:	ffffe097          	auipc	ra,0xffffe
    800055c4:	904080e7          	jalr	-1788(ra) # 80002ec4 <argint>
    return -1;
    800055c8:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    800055ca:	02054763          	bltz	a0,800055f8 <sys_write+0x5c>
    800055ce:	fd840593          	addi	a1,s0,-40
    800055d2:	4505                	li	a0,1
    800055d4:	ffffe097          	auipc	ra,0xffffe
    800055d8:	912080e7          	jalr	-1774(ra) # 80002ee6 <argaddr>
    return -1;
    800055dc:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argaddr(1, &p) < 0)
    800055de:	00054d63          	bltz	a0,800055f8 <sys_write+0x5c>
  return filewrite(f, p, n);
    800055e2:	fe442603          	lw	a2,-28(s0)
    800055e6:	fd843583          	ld	a1,-40(s0)
    800055ea:	fe843503          	ld	a0,-24(s0)
    800055ee:	fffff097          	auipc	ra,0xfffff
    800055f2:	4fa080e7          	jalr	1274(ra) # 80004ae8 <filewrite>
    800055f6:	87aa                	mv	a5,a0
}
    800055f8:	853e                	mv	a0,a5
    800055fa:	70a2                	ld	ra,40(sp)
    800055fc:	7402                	ld	s0,32(sp)
    800055fe:	6145                	addi	sp,sp,48
    80005600:	8082                	ret

0000000080005602 <sys_close>:
{
    80005602:	1101                	addi	sp,sp,-32
    80005604:	ec06                	sd	ra,24(sp)
    80005606:	e822                	sd	s0,16(sp)
    80005608:	1000                	addi	s0,sp,32
  if(argfd(0, &fd, &f) < 0)
    8000560a:	fe040613          	addi	a2,s0,-32
    8000560e:	fec40593          	addi	a1,s0,-20
    80005612:	4501                	li	a0,0
    80005614:	00000097          	auipc	ra,0x0
    80005618:	cc2080e7          	jalr	-830(ra) # 800052d6 <argfd>
    return -1;
    8000561c:	57fd                	li	a5,-1
  if(argfd(0, &fd, &f) < 0)
    8000561e:	02054463          	bltz	a0,80005646 <sys_close+0x44>
  myproc()->ofile[fd] = 0;
    80005622:	ffffc097          	auipc	ra,0xffffc
    80005626:	76a080e7          	jalr	1898(ra) # 80001d8c <myproc>
    8000562a:	fec42783          	lw	a5,-20(s0)
    8000562e:	07e9                	addi	a5,a5,26
    80005630:	078e                	slli	a5,a5,0x3
    80005632:	97aa                	add	a5,a5,a0
    80005634:	0007b423          	sd	zero,8(a5)
  fileclose(f);
    80005638:	fe043503          	ld	a0,-32(s0)
    8000563c:	fffff097          	auipc	ra,0xfffff
    80005640:	2b0080e7          	jalr	688(ra) # 800048ec <fileclose>
  return 0;
    80005644:	4781                	li	a5,0
}
    80005646:	853e                	mv	a0,a5
    80005648:	60e2                	ld	ra,24(sp)
    8000564a:	6442                	ld	s0,16(sp)
    8000564c:	6105                	addi	sp,sp,32
    8000564e:	8082                	ret

0000000080005650 <sys_fstat>:
{
    80005650:	1101                	addi	sp,sp,-32
    80005652:	ec06                	sd	ra,24(sp)
    80005654:	e822                	sd	s0,16(sp)
    80005656:	1000                	addi	s0,sp,32
  if(argfd(0, 0, &f) < 0 || argaddr(1, &st) < 0)
    80005658:	fe840613          	addi	a2,s0,-24
    8000565c:	4581                	li	a1,0
    8000565e:	4501                	li	a0,0
    80005660:	00000097          	auipc	ra,0x0
    80005664:	c76080e7          	jalr	-906(ra) # 800052d6 <argfd>
    return -1;
    80005668:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argaddr(1, &st) < 0)
    8000566a:	02054563          	bltz	a0,80005694 <sys_fstat+0x44>
    8000566e:	fe040593          	addi	a1,s0,-32
    80005672:	4505                	li	a0,1
    80005674:	ffffe097          	auipc	ra,0xffffe
    80005678:	872080e7          	jalr	-1934(ra) # 80002ee6 <argaddr>
    return -1;
    8000567c:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0 || argaddr(1, &st) < 0)
    8000567e:	00054b63          	bltz	a0,80005694 <sys_fstat+0x44>
  return filestat(f, st);
    80005682:	fe043583          	ld	a1,-32(s0)
    80005686:	fe843503          	ld	a0,-24(s0)
    8000568a:	fffff097          	auipc	ra,0xfffff
    8000568e:	32a080e7          	jalr	810(ra) # 800049b4 <filestat>
    80005692:	87aa                	mv	a5,a0
}
    80005694:	853e                	mv	a0,a5
    80005696:	60e2                	ld	ra,24(sp)
    80005698:	6442                	ld	s0,16(sp)
    8000569a:	6105                	addi	sp,sp,32
    8000569c:	8082                	ret

000000008000569e <sys_link>:
{
    8000569e:	7169                	addi	sp,sp,-304
    800056a0:	f606                	sd	ra,296(sp)
    800056a2:	f222                	sd	s0,288(sp)
    800056a4:	ee26                	sd	s1,280(sp)
    800056a6:	ea4a                	sd	s2,272(sp)
    800056a8:	1a00                	addi	s0,sp,304
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    800056aa:	08000613          	li	a2,128
    800056ae:	ed040593          	addi	a1,s0,-304
    800056b2:	4501                	li	a0,0
    800056b4:	ffffe097          	auipc	ra,0xffffe
    800056b8:	854080e7          	jalr	-1964(ra) # 80002f08 <argstr>
    return -1;
    800056bc:	57fd                	li	a5,-1
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    800056be:	10054e63          	bltz	a0,800057da <sys_link+0x13c>
    800056c2:	08000613          	li	a2,128
    800056c6:	f5040593          	addi	a1,s0,-176
    800056ca:	4505                	li	a0,1
    800056cc:	ffffe097          	auipc	ra,0xffffe
    800056d0:	83c080e7          	jalr	-1988(ra) # 80002f08 <argstr>
    return -1;
    800056d4:	57fd                	li	a5,-1
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    800056d6:	10054263          	bltz	a0,800057da <sys_link+0x13c>
  begin_op();
    800056da:	fffff097          	auipc	ra,0xfffff
    800056de:	d46080e7          	jalr	-698(ra) # 80004420 <begin_op>
  if((ip = namei(old)) == 0){
    800056e2:	ed040513          	addi	a0,s0,-304
    800056e6:	fffff097          	auipc	ra,0xfffff
    800056ea:	b1e080e7          	jalr	-1250(ra) # 80004204 <namei>
    800056ee:	84aa                	mv	s1,a0
    800056f0:	c551                	beqz	a0,8000577c <sys_link+0xde>
  ilock(ip);
    800056f2:	ffffe097          	auipc	ra,0xffffe
    800056f6:	35c080e7          	jalr	860(ra) # 80003a4e <ilock>
  if(ip->type == T_DIR){
    800056fa:	04449703          	lh	a4,68(s1)
    800056fe:	4785                	li	a5,1
    80005700:	08f70463          	beq	a4,a5,80005788 <sys_link+0xea>
  ip->nlink++;
    80005704:	04a4d783          	lhu	a5,74(s1)
    80005708:	2785                	addiw	a5,a5,1
    8000570a:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    8000570e:	8526                	mv	a0,s1
    80005710:	ffffe097          	auipc	ra,0xffffe
    80005714:	274080e7          	jalr	628(ra) # 80003984 <iupdate>
  iunlock(ip);
    80005718:	8526                	mv	a0,s1
    8000571a:	ffffe097          	auipc	ra,0xffffe
    8000571e:	3f6080e7          	jalr	1014(ra) # 80003b10 <iunlock>
  if((dp = nameiparent(new, name)) == 0)
    80005722:	fd040593          	addi	a1,s0,-48
    80005726:	f5040513          	addi	a0,s0,-176
    8000572a:	fffff097          	auipc	ra,0xfffff
    8000572e:	af8080e7          	jalr	-1288(ra) # 80004222 <nameiparent>
    80005732:	892a                	mv	s2,a0
    80005734:	c935                	beqz	a0,800057a8 <sys_link+0x10a>
  ilock(dp);
    80005736:	ffffe097          	auipc	ra,0xffffe
    8000573a:	318080e7          	jalr	792(ra) # 80003a4e <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
    8000573e:	00092703          	lw	a4,0(s2)
    80005742:	409c                	lw	a5,0(s1)
    80005744:	04f71d63          	bne	a4,a5,8000579e <sys_link+0x100>
    80005748:	40d0                	lw	a2,4(s1)
    8000574a:	fd040593          	addi	a1,s0,-48
    8000574e:	854a                	mv	a0,s2
    80005750:	fffff097          	auipc	ra,0xfffff
    80005754:	9f2080e7          	jalr	-1550(ra) # 80004142 <dirlink>
    80005758:	04054363          	bltz	a0,8000579e <sys_link+0x100>
  iunlockput(dp);
    8000575c:	854a                	mv	a0,s2
    8000575e:	ffffe097          	auipc	ra,0xffffe
    80005762:	552080e7          	jalr	1362(ra) # 80003cb0 <iunlockput>
  iput(ip);
    80005766:	8526                	mv	a0,s1
    80005768:	ffffe097          	auipc	ra,0xffffe
    8000576c:	4a0080e7          	jalr	1184(ra) # 80003c08 <iput>
  end_op();
    80005770:	fffff097          	auipc	ra,0xfffff
    80005774:	d30080e7          	jalr	-720(ra) # 800044a0 <end_op>
  return 0;
    80005778:	4781                	li	a5,0
    8000577a:	a085                	j	800057da <sys_link+0x13c>
    end_op();
    8000577c:	fffff097          	auipc	ra,0xfffff
    80005780:	d24080e7          	jalr	-732(ra) # 800044a0 <end_op>
    return -1;
    80005784:	57fd                	li	a5,-1
    80005786:	a891                	j	800057da <sys_link+0x13c>
    iunlockput(ip);
    80005788:	8526                	mv	a0,s1
    8000578a:	ffffe097          	auipc	ra,0xffffe
    8000578e:	526080e7          	jalr	1318(ra) # 80003cb0 <iunlockput>
    end_op();
    80005792:	fffff097          	auipc	ra,0xfffff
    80005796:	d0e080e7          	jalr	-754(ra) # 800044a0 <end_op>
    return -1;
    8000579a:	57fd                	li	a5,-1
    8000579c:	a83d                	j	800057da <sys_link+0x13c>
    iunlockput(dp);
    8000579e:	854a                	mv	a0,s2
    800057a0:	ffffe097          	auipc	ra,0xffffe
    800057a4:	510080e7          	jalr	1296(ra) # 80003cb0 <iunlockput>
  ilock(ip);
    800057a8:	8526                	mv	a0,s1
    800057aa:	ffffe097          	auipc	ra,0xffffe
    800057ae:	2a4080e7          	jalr	676(ra) # 80003a4e <ilock>
  ip->nlink--;
    800057b2:	04a4d783          	lhu	a5,74(s1)
    800057b6:	37fd                	addiw	a5,a5,-1
    800057b8:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    800057bc:	8526                	mv	a0,s1
    800057be:	ffffe097          	auipc	ra,0xffffe
    800057c2:	1c6080e7          	jalr	454(ra) # 80003984 <iupdate>
  iunlockput(ip);
    800057c6:	8526                	mv	a0,s1
    800057c8:	ffffe097          	auipc	ra,0xffffe
    800057cc:	4e8080e7          	jalr	1256(ra) # 80003cb0 <iunlockput>
  end_op();
    800057d0:	fffff097          	auipc	ra,0xfffff
    800057d4:	cd0080e7          	jalr	-816(ra) # 800044a0 <end_op>
  return -1;
    800057d8:	57fd                	li	a5,-1
}
    800057da:	853e                	mv	a0,a5
    800057dc:	70b2                	ld	ra,296(sp)
    800057de:	7412                	ld	s0,288(sp)
    800057e0:	64f2                	ld	s1,280(sp)
    800057e2:	6952                	ld	s2,272(sp)
    800057e4:	6155                	addi	sp,sp,304
    800057e6:	8082                	ret

00000000800057e8 <sys_unlink>:
{
    800057e8:	7151                	addi	sp,sp,-240
    800057ea:	f586                	sd	ra,232(sp)
    800057ec:	f1a2                	sd	s0,224(sp)
    800057ee:	eda6                	sd	s1,216(sp)
    800057f0:	e9ca                	sd	s2,208(sp)
    800057f2:	e5ce                	sd	s3,200(sp)
    800057f4:	1980                	addi	s0,sp,240
  if(argstr(0, path, MAXPATH) < 0)
    800057f6:	08000613          	li	a2,128
    800057fa:	f3040593          	addi	a1,s0,-208
    800057fe:	4501                	li	a0,0
    80005800:	ffffd097          	auipc	ra,0xffffd
    80005804:	708080e7          	jalr	1800(ra) # 80002f08 <argstr>
    80005808:	18054163          	bltz	a0,8000598a <sys_unlink+0x1a2>
  begin_op();
    8000580c:	fffff097          	auipc	ra,0xfffff
    80005810:	c14080e7          	jalr	-1004(ra) # 80004420 <begin_op>
  if((dp = nameiparent(path, name)) == 0){
    80005814:	fb040593          	addi	a1,s0,-80
    80005818:	f3040513          	addi	a0,s0,-208
    8000581c:	fffff097          	auipc	ra,0xfffff
    80005820:	a06080e7          	jalr	-1530(ra) # 80004222 <nameiparent>
    80005824:	84aa                	mv	s1,a0
    80005826:	c979                	beqz	a0,800058fc <sys_unlink+0x114>
  ilock(dp);
    80005828:	ffffe097          	auipc	ra,0xffffe
    8000582c:	226080e7          	jalr	550(ra) # 80003a4e <ilock>
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
    80005830:	00003597          	auipc	a1,0x3
    80005834:	f3058593          	addi	a1,a1,-208 # 80008760 <syscalls+0x2b8>
    80005838:	fb040513          	addi	a0,s0,-80
    8000583c:	ffffe097          	auipc	ra,0xffffe
    80005840:	6dc080e7          	jalr	1756(ra) # 80003f18 <namecmp>
    80005844:	14050a63          	beqz	a0,80005998 <sys_unlink+0x1b0>
    80005848:	00003597          	auipc	a1,0x3
    8000584c:	f2058593          	addi	a1,a1,-224 # 80008768 <syscalls+0x2c0>
    80005850:	fb040513          	addi	a0,s0,-80
    80005854:	ffffe097          	auipc	ra,0xffffe
    80005858:	6c4080e7          	jalr	1732(ra) # 80003f18 <namecmp>
    8000585c:	12050e63          	beqz	a0,80005998 <sys_unlink+0x1b0>
  if((ip = dirlookup(dp, name, &off)) == 0)
    80005860:	f2c40613          	addi	a2,s0,-212
    80005864:	fb040593          	addi	a1,s0,-80
    80005868:	8526                	mv	a0,s1
    8000586a:	ffffe097          	auipc	ra,0xffffe
    8000586e:	6c8080e7          	jalr	1736(ra) # 80003f32 <dirlookup>
    80005872:	892a                	mv	s2,a0
    80005874:	12050263          	beqz	a0,80005998 <sys_unlink+0x1b0>
  ilock(ip);
    80005878:	ffffe097          	auipc	ra,0xffffe
    8000587c:	1d6080e7          	jalr	470(ra) # 80003a4e <ilock>
  if(ip->nlink < 1)
    80005880:	04a91783          	lh	a5,74(s2)
    80005884:	08f05263          	blez	a5,80005908 <sys_unlink+0x120>
  if(ip->type == T_DIR && !isdirempty(ip)){
    80005888:	04491703          	lh	a4,68(s2)
    8000588c:	4785                	li	a5,1
    8000588e:	08f70563          	beq	a4,a5,80005918 <sys_unlink+0x130>
  memset(&de, 0, sizeof(de));
    80005892:	4641                	li	a2,16
    80005894:	4581                	li	a1,0
    80005896:	fc040513          	addi	a0,s0,-64
    8000589a:	ffffb097          	auipc	ra,0xffffb
    8000589e:	446080e7          	jalr	1094(ra) # 80000ce0 <memset>
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    800058a2:	4741                	li	a4,16
    800058a4:	f2c42683          	lw	a3,-212(s0)
    800058a8:	fc040613          	addi	a2,s0,-64
    800058ac:	4581                	li	a1,0
    800058ae:	8526                	mv	a0,s1
    800058b0:	ffffe097          	auipc	ra,0xffffe
    800058b4:	54a080e7          	jalr	1354(ra) # 80003dfa <writei>
    800058b8:	47c1                	li	a5,16
    800058ba:	0af51563          	bne	a0,a5,80005964 <sys_unlink+0x17c>
  if(ip->type == T_DIR){
    800058be:	04491703          	lh	a4,68(s2)
    800058c2:	4785                	li	a5,1
    800058c4:	0af70863          	beq	a4,a5,80005974 <sys_unlink+0x18c>
  iunlockput(dp);
    800058c8:	8526                	mv	a0,s1
    800058ca:	ffffe097          	auipc	ra,0xffffe
    800058ce:	3e6080e7          	jalr	998(ra) # 80003cb0 <iunlockput>
  ip->nlink--;
    800058d2:	04a95783          	lhu	a5,74(s2)
    800058d6:	37fd                	addiw	a5,a5,-1
    800058d8:	04f91523          	sh	a5,74(s2)
  iupdate(ip);
    800058dc:	854a                	mv	a0,s2
    800058de:	ffffe097          	auipc	ra,0xffffe
    800058e2:	0a6080e7          	jalr	166(ra) # 80003984 <iupdate>
  iunlockput(ip);
    800058e6:	854a                	mv	a0,s2
    800058e8:	ffffe097          	auipc	ra,0xffffe
    800058ec:	3c8080e7          	jalr	968(ra) # 80003cb0 <iunlockput>
  end_op();
    800058f0:	fffff097          	auipc	ra,0xfffff
    800058f4:	bb0080e7          	jalr	-1104(ra) # 800044a0 <end_op>
  return 0;
    800058f8:	4501                	li	a0,0
    800058fa:	a84d                	j	800059ac <sys_unlink+0x1c4>
    end_op();
    800058fc:	fffff097          	auipc	ra,0xfffff
    80005900:	ba4080e7          	jalr	-1116(ra) # 800044a0 <end_op>
    return -1;
    80005904:	557d                	li	a0,-1
    80005906:	a05d                	j	800059ac <sys_unlink+0x1c4>
    panic("unlink: nlink < 1");
    80005908:	00003517          	auipc	a0,0x3
    8000590c:	e8850513          	addi	a0,a0,-376 # 80008790 <syscalls+0x2e8>
    80005910:	ffffb097          	auipc	ra,0xffffb
    80005914:	c2e080e7          	jalr	-978(ra) # 8000053e <panic>
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    80005918:	04c92703          	lw	a4,76(s2)
    8000591c:	02000793          	li	a5,32
    80005920:	f6e7f9e3          	bgeu	a5,a4,80005892 <sys_unlink+0xaa>
    80005924:	02000993          	li	s3,32
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80005928:	4741                	li	a4,16
    8000592a:	86ce                	mv	a3,s3
    8000592c:	f1840613          	addi	a2,s0,-232
    80005930:	4581                	li	a1,0
    80005932:	854a                	mv	a0,s2
    80005934:	ffffe097          	auipc	ra,0xffffe
    80005938:	3ce080e7          	jalr	974(ra) # 80003d02 <readi>
    8000593c:	47c1                	li	a5,16
    8000593e:	00f51b63          	bne	a0,a5,80005954 <sys_unlink+0x16c>
    if(de.inum != 0)
    80005942:	f1845783          	lhu	a5,-232(s0)
    80005946:	e7a1                	bnez	a5,8000598e <sys_unlink+0x1a6>
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    80005948:	29c1                	addiw	s3,s3,16
    8000594a:	04c92783          	lw	a5,76(s2)
    8000594e:	fcf9ede3          	bltu	s3,a5,80005928 <sys_unlink+0x140>
    80005952:	b781                	j	80005892 <sys_unlink+0xaa>
      panic("isdirempty: readi");
    80005954:	00003517          	auipc	a0,0x3
    80005958:	e5450513          	addi	a0,a0,-428 # 800087a8 <syscalls+0x300>
    8000595c:	ffffb097          	auipc	ra,0xffffb
    80005960:	be2080e7          	jalr	-1054(ra) # 8000053e <panic>
    panic("unlink: writei");
    80005964:	00003517          	auipc	a0,0x3
    80005968:	e5c50513          	addi	a0,a0,-420 # 800087c0 <syscalls+0x318>
    8000596c:	ffffb097          	auipc	ra,0xffffb
    80005970:	bd2080e7          	jalr	-1070(ra) # 8000053e <panic>
    dp->nlink--;
    80005974:	04a4d783          	lhu	a5,74(s1)
    80005978:	37fd                	addiw	a5,a5,-1
    8000597a:	04f49523          	sh	a5,74(s1)
    iupdate(dp);
    8000597e:	8526                	mv	a0,s1
    80005980:	ffffe097          	auipc	ra,0xffffe
    80005984:	004080e7          	jalr	4(ra) # 80003984 <iupdate>
    80005988:	b781                	j	800058c8 <sys_unlink+0xe0>
    return -1;
    8000598a:	557d                	li	a0,-1
    8000598c:	a005                	j	800059ac <sys_unlink+0x1c4>
    iunlockput(ip);
    8000598e:	854a                	mv	a0,s2
    80005990:	ffffe097          	auipc	ra,0xffffe
    80005994:	320080e7          	jalr	800(ra) # 80003cb0 <iunlockput>
  iunlockput(dp);
    80005998:	8526                	mv	a0,s1
    8000599a:	ffffe097          	auipc	ra,0xffffe
    8000599e:	316080e7          	jalr	790(ra) # 80003cb0 <iunlockput>
  end_op();
    800059a2:	fffff097          	auipc	ra,0xfffff
    800059a6:	afe080e7          	jalr	-1282(ra) # 800044a0 <end_op>
  return -1;
    800059aa:	557d                	li	a0,-1
}
    800059ac:	70ae                	ld	ra,232(sp)
    800059ae:	740e                	ld	s0,224(sp)
    800059b0:	64ee                	ld	s1,216(sp)
    800059b2:	694e                	ld	s2,208(sp)
    800059b4:	69ae                	ld	s3,200(sp)
    800059b6:	616d                	addi	sp,sp,240
    800059b8:	8082                	ret

00000000800059ba <sys_open>:

uint64
sys_open(void)
{
    800059ba:	7131                	addi	sp,sp,-192
    800059bc:	fd06                	sd	ra,184(sp)
    800059be:	f922                	sd	s0,176(sp)
    800059c0:	f526                	sd	s1,168(sp)
    800059c2:	f14a                	sd	s2,160(sp)
    800059c4:	ed4e                	sd	s3,152(sp)
    800059c6:	0180                	addi	s0,sp,192
  int fd, omode;
  struct file *f;
  struct inode *ip;
  int n;

  if((n = argstr(0, path, MAXPATH)) < 0 || argint(1, &omode) < 0)
    800059c8:	08000613          	li	a2,128
    800059cc:	f5040593          	addi	a1,s0,-176
    800059d0:	4501                	li	a0,0
    800059d2:	ffffd097          	auipc	ra,0xffffd
    800059d6:	536080e7          	jalr	1334(ra) # 80002f08 <argstr>
    return -1;
    800059da:	54fd                	li	s1,-1
  if((n = argstr(0, path, MAXPATH)) < 0 || argint(1, &omode) < 0)
    800059dc:	0c054163          	bltz	a0,80005a9e <sys_open+0xe4>
    800059e0:	f4c40593          	addi	a1,s0,-180
    800059e4:	4505                	li	a0,1
    800059e6:	ffffd097          	auipc	ra,0xffffd
    800059ea:	4de080e7          	jalr	1246(ra) # 80002ec4 <argint>
    800059ee:	0a054863          	bltz	a0,80005a9e <sys_open+0xe4>

  begin_op();
    800059f2:	fffff097          	auipc	ra,0xfffff
    800059f6:	a2e080e7          	jalr	-1490(ra) # 80004420 <begin_op>

  if(omode & O_CREATE){
    800059fa:	f4c42783          	lw	a5,-180(s0)
    800059fe:	2007f793          	andi	a5,a5,512
    80005a02:	cbdd                	beqz	a5,80005ab8 <sys_open+0xfe>
    ip = create(path, T_FILE, 0, 0);
    80005a04:	4681                	li	a3,0
    80005a06:	4601                	li	a2,0
    80005a08:	4589                	li	a1,2
    80005a0a:	f5040513          	addi	a0,s0,-176
    80005a0e:	00000097          	auipc	ra,0x0
    80005a12:	972080e7          	jalr	-1678(ra) # 80005380 <create>
    80005a16:	892a                	mv	s2,a0
    if(ip == 0){
    80005a18:	c959                	beqz	a0,80005aae <sys_open+0xf4>
      end_op();
      return -1;
    }
  }

  if(ip->type == T_DEVICE && (ip->major < 0 || ip->major >= NDEV)){
    80005a1a:	04491703          	lh	a4,68(s2)
    80005a1e:	478d                	li	a5,3
    80005a20:	00f71763          	bne	a4,a5,80005a2e <sys_open+0x74>
    80005a24:	04695703          	lhu	a4,70(s2)
    80005a28:	47a5                	li	a5,9
    80005a2a:	0ce7ec63          	bltu	a5,a4,80005b02 <sys_open+0x148>
    iunlockput(ip);
    end_op();
    return -1;
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
    80005a2e:	fffff097          	auipc	ra,0xfffff
    80005a32:	e02080e7          	jalr	-510(ra) # 80004830 <filealloc>
    80005a36:	89aa                	mv	s3,a0
    80005a38:	10050263          	beqz	a0,80005b3c <sys_open+0x182>
    80005a3c:	00000097          	auipc	ra,0x0
    80005a40:	902080e7          	jalr	-1790(ra) # 8000533e <fdalloc>
    80005a44:	84aa                	mv	s1,a0
    80005a46:	0e054663          	bltz	a0,80005b32 <sys_open+0x178>
    iunlockput(ip);
    end_op();
    return -1;
  }

  if(ip->type == T_DEVICE){
    80005a4a:	04491703          	lh	a4,68(s2)
    80005a4e:	478d                	li	a5,3
    80005a50:	0cf70463          	beq	a4,a5,80005b18 <sys_open+0x15e>
    f->type = FD_DEVICE;
    f->major = ip->major;
  } else {
    f->type = FD_INODE;
    80005a54:	4789                	li	a5,2
    80005a56:	00f9a023          	sw	a5,0(s3)
    f->off = 0;
    80005a5a:	0209a023          	sw	zero,32(s3)
  }
  f->ip = ip;
    80005a5e:	0129bc23          	sd	s2,24(s3)
  f->readable = !(omode & O_WRONLY);
    80005a62:	f4c42783          	lw	a5,-180(s0)
    80005a66:	0017c713          	xori	a4,a5,1
    80005a6a:	8b05                	andi	a4,a4,1
    80005a6c:	00e98423          	sb	a4,8(s3)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
    80005a70:	0037f713          	andi	a4,a5,3
    80005a74:	00e03733          	snez	a4,a4
    80005a78:	00e984a3          	sb	a4,9(s3)

  if((omode & O_TRUNC) && ip->type == T_FILE){
    80005a7c:	4007f793          	andi	a5,a5,1024
    80005a80:	c791                	beqz	a5,80005a8c <sys_open+0xd2>
    80005a82:	04491703          	lh	a4,68(s2)
    80005a86:	4789                	li	a5,2
    80005a88:	08f70f63          	beq	a4,a5,80005b26 <sys_open+0x16c>
    itrunc(ip);
  }

  iunlock(ip);
    80005a8c:	854a                	mv	a0,s2
    80005a8e:	ffffe097          	auipc	ra,0xffffe
    80005a92:	082080e7          	jalr	130(ra) # 80003b10 <iunlock>
  end_op();
    80005a96:	fffff097          	auipc	ra,0xfffff
    80005a9a:	a0a080e7          	jalr	-1526(ra) # 800044a0 <end_op>

  return fd;
}
    80005a9e:	8526                	mv	a0,s1
    80005aa0:	70ea                	ld	ra,184(sp)
    80005aa2:	744a                	ld	s0,176(sp)
    80005aa4:	74aa                	ld	s1,168(sp)
    80005aa6:	790a                	ld	s2,160(sp)
    80005aa8:	69ea                	ld	s3,152(sp)
    80005aaa:	6129                	addi	sp,sp,192
    80005aac:	8082                	ret
      end_op();
    80005aae:	fffff097          	auipc	ra,0xfffff
    80005ab2:	9f2080e7          	jalr	-1550(ra) # 800044a0 <end_op>
      return -1;
    80005ab6:	b7e5                	j	80005a9e <sys_open+0xe4>
    if((ip = namei(path)) == 0){
    80005ab8:	f5040513          	addi	a0,s0,-176
    80005abc:	ffffe097          	auipc	ra,0xffffe
    80005ac0:	748080e7          	jalr	1864(ra) # 80004204 <namei>
    80005ac4:	892a                	mv	s2,a0
    80005ac6:	c905                	beqz	a0,80005af6 <sys_open+0x13c>
    ilock(ip);
    80005ac8:	ffffe097          	auipc	ra,0xffffe
    80005acc:	f86080e7          	jalr	-122(ra) # 80003a4e <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
    80005ad0:	04491703          	lh	a4,68(s2)
    80005ad4:	4785                	li	a5,1
    80005ad6:	f4f712e3          	bne	a4,a5,80005a1a <sys_open+0x60>
    80005ada:	f4c42783          	lw	a5,-180(s0)
    80005ade:	dba1                	beqz	a5,80005a2e <sys_open+0x74>
      iunlockput(ip);
    80005ae0:	854a                	mv	a0,s2
    80005ae2:	ffffe097          	auipc	ra,0xffffe
    80005ae6:	1ce080e7          	jalr	462(ra) # 80003cb0 <iunlockput>
      end_op();
    80005aea:	fffff097          	auipc	ra,0xfffff
    80005aee:	9b6080e7          	jalr	-1610(ra) # 800044a0 <end_op>
      return -1;
    80005af2:	54fd                	li	s1,-1
    80005af4:	b76d                	j	80005a9e <sys_open+0xe4>
      end_op();
    80005af6:	fffff097          	auipc	ra,0xfffff
    80005afa:	9aa080e7          	jalr	-1622(ra) # 800044a0 <end_op>
      return -1;
    80005afe:	54fd                	li	s1,-1
    80005b00:	bf79                	j	80005a9e <sys_open+0xe4>
    iunlockput(ip);
    80005b02:	854a                	mv	a0,s2
    80005b04:	ffffe097          	auipc	ra,0xffffe
    80005b08:	1ac080e7          	jalr	428(ra) # 80003cb0 <iunlockput>
    end_op();
    80005b0c:	fffff097          	auipc	ra,0xfffff
    80005b10:	994080e7          	jalr	-1644(ra) # 800044a0 <end_op>
    return -1;
    80005b14:	54fd                	li	s1,-1
    80005b16:	b761                	j	80005a9e <sys_open+0xe4>
    f->type = FD_DEVICE;
    80005b18:	00f9a023          	sw	a5,0(s3)
    f->major = ip->major;
    80005b1c:	04691783          	lh	a5,70(s2)
    80005b20:	02f99223          	sh	a5,36(s3)
    80005b24:	bf2d                	j	80005a5e <sys_open+0xa4>
    itrunc(ip);
    80005b26:	854a                	mv	a0,s2
    80005b28:	ffffe097          	auipc	ra,0xffffe
    80005b2c:	034080e7          	jalr	52(ra) # 80003b5c <itrunc>
    80005b30:	bfb1                	j	80005a8c <sys_open+0xd2>
      fileclose(f);
    80005b32:	854e                	mv	a0,s3
    80005b34:	fffff097          	auipc	ra,0xfffff
    80005b38:	db8080e7          	jalr	-584(ra) # 800048ec <fileclose>
    iunlockput(ip);
    80005b3c:	854a                	mv	a0,s2
    80005b3e:	ffffe097          	auipc	ra,0xffffe
    80005b42:	172080e7          	jalr	370(ra) # 80003cb0 <iunlockput>
    end_op();
    80005b46:	fffff097          	auipc	ra,0xfffff
    80005b4a:	95a080e7          	jalr	-1702(ra) # 800044a0 <end_op>
    return -1;
    80005b4e:	54fd                	li	s1,-1
    80005b50:	b7b9                	j	80005a9e <sys_open+0xe4>

0000000080005b52 <sys_mkdir>:
// }


uint64
sys_mkdir(void)
{
    80005b52:	7175                	addi	sp,sp,-144
    80005b54:	e506                	sd	ra,136(sp)
    80005b56:	e122                	sd	s0,128(sp)
    80005b58:	0900                	addi	s0,sp,144
  char path[MAXPATH];
  struct inode *ip;

  begin_op();
    80005b5a:	fffff097          	auipc	ra,0xfffff
    80005b5e:	8c6080e7          	jalr	-1850(ra) # 80004420 <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
    80005b62:	08000613          	li	a2,128
    80005b66:	f7040593          	addi	a1,s0,-144
    80005b6a:	4501                	li	a0,0
    80005b6c:	ffffd097          	auipc	ra,0xffffd
    80005b70:	39c080e7          	jalr	924(ra) # 80002f08 <argstr>
    80005b74:	02054963          	bltz	a0,80005ba6 <sys_mkdir+0x54>
    80005b78:	4681                	li	a3,0
    80005b7a:	4601                	li	a2,0
    80005b7c:	4585                	li	a1,1
    80005b7e:	f7040513          	addi	a0,s0,-144
    80005b82:	fffff097          	auipc	ra,0xfffff
    80005b86:	7fe080e7          	jalr	2046(ra) # 80005380 <create>
    80005b8a:	cd11                	beqz	a0,80005ba6 <sys_mkdir+0x54>
    end_op();
    return -1;
  }
  iunlockput(ip);
    80005b8c:	ffffe097          	auipc	ra,0xffffe
    80005b90:	124080e7          	jalr	292(ra) # 80003cb0 <iunlockput>
  end_op();
    80005b94:	fffff097          	auipc	ra,0xfffff
    80005b98:	90c080e7          	jalr	-1780(ra) # 800044a0 <end_op>
  return 0;
    80005b9c:	4501                	li	a0,0
}
    80005b9e:	60aa                	ld	ra,136(sp)
    80005ba0:	640a                	ld	s0,128(sp)
    80005ba2:	6149                	addi	sp,sp,144
    80005ba4:	8082                	ret
    end_op();
    80005ba6:	fffff097          	auipc	ra,0xfffff
    80005baa:	8fa080e7          	jalr	-1798(ra) # 800044a0 <end_op>
    return -1;
    80005bae:	557d                	li	a0,-1
    80005bb0:	b7fd                	j	80005b9e <sys_mkdir+0x4c>

0000000080005bb2 <sys_mknod>:

uint64
sys_mknod(void)
{
    80005bb2:	7135                	addi	sp,sp,-160
    80005bb4:	ed06                	sd	ra,152(sp)
    80005bb6:	e922                	sd	s0,144(sp)
    80005bb8:	1100                	addi	s0,sp,160
  struct inode *ip;
  char path[MAXPATH];
  int major, minor;

  begin_op();
    80005bba:	fffff097          	auipc	ra,0xfffff
    80005bbe:	866080e7          	jalr	-1946(ra) # 80004420 <begin_op>
  if((argstr(0, path, MAXPATH)) < 0 ||
    80005bc2:	08000613          	li	a2,128
    80005bc6:	f7040593          	addi	a1,s0,-144
    80005bca:	4501                	li	a0,0
    80005bcc:	ffffd097          	auipc	ra,0xffffd
    80005bd0:	33c080e7          	jalr	828(ra) # 80002f08 <argstr>
    80005bd4:	04054a63          	bltz	a0,80005c28 <sys_mknod+0x76>
     argint(1, &major) < 0 ||
    80005bd8:	f6c40593          	addi	a1,s0,-148
    80005bdc:	4505                	li	a0,1
    80005bde:	ffffd097          	auipc	ra,0xffffd
    80005be2:	2e6080e7          	jalr	742(ra) # 80002ec4 <argint>
  if((argstr(0, path, MAXPATH)) < 0 ||
    80005be6:	04054163          	bltz	a0,80005c28 <sys_mknod+0x76>
     argint(2, &minor) < 0 ||
    80005bea:	f6840593          	addi	a1,s0,-152
    80005bee:	4509                	li	a0,2
    80005bf0:	ffffd097          	auipc	ra,0xffffd
    80005bf4:	2d4080e7          	jalr	724(ra) # 80002ec4 <argint>
     argint(1, &major) < 0 ||
    80005bf8:	02054863          	bltz	a0,80005c28 <sys_mknod+0x76>
     (ip = create(path, T_DEVICE, major, minor)) == 0){
    80005bfc:	f6841683          	lh	a3,-152(s0)
    80005c00:	f6c41603          	lh	a2,-148(s0)
    80005c04:	458d                	li	a1,3
    80005c06:	f7040513          	addi	a0,s0,-144
    80005c0a:	fffff097          	auipc	ra,0xfffff
    80005c0e:	776080e7          	jalr	1910(ra) # 80005380 <create>
     argint(2, &minor) < 0 ||
    80005c12:	c919                	beqz	a0,80005c28 <sys_mknod+0x76>
    end_op();
    return -1;
  }
  iunlockput(ip);
    80005c14:	ffffe097          	auipc	ra,0xffffe
    80005c18:	09c080e7          	jalr	156(ra) # 80003cb0 <iunlockput>
  end_op();
    80005c1c:	fffff097          	auipc	ra,0xfffff
    80005c20:	884080e7          	jalr	-1916(ra) # 800044a0 <end_op>
  return 0;
    80005c24:	4501                	li	a0,0
    80005c26:	a031                	j	80005c32 <sys_mknod+0x80>
    end_op();
    80005c28:	fffff097          	auipc	ra,0xfffff
    80005c2c:	878080e7          	jalr	-1928(ra) # 800044a0 <end_op>
    return -1;
    80005c30:	557d                	li	a0,-1
}
    80005c32:	60ea                	ld	ra,152(sp)
    80005c34:	644a                	ld	s0,144(sp)
    80005c36:	610d                	addi	sp,sp,160
    80005c38:	8082                	ret

0000000080005c3a <sys_chdir>:

uint64
sys_chdir(void)
{
    80005c3a:	7135                	addi	sp,sp,-160
    80005c3c:	ed06                	sd	ra,152(sp)
    80005c3e:	e922                	sd	s0,144(sp)
    80005c40:	e526                	sd	s1,136(sp)
    80005c42:	e14a                	sd	s2,128(sp)
    80005c44:	1100                	addi	s0,sp,160
  char path[MAXPATH];
  struct inode *ip;
  struct proc *p = myproc();
    80005c46:	ffffc097          	auipc	ra,0xffffc
    80005c4a:	146080e7          	jalr	326(ra) # 80001d8c <myproc>
    80005c4e:	892a                	mv	s2,a0
  
  begin_op();
    80005c50:	ffffe097          	auipc	ra,0xffffe
    80005c54:	7d0080e7          	jalr	2000(ra) # 80004420 <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = namei(path)) == 0){
    80005c58:	08000613          	li	a2,128
    80005c5c:	f6040593          	addi	a1,s0,-160
    80005c60:	4501                	li	a0,0
    80005c62:	ffffd097          	auipc	ra,0xffffd
    80005c66:	2a6080e7          	jalr	678(ra) # 80002f08 <argstr>
    80005c6a:	04054b63          	bltz	a0,80005cc0 <sys_chdir+0x86>
    80005c6e:	f6040513          	addi	a0,s0,-160
    80005c72:	ffffe097          	auipc	ra,0xffffe
    80005c76:	592080e7          	jalr	1426(ra) # 80004204 <namei>
    80005c7a:	84aa                	mv	s1,a0
    80005c7c:	c131                	beqz	a0,80005cc0 <sys_chdir+0x86>
    end_op();
    return -1;
  }
  ilock(ip);
    80005c7e:	ffffe097          	auipc	ra,0xffffe
    80005c82:	dd0080e7          	jalr	-560(ra) # 80003a4e <ilock>
  if(ip->type != T_DIR){
    80005c86:	04449703          	lh	a4,68(s1)
    80005c8a:	4785                	li	a5,1
    80005c8c:	04f71063          	bne	a4,a5,80005ccc <sys_chdir+0x92>
    iunlockput(ip);
    end_op();
    return -1;
  }
  iunlock(ip);
    80005c90:	8526                	mv	a0,s1
    80005c92:	ffffe097          	auipc	ra,0xffffe
    80005c96:	e7e080e7          	jalr	-386(ra) # 80003b10 <iunlock>
  iput(p->cwd);
    80005c9a:	15893503          	ld	a0,344(s2)
    80005c9e:	ffffe097          	auipc	ra,0xffffe
    80005ca2:	f6a080e7          	jalr	-150(ra) # 80003c08 <iput>
  end_op();
    80005ca6:	ffffe097          	auipc	ra,0xffffe
    80005caa:	7fa080e7          	jalr	2042(ra) # 800044a0 <end_op>
  p->cwd = ip;
    80005cae:	14993c23          	sd	s1,344(s2)
  return 0;
    80005cb2:	4501                	li	a0,0
}
    80005cb4:	60ea                	ld	ra,152(sp)
    80005cb6:	644a                	ld	s0,144(sp)
    80005cb8:	64aa                	ld	s1,136(sp)
    80005cba:	690a                	ld	s2,128(sp)
    80005cbc:	610d                	addi	sp,sp,160
    80005cbe:	8082                	ret
    end_op();
    80005cc0:	ffffe097          	auipc	ra,0xffffe
    80005cc4:	7e0080e7          	jalr	2016(ra) # 800044a0 <end_op>
    return -1;
    80005cc8:	557d                	li	a0,-1
    80005cca:	b7ed                	j	80005cb4 <sys_chdir+0x7a>
    iunlockput(ip);
    80005ccc:	8526                	mv	a0,s1
    80005cce:	ffffe097          	auipc	ra,0xffffe
    80005cd2:	fe2080e7          	jalr	-30(ra) # 80003cb0 <iunlockput>
    end_op();
    80005cd6:	ffffe097          	auipc	ra,0xffffe
    80005cda:	7ca080e7          	jalr	1994(ra) # 800044a0 <end_op>
    return -1;
    80005cde:	557d                	li	a0,-1
    80005ce0:	bfd1                	j	80005cb4 <sys_chdir+0x7a>

0000000080005ce2 <sys_exec>:

uint64
sys_exec(void)
{
    80005ce2:	7145                	addi	sp,sp,-464
    80005ce4:	e786                	sd	ra,456(sp)
    80005ce6:	e3a2                	sd	s0,448(sp)
    80005ce8:	ff26                	sd	s1,440(sp)
    80005cea:	fb4a                	sd	s2,432(sp)
    80005cec:	f74e                	sd	s3,424(sp)
    80005cee:	f352                	sd	s4,416(sp)
    80005cf0:	ef56                	sd	s5,408(sp)
    80005cf2:	0b80                	addi	s0,sp,464
  char path[MAXPATH], *argv[MAXARG];
  int i;
  uint64 uargv, uarg;

  if(argstr(0, path, MAXPATH) < 0 || argaddr(1, &uargv) < 0){
    80005cf4:	08000613          	li	a2,128
    80005cf8:	f4040593          	addi	a1,s0,-192
    80005cfc:	4501                	li	a0,0
    80005cfe:	ffffd097          	auipc	ra,0xffffd
    80005d02:	20a080e7          	jalr	522(ra) # 80002f08 <argstr>
    return -1;
    80005d06:	597d                	li	s2,-1
  if(argstr(0, path, MAXPATH) < 0 || argaddr(1, &uargv) < 0){
    80005d08:	0c054a63          	bltz	a0,80005ddc <sys_exec+0xfa>
    80005d0c:	e3840593          	addi	a1,s0,-456
    80005d10:	4505                	li	a0,1
    80005d12:	ffffd097          	auipc	ra,0xffffd
    80005d16:	1d4080e7          	jalr	468(ra) # 80002ee6 <argaddr>
    80005d1a:	0c054163          	bltz	a0,80005ddc <sys_exec+0xfa>
  }
  memset(argv, 0, sizeof(argv));
    80005d1e:	10000613          	li	a2,256
    80005d22:	4581                	li	a1,0
    80005d24:	e4040513          	addi	a0,s0,-448
    80005d28:	ffffb097          	auipc	ra,0xffffb
    80005d2c:	fb8080e7          	jalr	-72(ra) # 80000ce0 <memset>
  for(i=0;; i++){
    if(i >= NELEM(argv)){
    80005d30:	e4040493          	addi	s1,s0,-448
  memset(argv, 0, sizeof(argv));
    80005d34:	89a6                	mv	s3,s1
    80005d36:	4901                	li	s2,0
    if(i >= NELEM(argv)){
    80005d38:	02000a13          	li	s4,32
    80005d3c:	00090a9b          	sext.w	s5,s2
      goto bad;
    }
    if(fetchaddr(uargv+sizeof(uint64)*i, (uint64*)&uarg) < 0){
    80005d40:	00391513          	slli	a0,s2,0x3
    80005d44:	e3040593          	addi	a1,s0,-464
    80005d48:	e3843783          	ld	a5,-456(s0)
    80005d4c:	953e                	add	a0,a0,a5
    80005d4e:	ffffd097          	auipc	ra,0xffffd
    80005d52:	0dc080e7          	jalr	220(ra) # 80002e2a <fetchaddr>
    80005d56:	02054a63          	bltz	a0,80005d8a <sys_exec+0xa8>
      goto bad;
    }
    if(uarg == 0){
    80005d5a:	e3043783          	ld	a5,-464(s0)
    80005d5e:	c3b9                	beqz	a5,80005da4 <sys_exec+0xc2>
      argv[i] = 0;
      break;
    }
    argv[i] = kalloc();
    80005d60:	ffffb097          	auipc	ra,0xffffb
    80005d64:	d94080e7          	jalr	-620(ra) # 80000af4 <kalloc>
    80005d68:	85aa                	mv	a1,a0
    80005d6a:	00a9b023          	sd	a0,0(s3)
    if(argv[i] == 0)
    80005d6e:	cd11                	beqz	a0,80005d8a <sys_exec+0xa8>
      goto bad;
    if(fetchstr(uarg, argv[i], PGSIZE) < 0)
    80005d70:	6605                	lui	a2,0x1
    80005d72:	e3043503          	ld	a0,-464(s0)
    80005d76:	ffffd097          	auipc	ra,0xffffd
    80005d7a:	106080e7          	jalr	262(ra) # 80002e7c <fetchstr>
    80005d7e:	00054663          	bltz	a0,80005d8a <sys_exec+0xa8>
    if(i >= NELEM(argv)){
    80005d82:	0905                	addi	s2,s2,1
    80005d84:	09a1                	addi	s3,s3,8
    80005d86:	fb491be3          	bne	s2,s4,80005d3c <sys_exec+0x5a>
    kfree(argv[i]);

  return ret;

 bad:
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005d8a:	10048913          	addi	s2,s1,256
    80005d8e:	6088                	ld	a0,0(s1)
    80005d90:	c529                	beqz	a0,80005dda <sys_exec+0xf8>
    kfree(argv[i]);
    80005d92:	ffffb097          	auipc	ra,0xffffb
    80005d96:	c66080e7          	jalr	-922(ra) # 800009f8 <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005d9a:	04a1                	addi	s1,s1,8
    80005d9c:	ff2499e3          	bne	s1,s2,80005d8e <sys_exec+0xac>
  return -1;
    80005da0:	597d                	li	s2,-1
    80005da2:	a82d                	j	80005ddc <sys_exec+0xfa>
      argv[i] = 0;
    80005da4:	0a8e                	slli	s5,s5,0x3
    80005da6:	fc040793          	addi	a5,s0,-64
    80005daa:	9abe                	add	s5,s5,a5
    80005dac:	e80ab023          	sd	zero,-384(s5)
  int ret = exec(path, argv);
    80005db0:	e4040593          	addi	a1,s0,-448
    80005db4:	f4040513          	addi	a0,s0,-192
    80005db8:	fffff097          	auipc	ra,0xfffff
    80005dbc:	194080e7          	jalr	404(ra) # 80004f4c <exec>
    80005dc0:	892a                	mv	s2,a0
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005dc2:	10048993          	addi	s3,s1,256
    80005dc6:	6088                	ld	a0,0(s1)
    80005dc8:	c911                	beqz	a0,80005ddc <sys_exec+0xfa>
    kfree(argv[i]);
    80005dca:	ffffb097          	auipc	ra,0xffffb
    80005dce:	c2e080e7          	jalr	-978(ra) # 800009f8 <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005dd2:	04a1                	addi	s1,s1,8
    80005dd4:	ff3499e3          	bne	s1,s3,80005dc6 <sys_exec+0xe4>
    80005dd8:	a011                	j	80005ddc <sys_exec+0xfa>
  return -1;
    80005dda:	597d                	li	s2,-1
}
    80005ddc:	854a                	mv	a0,s2
    80005dde:	60be                	ld	ra,456(sp)
    80005de0:	641e                	ld	s0,448(sp)
    80005de2:	74fa                	ld	s1,440(sp)
    80005de4:	795a                	ld	s2,432(sp)
    80005de6:	79ba                	ld	s3,424(sp)
    80005de8:	7a1a                	ld	s4,416(sp)
    80005dea:	6afa                	ld	s5,408(sp)
    80005dec:	6179                	addi	sp,sp,464
    80005dee:	8082                	ret

0000000080005df0 <sys_pipe>:

uint64
sys_pipe(void)
{
    80005df0:	7139                	addi	sp,sp,-64
    80005df2:	fc06                	sd	ra,56(sp)
    80005df4:	f822                	sd	s0,48(sp)
    80005df6:	f426                	sd	s1,40(sp)
    80005df8:	0080                	addi	s0,sp,64
  uint64 fdarray; // user pointer to array of two integers
  struct file *rf, *wf;
  int fd0, fd1;
  struct proc *p = myproc();
    80005dfa:	ffffc097          	auipc	ra,0xffffc
    80005dfe:	f92080e7          	jalr	-110(ra) # 80001d8c <myproc>
    80005e02:	84aa                	mv	s1,a0

  if(argaddr(0, &fdarray) < 0)
    80005e04:	fd840593          	addi	a1,s0,-40
    80005e08:	4501                	li	a0,0
    80005e0a:	ffffd097          	auipc	ra,0xffffd
    80005e0e:	0dc080e7          	jalr	220(ra) # 80002ee6 <argaddr>
    return -1;
    80005e12:	57fd                	li	a5,-1
  if(argaddr(0, &fdarray) < 0)
    80005e14:	0e054063          	bltz	a0,80005ef4 <sys_pipe+0x104>
  if(pipealloc(&rf, &wf) < 0)
    80005e18:	fc840593          	addi	a1,s0,-56
    80005e1c:	fd040513          	addi	a0,s0,-48
    80005e20:	fffff097          	auipc	ra,0xfffff
    80005e24:	dfc080e7          	jalr	-516(ra) # 80004c1c <pipealloc>
    return -1;
    80005e28:	57fd                	li	a5,-1
  if(pipealloc(&rf, &wf) < 0)
    80005e2a:	0c054563          	bltz	a0,80005ef4 <sys_pipe+0x104>
  fd0 = -1;
    80005e2e:	fcf42223          	sw	a5,-60(s0)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
    80005e32:	fd043503          	ld	a0,-48(s0)
    80005e36:	fffff097          	auipc	ra,0xfffff
    80005e3a:	508080e7          	jalr	1288(ra) # 8000533e <fdalloc>
    80005e3e:	fca42223          	sw	a0,-60(s0)
    80005e42:	08054c63          	bltz	a0,80005eda <sys_pipe+0xea>
    80005e46:	fc843503          	ld	a0,-56(s0)
    80005e4a:	fffff097          	auipc	ra,0xfffff
    80005e4e:	4f4080e7          	jalr	1268(ra) # 8000533e <fdalloc>
    80005e52:	fca42023          	sw	a0,-64(s0)
    80005e56:	06054863          	bltz	a0,80005ec6 <sys_pipe+0xd6>
      p->ofile[fd0] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  }
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    80005e5a:	4691                	li	a3,4
    80005e5c:	fc440613          	addi	a2,s0,-60
    80005e60:	fd843583          	ld	a1,-40(s0)
    80005e64:	6ca8                	ld	a0,88(s1)
    80005e66:	ffffc097          	auipc	ra,0xffffc
    80005e6a:	80c080e7          	jalr	-2036(ra) # 80001672 <copyout>
    80005e6e:	02054063          	bltz	a0,80005e8e <sys_pipe+0x9e>
     copyout(p->pagetable, fdarray+sizeof(fd0), (char *)&fd1, sizeof(fd1)) < 0){
    80005e72:	4691                	li	a3,4
    80005e74:	fc040613          	addi	a2,s0,-64
    80005e78:	fd843583          	ld	a1,-40(s0)
    80005e7c:	0591                	addi	a1,a1,4
    80005e7e:	6ca8                	ld	a0,88(s1)
    80005e80:	ffffb097          	auipc	ra,0xffffb
    80005e84:	7f2080e7          	jalr	2034(ra) # 80001672 <copyout>
    p->ofile[fd1] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  }
  return 0;
    80005e88:	4781                	li	a5,0
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    80005e8a:	06055563          	bgez	a0,80005ef4 <sys_pipe+0x104>
    p->ofile[fd0] = 0;
    80005e8e:	fc442783          	lw	a5,-60(s0)
    80005e92:	07e9                	addi	a5,a5,26
    80005e94:	078e                	slli	a5,a5,0x3
    80005e96:	97a6                	add	a5,a5,s1
    80005e98:	0007b423          	sd	zero,8(a5)
    p->ofile[fd1] = 0;
    80005e9c:	fc042503          	lw	a0,-64(s0)
    80005ea0:	0569                	addi	a0,a0,26
    80005ea2:	050e                	slli	a0,a0,0x3
    80005ea4:	9526                	add	a0,a0,s1
    80005ea6:	00053423          	sd	zero,8(a0)
    fileclose(rf);
    80005eaa:	fd043503          	ld	a0,-48(s0)
    80005eae:	fffff097          	auipc	ra,0xfffff
    80005eb2:	a3e080e7          	jalr	-1474(ra) # 800048ec <fileclose>
    fileclose(wf);
    80005eb6:	fc843503          	ld	a0,-56(s0)
    80005eba:	fffff097          	auipc	ra,0xfffff
    80005ebe:	a32080e7          	jalr	-1486(ra) # 800048ec <fileclose>
    return -1;
    80005ec2:	57fd                	li	a5,-1
    80005ec4:	a805                	j	80005ef4 <sys_pipe+0x104>
    if(fd0 >= 0)
    80005ec6:	fc442783          	lw	a5,-60(s0)
    80005eca:	0007c863          	bltz	a5,80005eda <sys_pipe+0xea>
      p->ofile[fd0] = 0;
    80005ece:	01a78513          	addi	a0,a5,26
    80005ed2:	050e                	slli	a0,a0,0x3
    80005ed4:	9526                	add	a0,a0,s1
    80005ed6:	00053423          	sd	zero,8(a0)
    fileclose(rf);
    80005eda:	fd043503          	ld	a0,-48(s0)
    80005ede:	fffff097          	auipc	ra,0xfffff
    80005ee2:	a0e080e7          	jalr	-1522(ra) # 800048ec <fileclose>
    fileclose(wf);
    80005ee6:	fc843503          	ld	a0,-56(s0)
    80005eea:	fffff097          	auipc	ra,0xfffff
    80005eee:	a02080e7          	jalr	-1534(ra) # 800048ec <fileclose>
    return -1;
    80005ef2:	57fd                	li	a5,-1
}
    80005ef4:	853e                	mv	a0,a5
    80005ef6:	70e2                	ld	ra,56(sp)
    80005ef8:	7442                	ld	s0,48(sp)
    80005efa:	74a2                	ld	s1,40(sp)
    80005efc:	6121                	addi	sp,sp,64
    80005efe:	8082                	ret

0000000080005f00 <kernelvec>:
    80005f00:	7111                	addi	sp,sp,-256
    80005f02:	e006                	sd	ra,0(sp)
    80005f04:	e40a                	sd	sp,8(sp)
    80005f06:	e80e                	sd	gp,16(sp)
    80005f08:	ec12                	sd	tp,24(sp)
    80005f0a:	f016                	sd	t0,32(sp)
    80005f0c:	f41a                	sd	t1,40(sp)
    80005f0e:	f81e                	sd	t2,48(sp)
    80005f10:	fc22                	sd	s0,56(sp)
    80005f12:	e0a6                	sd	s1,64(sp)
    80005f14:	e4aa                	sd	a0,72(sp)
    80005f16:	e8ae                	sd	a1,80(sp)
    80005f18:	ecb2                	sd	a2,88(sp)
    80005f1a:	f0b6                	sd	a3,96(sp)
    80005f1c:	f4ba                	sd	a4,104(sp)
    80005f1e:	f8be                	sd	a5,112(sp)
    80005f20:	fcc2                	sd	a6,120(sp)
    80005f22:	e146                	sd	a7,128(sp)
    80005f24:	e54a                	sd	s2,136(sp)
    80005f26:	e94e                	sd	s3,144(sp)
    80005f28:	ed52                	sd	s4,152(sp)
    80005f2a:	f156                	sd	s5,160(sp)
    80005f2c:	f55a                	sd	s6,168(sp)
    80005f2e:	f95e                	sd	s7,176(sp)
    80005f30:	fd62                	sd	s8,184(sp)
    80005f32:	e1e6                	sd	s9,192(sp)
    80005f34:	e5ea                	sd	s10,200(sp)
    80005f36:	e9ee                	sd	s11,208(sp)
    80005f38:	edf2                	sd	t3,216(sp)
    80005f3a:	f1f6                	sd	t4,224(sp)
    80005f3c:	f5fa                	sd	t5,232(sp)
    80005f3e:	f9fe                	sd	t6,240(sp)
    80005f40:	db7fc0ef          	jal	ra,80002cf6 <kerneltrap>
    80005f44:	6082                	ld	ra,0(sp)
    80005f46:	6122                	ld	sp,8(sp)
    80005f48:	61c2                	ld	gp,16(sp)
    80005f4a:	7282                	ld	t0,32(sp)
    80005f4c:	7322                	ld	t1,40(sp)
    80005f4e:	73c2                	ld	t2,48(sp)
    80005f50:	7462                	ld	s0,56(sp)
    80005f52:	6486                	ld	s1,64(sp)
    80005f54:	6526                	ld	a0,72(sp)
    80005f56:	65c6                	ld	a1,80(sp)
    80005f58:	6666                	ld	a2,88(sp)
    80005f5a:	7686                	ld	a3,96(sp)
    80005f5c:	7726                	ld	a4,104(sp)
    80005f5e:	77c6                	ld	a5,112(sp)
    80005f60:	7866                	ld	a6,120(sp)
    80005f62:	688a                	ld	a7,128(sp)
    80005f64:	692a                	ld	s2,136(sp)
    80005f66:	69ca                	ld	s3,144(sp)
    80005f68:	6a6a                	ld	s4,152(sp)
    80005f6a:	7a8a                	ld	s5,160(sp)
    80005f6c:	7b2a                	ld	s6,168(sp)
    80005f6e:	7bca                	ld	s7,176(sp)
    80005f70:	7c6a                	ld	s8,184(sp)
    80005f72:	6c8e                	ld	s9,192(sp)
    80005f74:	6d2e                	ld	s10,200(sp)
    80005f76:	6dce                	ld	s11,208(sp)
    80005f78:	6e6e                	ld	t3,216(sp)
    80005f7a:	7e8e                	ld	t4,224(sp)
    80005f7c:	7f2e                	ld	t5,232(sp)
    80005f7e:	7fce                	ld	t6,240(sp)
    80005f80:	6111                	addi	sp,sp,256
    80005f82:	10200073          	sret
    80005f86:	00000013          	nop
    80005f8a:	00000013          	nop
    80005f8e:	0001                	nop

0000000080005f90 <timervec>:
    80005f90:	34051573          	csrrw	a0,mscratch,a0
    80005f94:	e10c                	sd	a1,0(a0)
    80005f96:	e510                	sd	a2,8(a0)
    80005f98:	e914                	sd	a3,16(a0)
    80005f9a:	6d0c                	ld	a1,24(a0)
    80005f9c:	7110                	ld	a2,32(a0)
    80005f9e:	6194                	ld	a3,0(a1)
    80005fa0:	96b2                	add	a3,a3,a2
    80005fa2:	e194                	sd	a3,0(a1)
    80005fa4:	4589                	li	a1,2
    80005fa6:	14459073          	csrw	sip,a1
    80005faa:	6914                	ld	a3,16(a0)
    80005fac:	6510                	ld	a2,8(a0)
    80005fae:	610c                	ld	a1,0(a0)
    80005fb0:	34051573          	csrrw	a0,mscratch,a0
    80005fb4:	30200073          	mret
	...

0000000080005fba <plicinit>:
// the riscv Platform Level Interrupt Controller (PLIC).
//

void
plicinit(void)
{
    80005fba:	1141                	addi	sp,sp,-16
    80005fbc:	e422                	sd	s0,8(sp)
    80005fbe:	0800                	addi	s0,sp,16
  // set desired IRQ priorities non-zero (otherwise disabled).
  *(uint32*)(PLIC + UART0_IRQ*4) = 1;
    80005fc0:	0c0007b7          	lui	a5,0xc000
    80005fc4:	4705                	li	a4,1
    80005fc6:	d798                	sw	a4,40(a5)
  *(uint32*)(PLIC + VIRTIO0_IRQ*4) = 1;
    80005fc8:	c3d8                	sw	a4,4(a5)
}
    80005fca:	6422                	ld	s0,8(sp)
    80005fcc:	0141                	addi	sp,sp,16
    80005fce:	8082                	ret

0000000080005fd0 <plicinithart>:

void
plicinithart(void)
{
    80005fd0:	1141                	addi	sp,sp,-16
    80005fd2:	e406                	sd	ra,8(sp)
    80005fd4:	e022                	sd	s0,0(sp)
    80005fd6:	0800                	addi	s0,sp,16
  int hart = cpuid();
    80005fd8:	ffffc097          	auipc	ra,0xffffc
    80005fdc:	d88080e7          	jalr	-632(ra) # 80001d60 <cpuid>
  
  // set uart's enable bit for this hart's S-mode. 
  *(uint32*)PLIC_SENABLE(hart)= (1 << UART0_IRQ) | (1 << VIRTIO0_IRQ);
    80005fe0:	0085171b          	slliw	a4,a0,0x8
    80005fe4:	0c0027b7          	lui	a5,0xc002
    80005fe8:	97ba                	add	a5,a5,a4
    80005fea:	40200713          	li	a4,1026
    80005fee:	08e7a023          	sw	a4,128(a5) # c002080 <_entry-0x73ffdf80>

  // set this hart's S-mode priority threshold to 0.
  *(uint32*)PLIC_SPRIORITY(hart) = 0;
    80005ff2:	00d5151b          	slliw	a0,a0,0xd
    80005ff6:	0c2017b7          	lui	a5,0xc201
    80005ffa:	953e                	add	a0,a0,a5
    80005ffc:	00052023          	sw	zero,0(a0)
}
    80006000:	60a2                	ld	ra,8(sp)
    80006002:	6402                	ld	s0,0(sp)
    80006004:	0141                	addi	sp,sp,16
    80006006:	8082                	ret

0000000080006008 <plic_claim>:

// ask the PLIC what interrupt we should serve.
int
plic_claim(void)
{
    80006008:	1141                	addi	sp,sp,-16
    8000600a:	e406                	sd	ra,8(sp)
    8000600c:	e022                	sd	s0,0(sp)
    8000600e:	0800                	addi	s0,sp,16
  int hart = cpuid();
    80006010:	ffffc097          	auipc	ra,0xffffc
    80006014:	d50080e7          	jalr	-688(ra) # 80001d60 <cpuid>
  int irq = *(uint32*)PLIC_SCLAIM(hart);
    80006018:	00d5179b          	slliw	a5,a0,0xd
    8000601c:	0c201537          	lui	a0,0xc201
    80006020:	953e                	add	a0,a0,a5
  return irq;
}
    80006022:	4148                	lw	a0,4(a0)
    80006024:	60a2                	ld	ra,8(sp)
    80006026:	6402                	ld	s0,0(sp)
    80006028:	0141                	addi	sp,sp,16
    8000602a:	8082                	ret

000000008000602c <plic_complete>:

// tell the PLIC we've served this IRQ.
void
plic_complete(int irq)
{
    8000602c:	1101                	addi	sp,sp,-32
    8000602e:	ec06                	sd	ra,24(sp)
    80006030:	e822                	sd	s0,16(sp)
    80006032:	e426                	sd	s1,8(sp)
    80006034:	1000                	addi	s0,sp,32
    80006036:	84aa                	mv	s1,a0
  int hart = cpuid();
    80006038:	ffffc097          	auipc	ra,0xffffc
    8000603c:	d28080e7          	jalr	-728(ra) # 80001d60 <cpuid>
  *(uint32*)PLIC_SCLAIM(hart) = irq;
    80006040:	00d5151b          	slliw	a0,a0,0xd
    80006044:	0c2017b7          	lui	a5,0xc201
    80006048:	97aa                	add	a5,a5,a0
    8000604a:	c3c4                	sw	s1,4(a5)
}
    8000604c:	60e2                	ld	ra,24(sp)
    8000604e:	6442                	ld	s0,16(sp)
    80006050:	64a2                	ld	s1,8(sp)
    80006052:	6105                	addi	sp,sp,32
    80006054:	8082                	ret

0000000080006056 <free_desc>:
}

// mark a descriptor as free.
static void
free_desc(int i)
{
    80006056:	1141                	addi	sp,sp,-16
    80006058:	e406                	sd	ra,8(sp)
    8000605a:	e022                	sd	s0,0(sp)
    8000605c:	0800                	addi	s0,sp,16
  if(i >= NUM)
    8000605e:	479d                	li	a5,7
    80006060:	06a7c963          	blt	a5,a0,800060d2 <free_desc+0x7c>
    panic("free_desc 1");
  if(disk.free[i])
    80006064:	0001d797          	auipc	a5,0x1d
    80006068:	f9c78793          	addi	a5,a5,-100 # 80023000 <disk>
    8000606c:	00a78733          	add	a4,a5,a0
    80006070:	6789                	lui	a5,0x2
    80006072:	97ba                	add	a5,a5,a4
    80006074:	0187c783          	lbu	a5,24(a5) # 2018 <_entry-0x7fffdfe8>
    80006078:	e7ad                	bnez	a5,800060e2 <free_desc+0x8c>
    panic("free_desc 2");
  disk.desc[i].addr = 0;
    8000607a:	00451793          	slli	a5,a0,0x4
    8000607e:	0001f717          	auipc	a4,0x1f
    80006082:	f8270713          	addi	a4,a4,-126 # 80025000 <disk+0x2000>
    80006086:	6314                	ld	a3,0(a4)
    80006088:	96be                	add	a3,a3,a5
    8000608a:	0006b023          	sd	zero,0(a3)
  disk.desc[i].len = 0;
    8000608e:	6314                	ld	a3,0(a4)
    80006090:	96be                	add	a3,a3,a5
    80006092:	0006a423          	sw	zero,8(a3)
  disk.desc[i].flags = 0;
    80006096:	6314                	ld	a3,0(a4)
    80006098:	96be                	add	a3,a3,a5
    8000609a:	00069623          	sh	zero,12(a3)
  disk.desc[i].next = 0;
    8000609e:	6318                	ld	a4,0(a4)
    800060a0:	97ba                	add	a5,a5,a4
    800060a2:	00079723          	sh	zero,14(a5)
  disk.free[i] = 1;
    800060a6:	0001d797          	auipc	a5,0x1d
    800060aa:	f5a78793          	addi	a5,a5,-166 # 80023000 <disk>
    800060ae:	97aa                	add	a5,a5,a0
    800060b0:	6509                	lui	a0,0x2
    800060b2:	953e                	add	a0,a0,a5
    800060b4:	4785                	li	a5,1
    800060b6:	00f50c23          	sb	a5,24(a0) # 2018 <_entry-0x7fffdfe8>
  wakeup(&disk.free[0]);
    800060ba:	0001f517          	auipc	a0,0x1f
    800060be:	f5e50513          	addi	a0,a0,-162 # 80025018 <disk+0x2018>
    800060c2:	ffffc097          	auipc	ra,0xffffc
    800060c6:	514080e7          	jalr	1300(ra) # 800025d6 <wakeup>
}
    800060ca:	60a2                	ld	ra,8(sp)
    800060cc:	6402                	ld	s0,0(sp)
    800060ce:	0141                	addi	sp,sp,16
    800060d0:	8082                	ret
    panic("free_desc 1");
    800060d2:	00002517          	auipc	a0,0x2
    800060d6:	6fe50513          	addi	a0,a0,1790 # 800087d0 <syscalls+0x328>
    800060da:	ffffa097          	auipc	ra,0xffffa
    800060de:	464080e7          	jalr	1124(ra) # 8000053e <panic>
    panic("free_desc 2");
    800060e2:	00002517          	auipc	a0,0x2
    800060e6:	6fe50513          	addi	a0,a0,1790 # 800087e0 <syscalls+0x338>
    800060ea:	ffffa097          	auipc	ra,0xffffa
    800060ee:	454080e7          	jalr	1108(ra) # 8000053e <panic>

00000000800060f2 <virtio_disk_init>:
{
    800060f2:	1101                	addi	sp,sp,-32
    800060f4:	ec06                	sd	ra,24(sp)
    800060f6:	e822                	sd	s0,16(sp)
    800060f8:	e426                	sd	s1,8(sp)
    800060fa:	1000                	addi	s0,sp,32
  initlock(&disk.vdisk_lock, "virtio_disk");
    800060fc:	00002597          	auipc	a1,0x2
    80006100:	6f458593          	addi	a1,a1,1780 # 800087f0 <syscalls+0x348>
    80006104:	0001f517          	auipc	a0,0x1f
    80006108:	02450513          	addi	a0,a0,36 # 80025128 <disk+0x2128>
    8000610c:	ffffb097          	auipc	ra,0xffffb
    80006110:	a48080e7          	jalr	-1464(ra) # 80000b54 <initlock>
  if(*R(VIRTIO_MMIO_MAGIC_VALUE) != 0x74726976 ||
    80006114:	100017b7          	lui	a5,0x10001
    80006118:	4398                	lw	a4,0(a5)
    8000611a:	2701                	sext.w	a4,a4
    8000611c:	747277b7          	lui	a5,0x74727
    80006120:	97678793          	addi	a5,a5,-1674 # 74726976 <_entry-0xb8d968a>
    80006124:	0ef71163          	bne	a4,a5,80006206 <virtio_disk_init+0x114>
     *R(VIRTIO_MMIO_VERSION) != 1 ||
    80006128:	100017b7          	lui	a5,0x10001
    8000612c:	43dc                	lw	a5,4(a5)
    8000612e:	2781                	sext.w	a5,a5
  if(*R(VIRTIO_MMIO_MAGIC_VALUE) != 0x74726976 ||
    80006130:	4705                	li	a4,1
    80006132:	0ce79a63          	bne	a5,a4,80006206 <virtio_disk_init+0x114>
     *R(VIRTIO_MMIO_DEVICE_ID) != 2 ||
    80006136:	100017b7          	lui	a5,0x10001
    8000613a:	479c                	lw	a5,8(a5)
    8000613c:	2781                	sext.w	a5,a5
     *R(VIRTIO_MMIO_VERSION) != 1 ||
    8000613e:	4709                	li	a4,2
    80006140:	0ce79363          	bne	a5,a4,80006206 <virtio_disk_init+0x114>
     *R(VIRTIO_MMIO_VENDOR_ID) != 0x554d4551){
    80006144:	100017b7          	lui	a5,0x10001
    80006148:	47d8                	lw	a4,12(a5)
    8000614a:	2701                	sext.w	a4,a4
     *R(VIRTIO_MMIO_DEVICE_ID) != 2 ||
    8000614c:	554d47b7          	lui	a5,0x554d4
    80006150:	55178793          	addi	a5,a5,1361 # 554d4551 <_entry-0x2ab2baaf>
    80006154:	0af71963          	bne	a4,a5,80006206 <virtio_disk_init+0x114>
  *R(VIRTIO_MMIO_STATUS) = status;
    80006158:	100017b7          	lui	a5,0x10001
    8000615c:	4705                	li	a4,1
    8000615e:	dbb8                	sw	a4,112(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    80006160:	470d                	li	a4,3
    80006162:	dbb8                	sw	a4,112(a5)
  uint64 features = *R(VIRTIO_MMIO_DEVICE_FEATURES);
    80006164:	4b94                	lw	a3,16(a5)
  features &= ~(1 << VIRTIO_RING_F_INDIRECT_DESC);
    80006166:	c7ffe737          	lui	a4,0xc7ffe
    8000616a:	75f70713          	addi	a4,a4,1887 # ffffffffc7ffe75f <end+0xffffffff47fd875f>
    8000616e:	8f75                	and	a4,a4,a3
  *R(VIRTIO_MMIO_DRIVER_FEATURES) = features;
    80006170:	2701                	sext.w	a4,a4
    80006172:	d398                	sw	a4,32(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    80006174:	472d                	li	a4,11
    80006176:	dbb8                	sw	a4,112(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    80006178:	473d                	li	a4,15
    8000617a:	dbb8                	sw	a4,112(a5)
  *R(VIRTIO_MMIO_GUEST_PAGE_SIZE) = PGSIZE;
    8000617c:	6705                	lui	a4,0x1
    8000617e:	d798                	sw	a4,40(a5)
  *R(VIRTIO_MMIO_QUEUE_SEL) = 0;
    80006180:	0207a823          	sw	zero,48(a5) # 10001030 <_entry-0x6fffefd0>
  uint32 max = *R(VIRTIO_MMIO_QUEUE_NUM_MAX);
    80006184:	5bdc                	lw	a5,52(a5)
    80006186:	2781                	sext.w	a5,a5
  if(max == 0)
    80006188:	c7d9                	beqz	a5,80006216 <virtio_disk_init+0x124>
  if(max < NUM)
    8000618a:	471d                	li	a4,7
    8000618c:	08f77d63          	bgeu	a4,a5,80006226 <virtio_disk_init+0x134>
  *R(VIRTIO_MMIO_QUEUE_NUM) = NUM;
    80006190:	100014b7          	lui	s1,0x10001
    80006194:	47a1                	li	a5,8
    80006196:	dc9c                	sw	a5,56(s1)
  memset(disk.pages, 0, sizeof(disk.pages));
    80006198:	6609                	lui	a2,0x2
    8000619a:	4581                	li	a1,0
    8000619c:	0001d517          	auipc	a0,0x1d
    800061a0:	e6450513          	addi	a0,a0,-412 # 80023000 <disk>
    800061a4:	ffffb097          	auipc	ra,0xffffb
    800061a8:	b3c080e7          	jalr	-1220(ra) # 80000ce0 <memset>
  *R(VIRTIO_MMIO_QUEUE_PFN) = ((uint64)disk.pages) >> PGSHIFT;
    800061ac:	0001d717          	auipc	a4,0x1d
    800061b0:	e5470713          	addi	a4,a4,-428 # 80023000 <disk>
    800061b4:	00c75793          	srli	a5,a4,0xc
    800061b8:	2781                	sext.w	a5,a5
    800061ba:	c0bc                	sw	a5,64(s1)
  disk.desc = (struct virtq_desc *) disk.pages;
    800061bc:	0001f797          	auipc	a5,0x1f
    800061c0:	e4478793          	addi	a5,a5,-444 # 80025000 <disk+0x2000>
    800061c4:	e398                	sd	a4,0(a5)
  disk.avail = (struct virtq_avail *)(disk.pages + NUM*sizeof(struct virtq_desc));
    800061c6:	0001d717          	auipc	a4,0x1d
    800061ca:	eba70713          	addi	a4,a4,-326 # 80023080 <disk+0x80>
    800061ce:	e798                	sd	a4,8(a5)
  disk.used = (struct virtq_used *) (disk.pages + PGSIZE);
    800061d0:	0001e717          	auipc	a4,0x1e
    800061d4:	e3070713          	addi	a4,a4,-464 # 80024000 <disk+0x1000>
    800061d8:	eb98                	sd	a4,16(a5)
    disk.free[i] = 1;
    800061da:	4705                	li	a4,1
    800061dc:	00e78c23          	sb	a4,24(a5)
    800061e0:	00e78ca3          	sb	a4,25(a5)
    800061e4:	00e78d23          	sb	a4,26(a5)
    800061e8:	00e78da3          	sb	a4,27(a5)
    800061ec:	00e78e23          	sb	a4,28(a5)
    800061f0:	00e78ea3          	sb	a4,29(a5)
    800061f4:	00e78f23          	sb	a4,30(a5)
    800061f8:	00e78fa3          	sb	a4,31(a5)
}
    800061fc:	60e2                	ld	ra,24(sp)
    800061fe:	6442                	ld	s0,16(sp)
    80006200:	64a2                	ld	s1,8(sp)
    80006202:	6105                	addi	sp,sp,32
    80006204:	8082                	ret
    panic("could not find virtio disk");
    80006206:	00002517          	auipc	a0,0x2
    8000620a:	5fa50513          	addi	a0,a0,1530 # 80008800 <syscalls+0x358>
    8000620e:	ffffa097          	auipc	ra,0xffffa
    80006212:	330080e7          	jalr	816(ra) # 8000053e <panic>
    panic("virtio disk has no queue 0");
    80006216:	00002517          	auipc	a0,0x2
    8000621a:	60a50513          	addi	a0,a0,1546 # 80008820 <syscalls+0x378>
    8000621e:	ffffa097          	auipc	ra,0xffffa
    80006222:	320080e7          	jalr	800(ra) # 8000053e <panic>
    panic("virtio disk max queue too short");
    80006226:	00002517          	auipc	a0,0x2
    8000622a:	61a50513          	addi	a0,a0,1562 # 80008840 <syscalls+0x398>
    8000622e:	ffffa097          	auipc	ra,0xffffa
    80006232:	310080e7          	jalr	784(ra) # 8000053e <panic>

0000000080006236 <virtio_disk_rw>:
  return 0;
}

void
virtio_disk_rw(struct buf *b, int write)
{
    80006236:	7159                	addi	sp,sp,-112
    80006238:	f486                	sd	ra,104(sp)
    8000623a:	f0a2                	sd	s0,96(sp)
    8000623c:	eca6                	sd	s1,88(sp)
    8000623e:	e8ca                	sd	s2,80(sp)
    80006240:	e4ce                	sd	s3,72(sp)
    80006242:	e0d2                	sd	s4,64(sp)
    80006244:	fc56                	sd	s5,56(sp)
    80006246:	f85a                	sd	s6,48(sp)
    80006248:	f45e                	sd	s7,40(sp)
    8000624a:	f062                	sd	s8,32(sp)
    8000624c:	ec66                	sd	s9,24(sp)
    8000624e:	e86a                	sd	s10,16(sp)
    80006250:	1880                	addi	s0,sp,112
    80006252:	892a                	mv	s2,a0
    80006254:	8d2e                	mv	s10,a1
  uint64 sector = b->blockno * (BSIZE / 512);
    80006256:	00c52c83          	lw	s9,12(a0)
    8000625a:	001c9c9b          	slliw	s9,s9,0x1
    8000625e:	1c82                	slli	s9,s9,0x20
    80006260:	020cdc93          	srli	s9,s9,0x20

  acquire(&disk.vdisk_lock);
    80006264:	0001f517          	auipc	a0,0x1f
    80006268:	ec450513          	addi	a0,a0,-316 # 80025128 <disk+0x2128>
    8000626c:	ffffb097          	auipc	ra,0xffffb
    80006270:	978080e7          	jalr	-1672(ra) # 80000be4 <acquire>
  for(int i = 0; i < 3; i++){
    80006274:	4981                	li	s3,0
  for(int i = 0; i < NUM; i++){
    80006276:	4c21                	li	s8,8
      disk.free[i] = 0;
    80006278:	0001db97          	auipc	s7,0x1d
    8000627c:	d88b8b93          	addi	s7,s7,-632 # 80023000 <disk>
    80006280:	6b09                	lui	s6,0x2
  for(int i = 0; i < 3; i++){
    80006282:	4a8d                	li	s5,3
  for(int i = 0; i < NUM; i++){
    80006284:	8a4e                	mv	s4,s3
    80006286:	a051                	j	8000630a <virtio_disk_rw+0xd4>
      disk.free[i] = 0;
    80006288:	00fb86b3          	add	a3,s7,a5
    8000628c:	96da                	add	a3,a3,s6
    8000628e:	00068c23          	sb	zero,24(a3)
    idx[i] = alloc_desc();
    80006292:	c21c                	sw	a5,0(a2)
    if(idx[i] < 0){
    80006294:	0207c563          	bltz	a5,800062be <virtio_disk_rw+0x88>
  for(int i = 0; i < 3; i++){
    80006298:	2485                	addiw	s1,s1,1
    8000629a:	0711                	addi	a4,a4,4
    8000629c:	25548063          	beq	s1,s5,800064dc <virtio_disk_rw+0x2a6>
    idx[i] = alloc_desc();
    800062a0:	863a                	mv	a2,a4
  for(int i = 0; i < NUM; i++){
    800062a2:	0001f697          	auipc	a3,0x1f
    800062a6:	d7668693          	addi	a3,a3,-650 # 80025018 <disk+0x2018>
    800062aa:	87d2                	mv	a5,s4
    if(disk.free[i]){
    800062ac:	0006c583          	lbu	a1,0(a3)
    800062b0:	fde1                	bnez	a1,80006288 <virtio_disk_rw+0x52>
  for(int i = 0; i < NUM; i++){
    800062b2:	2785                	addiw	a5,a5,1
    800062b4:	0685                	addi	a3,a3,1
    800062b6:	ff879be3          	bne	a5,s8,800062ac <virtio_disk_rw+0x76>
    idx[i] = alloc_desc();
    800062ba:	57fd                	li	a5,-1
    800062bc:	c21c                	sw	a5,0(a2)
      for(int j = 0; j < i; j++)
    800062be:	02905a63          	blez	s1,800062f2 <virtio_disk_rw+0xbc>
        free_desc(idx[j]);
    800062c2:	f9042503          	lw	a0,-112(s0)
    800062c6:	00000097          	auipc	ra,0x0
    800062ca:	d90080e7          	jalr	-624(ra) # 80006056 <free_desc>
      for(int j = 0; j < i; j++)
    800062ce:	4785                	li	a5,1
    800062d0:	0297d163          	bge	a5,s1,800062f2 <virtio_disk_rw+0xbc>
        free_desc(idx[j]);
    800062d4:	f9442503          	lw	a0,-108(s0)
    800062d8:	00000097          	auipc	ra,0x0
    800062dc:	d7e080e7          	jalr	-642(ra) # 80006056 <free_desc>
      for(int j = 0; j < i; j++)
    800062e0:	4789                	li	a5,2
    800062e2:	0097d863          	bge	a5,s1,800062f2 <virtio_disk_rw+0xbc>
        free_desc(idx[j]);
    800062e6:	f9842503          	lw	a0,-104(s0)
    800062ea:	00000097          	auipc	ra,0x0
    800062ee:	d6c080e7          	jalr	-660(ra) # 80006056 <free_desc>
  int idx[3];
  while(1){
    if(alloc3_desc(idx) == 0) {
      break;
    }
    sleep(&disk.free[0], &disk.vdisk_lock);
    800062f2:	0001f597          	auipc	a1,0x1f
    800062f6:	e3658593          	addi	a1,a1,-458 # 80025128 <disk+0x2128>
    800062fa:	0001f517          	auipc	a0,0x1f
    800062fe:	d1e50513          	addi	a0,a0,-738 # 80025018 <disk+0x2018>
    80006302:	ffffc097          	auipc	ra,0xffffc
    80006306:	148080e7          	jalr	328(ra) # 8000244a <sleep>
  for(int i = 0; i < 3; i++){
    8000630a:	f9040713          	addi	a4,s0,-112
    8000630e:	84ce                	mv	s1,s3
    80006310:	bf41                	j	800062a0 <virtio_disk_rw+0x6a>
  // qemu's virtio-blk.c reads them.

  struct virtio_blk_req *buf0 = &disk.ops[idx[0]];

  if(write)
    buf0->type = VIRTIO_BLK_T_OUT; // write the disk
    80006312:	20058713          	addi	a4,a1,512
    80006316:	00471693          	slli	a3,a4,0x4
    8000631a:	0001d717          	auipc	a4,0x1d
    8000631e:	ce670713          	addi	a4,a4,-794 # 80023000 <disk>
    80006322:	9736                	add	a4,a4,a3
    80006324:	4685                	li	a3,1
    80006326:	0ad72423          	sw	a3,168(a4)
  else
    buf0->type = VIRTIO_BLK_T_IN; // read the disk
  buf0->reserved = 0;
    8000632a:	20058713          	addi	a4,a1,512
    8000632e:	00471693          	slli	a3,a4,0x4
    80006332:	0001d717          	auipc	a4,0x1d
    80006336:	cce70713          	addi	a4,a4,-818 # 80023000 <disk>
    8000633a:	9736                	add	a4,a4,a3
    8000633c:	0a072623          	sw	zero,172(a4)
  buf0->sector = sector;
    80006340:	0b973823          	sd	s9,176(a4)

  disk.desc[idx[0]].addr = (uint64) buf0;
    80006344:	7679                	lui	a2,0xffffe
    80006346:	963e                	add	a2,a2,a5
    80006348:	0001f697          	auipc	a3,0x1f
    8000634c:	cb868693          	addi	a3,a3,-840 # 80025000 <disk+0x2000>
    80006350:	6298                	ld	a4,0(a3)
    80006352:	9732                	add	a4,a4,a2
    80006354:	e308                	sd	a0,0(a4)
  disk.desc[idx[0]].len = sizeof(struct virtio_blk_req);
    80006356:	6298                	ld	a4,0(a3)
    80006358:	9732                	add	a4,a4,a2
    8000635a:	4541                	li	a0,16
    8000635c:	c708                	sw	a0,8(a4)
  disk.desc[idx[0]].flags = VRING_DESC_F_NEXT;
    8000635e:	6298                	ld	a4,0(a3)
    80006360:	9732                	add	a4,a4,a2
    80006362:	4505                	li	a0,1
    80006364:	00a71623          	sh	a0,12(a4)
  disk.desc[idx[0]].next = idx[1];
    80006368:	f9442703          	lw	a4,-108(s0)
    8000636c:	6288                	ld	a0,0(a3)
    8000636e:	962a                	add	a2,a2,a0
    80006370:	00e61723          	sh	a4,14(a2) # ffffffffffffe00e <end+0xffffffff7ffd800e>

  disk.desc[idx[1]].addr = (uint64) b->data;
    80006374:	0712                	slli	a4,a4,0x4
    80006376:	6290                	ld	a2,0(a3)
    80006378:	963a                	add	a2,a2,a4
    8000637a:	05890513          	addi	a0,s2,88
    8000637e:	e208                	sd	a0,0(a2)
  disk.desc[idx[1]].len = BSIZE;
    80006380:	6294                	ld	a3,0(a3)
    80006382:	96ba                	add	a3,a3,a4
    80006384:	40000613          	li	a2,1024
    80006388:	c690                	sw	a2,8(a3)
  if(write)
    8000638a:	140d0063          	beqz	s10,800064ca <virtio_disk_rw+0x294>
    disk.desc[idx[1]].flags = 0; // device reads b->data
    8000638e:	0001f697          	auipc	a3,0x1f
    80006392:	c726b683          	ld	a3,-910(a3) # 80025000 <disk+0x2000>
    80006396:	96ba                	add	a3,a3,a4
    80006398:	00069623          	sh	zero,12(a3)
  else
    disk.desc[idx[1]].flags = VRING_DESC_F_WRITE; // device writes b->data
  disk.desc[idx[1]].flags |= VRING_DESC_F_NEXT;
    8000639c:	0001d817          	auipc	a6,0x1d
    800063a0:	c6480813          	addi	a6,a6,-924 # 80023000 <disk>
    800063a4:	0001f517          	auipc	a0,0x1f
    800063a8:	c5c50513          	addi	a0,a0,-932 # 80025000 <disk+0x2000>
    800063ac:	6114                	ld	a3,0(a0)
    800063ae:	96ba                	add	a3,a3,a4
    800063b0:	00c6d603          	lhu	a2,12(a3)
    800063b4:	00166613          	ori	a2,a2,1
    800063b8:	00c69623          	sh	a2,12(a3)
  disk.desc[idx[1]].next = idx[2];
    800063bc:	f9842683          	lw	a3,-104(s0)
    800063c0:	6110                	ld	a2,0(a0)
    800063c2:	9732                	add	a4,a4,a2
    800063c4:	00d71723          	sh	a3,14(a4)

  disk.info[idx[0]].status = 0xff; // device writes 0 on success
    800063c8:	20058613          	addi	a2,a1,512
    800063cc:	0612                	slli	a2,a2,0x4
    800063ce:	9642                	add	a2,a2,a6
    800063d0:	577d                	li	a4,-1
    800063d2:	02e60823          	sb	a4,48(a2)
  disk.desc[idx[2]].addr = (uint64) &disk.info[idx[0]].status;
    800063d6:	00469713          	slli	a4,a3,0x4
    800063da:	6114                	ld	a3,0(a0)
    800063dc:	96ba                	add	a3,a3,a4
    800063de:	03078793          	addi	a5,a5,48
    800063e2:	97c2                	add	a5,a5,a6
    800063e4:	e29c                	sd	a5,0(a3)
  disk.desc[idx[2]].len = 1;
    800063e6:	611c                	ld	a5,0(a0)
    800063e8:	97ba                	add	a5,a5,a4
    800063ea:	4685                	li	a3,1
    800063ec:	c794                	sw	a3,8(a5)
  disk.desc[idx[2]].flags = VRING_DESC_F_WRITE; // device writes the status
    800063ee:	611c                	ld	a5,0(a0)
    800063f0:	97ba                	add	a5,a5,a4
    800063f2:	4809                	li	a6,2
    800063f4:	01079623          	sh	a6,12(a5)
  disk.desc[idx[2]].next = 0;
    800063f8:	611c                	ld	a5,0(a0)
    800063fa:	973e                	add	a4,a4,a5
    800063fc:	00071723          	sh	zero,14(a4)

  // record struct buf for virtio_disk_intr().
  b->disk = 1;
    80006400:	00d92223          	sw	a3,4(s2)
  disk.info[idx[0]].b = b;
    80006404:	03263423          	sd	s2,40(a2)

  // tell the device the first index in our chain of descriptors.
  disk.avail->ring[disk.avail->idx % NUM] = idx[0];
    80006408:	6518                	ld	a4,8(a0)
    8000640a:	00275783          	lhu	a5,2(a4)
    8000640e:	8b9d                	andi	a5,a5,7
    80006410:	0786                	slli	a5,a5,0x1
    80006412:	97ba                	add	a5,a5,a4
    80006414:	00b79223          	sh	a1,4(a5)

  __sync_synchronize();
    80006418:	0ff0000f          	fence

  // tell the device another avail ring entry is available.
  disk.avail->idx += 1; // not % NUM ...
    8000641c:	6518                	ld	a4,8(a0)
    8000641e:	00275783          	lhu	a5,2(a4)
    80006422:	2785                	addiw	a5,a5,1
    80006424:	00f71123          	sh	a5,2(a4)

  __sync_synchronize();
    80006428:	0ff0000f          	fence

  *R(VIRTIO_MMIO_QUEUE_NOTIFY) = 0; // value is queue number
    8000642c:	100017b7          	lui	a5,0x10001
    80006430:	0407a823          	sw	zero,80(a5) # 10001050 <_entry-0x6fffefb0>

  // Wait for virtio_disk_intr() to say request has finished.
  while(b->disk == 1) {
    80006434:	00492703          	lw	a4,4(s2)
    80006438:	4785                	li	a5,1
    8000643a:	02f71163          	bne	a4,a5,8000645c <virtio_disk_rw+0x226>
    sleep(b, &disk.vdisk_lock);
    8000643e:	0001f997          	auipc	s3,0x1f
    80006442:	cea98993          	addi	s3,s3,-790 # 80025128 <disk+0x2128>
  while(b->disk == 1) {
    80006446:	4485                	li	s1,1
    sleep(b, &disk.vdisk_lock);
    80006448:	85ce                	mv	a1,s3
    8000644a:	854a                	mv	a0,s2
    8000644c:	ffffc097          	auipc	ra,0xffffc
    80006450:	ffe080e7          	jalr	-2(ra) # 8000244a <sleep>
  while(b->disk == 1) {
    80006454:	00492783          	lw	a5,4(s2)
    80006458:	fe9788e3          	beq	a5,s1,80006448 <virtio_disk_rw+0x212>
  }

  disk.info[idx[0]].b = 0;
    8000645c:	f9042903          	lw	s2,-112(s0)
    80006460:	20090793          	addi	a5,s2,512
    80006464:	00479713          	slli	a4,a5,0x4
    80006468:	0001d797          	auipc	a5,0x1d
    8000646c:	b9878793          	addi	a5,a5,-1128 # 80023000 <disk>
    80006470:	97ba                	add	a5,a5,a4
    80006472:	0207b423          	sd	zero,40(a5)
    int flag = disk.desc[i].flags;
    80006476:	0001f997          	auipc	s3,0x1f
    8000647a:	b8a98993          	addi	s3,s3,-1142 # 80025000 <disk+0x2000>
    8000647e:	00491713          	slli	a4,s2,0x4
    80006482:	0009b783          	ld	a5,0(s3)
    80006486:	97ba                	add	a5,a5,a4
    80006488:	00c7d483          	lhu	s1,12(a5)
    int nxt = disk.desc[i].next;
    8000648c:	854a                	mv	a0,s2
    8000648e:	00e7d903          	lhu	s2,14(a5)
    free_desc(i);
    80006492:	00000097          	auipc	ra,0x0
    80006496:	bc4080e7          	jalr	-1084(ra) # 80006056 <free_desc>
    if(flag & VRING_DESC_F_NEXT)
    8000649a:	8885                	andi	s1,s1,1
    8000649c:	f0ed                	bnez	s1,8000647e <virtio_disk_rw+0x248>
  free_chain(idx[0]);

  release(&disk.vdisk_lock);
    8000649e:	0001f517          	auipc	a0,0x1f
    800064a2:	c8a50513          	addi	a0,a0,-886 # 80025128 <disk+0x2128>
    800064a6:	ffffa097          	auipc	ra,0xffffa
    800064aa:	7f2080e7          	jalr	2034(ra) # 80000c98 <release>
}
    800064ae:	70a6                	ld	ra,104(sp)
    800064b0:	7406                	ld	s0,96(sp)
    800064b2:	64e6                	ld	s1,88(sp)
    800064b4:	6946                	ld	s2,80(sp)
    800064b6:	69a6                	ld	s3,72(sp)
    800064b8:	6a06                	ld	s4,64(sp)
    800064ba:	7ae2                	ld	s5,56(sp)
    800064bc:	7b42                	ld	s6,48(sp)
    800064be:	7ba2                	ld	s7,40(sp)
    800064c0:	7c02                	ld	s8,32(sp)
    800064c2:	6ce2                	ld	s9,24(sp)
    800064c4:	6d42                	ld	s10,16(sp)
    800064c6:	6165                	addi	sp,sp,112
    800064c8:	8082                	ret
    disk.desc[idx[1]].flags = VRING_DESC_F_WRITE; // device writes b->data
    800064ca:	0001f697          	auipc	a3,0x1f
    800064ce:	b366b683          	ld	a3,-1226(a3) # 80025000 <disk+0x2000>
    800064d2:	96ba                	add	a3,a3,a4
    800064d4:	4609                	li	a2,2
    800064d6:	00c69623          	sh	a2,12(a3)
    800064da:	b5c9                	j	8000639c <virtio_disk_rw+0x166>
  struct virtio_blk_req *buf0 = &disk.ops[idx[0]];
    800064dc:	f9042583          	lw	a1,-112(s0)
    800064e0:	20058793          	addi	a5,a1,512
    800064e4:	0792                	slli	a5,a5,0x4
    800064e6:	0001d517          	auipc	a0,0x1d
    800064ea:	bc250513          	addi	a0,a0,-1086 # 800230a8 <disk+0xa8>
    800064ee:	953e                	add	a0,a0,a5
  if(write)
    800064f0:	e20d11e3          	bnez	s10,80006312 <virtio_disk_rw+0xdc>
    buf0->type = VIRTIO_BLK_T_IN; // read the disk
    800064f4:	20058713          	addi	a4,a1,512
    800064f8:	00471693          	slli	a3,a4,0x4
    800064fc:	0001d717          	auipc	a4,0x1d
    80006500:	b0470713          	addi	a4,a4,-1276 # 80023000 <disk>
    80006504:	9736                	add	a4,a4,a3
    80006506:	0a072423          	sw	zero,168(a4)
    8000650a:	b505                	j	8000632a <virtio_disk_rw+0xf4>

000000008000650c <virtio_disk_intr>:

void
virtio_disk_intr()
{
    8000650c:	1101                	addi	sp,sp,-32
    8000650e:	ec06                	sd	ra,24(sp)
    80006510:	e822                	sd	s0,16(sp)
    80006512:	e426                	sd	s1,8(sp)
    80006514:	e04a                	sd	s2,0(sp)
    80006516:	1000                	addi	s0,sp,32
  acquire(&disk.vdisk_lock);
    80006518:	0001f517          	auipc	a0,0x1f
    8000651c:	c1050513          	addi	a0,a0,-1008 # 80025128 <disk+0x2128>
    80006520:	ffffa097          	auipc	ra,0xffffa
    80006524:	6c4080e7          	jalr	1732(ra) # 80000be4 <acquire>
  // we've seen this interrupt, which the following line does.
  // this may race with the device writing new entries to
  // the "used" ring, in which case we may process the new
  // completion entries in this interrupt, and have nothing to do
  // in the next interrupt, which is harmless.
  *R(VIRTIO_MMIO_INTERRUPT_ACK) = *R(VIRTIO_MMIO_INTERRUPT_STATUS) & 0x3;
    80006528:	10001737          	lui	a4,0x10001
    8000652c:	533c                	lw	a5,96(a4)
    8000652e:	8b8d                	andi	a5,a5,3
    80006530:	d37c                	sw	a5,100(a4)

  __sync_synchronize();
    80006532:	0ff0000f          	fence

  // the device increments disk.used->idx when it
  // adds an entry to the used ring.

  while(disk.used_idx != disk.used->idx){
    80006536:	0001f797          	auipc	a5,0x1f
    8000653a:	aca78793          	addi	a5,a5,-1334 # 80025000 <disk+0x2000>
    8000653e:	6b94                	ld	a3,16(a5)
    80006540:	0207d703          	lhu	a4,32(a5)
    80006544:	0026d783          	lhu	a5,2(a3)
    80006548:	06f70163          	beq	a4,a5,800065aa <virtio_disk_intr+0x9e>
    __sync_synchronize();
    int id = disk.used->ring[disk.used_idx % NUM].id;
    8000654c:	0001d917          	auipc	s2,0x1d
    80006550:	ab490913          	addi	s2,s2,-1356 # 80023000 <disk>
    80006554:	0001f497          	auipc	s1,0x1f
    80006558:	aac48493          	addi	s1,s1,-1364 # 80025000 <disk+0x2000>
    __sync_synchronize();
    8000655c:	0ff0000f          	fence
    int id = disk.used->ring[disk.used_idx % NUM].id;
    80006560:	6898                	ld	a4,16(s1)
    80006562:	0204d783          	lhu	a5,32(s1)
    80006566:	8b9d                	andi	a5,a5,7
    80006568:	078e                	slli	a5,a5,0x3
    8000656a:	97ba                	add	a5,a5,a4
    8000656c:	43dc                	lw	a5,4(a5)

    if(disk.info[id].status != 0)
    8000656e:	20078713          	addi	a4,a5,512
    80006572:	0712                	slli	a4,a4,0x4
    80006574:	974a                	add	a4,a4,s2
    80006576:	03074703          	lbu	a4,48(a4) # 10001030 <_entry-0x6fffefd0>
    8000657a:	e731                	bnez	a4,800065c6 <virtio_disk_intr+0xba>
      panic("virtio_disk_intr status");

    struct buf *b = disk.info[id].b;
    8000657c:	20078793          	addi	a5,a5,512
    80006580:	0792                	slli	a5,a5,0x4
    80006582:	97ca                	add	a5,a5,s2
    80006584:	7788                	ld	a0,40(a5)
    b->disk = 0;   // disk is done with buf
    80006586:	00052223          	sw	zero,4(a0)
    wakeup(b);
    8000658a:	ffffc097          	auipc	ra,0xffffc
    8000658e:	04c080e7          	jalr	76(ra) # 800025d6 <wakeup>

    disk.used_idx += 1;
    80006592:	0204d783          	lhu	a5,32(s1)
    80006596:	2785                	addiw	a5,a5,1
    80006598:	17c2                	slli	a5,a5,0x30
    8000659a:	93c1                	srli	a5,a5,0x30
    8000659c:	02f49023          	sh	a5,32(s1)
  while(disk.used_idx != disk.used->idx){
    800065a0:	6898                	ld	a4,16(s1)
    800065a2:	00275703          	lhu	a4,2(a4)
    800065a6:	faf71be3          	bne	a4,a5,8000655c <virtio_disk_intr+0x50>
  }

  release(&disk.vdisk_lock);
    800065aa:	0001f517          	auipc	a0,0x1f
    800065ae:	b7e50513          	addi	a0,a0,-1154 # 80025128 <disk+0x2128>
    800065b2:	ffffa097          	auipc	ra,0xffffa
    800065b6:	6e6080e7          	jalr	1766(ra) # 80000c98 <release>
}
    800065ba:	60e2                	ld	ra,24(sp)
    800065bc:	6442                	ld	s0,16(sp)
    800065be:	64a2                	ld	s1,8(sp)
    800065c0:	6902                	ld	s2,0(sp)
    800065c2:	6105                	addi	sp,sp,32
    800065c4:	8082                	ret
      panic("virtio_disk_intr status");
    800065c6:	00002517          	auipc	a0,0x2
    800065ca:	29a50513          	addi	a0,a0,666 # 80008860 <syscalls+0x3b8>
    800065ce:	ffffa097          	auipc	ra,0xffffa
    800065d2:	f70080e7          	jalr	-144(ra) # 8000053e <panic>

00000000800065d6 <cas>:
    800065d6:	100522af          	lr.w	t0,(a0)
    800065da:	00b29563          	bne	t0,a1,800065e4 <fail>
    800065de:	18c5252f          	sc.w	a0,a2,(a0)
    800065e2:	8082                	ret

00000000800065e4 <fail>:
    800065e4:	4505                	li	a0,1
    800065e6:	8082                	ret
    800065e8:	100522af          	lr.w	t0,(a0)
    800065ec:	feb29ce3          	bne	t0,a1,800065e4 <fail>
    800065f0:	18c5252f          	sc.w	a0,a2,(a0)
    800065f4:	8082                	ret
	...

0000000080007000 <_trampoline>:
    80007000:	14051573          	csrrw	a0,sscratch,a0
    80007004:	02153423          	sd	ra,40(a0)
    80007008:	02253823          	sd	sp,48(a0)
    8000700c:	02353c23          	sd	gp,56(a0)
    80007010:	04453023          	sd	tp,64(a0)
    80007014:	04553423          	sd	t0,72(a0)
    80007018:	04653823          	sd	t1,80(a0)
    8000701c:	04753c23          	sd	t2,88(a0)
    80007020:	f120                	sd	s0,96(a0)
    80007022:	f524                	sd	s1,104(a0)
    80007024:	fd2c                	sd	a1,120(a0)
    80007026:	e150                	sd	a2,128(a0)
    80007028:	e554                	sd	a3,136(a0)
    8000702a:	e958                	sd	a4,144(a0)
    8000702c:	ed5c                	sd	a5,152(a0)
    8000702e:	0b053023          	sd	a6,160(a0)
    80007032:	0b153423          	sd	a7,168(a0)
    80007036:	0b253823          	sd	s2,176(a0)
    8000703a:	0b353c23          	sd	s3,184(a0)
    8000703e:	0d453023          	sd	s4,192(a0)
    80007042:	0d553423          	sd	s5,200(a0)
    80007046:	0d653823          	sd	s6,208(a0)
    8000704a:	0d753c23          	sd	s7,216(a0)
    8000704e:	0f853023          	sd	s8,224(a0)
    80007052:	0f953423          	sd	s9,232(a0)
    80007056:	0fa53823          	sd	s10,240(a0)
    8000705a:	0fb53c23          	sd	s11,248(a0)
    8000705e:	11c53023          	sd	t3,256(a0)
    80007062:	11d53423          	sd	t4,264(a0)
    80007066:	11e53823          	sd	t5,272(a0)
    8000706a:	11f53c23          	sd	t6,280(a0)
    8000706e:	140022f3          	csrr	t0,sscratch
    80007072:	06553823          	sd	t0,112(a0)
    80007076:	00853103          	ld	sp,8(a0)
    8000707a:	02053203          	ld	tp,32(a0)
    8000707e:	01053283          	ld	t0,16(a0)
    80007082:	00053303          	ld	t1,0(a0)
    80007086:	18031073          	csrw	satp,t1
    8000708a:	12000073          	sfence.vma
    8000708e:	8282                	jr	t0

0000000080007090 <userret>:
    80007090:	18059073          	csrw	satp,a1
    80007094:	12000073          	sfence.vma
    80007098:	07053283          	ld	t0,112(a0)
    8000709c:	14029073          	csrw	sscratch,t0
    800070a0:	02853083          	ld	ra,40(a0)
    800070a4:	03053103          	ld	sp,48(a0)
    800070a8:	03853183          	ld	gp,56(a0)
    800070ac:	04053203          	ld	tp,64(a0)
    800070b0:	04853283          	ld	t0,72(a0)
    800070b4:	05053303          	ld	t1,80(a0)
    800070b8:	05853383          	ld	t2,88(a0)
    800070bc:	7120                	ld	s0,96(a0)
    800070be:	7524                	ld	s1,104(a0)
    800070c0:	7d2c                	ld	a1,120(a0)
    800070c2:	6150                	ld	a2,128(a0)
    800070c4:	6554                	ld	a3,136(a0)
    800070c6:	6958                	ld	a4,144(a0)
    800070c8:	6d5c                	ld	a5,152(a0)
    800070ca:	0a053803          	ld	a6,160(a0)
    800070ce:	0a853883          	ld	a7,168(a0)
    800070d2:	0b053903          	ld	s2,176(a0)
    800070d6:	0b853983          	ld	s3,184(a0)
    800070da:	0c053a03          	ld	s4,192(a0)
    800070de:	0c853a83          	ld	s5,200(a0)
    800070e2:	0d053b03          	ld	s6,208(a0)
    800070e6:	0d853b83          	ld	s7,216(a0)
    800070ea:	0e053c03          	ld	s8,224(a0)
    800070ee:	0e853c83          	ld	s9,232(a0)
    800070f2:	0f053d03          	ld	s10,240(a0)
    800070f6:	0f853d83          	ld	s11,248(a0)
    800070fa:	10053e03          	ld	t3,256(a0)
    800070fe:	10853e83          	ld	t4,264(a0)
    80007102:	11053f03          	ld	t5,272(a0)
    80007106:	11853f83          	ld	t6,280(a0)
    8000710a:	14051573          	csrrw	a0,sscratch,a0
    8000710e:	10200073          	sret
	...
